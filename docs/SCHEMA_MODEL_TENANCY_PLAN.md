...
## Current state
- Host models: `Company` is user-owned; `Product` now uses `BelongsToCompany` for tenant scoping; `UserOpenai`, `Chatbot` and others hold `company_id`/`team_id` without a global scope; `Team` manages crew membership. Tenant-aware traits live in `app/Models/Concerns`.
...
2. Apply `BelongsToCompany` to every imported WorkCore model and any host model that becomes tenant-specific (customers, enquiries, sites, service jobs, quotes, invoices, payments, expenses, attendance, leave, shifts, tickets). Host `Product` is now scoped this way.
