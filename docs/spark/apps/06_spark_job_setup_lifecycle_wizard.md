# [spark] Job Setup Lifecycle Wizard

## 1. Purpose
Build the wizard that turns a created job into a properly prepared operational record with checklist, crew, notes, recurrence, and execution details. This surfaces in Work Hub and finalises job readiness before field execution begins.

## 2. Why this app is priority
A job that is not properly prepared cannot be executed well in the field. This wizard ensures every job has a crew, checklist, access notes, and schedule before dispatch.

## 3. Mobile-first UX requirements
- Multi-step but fast
- Inline help for field-specific concepts
- Large controls for crew assignment and checklist selections
- Resume and save-progress states

## 4. Chat-launch / wizard-launch behavior
- Launch from chat intents like "set up tomorrow's office clean" or from the end of the Lead-to-Job wizard
- Deep-link from Work Hub cards such as unprepared job alerts

## 5. Data boundaries and persistence rules
- Persist preparation truth to core job/checklist/site structures only
- Temporary wizard state can exist locally, but is not the source of truth

## 6. Docs to read first
- Nexus bundle: `DOC80_Jobs_Mode_Blueprint.md`
- Nexus bundle: `DOC18_View_Surface_Map.md`
- Nexus bundle: `DOC22_Lifecycle_State_Machine.md`
- `09_MOBILE_APPS_AND_CLIENT_SURFACES/25_MOBILE_SURFACE_CAPABILITY_MATRIX.md`
- `05_DATA_SCHEMA_AND_TABLES/DOC121_Node_vs_PWA_Data_Boundary.md`

## 7. Drift reconciliation note
Use current Work Hub language instead of older Jobs Mode UI language when writing acceptance criteria and component names. Any references to "Jobs Mode" are remapped to "Work Hub".

## 8. Scope included
- Crew assignment step
- Checklist selection or creation handoff
- Access notes step
- Recurrence settings
- Operational readiness summary

## 9. Scope excluded
- Live route optimization
- Full dispatch board
- Payroll / invoicing

## 10. Acceptance criteria
- [ ] Wizard completes a practical job setup flow
- [ ] Mobile-first and field-ready
- [ ] Can launch from chat or the Lead-to-Job wizard
- [ ] Produces a readiness summary with next step links

## 11. Suggested component breakdown
- `JobMetaStep`
- `CrewAssignmentStep`
- `ChecklistConfigStep`
- `AccessNotesStep`
- `RecurrenceStep`
- `ReadinessSummaryStep`

## 12. Suggested API/data dependencies
- job detail endpoint
- crew/team availability endpoint
- checklist templates endpoint
- recurrence options metadata

## 13. Output expectation for Spark
Produce a standalone mobile-first React wizard that prepares jobs for execution and later nests cleanly inside Work Hub.

---
**Labels:** `spark`, `nexus`, `lifecycle`, `wizard`, `work-hub`, `mobile`
