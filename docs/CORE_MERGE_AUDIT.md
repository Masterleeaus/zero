...
- **Controllers**: AI/marketplace/chatbot first-party controllers plus native core CRM controllers under `App\Http\Controllers\Core\Crm` with real logic (list/create/update/show customers and enquiries). Work domain now lives under `App\Http\Controllers\Core\Work` with native logic for sites, service jobs, and checklists (filters, validation, tenancy-aware queries). Money domain now has lifecycle controllers (`QuoteController`, `InvoiceController`, `PaymentController`) with create/edit/status transitions, invoice payments, quote→service-job conversion, quote→invoice conversion, and line-item syncing.
