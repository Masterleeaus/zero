...
- **Controllers**: AI/marketplace/chatbot first-party controllers plus native core CRM controllers under `App\Http\Controllers\Core\Crm` with real logic (list/create/update/show customers and enquiries). Work domain now lives under `App\Http\Controllers\Core\Work` with native logic for sites, service jobs, and checklists (filters, validation, tenancy-aware queries, status options, customer binding). Money domain now has lifecycle controllers (`QuoteController`, `InvoiceController`, `PaymentController`) with create/edit/status transitions, quote→service-job conversion, quote→invoice conversion, line-item syncing, payments recording with balance recalculation, and company-scoped numbering helpers. Insights controller now renders overview metrics instead of placeholders.
...
- **Signals**: `QuoteAccepted` dispatched on quote status update to accepted; `InvoiceIssued` dispatched when an invoice is created/transitioned to issued; `InvoicePaid` dispatched when an invoice balance hits zero. No automation listeners yet—reserved as lifecycle hooks.
...
- **Authorization**: Money domain now uses dedicated policies for `Quote`, `Invoice`, and `Payment` (company_id match) guarding view/create/update/convert/mark-paid/record-payment paths beyond route-model binding.
- **Navigation**: dynamic menu via `App\Services\Common\MenuService` + `App\Models\Common\Menu`/`MenuGroup`; rendered with panel components (`resources/views/default/components/navbar`, `floating-menu`, `bottom-menu`, `table`, `card`, etc.). Money parent menu with Quotes/Invoices/Payments links points to the native routes.
- **Views**: native layouts under `resources/views/default/layout` and `resources/views/default/panel/*`; CRM screens now live at `resources/views/default/panel/user/crm/*` (customers/enquiries). Work screens live at `resources/views/default/panel/user/work/*` (sites, service jobs, checklists) using native panel components. Money screens live at `resources/views/default/panel/user/money/*` (quotes/invoices/payments forms with line-item editing UI, conversion actions, totals summaries, payment tables).
