...
- **Controllers**: AI/marketplace/chatbot first-party controllers plus native core CRM controllers under `App\Http\Controllers\Core\Crm` with real logic (list/create/update/show customers and enquiries). Work domain now lives under `App\Http\Controllers\Core\Work` with native logic for sites, service jobs, and checklists (filters, validation, tenancy-aware queries).
...
- **Views**: native layouts under `resources/views/default/layout` and `resources/views/default/panel/*`; CRM screens now live at `resources/views/default/panel/user/crm/*` (customers/enquiries). Work screens live at `resources/views/default/panel/work/*` (sites, service jobs, checklists) using native panel components.
