...
class AIController extends Controller
{
    ...
    public function audioIsolator($file, $post, $user): JsonResponse
    {
        ...
        if (empty($apiKey)) {
            return response()->json([
                'type'   => 'error',
                'errors' => [__('Elevenlabs API key is required.')],
            ], 401);
        }
    ...
    public function updateWriting(Request $request)
    {
        ...
        $content = $aiService->complete(
            'You are a helpful assistant. You must response only with answer. Response language must be: ' . $languageCode,
            "$prompt\n\nContent: \"$content\""
        );
    ...
    public function processOpenAIGptImage1(?EntityEnum $model, array $param): array
    {
        ...
        throw ValidationException::withMessages([
            'message' => trans('Image generation failed'),
        ]);
    }
