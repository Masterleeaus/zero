        ...
        <x-card>
            <form method="post" action="{{ $quote->exists ? route('dashboard.money.quotes.update', $quote) : route('dashboard.money.quotes.store') }}">
                ...
                <div class="grid md:grid-cols-2 gap-4">
                    ...
                    <x-input name="subtotal" type="number" step="0.01" label="{{ __('Subtotal') }}" value="{{ old('subtotal', $quote->subtotal) }}" required />
                    <x-input name="tax" type="number" step="0.01" label="{{ __('Tax') }}" value="{{ old('tax', $quote->tax) }}" required />
                    <x-input name="total" type="number" step="0.01" label="{{ __('Total') }}" value="{{ old('total', $quote->total) }}" required />
                </div>
                <div class="mt-4">
