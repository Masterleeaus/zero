        ...
        <x-card>
            <form method="post" action="{{ $invoice->exists ? route('dashboard.money.invoices.update', $invoice) : route('dashboard.money.invoices.store') }}">
                ...
                <div class="grid md:grid-cols-2 gap-4">
                    ...
                    <x-input name="subtotal" type="number" step="0.01" label="{{ __('Subtotal') }}" value="{{ old('subtotal', $invoice->subtotal) }}" required />
                    <x-input name="tax" type="number" step="0.01" label="{{ __('Tax') }}" value="{{ old('tax', $invoice->tax) }}" required />
                    <x-input name="total" type="number" step="0.01" label="{{ __('Total') }}" value="{{ old('total', $invoice->total) }}" required />
