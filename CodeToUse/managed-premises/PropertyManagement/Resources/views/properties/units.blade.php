...
@section('content')
    ...
    <div class="card mb-3">
        ...
        <div class="card-body">
            <x-form method="POST" :action="route('propertymanagement.properties.units.store', $property->id)">
                @csrf
                <div class="row">
                    ...
                    <tbody>
                        @forelse($property->units as $unit)
                            <tr>
                                ...
                                <td>{{ $unit->area }}</td>
                                <td class="text-right">
                                    <form method="POST" action="{{ route('propertymanagement.properties.units.destroy', [$property->id, $unit->id]) }}" onsubmit="return confirm('@lang('app.areYouSure')')">
                                        @csrf
                                        @method('DELETE')
