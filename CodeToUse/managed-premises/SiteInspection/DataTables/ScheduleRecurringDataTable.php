...
class ScheduleRecurringDataTable extends BaseDataTable
{
    ...
    {
        parent::__construct();
        $this->viewSchedulePermission = user()->permission('view_inspection');
        $this->deleteSchedulePermission = user()->permission('delete_inspection');
        $this->editSchedulePermission = user()->permission('edit_inspection');
    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->addColumn('action', function ($row) {
                ...
                $action .= '<a href="' . route('recurring-schedules.show', [$row->id]) . '" class="dropdown-item"><i class="fa fa-eye mr-2"></i>' . __('app.view') . '</a>';

                if ($this->editSchedulePermission == 'all' ) {
                    $action .= '<a class="dropdown-item" href="' . route('recurring-schedules.edit', $row->id) . '" >
    ...
    public function query(RecurringSchedule $model)
    {
        ...
        if ($request->startDate !== null && $request->startDate != 'null' && $request->startDate != '') {
            ...
            $model = $model->where(DB::raw('DATE(schedule_recurring.`issue_date`)'), '>=', $startDate);
        }

        if ($request->endDate !== null && $request->endDate != 'null' && $request->endDate != '') {
            $endDate = Carbon::createFromFormat($this->company->date_format, $request->endDate)->toDateString();
            $model = $model->where(DB::raw('DATE(schedule_recurring.`issue_date`)'), '<=', $endDate);
        }

        if ($request->status != 'all' && !is_null($request->status)) {
            $model = $model->where('schedule_recurring.status', '=', $request->status);
        }


        if ($request->searchText != '') {
            $model = $model->where(function ($query) {
                $query->where('schedule_recurring.subject', 'like', '%' . request('searchText') . '%')
                    ->orWhere('schedule_recurring.lokasi', 'like', '%' . request('searchText') . '%');
            });
        }
