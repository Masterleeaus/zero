...
class SchedulesDataTable extends BaseDataTable
{
    ...
    public function query(Schedule $model)
    {
        ...
        if ($request->startDate !== null && $request->startDate != 'null' && $request->startDate != '') {
            ...
            $model = $model->where(DB::raw('DATE(schedules.`issue_date`)'), '>=', $startDate);
        }

        if ($request->endDate !== null && $request->endDate != 'null' && $request->endDate != '') {
            $endDate = Carbon::createFromFormat($this->company->date_format, $request->endDate)->toDateString();
            $model = $model->where(DB::raw('DATE(schedules.`issue_date`)'), '<=', $endDate);
        }

        if ($request->status != 'all' && !is_null($request->status)) {
            $model = $model->where('schedules.status', '=', $request->status);
        }

        if ($request->searchText != '') {
            $model = $model->where(function ($query) {
                $query->where('schedules.subject', 'like', '%' . request('searchText') . '%')
                    ->orWhere('schedules.pic', 'like', '%' . request('searchText') . '%')
                    ->orWhere('schedules.remark', 'like', '%' . request('searchText') . '%');
            });
        }
    ...
    {
        return $this->setBuilder('schedules-table', 0)
            ->parameters([
                'initComplete' => 'function () {
                   window.LaravelDataTables["schedules-table"].buttons().container()
