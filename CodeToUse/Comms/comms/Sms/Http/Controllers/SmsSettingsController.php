...
class SmsSettingsController extends AccountBaseController
{
    ...
    public function update(StoreSmsSetting $request, $id)
    {
        ...
        if ($request->active_gateway == 'telegram') {
            ...
            // Sync Titan Talk SMS channel settings if Titan Talk module is installed
            try {
                if (class_exists(\Modules\AIConverse\Models\Channel::class)) {
                    $channel = \Modules\AIConverse\Models\Channel::firstOrCreate(
                        ['tenant_id' => null, 'driver' => 'sms'],
                        ['name' => 'SMS', 'enabled' => (bool) $smsSetting->status, 'config' => []]
                    );

                    $config = [
                        'provider'   => $request->active_gateway,
                        'from'       => null,
                        'api_key'    => null,
                        'api_secret' => null,
                    ];
                    ...
                    if ($request->active_gateway == 'twilio') {
                        $config['from']       = $smsSetting->from_number;
                        $config['api_key']    = $smsSetting->account_sid;
                        $config['api_secret'] = $smsSetting->auth_token;
                    } elseif ($request->active_gateway == 'nexmo') {
                        $config['from']       = $smsSetting->nexmo_from_number;
                        $config['api_key']    = $smsSetting->nexmo_api_key;
                        $config['api_secret'] = $smsSetting->nexmo_api_secret;
                    } elseif ($request->active_gateway == 'msg91') {
                        $config['from']       = $smsSetting->msg91_from;
                        $config['api_key']    = $smsSetting->msg91_auth_key;
                    }

                    $channel->enabled = (bool) ($smsSetting->status || $smsSetting->nexmo_status || $smsSetting->msg91_status);
                    $existingConfig = is_array($channel->config) ? $channel->config : [];
                    $channel->config = array_merge($existingConfig, $config);
                    $channel->save();
                }
            } catch (\Throwable $e) {
                // Titan Talk might not be installed; ignore failures
            }
        }
