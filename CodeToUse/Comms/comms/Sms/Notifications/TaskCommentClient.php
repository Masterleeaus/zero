...
class TaskCommentClient extends Notification implements ShouldQueue
{
    ...
    public function __construct(Task $task, $comment)
    {
        ...
        $this->smsSetting = SmsNotificationSetting::where('slug', 'task-comment')->where('company_id', $this->company->id)->first();
    ...
    public function toTelegram($notifiable)
    {
        return TelegramMessage::create()
            ...
            // Markdown supported.
            ->content($this->message)
            ->button(__('email.taskComment.action'), route('tasks.show', [$this->task->id, 'view' => 'comments']));
    }
}
