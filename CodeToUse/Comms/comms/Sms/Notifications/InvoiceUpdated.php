...
class InvoiceUpdated extends Notification implements ShouldQueue
{
    ...
    public function __construct(Invoice $invoice)
    {
        ...
        $this->smsSetting = SmsNotificationSetting::where('slug', 'invoice-updated')->where('company_id', $this->company->id)->first();
    ...
    public function via($notifiable)
    {
        ...
        $this->message = __('email.invoice.updateTextSms', ['invoice_number' => $this->invoice->invoice_number]);
    ...
    public function toTelegram($notifiable)
    {
        return TelegramMessage::create()
            ...
            // Markdown supported.
            ->content($this->message)
            ->button(__('email.viewInvoice'), route('front.invoice', $this->invoice->hash));
    }
}
