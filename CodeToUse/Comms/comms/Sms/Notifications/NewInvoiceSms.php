...
class NewInvoiceSms extends Notification implements ShouldQueue
{
    ...
    public function __construct(Invoice $invoice)
    {
        ...
        $this->smsSetting = SmsNotificationSetting::where('slug', 'invoice-created')->where('company_id', $this->company->id)->first();
    ...
    public function via($notifiable)
    {
        ...
        $this->message = __('email.invoice.subject').' - '.$this->invoice->invoice_number;
    ...
    public function toTwilio($notifiable)
    {
        $this->toWhatsapp(
            ...
            $notifiable,
            __($this->smsSetting->slug->translationString(), ['invoiceNumber' => $this->invoice->invoice_number]),
            ['1' => $this->invoice->invoice_number]
        );
    ...
    public function toTelegram($notifiable)
    {
        return TelegramMessage::create()
            ...
            // Markdown supported.
            ->content($this->message)
            ->button(__('app.view'), route('front.invoice', $this->invoice->hash));
    }
}
