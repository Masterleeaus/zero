...
class MultipleLeaveApplication extends Notification implements ShouldQueue
{
    ...
    public function __construct(Leave $leave, $multiDates)
    {
        ...
        $this->smsSetting = SmsNotificationSetting::where('slug', 'multiple-leave-application')->where('company_id', $this->company->id)->first();
    ...
    public function toTwilio($notifiable)
    {
        $this->toWhatsapp(
            ...
            $notifiable,
            __($this->smsSetting->slug->translationString(), ['name' => $this->leave->user->name, 'reason' => $this->leave->reason]),
            ['1' => $this->leave->user->name, '2' => $this->leave->reason]
        );
