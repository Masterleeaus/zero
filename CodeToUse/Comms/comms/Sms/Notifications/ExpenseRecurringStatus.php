...
class ExpenseRecurringStatus extends Notification implements ShouldQueue
{
    ...
    public function __construct(ExpenseRecurring $expense)
    {
        ...
        $this->smsSetting = SmsNotificationSetting::where('slug', 'recurring-expense-status-updated')->where('company_id', $this->company->id)->first();
    ...
    public function toTwilio($notifiable)
    {
        $this->toWhatsapp(
            ...
            $notifiable,
            __($this->smsSetting->slug->translationString(), ['expenseName' => $this->expense->item_name, 'expenseStatus' => $this->expense->status]),
            ['1' => $this->expense->item_name, '2' => $this->expense->status]
        );
    ...
    public function toMsg91($notifiable)
    {
        ...
        if ($this->smsSetting->msg91_flow_id && sms_setting()->msg91_status) {
            ...
            return (new \Craftsys\Notifications\Messages\Msg91SMS)
                ...
                ->variable('expense_name', $this->expense->item_name)
                ->variable('expense_status', $this->expense->status);
        }
    }
    ...
    public function toTelegram($notifiable)
    {
        return TelegramMessage::create()
            ...
            // Markdown supported.
            ->content($this->message)
            ->button(__('email.expenseRecurringStatus.action'), route('recurring-expenses.show', $this->expense->id));
    }
}
