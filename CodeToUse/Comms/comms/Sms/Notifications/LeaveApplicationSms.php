...
class LeaveApplicationSms extends Notification implements ShouldQueue
{
    ...
    public function __construct(Leave $leave)
    {
        ...
        $this->smsSetting = SmsNotificationSetting::where('slug', 'new-leave-application')->where('company_id', $this->company->id)->first();
    ...
    public function toTwilio($notifiable)
    {
        $this->toWhatsapp(
            ...
            $notifiable,
            __($this->smsSetting->slug->translationString(), ['leaveDate' => $this->leave->leave_date->format($this->company->date_format)]),
            ['1' => $this->leave->leave_date->format($this->company->date_format)]
        );
