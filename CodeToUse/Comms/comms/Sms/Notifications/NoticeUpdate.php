...
class NoticeUpdate extends Notification implements ShouldQueue
{
    ...
    {
        $this->notice = $notice;
        $this->smsSetting = SmsNotificationSetting::where('slug', 'notice-updated')->first();

        $this->company = $this->notice->company;
        $this->smsSetting = SmsNotificationSetting::where('slug', 'notice-updated')->where('company_id', $this->company->id)->first();
    ...
    public function via($notifiable)
    {
        ...
        $this->message = __('email.noticeUpdate.text')."\n".($this->notice->heading);
    ...
    public function toTwilio($notifiable)
    {
        $this->toWhatsapp(
            ...
            $notifiable,
            __($this->smsSetting->slug->translationString(), ['heading' => $this->notice->heading]),
            ['1' => $this->notice->heading]
        );
    ...
    public function toMsg91($notifiable)
    {
        ...
        if ($this->smsSetting->msg91_flow_id && sms_setting()->msg91_status) {
            ...
            return (new \Craftsys\Notifications\Messages\Msg91SMS)
                ...
                ->variable('heading', Str::limit($this->notice->heading, 27, '...'));
        }
    }
    ...
    public function toTelegram($notifiable)
    {
        ...
        return TelegramMessage::create()
            ...
            // Markdown supported.
            ->content($this->message)
            ->button(__('email.noticeUpdate.action'), route('notices.show', $this->notice->id));
    }
}
