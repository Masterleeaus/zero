...
class NewTicketReply extends Notification implements ShouldQueue
{
    ...
    public function __construct(TicketReply $ticket)
    {
        ...
        $this->smsSetting = SmsNotificationSetting::where('slug', 'new-ticket-reply')->where('company_id', $this->company->id)->first();
    ...
    public function via($notifiable)
    {
        ...
        $this->message = __('email.ticketReply.subject')."\n".__('email.ticketReply.text').$this->ticket->ticket_number;
    ...
    public function toTwilio($notifiable)
    {
        $this->toWhatsapp(
            ...
            $notifiable,
            __($this->smsSetting->slug->translationString(), ['ticketNumber' => $this->ticket->ticket_number]),
            ['1' => $this->ticket->ticket_number]
        );
    ...
    public function toMsg91($notifiable)
    {
        ...
        if ($this->smsSetting->msg91_flow_id && sms_setting()->msg91_status) {
            ...
            return (new \Craftsys\Notifications\Messages\Msg91SMS)
                ...
                ->variable('ticket_number', $this->ticket->ticket_number);
        }
    }
    ...
    public function toTelegram($notifiable)
    {
        return TelegramMessage::create()
            ...
            // Markdown supported.
            ->content($this->message)
            ->button(__('email.ticketReply.action'), route('tickets.show', $this->ticket->ticket_number));
    }
}
