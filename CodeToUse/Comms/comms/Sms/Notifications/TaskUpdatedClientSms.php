...
class TaskUpdatedClientSms extends Notification implements ShouldQueue
{
    ...
    public function __construct(Task $task)
    {
        ...
        $this->smsSetting = SmsNotificationSetting::where('slug', 'task-update-client')->where('company_id', $this->company->id)->first();
    ...
    public function toTwilio($notifiable)
    {
        $this->toWhatsapp(
            ...
            $notifiable,
            __($this->smsSetting->slug->translationString(), ['heading' => $this->task->heading, 'taskId' => $this->task->id]),
            ['1' => $this->task->heading, '2' => $this->task->id]
        );
