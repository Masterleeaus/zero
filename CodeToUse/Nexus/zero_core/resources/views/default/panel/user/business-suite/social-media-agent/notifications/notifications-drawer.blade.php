@php
    $button_classname = @twMerge('lqd-social-media-agent-notification-trigger size-6 !p-0 max-lg:hidden max-lg:size-10 flex items-center justify-center hover:bg-transparent max-lg:rounded-full max-lg:border max-lg:dark:bg-white/[3%] relative', $attributes->get('class'));
@endphp

<div
    class="inline-flex items-center"
    x-data="socialMediaAgentNotifications"
>
    <x-button
        class="{{ $button_classname }}"
        size="none"
        href="#"
        title="{{ __('Service Ops Notifications') }}"
        variant="link"
    ...
    <x-sidedrawer id="social-media-agent-notifications-sidedrawer">
...
@pushOnce('script')
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data('socialMediaAgentNotifications', () => ({
                notifications: [],
                unreadsCount: 0,
                ...
                isClearingAll: false,
                endpoints: {
                    list: '{{ route('dashboard.user.social-media.agent.analyses.index') }}',
                    base: '{{ url('dashboard/user/social-media/agent/analyses') }}',
                    clearAll: '{{ route('dashboard.user.social-media.agent.analyses.clear-all') }}',
                },

                async init() {
                    this.notificationsSidedrawer = document.querySelector('#social-media-agent-notifications-sidedrawer');
