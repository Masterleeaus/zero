...
<div class="controls">
  ...
  <form method="POST" action="{{ route('inventory.wh.bulkDelete') }}" style="display:inline"> @csrf <button class="btn btn-danger" data-confirm="Move selected to trash?">Bulk Delete</button></form>
</div>
<script>
...
<p>
<a class="btn" href="{{ route('inventory.wh.export') }}">Export CSV</a>
<form method="POST" action="{{ route('inventory.wh.import') }}" enctype="multipart/form-data" style="display:inline-block;margin-left:12px">
@csrf <input type="file" name="csv" accept=".csv"> <button class="btn btn-ok">Import CSV</button>
</form>
</p>

<a class="btn" href="{{ route('inventory.wh.create') }}" class="btn btn-primary">+ New Warehouse</a>
<table data-enhanced border="1" cellspacing="0" cellpadding="6">
<tr><th><input type="checkbox" id="chk-all"></th><th>ID</th><th>Name</th><th>Code</th><th>Location</th><th></th></tr>
@foreach($rows as $w)
...
<td>{{ $w->location }}</td>
<td>
  <a class="btn" href="{{ route('inventory.wh.edit',$w) }}">Edit</a>
  <form action="{{ route('inventory.wh.destroy',$w) }}" method="POST" style="display:inline">@csrf @method('DELETE') <button data-confirm="Delete this record?">Delete</button></form>
</td>
</tr>
@endforeach
</table>
...
</script>\n<p><a class="btn" href="{{ route('inventory.wh.trashed') }}">Trashed</a></p>