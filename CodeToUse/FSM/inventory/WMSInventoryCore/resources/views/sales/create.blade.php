...
@section('page-script')
  <script>
    const pageData = {
      urls: {
        ...
        store: @json(route('wmsinventorycore.sales.store')),
      ...
      labels: {
        ...
        salesOrderCreated: @json(__('Sales order created successfully.')),
        error: @json(__('Error!')),
  ...
  @vite(['Modules/WMSInventoryCore/resources/assets/js/wms-inventory-sale-form.js'])
@endsection
...
<div class="row">
  <div class="col-12">
    <form id="salesForm" action="{{ route('wmsinventorycore.sales.store') }}" method="POST">
      @csrf
        ...
        <div class="card-body">
          <div class="row">
            <div class="col-md-3 mb-3">
              ...
              <input type="text" class="form-control" id="so_number" name="so_number" value="{{ old('so_number') }}" placeholder="{{ __('Auto-generated') }}" readonly />
            </div>
            
            <div class="col-md-3 mb-3">
              <label class="form-label" for="sale_date">{{ __('Sale Date') }} <span class="text-danger">*</span></label>
              <input type="text" class="form-control datepicker" id="sale_date" name="sale_date" value="{{ old('sale_date', date('Y-m-d')) }}" required />
            </div>
          ...
          <div class="card">
            ...
            <div class="card-body">
              <div class="mb-3">
                ...
                <textarea class="form-control" id="notes" name="notes" rows="4" placeholder="{{ __('Enter any notes for this sales order') }}">{{ old('notes') }}</textarea>
              </div>
          ...
          <div class="card">
            <div class="card-body">
              <div class="d-flex gap-3">
                <button type="submit" class="btn btn-primary">{{ __('Create Sales Order') }}</button>
                <a href="{{ route('wmsinventorycore.sales.index') }}" class="btn btn-label-secondary">{{ __('Cancel') }}</a>
              </div>
            </div>
          </div>
        </div>
