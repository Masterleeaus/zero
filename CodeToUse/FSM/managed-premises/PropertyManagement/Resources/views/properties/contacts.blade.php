...
@section('content')
    <div class="d-flex justify-content-between align-items-center mb-3">
        ...
        <a class="btn btn-outline-secondary" href="{{ route('propertymanagement.properties.show', $property->id) }}">Back</a>
    </div>

    <div class="card mb-3">
        <div class="card-header">Add contact</div>
        <div class="card-body">
            <x-form method="POST" :action="route('propertymanagement.properties.contacts.store', $property->id)">
                @csrf
                <div class="row">
                    <div class="col-md-3">
            ...
            <table class="table table-hover">
                ...
                @forelse($property->contacts as $contact)
                    <tr>
                        ...
                        <td>{{ $contact->email }}</td>
                        <td class="text-right">
                            <form method="POST" action="{{ route('propertymanagement.properties.contacts.destroy', [$property->id, $contact->id]) }}" onsubmit="return confirm('@lang('app.areYouSure')')">
                                @csrf
                                @method('DELETE')
