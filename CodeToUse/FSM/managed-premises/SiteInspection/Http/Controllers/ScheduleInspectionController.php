...
class ScheduleInspectionController extends AccountBaseController
{
    ...
    public function index(ScheduleInspectionDataTable $dataTable)
    {
        ...
        return $dataTable->render('siteinspection::schedule-siteinspection.index', $this->data);
    }
    ...
    public function show($id)
    {
        ...
        return view('siteinspection::schedule-siteinspection.edit', $this->data);
    }
    ...
    public function refreshCount(Request $request)
    {
        ...
        $schedules = Schedule::with('agent');

        if (!is_null($request->startDate) && $request->startDate != '') {
            $startDate = Carbon::createFromFormat($this->company->date_format, $request->startDate)->toDateString();
            $schedules->where(DB::raw('DATE(`updated_at`)'), '>=', $startDate);
        }

        if (!is_null($request->endDate) && $request->endDate != '') {
            $endDate = Carbon::createFromFormat($this->company->date_format, $request->endDate)->toDateString();
            $schedules->where(DB::raw('DATE(`updated_at`)'), '<=', $endDate);
        }

        if (!is_null($request->agentId) && $request->agentId != 'all') {
            $schedules->where('agent_id', '=', $request->agentId);
        }

        if (!is_null($request->priority) && $request->priority != 'all') {
            $schedules->where('priority', '=', $request->priority);
        }


        $schedules = $schedules->get();

        $openTickets = $schedules->filter(function ($value, $key) {
            return $value->status == 'open';
        })->count();

        $pendingTickets = $schedules->filter(function ($value, $key) {
            return $value->status == 'pending';
        })->count();

        $resolvedTickets = $schedules->filter(function ($value, $key) {
            return $value->status == 'resolved';
        })->count();

        $closedTickets = $schedules->filter(function ($value, $key) {
            return $value->status == 'closed';
        })->count();

        $totalTickets = $schedules->count();
