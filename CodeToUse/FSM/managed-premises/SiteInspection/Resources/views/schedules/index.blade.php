        ...
        <div class="d-block d-lg-flex d-md-flex">
            <div id="table-actions" class="flex-grow-1 align-items-center mb-2 mb-lg-0 mb-md-0">
                @if ($addSchedulesPermission == 'all')
                    <x-forms.link-primary :link="route('schedules.create')" class="mr-3 float-left mb-2 mb-lg-0 mb-md-0"
                        icon="plus">
                        Add Schedule
...
@push('scripts')
    ...
    <script>
        $('#schedules-table').on('preXhr.dt', function(e, settings, data) {
        ...
        const showTable = () => {
            window.LaravelDataTables["schedules-table"].draw(false);
        }
        ...
        const applyQuickAction = () => {
            var rowdIds = $("#schedules-table input:checkbox:checked").map(function() {
                return $(this).val();
            }).get();

            var url = "{{ route('schedules.apply_quick_action') }}?row_ids=" + rowdIds;
        ...
        $('body').on('click', '.cancel-schedule', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    var url = "{{ route('schedules.update_status', ':id') }}";
                    url = url.replace(':id', id);
