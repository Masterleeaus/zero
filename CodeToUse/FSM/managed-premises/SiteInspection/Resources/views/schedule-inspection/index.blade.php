...
@push('scripts')
    ...
    <script>
        ...
        $('#agent_id, #ticket-status, #priority')
            .on('change keyup',
                function() {
        ...
        $('body').on('click', '.delete-table-row', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    var url = "{{ route('schedule-siteinspection.destroy', ':id') }}";
        ...
        $('#ticket-table').on('change', '.change-status', function() {
            var url = "{{ route('schedule-siteinspection.change-status') }}";
            var token = "{{ csrf_token() }}";
            var id = $(this).data('ticket-id');
            var status = $(this).val();
        ...
        const applyQuickAction = () => {
            ...
            var url = "{{ route('siteinspection.apply_quick_action') }}?row_ids=" + rowdIds;
        ...
        function refreshCount() {
            ...
            var url = "{{ route('schedule-siteinspection.refresh_count') }}";
            $.easyAjax({
                type: 'POST',
                url: url,
                data: {
