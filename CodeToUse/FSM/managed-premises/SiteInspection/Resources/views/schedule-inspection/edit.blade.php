...
@php
    $editInspectionPermission = user()->permission('edit_inspection');
    $deleteInspectionPermission = user()->permission('delete_inspection');
@endphp
        ...
        <div class="ticket-left w-100">
            <x-form id="updateTicket2" method="PUT">
                ...
                <div class="ticket-reply-back justify-content-start px-lg-4 px-md-4 px-3 py-3  d-flex bg-white border-top-grey border-right-grey"
                    ...
                    <x-forms.link-secondary :link="route('schedule-siteinspection.index')" icon="arrow-left">@lang('app.back')
...
@push('scripts')
    ...
    <script>
        ...
        taskDropzone.on('completemultiple', function() {
            ...
            window.location.href = "{{ route('schedule-siteinspection.show', $schedule->id) }}";
        });
        ...
        $('.submit-ticket').click(function() {
            ...
            $('#status').val(status);
            const url = "{{ route('schedule-siteinspection.update', $schedule->id) }}";
                    ...
                    if (response.status == 'success') {
                        if (taskDropzone.getQueuedFiles().length > 0) {
                            ...
                            window.location.href =
                                "{{ route('schedule-siteinspection.show', $schedule->id) }}";
                        }
                    }
                }
            });
        ...
        $('.submit-update').click(function() {
            $.easyAjax({
                url: "{{ route('schedule-siteinspection.update_other_data', $schedule->id) }}",
        ...
        $('body').on('click', '.delete-ticket', function() {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    var url = "{{ route('schedule-siteinspection.destroy', $schedule->id) }}";
                    ...
                    $.easyAjax({
                        ...
                        success: function(response) {
                            if (response.status == "success") {
                                window.location.href =
                                    "{{ route('schedule-siteinspection.index') }}";
                            }
                        }
                    });
                }
