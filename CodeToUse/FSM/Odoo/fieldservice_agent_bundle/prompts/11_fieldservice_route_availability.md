# META PROMPT — Integrate `fieldservice_route_availability` Into Titan Zero Backend

## Objective
Combine route planning with availability validation.

## Mapping target
**Primary Titan domain:** route planner validation

## Copilot instructions
You are integrating the Odoo addon **fieldservice_route_availability** into the existing **zero-main** Titan Zero Laravel repo as a **backend overlay**, not a new subsystem.
...
Reject impossible route sequences, check time windows, cleaner availability, territory constraints, conflict flags, planner diagnostics.
...
## Avoid
Do not introduce heavy external solvers yet.
