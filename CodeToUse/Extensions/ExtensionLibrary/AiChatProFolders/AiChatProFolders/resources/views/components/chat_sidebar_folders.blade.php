...
>
    ...
    >
        ...
        {{-- Empty State --}}
        <li
            class="chat-list-item flex shrink-0 flex-col items-center justify-center gap-2 p-8 text-center text-foreground/50"
            x-show="!initialLoading && !fetchingChats && chatsListToShow.length === 0 && !loadingMore"
            x-cloak
        >
            <x-tabler-message-circle-off class="size-8 opacity-50" />
            <span x-text="openFolder ? '{{ __('No chats in this folder') }}' : '{{ __('No chats yet') }}'"></span>
        </li>
    </ul>
