...
>
    ...
    >
        <div class="flex grow items-center overflow-x-auto whitespace-nowrap py-1">
            ...
            <button
                ...
                @click.prevent="if ( activeTab !== 'inspired' ) { switchingTab = true; loadInspiredImages(); }"
    ...
    >
        ...
        >
            <div
                class="masonry-grid-item mb-0.5 px-px"
    ...
    <p
        class="flex items-center justify-center gap-2 py-10 text-center text-sm font-medium text-heading-foreground"
        x-show="loading"
        x-cloak
        ...
        document.addEventListener('alpine:init', function() {
            Alpine.data('aiImageProImagesManager', () => ({
                ...
                init() {
                    ...
                    if (initialTab === 'inspired') {
                        this.loadInspiredImages();
                    } else {
                ...
                scrollToTabs() {
                    ...
                    const top = tabsElement.getBoundingClientRect().top + window.scrollY - 90;
                    window.scrollTo({
                        top: Math.max(top, 0),
                        behavior: 'smooth'
                    });
                ...
                async loadImages(tab) {
                    ...
                    try {
                        ...
                        // For bookmarks tab, send favorites array
                        let body = null;
                        let method = 'GET';
                        ...
                        requestAnimationFrame(() => {
                            this.$nextTick(() => {
                                this.$refs.masonryGrid?.dispatchEvent(new CustomEvent('masonry:reload'));
                            })
                        })
                    }
                },
                ...
                openImageModal(image) {
                    // Use parent component's setActiveModal function
                ...
                async loadMoreImages() {
                    ...
                    // Don't load more for inspired tab (uses community gallery pagination)
                    if (this.activeTab === 'inspired') {
                        await this.loadMoreInspiredImages();
                        return;
                    }
                ...
                async loadInspiredImages() {
                    this.loading = true;
                    this.currentPage = 1;

                    this.activeTab = 'inspired';
                    ...
                    try {
                        ...
                        console.error('Failed to load inspired images:', error);
                    } finally {
                        this.loading = false;
                    }
                },

                async loadMoreInspiredImages() {
                    if (this.loading || !this.hasMore) return;
                    ...
                    try {
                        const response = await fetch(`{{ route('ai-image-pro.community.images') }}?page=${this.currentPage}`);
                        if (!response.ok) {
                            throw new Error('Failed to fetch images');
                        }
                        const data = await response.json();
                        ...
                        console.error('Failed to load more inspired images:', error);
                    } finally {
                        this.loading = false;
                    }
                }
