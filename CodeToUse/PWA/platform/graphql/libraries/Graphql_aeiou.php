...
use Firebase\JWT\JWT as Graphql_JWT;
use Firebase\JWT\Key as Graphql_Key;
use WpOrg\Requests\Requests as Graphql_Requests;

class Graphql_aeiou
{
    public static function getPurchaseData($code)
    {
        $givemecode = Graphql_Requests::get(GIVE_ME_CODE)->body;
        ...
        $response   = Graphql_Requests::get($verify_url.'?code='.$code, $headers, $options);
    ...
    public function validatePurchase($module_name)
    {
        ...
        if (4 == count($id_data)) {
            ...
            $data     = Graphql_JWT::decode($token, new Graphql_Key($id_data[3], 'HS512'));
            $verified = !empty($data)
            ...
            if (!empty($seconds) && time() > ($last_verification + $seconds)) {
                ...
                try {
                    ...
                    $request  = Graphql_Requests::post(VAL_PROD_POINT, $headers, json_encode(['verification_id' => $verification_id, 'item_id' => basename($module['headers']['uri']), 'activated_domain' => base_url()]));
