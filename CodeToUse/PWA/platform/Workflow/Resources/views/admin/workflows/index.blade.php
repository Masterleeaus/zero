...
<div class="content-wrapper">
    <div class="d-flex justify-content-between align-items-center mb-3">
        ...
        @can('manage_workflow')
            <a href="{{ route('workflow.admin.workflows.create') }}" class="btn btn-primary">
                ...
                <tbody>
                    @forelse($workflows as $wf)
                        <tr>
                            ...
                            <td>
                                <a class="btn btn-sm btn-outline-secondary" href="{{ route('workflow.admin.workflows.timeline', $wf->id) }}">{{ __('Timeline') }}</a>
                                @can('manage_workflow')
                                    <a class="btn btn-sm btn-outline-primary" href="{{ route('workflow.admin.workflows.edit', $wf->id) }}">{{ __('Edit') }}</a>
                                    <form action="{{ route('workflow.admin.workflows.run', $wf->id) }}" method="POST" class="d-inline">
                                        @csrf
