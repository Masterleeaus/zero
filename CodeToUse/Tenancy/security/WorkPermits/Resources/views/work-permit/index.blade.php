...
@php
    $addUnitPermission = user()->permission('add_work_permits');
@endphp
...
@section('filter-section')
    <x-filters.filter-box>
        ...
        <div class="select-box d-flex py-2 px-lg-2 px-md-2 px-0 border-right-grey border-right-grey-sm-0">
            <p class="mb-0 pr-2 f-14 text-dark-grey d-flex align-items-center">@lang('trworkpermits::app.menu.statusApprove')</p>
            <div class="select-status">
                <select class="form-control select-picker" name="status_approve" id="status_approve">
                    ...
                    <option value="approved">@lang('trworkpermits::app.menu.approve')</option>
                    <option value="notApprove">@lang('trworkpermits::app.menu.notApprove')</option>
                </select>
            </div>
        </div>
        <!-- STATUS APPROVAL END -->
        <!-- STATUS APPROVAL START -->
        <div class="select-box d-flex py-2 px-lg-2 px-md-2 px-0 border-right-grey border-right-grey-sm-0">
            <p class="mb-0 pr-2 f-14 text-dark-grey d-flex align-items-center">@lang('trworkpermits::app.menu.statusApprovalBm')</p>
            <div class="select-status">
        ...
        <div class="select-box d-flex py-2 px-lg-2 px-md-2 px-0 border-right-grey border-right-grey-sm-0">
            <p class="mb-0 pr-2 f-14 text-dark-grey d-flex align-items-center">@lang('trworkpermits::app.menu.statusValidate')</p>
            <div class="select-status">
                <select class="form-control select-picker" name="status_validate" id="status_validate">
                    <option value="all" selected>@lang('app.all')</option>
                    <option value="validated">@lang('trworkpermits::app.menu.validated')</option>
                    <option value="notValidated">@lang('trworkpermits::app.menu.notValidate')</option>
                </select>
            </div>
        </div>
        <!-- STATUS VALIDATE END -->
        ...
        <div class="d-block d-lg-flex d-md-flex justify-content-between action-bar">
            <div id="table-actions" class="flex-grow-1 align-items-center">
                @if ($addUnitPermission == 'all' || $addUnitPermission == 'owned')
                    <x-forms.link-primary :link="route('work-permits.create')" class="mr-3 openRightModal float-left" icon="plus">
                        @lang('app.add') @lang('trworkpermits::modules.workPermit')
    ...
    <script>
        ...
        $('body').on('click', '.delete-table-row', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    var url = "{{ route('work-permits.destroy', ':id') }}";
        ...
        $('body').on('click', '.approve', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    var url = "{{ route('work-permits.approved', ':id') }}";
                    url = url.replace(':id', id);
        ...
        $('body').on('click', '.approve_bm', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    var url = "{{ route('work-permits.approved_bm', ':id') }}";
                    url = url.replace(':id', id);
                    window.location.href = url;
                }
            });
        ...
        const applyQuickAction = () => {
            ...
            const url = "{{ route('work-permits.apply_quick_action') }}?row_ids=" + rowdIds;
