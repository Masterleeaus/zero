...
namespace Modules\TrInOutPermit\DataTables;
...
class TrInOutPermitDataTable extends BaseDataTable
{
    ...
    {
        parent::__construct();
        $this->editUnitPermission = user()->permission('edit_trinoutpermit');
        $this->deleteUnitPermission = user()->permission('delete_trinoutpermit');
        $this->viewUnitPermission = user()->permission('view_trinoutpermit');
    }
    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->addColumn('action', function ($row) {
                ...
                $action .= '<a href="' . route('trinoutpermit.show', [$row->id]) . '" class="dropdown-item openRightModal"><i class="fa fa-eye mr-2"></i>' . __('app.view') . '</a>';

                if ($this->editUnitPermission == 'all' || $this->editUnitPermission == 'owned') {
                    if ($row->approved_by == '') {
                        $action .=
                    ...
                    if ($row->approved_bm == '') {
                        $action .=
                            '<a class="dropdown-item approve_bm" href="javascript:;" data-Unit-id="' . $row->id . '">
                                <i class="fas fa-user-check mr-2"></i>
                                ' . trans('trinoutpermit::app.menu.approve_bm') . '
                            </a>';
                    }
                    if ($row->approved_by == '') {
                        $action .= '<a class="dropdown-item openRightModal" href="' . route('trinoutpermit.edit', [$row->id]) . '">
                ...
                $action .= '<a class="dropdown-item" href="' . route('trinoutpermit.download', [$row->id]) . '">
    ...
    public function query(TrInOutPermit $model)
    {
        ...
        if (!is_null($request->startDate) && $request->startDate != '') {
            ...
            $units->where(DB::raw('tr_in_out_permit.date'), '>=', $startDate);
        }

        if (!is_null($request->endDate) && $request->endDate != '') {
            $endDate = Carbon::createFromFormat($this->company->date_format, $request->endDate)->toDateString();
            $units->where(DB::raw('tr_in_out_permit.date'), '<=', $endDate);
        }

        if ($this->viewUnitPermission == 'owned') {
            $units->where(function ($query) {
                $query->where('tr_in_out_permit.created_by', '=', user()->id);
            });
        }
    ...
    protected function getColumns()
    {
        ...
        return [
            ...
            __('trinoutpermit::app.menu.date') => ['data' => 'date', 'name' => 'date', 'exportable' => true, 'title' => __('trinoutpermit::app.menu.date')],
            __('trinoutpermit::app.menu.unit') => ['data' => 'unit_id', 'name' => 'unit_id', 'exportable' => true, 'title' => __('trinoutpermit::app.menu.unit')],
            __('trinoutpermit::app.menu.resident') => ['data' => 'name', 'name' => 'name', 'exportable' => true, 'title' => __('trinoutpermit::app.menu.resident')],
            __('trinoutpermit::app.menu.delivererName') => ['data' => 'pj', 'name' => 'pj', 'exportable' => true, 'title' => __('trinoutpermit::app.menu.delivererName')],
            __('trinoutpermit::app.menu.noHP') => ['data' => 'no_hp', 'name' => 'no_hp', 'exportable' => true, 'title' => __('trinoutpermit::app.menu.noHP')],
            __('app.type') => ['data' => 'keterangan', 'name' => 'keterangan', 'exportable' => true, 'title' => __('app.type')],
            __('trinoutpermit::app.menu.statusAproval') => ['data' => 'approved_by', 'name' => 'approved_by', 'exportable' => true, 'title' => __('trinoutpermit::app.menu.statusAproval')],
            __('trinoutpermit::app.menu.statusAprovalBm') => ['data' => 'approved_bm', 'name' => 'approved_bm', 'exportable' => true, 'title' => __('trinoutpermit::app.menu.statusAprovalBm')],
            __('trinoutpermit::app.menu.statusValidasi') => ['data' => 'validated_by', 'name' => 'validated_by', 'exportable' => true, 'title' => __('trinoutpermit::app.menu.statusValidasi')],
