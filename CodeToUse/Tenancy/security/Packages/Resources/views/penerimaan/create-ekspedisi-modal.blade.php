<div class="modal-header">
    <h5 class="modal-title" id="modelHeading">@lang('app.add') @lang('trpackage::app.menu.ekspedisi')</h5>
...
<div class="modal-body">
    <div class="portlet-body">
        <x-table class="table-bordered" headType="thead-light">
            ...
            @forelse($ekspedisi as $key => $items)
                <tr id="cat-{{ $items->id }}">
                    <td>{{ $key + 1 }}</td>
                ...
                <div class="row">
                    <div class="col-lg-12">
                        <x-forms.text fieldId="name" :fieldLabel="__('trpackage::app.menu.name')" fieldName="name" fieldRequired="true"
                            fieldPlaceholder="e.g. JNE, TIKI, etc.">
                        </x-forms.text>
                    </div>
                </div>
            </div>
...
<script>
        ...
        $.easyAjax({
            ...
            success: function(response) {
                if (response.status == "success") {
                    if ($('#ekspedisi').length > 0) {
                        $('#ekspedisi').html(response.optionData);
                        $('#ekspedisi').selectpicker('refresh');
                        $(MODAL_LG).modal('hide');
                    } else {
    ...
    $('.delete-category').click(function() {
        ...
        let url = "{{ route('ekspedisi.destroy', ':id') }}";
    ...
    $('.edit-name').focus(function() {
        ...
        if ($(this).data("initialText") !== $(this).html()) {
            ...
            let value = $(this).html();
            let url = "{{ route('ekspedisi.update', ':id') }}";
            url = url.replace(':id', id);
