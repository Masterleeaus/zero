...
@php
    $addInvoicesPermission = user()->permission('add_parking');
@endphp
    ...
    <x-filters.filter-box>
        ...
        <x-filters.more-filter-box>
            <div class="more-filter-items">
                <label class="f-14 text-dark-grey mb-12 text-capitalize" for="usr">@lang('parking::app.menu.status')</label>
            ...
            <div class="more-filter-items">
                <label class="f-14 text-dark-grey mb-12 text-capitalize" for="usr">@lang('parking::app.menu.reqType')</label>
                <div class="select-filter mb-4">
                    <div class="select-others">
                        <select class="form-control select-picker" name="req" id="req" data-live-search="true"
                            data-size="8">
                            <option value="all">@lang('app.all')</option>
        ...
        <div class="d-block d-lg-flex d-md-flex">
            <div id="table-actions" class="flex-grow-1 align-items-center mb-2 mb-lg-0 mb-md-0">
                @if ($addInvoicesPermission == 'all')
                    <x-forms.link-primary :link="route('parking.create')" class="mr-3 float-left mb-2 mb-lg-0 mb-md-0" icon="plus">
                        @lang('parking::app.parkir.addParkir')
...
@push('scripts')
    ...
    <script>
        $('#Parking-table').on('preXhr.dt', function(e, settings, data) {
            ...
            if (status == "all" || req == "all" || searchText == "") {

            }
        });

        const showTable = () => {
            window.LaravelDataTables["Parking-table"].draw(false);
        }

        $('#status, #req').on('change keyup',
            function() {
                if ($('#status').val() != "all") {
