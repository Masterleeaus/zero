...
class TwilioConversationService
{
    ...
    public function handleVoice(): string
    {
        ...
        if ((bool) config('unified-communication.routing.ivr_menu', true)) {
            return app(CallRouter::class)->routeIncomingCall(
                conversation: $conversation,
     ...
     */
    public function processVoiceTranscript(string $transcript, ChatbotConversation $conversation, ?string $preResolvedResponse = null): string
    {
        $chatbot = $conversation->chatbot;
        $twilio = app(TwilioVoiceService::class)
            ->setChatbotChannel(ChatbotChannel::find($this->channelId));
        ...
        // Prefer pre-resolved orchestration response when available
        $response = $preResolvedResponse
            ?? $this->resolveVoiceCommand($transcript, $conversation)
