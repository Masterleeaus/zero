...
use App\Extensions\Chatbot\System\Http\Controllers\Overlay\ChatbotCustomerPortalController;
use App\Extensions\Chatbot\System\Http\Controllers\Overlay\ChatbotCommandController;
use App\Extensions\Chatbot\System\Http\Controllers\Overlay\ChatbotAgentController;
use App\Extensions\Chatbot\System\Http\Controllers\Webhooks\GenericWebhookController;
use App\Extensions\Chatbot\System\Http\Controllers\Webhooks\MessengerWebhookController;
use App\Extensions\Chatbot\System\Http\Controllers\Webhooks\TelegramWebhookController;
use App\Extensions\Chatbot\System\Http\Controllers\Webhooks\VoiceWebhookController;
use App\Extensions\Chatbot\System\Http\Controllers\Webhooks\WhatsAppWebhookController;
...
use App\Extensions\Chatbot\System\Policies\ChatbotConversationPolicy;
    ...
    private function registerRoutes(): static
    {
        $this->router()
            ...
            ->group([
                ...
                $router->post('{chatbot}/channel/{channel}', GenericWebhookController::class)->name('webhook');
                $router->post('telegram/{chatbot}', TelegramWebhookController::class)->name('webhook.telegram');
                $router->post('whatsapp/{chatbot}', WhatsAppWebhookController::class)->name('webhook.whatsapp');
                $router->post('messenger/{chatbot}', MessengerWebhookController::class)->name('webhook.messenger');
                $router->post('voice/{chatbot}', VoiceWebhookController::class)->name('webhook.voice');
            })
            ...
            ->group([
                ...
                $route->group([
                    'prefix' => 'dashboard/chatbot-overlay/command',
                    'as' => 'dashboard.chatbot.overlay.command.',
                    'controller' => ChatbotCommandController::class,
                ], function (Router $router) {
                    $router->get('', 'index')->name('index');
                    $router->get('{chatbot}', 'show')->name('show');
                    $router->get('{chatbot}/analytics', 'analytics')->name('analytics');
                    $router->post('{chatbot}/channels', 'saveChannel')->name('channels.save');
                    $router->get('{chatbot}/webhook-url/{channel}', 'webhookUrl')->name('webhook-url');
                });

                $route->group([
                    'prefix' => 'dashboard/chatbot-overlay/agent',
                    'as' => 'dashboard.chatbot.overlay.agent.',
                    'controller' => ChatbotAgentController::class,
                ], function (Router $router) {
                    $router->get('', 'index')->name('index');
                    $router->get('{conversation}', 'show')->name('show');
                    $router->post('{conversation}/claim', 'claim')->name('claim');
                    $router->post('{conversation}/reply', 'reply')->name('reply');
                    $router->post('{conversation}/note', 'note')->name('note');
                    $router->post('{conversation}/transfer', 'transfer')->name('transfer');
                    $router->post('{conversation}/close', 'close')->name('close');
                    $router->get('unread-count', 'unreadCount')->name('unread');
                });

                $route->group([
                    'prefix' => 'dashboard/chatbot-overlay/customer',
                    'as' => 'dashboard.chatbot.overlay.customer.',
                    'controller' => ChatbotCustomerPortalController::class,
                ], function (Router $router) {
                    $router->get('', 'index')->name('index');
                    $router->post('', 'create')->name('create');
                    $router->get('{conversation}', 'show')->name('show');
                    $router->post('{conversation}/message', 'sendMessage')->name('message');
                    $router->post('{conversation}/reopen', 'reopen')->name('reopen');
                    $router->post('{conversation}/feedback', 'feedback')->name('feedback');
                    $router->get('{conversation}/export', 'export')->name('export');
                });
