...
class ChannelWebhookService
{
    ...
    public function ingest(Chatbot $chatbot, ChannelTypeEnum|string $channel, Request $request): array
    {
        ...
        return DB::transaction(function () use ($chatbot, $channel, $normalized, $tenant) {
            ...
            $omniMirror = $this->omniDualWriteManager->ingest([
                'company_id' => $tenant['company_id'],
                'agent_id' => $chatbot->id,
                'channel_type' => $channel->value,
                'channel_id' => $normalized['external_customer_id'] ?: $conversation->session_id,
                'session_id' => $conversation->session_id,
                'customer_name' => $customer->name,
                'role' => 'user',
                'message_type' => $channel->value === 'voice' ? 'voice_transcript' : 'text',
                'content' => $normalized['message'] !== '' ? $normalized['message'] : null,
                ...
                'message_metadata' => [
                    'legacy_history_id' => $history?->id,
                    'legacy_channel' => $channel->value,
                ],
            ]);
            ...
            return [
                ...
                'omni_conversation_id' => $omniMirror['conversation']->id ?? null,
                'omni_message_id' => $omniMirror['message']->id ?? null,
            ];
        });
    }
}
