...
class Chatbot extends Model
{
    ...
    public function scopeOwnedByUser($query, ?int $userId, $workspaceId = null)
    {
        return $query->when($workspaceId, function ($q) use ($workspaceId) {
            $q->where(function ($nested) use ($workspaceId) {
                $nested->where('workspace_id', $workspaceId)
                    ->orWhere('team_id', $workspaceId)
                    ->orWhere('company_id', $workspaceId);
            });
    ...
    public function workspaceKey(): ?int
    {
        return $this->workspace_id ?: $this->company_id ?: $this->team_id ?: null;
    }
