<div class="py-10">
    @if ($agents->isEmpty())
        ...
        <p class="mb-5">
            @lang('Create your first AI social media agent to automate your posting across multiple platforms')
        </p>
        <x-button
                        ...
                        >
                            ...
                            >
                                <x-button
                                    ...
                                    variant="link"
                                    href="{{ route('dashboard.user.social-media.agent.edit', $agent) }}"
                                    title="@lang('Edit')"
                                >
                                    @lang('Edit')
								...
								@if(\App\Helpers\Classes\Helper::appIsDemo())
									<form
										class="w-full"
{{--										action="{{ route('dashboard.user.social-media.agent.destroy', $agent) }}"--}}
{{--										method="POST"--}}
                    ...
                    @if (filled($agent->platform_ids))
                        <div class="mb-4 flex flex-wrap justify-center gap-2">
                            @foreach ($agent->platform_ids as $platform_id)
                                @php
                                    $platform = $platforms->firstWhere('id', $platform_id);
                                ...
                                <figure class="w-5 transition-all hover:scale-125">
                                    <img
                    ...
                    <div class="border-t py-3.5 text-start">
                        <div class="grid grid-cols-1 gap-2 md:grid-cols-2">
                            <div>
                                <p class="mb-1.5 text-3xs font-medium">
                                    @lang('Avarage Impressions')
                                </p>
                                <p class="mb-0 font-medium text-heading-foreground">
                                    {{ \App\Helpers\Classes\Helper::appIsDemo() ? number_format(rand(120, 690)) : ($agent->average_impressions ?? '---') }}
                                </p>
                            </div>
                            <div>
                                <p class="mb-1.5 text-3xs font-medium">
                                    @lang('Avarage Engagement')
                                </p>
                                <p class="mb-0 font-medium text-heading-foreground">
                                    {{ \App\Helpers\Classes\Helper::appIsDemo() ? number_format(rand(80, 150) / 10, 1) : ($agent->average_engagement ?? '---') }}
                                </p>
                            </div>
                        </div>
                    </div>
