...
class ChatbotConversation extends Model
{
    ...
    {
        return $query->when($workspaceId, function (Builder $builder) use ($workspaceId) {
            $builder->where(function (Builder $nested) use ($workspaceId) {
                $nested->where('company_id', $workspaceId)->orWhere('team_id', $workspaceId);
            });
        });
    }
}
