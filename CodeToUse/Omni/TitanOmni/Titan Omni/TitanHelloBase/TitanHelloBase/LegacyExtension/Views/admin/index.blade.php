...
@section('titlebar_actions')
    <x-button
        href="{{ route('admin.extensions.titan-hello.settings.index') }}"
...
@section('content')
    <div class="py-10">
        ...
        >
            {{-- Titan Hello (Phone Answering) Dashboard Widgets --}}
            <div class="grid gap-6 mb-6 lg:grid-cols-3">
                <div class="p-5 rounded-xl border bg-white dark:bg-zinc-900">
                    <div class="font-semibold mb-2">@lang('Recent Calls (24h)')</div>
                    <div class="space-y-2 text-sm">
                        @forelse(($recentCalls ?? []) as $c)
                            <div class="flex justify-between gap-3">
                                <div class="truncate">
                                    <div class="font-medium truncate">{{ $c->from_number ?? 'Unknown' }}</div>
                                    <div class="opacity-70 truncate">{{ $c->job_type ?? ($c->suburb ? ('• ' . $c->suburb) : '') }}</div>
                                </div>
                                <div class="text-right">
                                    <div class="uppercase text-xs opacity-70">{{ $c->status }}</div>
                                    <div class="text-xs opacity-70">{{ optional($c->created_at)->diffForHumans() }}</div>
                                </div>
                            </div>
                        @empty
                            <div class="opacity-70">@lang('No calls yet.')</div>
                        @endforelse
                    </div>
                </div>

                <div class="p-5 rounded-xl border bg-white dark:bg-zinc-900">
                    <div class="font-semibold mb-2">@lang('Missed Calls')</div>
                    <div class="space-y-2 text-sm">
                        @forelse(($missedCalls ?? []) as $c)
                            <div class="flex justify-between gap-3">
                                <div class="truncate">
                                    <div class="font-medium truncate">{{ $c->from_number ?? 'Unknown' }}</div>
                                    <div class="opacity-70 truncate">{{ $c->to_number ?? '' }}</div>
                                </div>
                                <div class="text-right">
                                    <div class="uppercase text-xs opacity-70">{{ $c->status }}</div>
                                    <div class="text-xs opacity-70">{{ optional($c->created_at)->diffForHumans() }}</div>
                        ...
                        @empty
                            <div class="opacity-70">@lang('No missed calls in the last 24h.')</div>
                        @endforelse
                    </div>
                </div>

                <div class="p-5 rounded-xl border bg-white dark:bg-zinc-900">
                    <div class="font-semibold mb-2">@lang('Hot Leads')</div>
                    <div class="space-y-2 text-sm">
                        @forelse(($hotLeads ?? []) as $l)
                            <div class="flex justify-between gap-3">
                                <div class="truncate">
                                    <div class="font-medium truncate">{{ $l->caller_name ?: ($l->caller_phone ?? 'Lead') }}</div>
                                    <div class="opacity-70 truncate">{{ trim(($l->job_type ?? '') . ($l->suburb ? (' • ' . $l->suburb) : '')) }}</div>
                                </div>
                                <div class="text-right">
                                    <div class="uppercase text-xs">{{ $l->urgency }}</div>
                                    <div class="text-xs opacity-70">{{ optional($l->created_at)->diffForHumans() }}</div>
                                </div>
                            </div>
                        @empty
                            <div class="opacity-70">@lang('No hot leads yet.')</div>
                        @endforelse
                    </div>
                </div>
            </div>

            @include('titan-hello::admin.partials.actions-grid')

            @include('titan-hello::admin.partials.chatbots-list', ['chatbots' => $chatbots])

            @include('titan-hello::admin.partials.edit-window.edit-window', ['avatars' => $avatars])
        </div>

        @include('titan-hello::admin.partials.chats-history.chats-history')
    </div>
@endsection
                ...
                Alpine.data('externalChatbotEditor', () => ({
                    ...
                    async createNewChatbot() {
                        ...
                        const res = await fetch(
                            '{{ route('dashboard.titan-hello.store') }}', {
                                method: 'POST',
                                headers: {
                                    'Accept': 'application/json',
                                    'Content-Type': 'application/json',
                    ...
                    async updateChatbot(chatbot) {
                        ...
                        const res = await fetch(
                            '{{ route('dashboard.titan-hello.update') }}', {
                                method: 'PUT',
                                headers: {
                                    'Accept': 'application/json',
                                    'Content-Type': 'application/json',
