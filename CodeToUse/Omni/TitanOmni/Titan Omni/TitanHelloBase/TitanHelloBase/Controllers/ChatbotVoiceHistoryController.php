...
use Extensions\TitanHello\Helpers\Requests\VoiceChatHistoryStoreRequest;
use Extensions\TitanHello\Helpers\Resources\ChatbotConversationHistoryResource;
...
class ChatbotVoiceHistoryController extends Controller
{
    ...
    {
        $inProgressConvs = ExtVoicechabotConversation::where('status', 'in-progress')->orWhere('status', 'processing')->get();
    ...
    public function storeConversation(string $uuid, VoiceChatHistoryStoreRequest $request): JsonResponse
    {
        ...
        try {
            $conversation = $chatbot->conversations()->where('conversation_id', $request->validated('conversation_id'))->first();
            if (! $conversation) {
