...
class TemplateDataTable extends BaseDataTable
{
    ...
    public function dataTable($query)
    {
        ...
        $datatables->addColumn('action', function ($row) {
            ...
            $action .= '<a href="' . route('documents.template.show', [$row->id]) . '" class="dropdown-item openRightModal"><i class="fa fa-eye mr-2"></i>' . __('app.view') . '</a>';

            if ($this->editPermission != 'none') {
                $action .= '<a href="' . route('documents.template.edit', [$row->id]) . '" class="dropdown-item openRightModal"><i class="fa fa-edit mr-2"></i>' . __('app.edit') . '</a>';
            }
        ...
        $datatables->editColumn('title', function($row){
            return '<a href="' . route('documents.template.show', [$row->id]) . '" class="text-darkest-grey openRightModal">' . $row->title . '</a>';
        });
