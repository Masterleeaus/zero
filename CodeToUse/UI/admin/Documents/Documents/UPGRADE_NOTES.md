...
## Pass 3.1 — Templates tabs (Docs vs SWMS)
- Templates page now has two tabs: **Docs** and **SWMS** (filtered by template `category` = `SWMS`).
- Added search on templates (name/description/category).
- If no templates exist, page shows seeding commands:
  - `php artisan module:seed Documents`
  - or `php artisan db:seed --class="Modules\\Documents\\Database\\Seeders\\DocumentsTemplateSeeder"`
