    ...
    <div class="col-sm-6">
        <x-form id="letterForm">
            <div class="card bg-white rounded">
                ...
                <div class="row p-20">
                    <div class="col-md-12">
                        <x-forms.select fieldId="template_id" :fieldLabel="__('documents::app.fields.letterType')" fieldName="template_id" search="true">
                            <option value="">--</option>
                            @foreach ($templates as $template)
                                <option @selected($template->id == $Documents?->template_id) value="{{ $template->id }}">{{ $template->title }}
                                </option>
                            @endforeach
                        </x-forms.select>
                    </div>
                    ...
                    <div class="col-md-12">
                        <x-forms.select fieldId="employees" :fieldLabel="__('app.menu.employees')" :popover="__('documents::app.employeeInfo')" fieldName="user_id"
                            ...
                            @foreach ($employees as $employee)
                                <x-user-option  :user="$employee" :selected="$employee->id == $Documents?->user_id" />
                            @endforeach
                        </x-forms.select>
                    </div>
                    <div @class([
                        'col-md-12',
                        'd-none' => request()->letterId && !$Documents?->name,
                ...
                <div class="row px-3">
                    <div class="col-md-3">
                        <x-forms.number fieldId="left" :fieldLabel="__('documents::app.fields.left')" fieldName="left" :fieldValue="$Documents?->left ?: 20" />
                    </div>
                    <div class="col-md-3">
                        <x-forms.number fieldId="right" :fieldLabel="__('documents::app.fields.right')" fieldName="right" :fieldValue="$Documents?->right ?: 20" />
                    </div>
                    <div class="col-md-3">
                        <x-forms.number fieldId="top" :fieldLabel="__('documents::app.fields.top')" fieldName="top" :fieldValue="$Documents?->top ?: 20" />
                    </div>
                    <div class="col-md-3">
                        <x-forms.number fieldId="bottom" :fieldLabel="__('documents::app.fields.bottom')" fieldName="bottom" :fieldValue="$Documents?->bottom ?: 20" />
                    </div>
                </div>
                ...
                <x-form-actions>
                    <x-forms.button-primary id="save-Documents" icon="check">@lang('app.save')</x-forms.button-primary>
...
<script>
    $(document).ready(function() {
        ...
        $('#save-Documents').click(function() {
            var url = "{{ route('documents.generate.store') }}";
