        ...
        <x-form id="letterForm" method="PUT">
            <div class="card bg-white rounded">
                <h4 class="mb-0 p-20 f-21 font-weight-normal border-bottom">
                    @lang('documents::app.letterDetails')</h4>
                <div class="row p-20">
                    <div class="col-md-12">
                        <x-forms.text :fieldLabel="__('documents::app.fields.letterType')" fieldName="" fieldId="" :fieldValue="$Documents->template->title"
                            :fieldReadOnly="true" />
                        <input type="hidden" name="template_id" value="{{ $Documents->template_id }}">
                    </div>
                    @if ($Documents->name)
                        <div class="col-md-12">
                            <x-forms.select fieldId="employees" :fieldLabel="__('app.menu.employees')" :popover="__('documents::app.employeeInfo')" fieldName="user_id"
                                search="true">
                                <option value="">--</option>
                        ...
                        <div class="col-md-12" id="employeeNameField">
                            <x-forms.text :fieldLabel="__('documents::app.fields.employeeName')" fieldName="employeeName" fieldId="employeeName"
                                :fieldValue="$Documents->name" />
                        </div>
                    @else
                        <div class="col-md-12">
                            <x-forms.text :fieldLabel="__('documents::app.fields.employeeName')" fieldName="" fieldId="" :fieldValue="$Documents->employee_name"
                                :fieldReadOnly="true" />
                        </div>
                        <input type="hidden" name="user_id" value="{{ $Documents->user_id }}">
                    @endif
                </div>

                <h4 class="m-b-20 ml-3 mt-3 f-18 text-success">{{ __('documents::app.adjustSpaceSetting') }}</h4>
                <div class="row px-3">
                    <div class="col-md-3">
                        <x-forms.number fieldId="left" :fieldLabel="__('documents::app.fields.left')" fieldName="left" :fieldValue="$Documents->left" />
                    </div>
                    <div class="col-md-3">
                        <x-forms.number fieldId="right" :fieldLabel="__('documents::app.fields.right')" fieldName="right" :fieldValue="$Documents->right" />
                    </div>
                    <div class="col-md-3">
                        <x-forms.number fieldId="top" :fieldLabel="__('documents::app.fields.top')" fieldName="top" :fieldValue="$Documents->top" />
                    </div>
                    <div class="col-md-3">
                        <x-forms.number fieldId="bottom" :fieldLabel="__('documents::app.fields.bottom')" fieldName="bottom" :fieldValue="$Documents->bottom" />
                    </div>
                </div>
                ...
                <div class="col-md-12 pb-2">
                    ...
                    <div id="description">{!! $Documents->description !!}</div>
                ...
                <x-form-actions>
                    <x-forms.button-primary id="update-Documents" icon="check">@lang('app.update')</x-forms.button-primary>
                    <x-forms.button-cancel :link="route('documents.generate.index')" class="border-0 mr-3">@lang('app.cancel')
...
<script>
    $(document).ready(function() {
        ...
        function generatePreview(content = '') {
            ...
            // replace all Documents variables
        ...
        $('#downloadButton').on('click', function() {
            var url = "{{ route('documents.download.preview.store') }}";
        ...
        $('#letterForm').on('change', '#template_id', function() {
            ...
            var url = "{{ route('documents.ajax.template', ':id') }}";
            ...
            $.easyAjax({
                ...
                redirect: true,
                success: function(response) {
                    quill.pasteHTML(response.documents.description);
                    generatePreview();
                }
            });
        });
        ...
        $('#letterForm').on('change', '#employees', function() {
            ...
            var url = "{{ route('documents.employee', ':id') }}";
        ...
        $('#update-Documents').click(function() {
            var url = "{{ route('documents.generate.update', $Documents->id) }}";
            ...
            $.easyAjax({
                ...
                type: "POST",
                blockUI: true,
                buttonSelector: "#update-Documents",
                disableButton: true,
