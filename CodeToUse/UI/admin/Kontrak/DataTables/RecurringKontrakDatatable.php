...
class RecurringKontrakDatatable extends BaseDataTable
{
    ...
    {
        parent::__construct();
        $this->viewSchedulePermission = user()->permission('view_kontrak');
        $this->deleteSchedulePermission = user()->permission('delete_kontrak');
        $this->editSchedulePermission = user()->permission('edit_kontrak');
    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->addColumn('action', function ($row) {
                ...
                $action .= '<a href="' . route('kontrak.show', [$row->id]) . '" class="dropdown-item"><i class="fa fa-eye mr-2"></i>' . __('app.view') . '</a>';

                if ($this->editSchedulePermission == 'all' ) {
                    $action .= '<a class="dropdown-item" href="' . route('kontrak.edit', $row->id) . '" >
            ...
            ->editColumn(
                'next_schedule_date',
                function ($row) {
                    $rotation = '<span class="px-1"><label class="badge badge-' . RecurringKontrak::ROTATION_COLOR[$row->rotation] . '">' . $row->rotation . '</label></span';
     ...
     */
    public function query(RecurringKontrak $model)
    {
        ...
        if ($request->startDate !== null && $request->startDate != 'null' && $request->startDate != '') {
            ...
            $model = $model->where(DB::raw('DATE(workorders_recurring.`issue_date`)'), '>=', $startDate);
        }

        if ($request->endDate !== null && $request->endDate != 'null' && $request->endDate != '') {
            $endDate = Carbon::createFromFormat($this->company->date_format, $request->endDate)->toDateString();
            $model = $model->where(DB::raw('DATE(workorders_recurring.`issue_date`)'), '<=', $endDate);
        }

        if ($request->status != 'all' && !is_null($request->status)) {
            $model = $model->where('workorders_recurring.status', '=', $request->status);
        }


        if ($request->searchText != '') {
            $model = $model->where(function ($query) {
                $query->where('workorders_recurring.work_description', 'like', '%' . request('searchText') . '%');
            });
        }
    ...
    {
        return $this->setBuilder('schedules-recurring-table', 0)
            ->parameters([
                'initComplete' => 'function () {
                   window.LaravelDataTables["schedules-recurring-table"].buttons().container()
    ...
    protected function getColumns()
    {
        ...
        $dsData = [
            ...
            __('kontrak::app.menu.unit') => ['data' => 'unit_id', 'name' => 'unit_id', 'exportable' => false, 'title' => __('kontrak::app.menu.unit')],
            __('modules.schedules.startDate') => ['data' => 'issue_date', 'name' => 'issue_date', 'title' => __('app.startDate')],
            __('kontrak::app.menu.nextSchedule') => ['data' => 'next_schedule_date', 'name' => 'next_schedule_date', 'title' => __('kontrak::app.menu.nextSchedule')],
