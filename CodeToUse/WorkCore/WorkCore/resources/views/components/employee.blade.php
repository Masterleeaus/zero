...
<div class="media align-items-center mw-250">
    ...
    @if (!is_null($user))
        <a href="{{ isset($disabledLink) ? 'javascript:;' : route('cleaners.show', [$user->id]) }}"
           class="position-relative {{ isset($disabledLink) ? 'disabled-link' : '' }}">
            @if ($active)
                <span class="text-light-green position-absolute f-8 user-online"
                      title="@lang('modules.customer.online')"><i class="fa fa-circle"></i></span>
            @endif
        ...
        <div class="media-body {{$user->status}} text-truncate">
            ...
            <p class="mb-0 f-12 text-dark-grey">
                {{ !is_null($user->employeeDetail) && !is_null($user->employeeDetail->role) ? $user->employeeDetail->role->name : ' ' }}
            </p>
        </div>
    @else
        --
