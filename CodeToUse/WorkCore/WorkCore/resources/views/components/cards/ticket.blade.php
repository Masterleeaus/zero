<div class="card issue / support-message rounded-0 border-0  @if (user()->id == $user->id) bg-white-shade @endif" id="message-{{ $message->id }}" @if ($message->type == 'note') style="background-image: linear-gradient(#FFB4B452, #FFB4B452);" @endif>
    <div class="card-horizontal">
        <div class="card-img">
            <a
                href="{{ !is_null($user->employeeDetail) ? route('cleaners.show', $user->id) : route('customers.show', $user->id) }}"><img
                    class="" src="{{ $user->image_url }}" alt="{{ $user->name }}"></a>
        </div>
        <div class="card-body border-0 pl-0">
            <div class="d-flex">
                <a href="{{ !is_null($user->employeeDetail) ? route('cleaners.show', $user->id) : route('customers.show', $user->id) }}">
            ...
            @php
                ...
                $customer = \App\Models\User::where('id', $message->added_by)->first();
            @endphp
            @if(($isClient == true) && ($message->added_by != $user->id) && $customer)
            ...
            <div class="form-control edit-note-message d-none" id="text-{{$message->id}}">
                ...
                <div class="form-group">
                    <div class="my-1 form-group">
                        <a class="f-15 f-w-500 edit-issue / support-file" href="javascript:;" id="edit-note-file"><i
                        ...
                        <x-slot name="action">
                            <div class="dropdown ml-auto file-action">
                                ...
                                <div class="dropdown-menu dropdown-menu-right border-grey rounded b-shadow-4 p-0"
                                    ...
                                    <a class="dropdown-item"
                                        href="{{ route('issue / support-files.download', md5($file->id)) }}">@lang('app.download')</a>
