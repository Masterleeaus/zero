...
@foreach ($discussion->replies as $key => $message)
    ...
    @endphp
    {{-- customers.show --}}
    <div class="card issue / support-message border-0 rounded-bottom
        @if ($userId == $replyUser->id) bg-white-shade @endif
        " id="message-{{ $message->id }}">
        <div class="card-horizontal">
            <div class="card-img">
                <a href="{{ route(($replyUser->hasRole('customer') ? 'customers.show' : 'cleaners.show'), $replyUser->id) }}"><img class=""
                        src="{{ $replyUser->image_url }}" alt="{{ $replyUser->name }}"></a>
            </div>
            <div class="card-body border-0 pl-0">
                <div class="d-flex">
                    <a href="{{ route(($replyUser->hasRole('customer') ? 'customers.show' : 'cleaners.show'), $replyUser->id) }}">
                ...
                @php
                    $isClient = \App\Models\User::isClient($replyUser->id);
                    $customer = \App\Models\User::where('id', $message->added_by)->first();
                @endphp
                @if(($isClient == true) && ($message->added_by != $replyUser->id) && $customer)
            ...
            <div class="issue / support-msg border-right-grey mt-3 ml-3" data-menu-vertical="1" data-menu-scroll="1"
