...
<div class="card border-0 invoice">
    ...
    <div class="card-body">
        <div class="invoice-table-wrapper">
            <table width="100%" class="">
                <tr class="inv-logo-heading">
                    ...
                    <td align="right" class="font-weight-bold f-21 text-dark text-uppercase mt-4 mt-lg-0 mt-md-0">
                        @lang('modules.quotes.estimateTemplate')</td>
                </tr>
                <tr class="inv-num">
            ...
            <table width="100%">
                <tr class="inv-unpaid">
                    <td class="f-14 text-dark">
                        @if ($invoice->enquiry && ($invoice->enquiry->client_name || $invoice->enquiry->client_email || $invoice->enquiry->mobile || $invoice->enquiry->company_name || $invoice->enquiry->address) && (invoice_setting()->show_client_name == 'yes' || invoice_setting()->show_client_email == 'yes' || invoice_setting()->show_client_phone == 'yes' || invoice_setting()->show_client_company_name == 'yes' || invoice_setting()->show_client_company_address == 'yes'))
                        <p class="mb-0 text-left">
                            <span class="text-dark-grey ">
                                @lang("modules.invoices.billedTo")
                            </span><br>

                            @if ($invoice->enquiry && $invoice->enquiry->client_name && invoice_setting()->show_client_name == 'yes')
                                {{ $invoice->enquiry->client_name_salutation }}<br>
                            @endif
                            @if ($invoice->enquiry && $invoice->enquiry->client_email && invoice_setting()->show_client_email == 'yes')
                                {{ $invoice->enquiry->client_email }}<br>
                            @endif
                            @if ($invoice->enquiry && $invoice->enquiry->mobile && invoice_setting()->show_client_phone == 'yes')
                                {{ $invoice->enquiry->mobile }}<br>
                            @endif
                            @if ($invoice->enquiry && $invoice->enquiry->company_name && invoice_setting()->show_client_company_name == 'yes')
                                {{ $invoice->enquiry->company_name }}<br>
                            @endif
                            @if ($invoice->enquiry && $invoice->enquiry->address && invoice_setting()->show_client_company_address == 'yes')
                                {!! nl2br($invoice->enquiry->address) !!}
                            @endif
                        </p>
                        @endif
                    </td>
    ...
    <div class="card-footer bg-white border-0 d-flex justify-content-start py-0 py-lg-4 py-md-4 mb-4 mb-lg-3 mb-md-3 ">
        <x-forms.button-cancel :link="route('quote-template.index')" class="border-0 ml-2 mt">
            @lang('app.cancel')
