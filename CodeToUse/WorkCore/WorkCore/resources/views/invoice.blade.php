                            ...
                            <tr class="inv-unpaid">
                                <td class="f-14 text-dark">
                                    <p class="mb-0 text-left">
                                        ...
                                        @if ($invoice->customer && $invoice->customer->name && $invoice->company->invoiceSetting->show_client_name == 'yes')
                                            {{ $invoice->customer->name_salutation }}<br>
                                        @endif

                                        @if ($invoice->customer && $invoice->customer->email && $invoice->company->invoiceSetting->show_client_email == 'yes')
                                            {{ $invoice->customer->email }}<br>
                                        @endif

                                        @if ($invoice->customer && $invoice->customer->mobile && $invoice->company->invoiceSetting->show_client_phone == 'yes')
                                        ...
                                        @if ($invoiceSetting->show_gst == 'yes' && !is_null($customer->clientDetails->gst_number))
                                            ...
                                            @if ($customer->clientDetails->tax_name)
                                                {{$customer->clientDetails->tax_name}}
                                                : {{$customer->clientDetails->gst_number}}
                                            @else
                                                @lang('app.gstIn'): {{ $customer->clientDetails->gst_number }}
                                            @endif
                                        @endif
...
<script>
    ...
    $('body').on('click', '#razorpayPaymentButton', function () {
        ...
        var amount = {{ intval($invoice->amountDue() * 100) }};


        var invoiceId = {{ $invoice->id }};
        @if ($invoice->site && $invoice->site->customer)
        var clientEmail = "{{ $invoice->site->customer->email }}";
        @else
        var clientEmail = "{{ $invoice->customer->email }}";
        @endif
