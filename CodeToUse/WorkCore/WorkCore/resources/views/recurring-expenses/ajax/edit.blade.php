                            ...
                            <x-forms.input-group>
                                <select class="form-control select-picker" name="user_id" id="user_id"
                                    ...
                                    @foreach ($cleaners as $cleaner)
                                        <option @if ($expense->user_id == $cleaner->id)
                                            selected
                                        @endif
                                        data-content="<div class='d-inline-block mr-1'><img
                                                class='taskEmployeeImg rounded-circle' src='{{ $cleaner->image_url }}'></div>
                                        {{ $cleaner->name }}"
                                        value="{{ $cleaner->id }}">{{ $cleaner->name }}</option>
                                    @endforeach
                                </select>
                            </x-forms.input-group>
                        </div>
                ...
                <div class="col-md-6 col-lg-4">
                    <x-forms.select fieldId="project_id" fieldName="project_id" :fieldLabel="__('app.site')"
                        ...
                        @foreach ($sites as $site)
                            <option data-currency-id="{{ $site->currency_id }}" @if ($expense->project_id == $site->id) selected @endif value="{{ $site->id }}">
                                {{ $site->project_name }}
                            </option>
                        @endforeach
                    </x-forms.select>
                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-4 mt-0 billingInterval">
                            <x-forms.number class="mr-0 mr-lg-2 mr-md-2 mt-0" :fieldLabel="__('modules.invoices.totalCount')"
                                ...
                                :minValue="count($expense->recurrings)"
                                />
                        </div>
                    </div>
                </div>
