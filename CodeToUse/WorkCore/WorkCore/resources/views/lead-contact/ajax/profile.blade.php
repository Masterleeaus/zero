...
<div class="row">
    ...
    <div class="col-xl-12 col-lg-12 col-md-12 mb-4 mb-xl-0 mb-lg-4 mb-md-0">
        ...
        <x-cards.data :title="__('modules.customer.profileInfo')">
                ...
                <div class="dropdown">
                    ...
                    <div class="dropdown-menu dropdown-menu-right border-grey rounded b-shadow-4 p-0"
                        ...
                        <a class="dropdown-item openRightModal"
                            href="{{ route('enquiry-contact.edit', $leadContact->id) }}">@lang('app.edit')</a>
                        @endif
                        ...
                        @if ($leadContact->client_id == null || $leadContact->client_id == '')
                            <a class="dropdown-item" href="{{route('customers.create') . '?enquiry=' . $leadContact->id }}">
                                @lang('modules.enquiry.changeToClient')
                            </a>
                        @endif
                    </div>
                </div>
            ...
            @if(!is_null($leadContact->added_by))
                <div class="col-12 px-0 pb-3 d-block d-lg-flex d-md-flex">
                    ...
                    <p class="mb-0 text-dark-grey f-14 ">
                        <x-cleaner :user="$leadContact->addedBy" />
                    </p>
                </div>
            @endif
            ...
            @if(!is_null($leadContact->lead_owner))
                <div class="col-12 px-0 pb-3 d-block d-lg-flex d-md-flex">
                    <p class="mb-0 text-lightest f-14 w-30 d-inline-block ">
                        @lang('app.owner')</p>
                    <p class="mb-0 text-dark-grey f-14 ">
                        <x-cleaner :user="$leadContact->leadOwner" />
                    </p>
                </div>
            @else
            ...
            <x-cards.data-row :label="__('modules.enquiry.source')" :value="$leadContact->leadSource ? $leadContact->leadSource->type : '--'" />

            <x-cards.data-row :label="__('modules.enquiry.companyName')" :value="!empty($leadContact->company_name) ? $leadContact->company_name : '--'" />

            <x-cards.data-row :label="__('modules.enquiry.website')" :value="$leadContact->website ?? '--'" />

            <x-cards.data-row :label="__('modules.enquiry.mobile')" :value="$leadContact->mobile ?? '--'" />

            <x-cards.data-row :label="__('modules.customer.officePhoneNumber')" :value="$leadContact->office ?? '--'" />
            ...
            <x-cards.data-row :label="__('modules.enquiry.address')" :value="$leadContact->address ?? '--'" />
