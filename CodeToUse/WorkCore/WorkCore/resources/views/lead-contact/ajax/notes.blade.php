        ...
        <div class="d-flex justify-content-between action-bar">
            <div id="table-actions" class="d-flex align-items-center">
                @if ($addLeadNotePermission == 'all' || $addLeadNotePermission == 'added' || $addLeadNotePermission == 'both')
                    <x-forms.link-primary :link="route('enquiry-notes.create').'?enquiry='.$leadContact->id"
...
<script>
    $('#enquiry-notes-table').on('preXhr.dt', function(e, settings, data) {
        var leadID = "{{ $leadContact->id }}";
        data['leadID'] = leadID;
    });

    const showTable = () => {
        window.LaravelDataTables["enquiry-notes-table"].draw(true);
    }
    ...
    $('body').on('click', '.delete-table-row-enquiry', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                var url = "{{ route('enquiry-notes.destroy', ':id') }}";
    ...
    const applyQuickAction = () => {
        var rowdIds = $("#enquiry-notes-table input:checkbox:checked").map(function() {
            return $(this).val();
        }).get();

        var url = "{{ route('enquiry-notes.apply_quick_action') }}?row_ids=" + rowdIds;
    ...
    $('body').on('click', '.ask-for-password', function() {
        let leadNoteId = $(this).data('enquiry-note-id');

        var url = "{{ route('enquiry-notes.ask_for_password', ':id') }}";
    ...
    var getNoteDetail = function(id) {
        ...
        var url = "{{ route('enquiry-notes.show', ':id') }}";
        url = url.replace(':id', id);
