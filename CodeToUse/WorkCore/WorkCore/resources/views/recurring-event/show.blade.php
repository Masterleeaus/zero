...
@php
$viewEventPermission = user()->permission('view_events');
@endphp
...
@section('filter-section')
    ...
    <div class="d-flex filter-box site-header bg-white">
        ...
        <div class="site-menu d-lg-flex" id="mob-customer-detail">
            ...
            <x-tab :href="route('recurring-event.show', $event->id)"
                :text="__('app.menu.eventRecurring').' '.__('app.info')" class="overview" />

            @if ($viewEventPermission != 'none')
                <x-tab :href="route('recurring-event.show', $event->id).'?tab=event'" ajax="false"
                    :text="__('app.menu.eventRecurring')" class="event" />
            @endif
