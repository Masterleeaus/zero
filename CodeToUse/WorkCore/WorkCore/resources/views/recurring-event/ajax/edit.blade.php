            ...
            <div class="add-customer bg-white rounded">
                ...
                <div class="row p-20">
                    ...
                    <div class="col-md-4">
                        <div class="form-group my-3">
                            <x-forms.label fieldId="colorselector" fieldRequired="true"
                                :fieldLabel="__('modules.service jobs.labelColor')">
                            </x-forms.label>
                    ...
                    <div class="col-md-12">
                        ...
                        <x-forms.input-group>
                            <select class="form-control multiple-users emp-event-zone" multiple name="team_id[]" id="employee_department"
                                    data-live-search="true">
                                @foreach ($teams as $team)
                                    <option
                                    data-content="<span class='p-2 border badge badge-pill badge-light'>{{ $team->team_name }}</span>"
                                    value="{{ $team->id }}" @if(in_array($team->id, $zones ?? [])) selected @endif>{{ $team->team_name }}</option>
                                @endforeach
                            </select>
                        </x-forms.input-group>
                    </div>

                    <div class="{{!in_array('customer',user_roles()) ? 'col-md-6' : 'col-md-12'}}">
                        <div class="form-group my-3">
                            @if(in_array('customer',user_roles()) || (!in_array('admin',user_roles()) && in_array('cleaner',user_roles()) && $viewEmployeePermission != 'all'))
                                <x-forms.text :fieldLabel="__('app.select').' '.__('app.cleaner')" fieldReadOnly="true"
                                ...
                                <x-forms.input-group>
                                    @php
                                        ...
                                        if ($hasDeactivatedSelected) {
                                            $deactivatedSelectedEmployees = $selectedEmployees->filter(function ($cleaner) use ($attendeeArray) {
                                                return in_array($cleaner->id, $attendeeArray) && $cleaner->status === 'deactive';
                                            });
                ...
                @if(!in_array('customer', user_roles()) && in_array('customers', user_modules()))
                    <div class="col-md-6">
                        <div class="form-group my-3">
                            @if((!in_array('admin',user_roles()) && in_array('cleaner',user_roles()) && $viewClientPermission != 'all'))
                                <x-forms.text :fieldLabel="__('app.select').' '.__('app.customer')" fieldReadOnly="true"
                                fieldName="customers" fieldId="selectAssignee2" :fieldValue="!($clientIds->isEmpty()) ? $clientIds->pluck('user.name')->implode(', ') : __('placeholders.noneSelectedText')" />

                                @foreach ($clientIds as $customer)
                                    <input type="hidden" name="user_id[]" value="{{ $customer->user_id }}">
                                @endforeach
                            @else
                                <x-forms.label fieldId="selectAssignee2"
                                    :fieldLabel="__('app.select').' '.__('app.customer')">
                                </x-forms.label>
                                <x-forms.input-group>
                                    <select class="form-control multiple-users" multiple name="user_id[]"
                ...
                @endif
                        ...
                        <div class="form-group my-3">
                            ...
                            @else
                                ...
                                <x-forms.input-group>
                                    ...
                                    $selectedEmployee = $cleaners->firstWhere('id', $event->host);
                    ...
                    @if($event->parent_id == null)
                        <div class="col-lg-2 my-3">
                            <x-forms.checkbox :fieldLabel="__('modules.events.repeat')" fieldName="repeat"
                                :popover="$event->parent_id !== null ? __('team chat.repeatEvent') : null"
                    ...
                    <div class="col-lg-3 my-3">
                        <x-forms.checkbox :fieldLabel="__('modules.service jobs.reminder')" fieldName="send_reminder"
    ...
    $(document).ready(function() {
        ...
        Dropzone.autoDiscover = false;
            ...
            eventDropzone.on('queuecomplete', function() {
                ...
                window.location.href = "{{ route('recurring-event.index') }}"
            });
