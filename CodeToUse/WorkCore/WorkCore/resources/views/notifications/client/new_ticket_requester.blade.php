...
if (!isset($notification->data['subject'])) {
    $ticketReply = \App\Models\Issue / Support::find($notification->data['id']);
...
@if ($notificationUser)
    <x-cards.notification :notification="$notification"  :link="route('issues / support.show', $notification->data['ticket_number'])" :image="$notificationUser->image_url"
