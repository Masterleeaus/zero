...
@section('filter-section')
    ...
    <x-filters.filter-box>
        ...
        <div class="select-box d-flex pr-2 border-right-grey border-right-grey-sm-0">
            <p class="mb-0 pr-2 f-14 text-dark-grey d-flex align-items-center">@lang('modules.customer.addedOn')</p>
        ...
        <div class="select-box d-flex py-2 px-lg-2 px-md-2 px-0 border-right-grey border-right-grey-sm-0">
            <p class="mb-0 pr-2 f-14 text-dark-grey d-flex align-items-center">@lang('app.customer')</p>
            <div class="select-status">
                <select class="form-control select-picker" name="customer" id="customer" data-live-search="true" data-size="8">
                    <option value="all">@lang('app.all')</option>
                    @foreach ($customers as $customer)
                        <x-user-option :user="$customer" />
                    @endforeach
                </select>
            </div>
        </div>
        ...
        <x-filters.more-filter-box>
            ...
            <div class="more-filter-items">
                ...
                <div class="select-filter mb-4">
                    <div class="select-others select-filter-site">
            ...
            <div class="more-filter-items">
                <label class="f-14 text-dark-grey mb-12 "
                    for="usr">@lang('modules.service agreements.contractType')</label>
                <div class="select-filter mb-4">
                    <div class="select-others">
                        <select class="form-control select-picker" id="contract_type_id" data-live-search="true"
                            ...
                            @foreach ($service agreements as $service agreement)
                                <option value="{{ $service agreement->id }}">{{ $service agreement->name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
            ...
            <div class="more-filter-items">
                <label class="f-14 text-dark-grey mb-12 " for="usr">@lang('app.verify') <i class="fa fa-question-circle" data-toggle="popover" data-placement="top" data-content="@lang('team chat.clientFilterVerification')" data-html="true" data-trigger="hover"></i></label>
                <div class="select-filter mb-4">
                    <div class="select-others">
        ...
        <div class="d-grid d-lg-flex d-md-flex action-bar">
            ...
            <div id="table-actions" class="flex-grow-1 align-items-center">
                @if ($addClientPermission == 'all' || $addClientPermission == 'added' || $addClientPermission == 'both')
                    <x-forms.link-primary :link="route('customers.create')" class="mr-3 openRightModal float-left mb-2 mb-lg-0 mb-md-0" icon="plus">
                        @lang('app.addClient')
                    </x-forms.link-primary>
                @endif

                @if ($addClientPermission == 'all' || $addClientPermission == 'added' || $addClientPermission == 'both')
                    <x-forms.link-secondary :link="route('customers.import')" class="mr-3 float-left mb-2 mb-lg-0 mb-md-0 d-sm-bloc d-none d-lg-block" icon="file-upload">
            ...
            <div class="btn-group mt-2 mt-lg-0 mt-md-0 ml-0 ml-lg-3 ml-md-3" role="group">
                <a href="{{ route('customers.index') }}" class="btn btn-secondary f-14 btn-active show-customers" data-toggle="tooltip"
                    data-original-title="@lang('app.menu.customers')"><i class="side-icon bi bi-list-ul"></i></a>
...
@push('scripts')
    ...
    <script>
        ...
        const showTable = () => {
            window.LaravelDataTables["customers-table"].draw(true);
        }

        $('#customer, #status, #filter_category_id, #filter_sub_category_id, #project_id, #contract_type_id, #country_id, #verification')
            .on('change keyup', function() {
                if ($('#status').val() !== "all") {
                    $('#reset-filters').removeClass('d-none');
                } else if ($('#customer').val() !== "all") {
                    $('#reset-filters').removeClass('d-none');
        ...
        $('#reset-filters,#reset-filters-2').click(function() {
            ...
            $('.show-customers').addClass("btn-active");
            $('#reset-filters').addClass('d-none');
            showTable();
        });
        ...
        $('body').on('click', '.verify-user', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    var url = "{{ route('customers.approve', ':id') }}";
        ...
        $('body').on('click', '.delete-table-row', function() {
            const id = $(this).data('user-id');
            var url = "{{ route('customers.finance_count', ':id') }}";
            url = url.replace(':id', id)
            var token = "{{ csrf_token() }}";
            $.easyAjax({
                type: 'GET',
                ...
                success: function(response) {
                    if (response.status == "success") {
                        Swal.fire({
                            title: "@lang('team chat.sweetAlertTitle')",
                            text: response.deleteClient + "@lang('team chat.recoverRecord')",
                            icon: 'warning',
                            showCancelButton: true,
                            focusConfirm: false,
                            confirmButtonText: "@lang('team chat.confirmDelete')",
        ...
        const applyQuickAction = () => {
            var rowdIds = $("#customers-table input:checkbox:checked").map(function() {
                return $(this).val();
            }).get();

            const url = "{{ route('customers.apply_quick_action') }}?row_ids=" + rowdIds;
