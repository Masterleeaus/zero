...
<div class="row">
    <div class="col-sm-12">
        <x-form id="save-customer-data-form">

            <div class="add-customer bg-white rounded">
                <h4 class="mb-0 p-20 f-21 font-weight-normal  border-bottom-grey">
                    @lang('modules.cleaners.accountDetails')</h4>

                @if (isset($enquiry->id)) <input type="hidden" name="enquiry"
                        value="{{ $enquiry->id }}"> @endif
                            ...
                            <div class="col-md-4">
                                <x-forms.select fieldId="salutation" fieldName="salutation"
                                    ...
                                    @foreach ($salutations as $salutation)
                                        <option value="{{ $salutation->value }}" @selected(isset($enquiry) && $salutation == $enquiry->salutation)>{{ $salutation->label() }}</option>
                                    @endforeach
                                </x-forms.select>
                            </div>
                            <div class="col-md-4">
                                <x-forms.text fieldId="name" :fieldLabel="__('modules.customer.clientName')" fieldName="name"
                                    ...
                                    :fieldValue="$enquiry->client_name ?? ''"></x-forms.text>
                            </div>
                            <div class="col-md-4">
                                <x-forms.email fieldId="email" :fieldLabel="__('app.email')" fieldName="email"
                                    ...
                                    :fieldValue="$enquiry->client_email ?? ''">
                                </x-forms.email>
                            </div>
                            <div class="col-md-4">
                                <x-forms.select fieldId="country" :fieldLabel="__('app.country')" fieldName="country"
                                    ...
                                    <option data-tokens="{{ $item->iso3 }}" data-phonecode = "{{$item->phonecode}}"
                                        ...
                                        @selected(isset($enquiry) && $item->nicename == $enquiry->country || ($item->id == user()->country_id))
                        ...
                        <x-forms.input-group>
                            <select class="form-control select-picker" name="category_id" id="category_id"
                                ...
                                @foreach ($categories as $category)
                                    <option @selected(isset($enquiry) && $enquiry->category_id == $category->id) value="{{ $category->id }}">
                            ...
                            @if ($addClientCategoryPermission == 'all')
                                <x-slot name="append">
                                    <button id="addClientCategory" type="button"
                                        ...
                                        data-toggle="tooltip" data-original-title="{{ __('app.add').' '.__('modules.customer.clientCategory') }}">
                                        @lang('app.add')</button>
                                </x-slot>
                            @endif
                        </x-forms.input-group>
                            ...
                            @if ($addClientSubCategoryPermission == 'all')
                                <x-slot name="append">
                                    <button id="addClientSubCategory" type="button"
                                        class="btn btn-outline-secondary border-grey"
                                        data-toggle="tooltip" data-original-title="{{ __('app.add').' '.__('modules.customer.clientSubCategory') }}"
                                        >@lang('app.add')</button>
                                </x-slot>
                            @endif
                        </x-forms.input-group>
                ...
                <div class="row p-20">
                    <div class="col-md-3">
                        <x-forms.text class="mb-3 mt-3 mt-lg-0 mt-md-0" fieldId="company_name"
                            ...
                            :fieldPlaceholder="__('placeholders.company')" :fieldValue="$enquiry->company_name ?? ''">
                        </x-forms.text>
                    </div>
                    <div class="col-md-3">
                        <x-forms.text class="mb-3 mt-3 mt-lg-0 mt-md-0" fieldId="website"
                            :fieldLabel="__('modules.customer.website')" fieldName="website"
                            :fieldPlaceholder="__('placeholders.website')" :fieldValue="$enquiry->website ?? ''">
                        </x-forms.text>
                    </div>
                    <div class="col-md-3">
                    ...
                    <div class="col-md-3">
                        <x-forms.text fieldId="office" :fieldLabel="__('modules.customer.officePhoneNumber')"
                            fieldName="office" :fieldPlaceholder="__('placeholders.mobileWithPlus')" :fieldValue="$enquiry->office ?? ''">
                        </x-forms.text>
                    </div>
                    <div class="col-md-3">
                        <x-forms.text fieldId="city" :fieldLabel="__('modules.stripeCustomerAddress.city')"
                            fieldName="city" :fieldPlaceholder="__('placeholders.city')" :fieldValue="$enquiry->city ?? ''">
                        </x-forms.text>
                    </div>
                    <div class="col-md-3">
                        <x-forms.text fieldId="state" :fieldLabel="__('modules.stripeCustomerAddress.state')"
                            fieldName="state" :fieldPlaceholder="__('placeholders.state')" :fieldValue="$enquiry->state ?? ''">
                        </x-forms.text>
                    </div>
                    <div class="col-md-3">
                        <x-forms.text fieldId="postalCode" :fieldLabel="__('modules.stripeCustomerAddress.postalCode')"
                            ...
                            :fieldValue="$enquiry->postal_code ?? ''">
                        </x-forms.text>
                    </div>
                    ...
                    <div class="col-md-6">
                        <div class="form-group my-3">
                            <x-forms.textarea class="mr-0 mr-lg-2 mr-md-2" :fieldLabel="__('app.shippingAddress')"
                                ...
                                :fieldPlaceholder="__('placeholders.address')" :fieldValue="$enquiry->address ?? ''">
                            </x-forms.textarea>
                        </div>
                    </div>
                    ...
                    <div class="col-lg-12">
                        <x-forms.file allowedFileExtensions="png jpg jpeg svg bmp" class="mr-0 mr-lg-2 mr-md-2"
                                               ...
                                               :fieldValue="(company()->logo_url)" fieldId="company_logo" :popover="__('team chat.fileFormat.ImageFile')"/>
                    </div>

                    @includeIf('einvoice::form.customer-create')
                ...
                <x-form-actions>
                    <x-forms.button-primary id="save-customer-form" class="mr-3" icon="check">@lang('app.save')
                    </x-forms.button-primary>
                    <x-forms.button-secondary class="mr-3" id="save-more-customer-form" icon="check-double">@lang('app.saveAddMore')
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#save-more-customer-form').click(function () {
            ...
            const url = "{{ route('customers.store') }}?add_more=true";
            // var data = $('#save-customer-data-form').serialize() + '&add_more=true';
            var data = $('#save-customer-data-form').serialize();

            // console.log(data);
            saveClient(data, url, "#save-more-customer-form");
        ...
        $('#save-customer-form').click(function() {
            ...
            saveClient(data, url, "#save-customer-form");
        ...
        function saveClient(data, url, buttonSelector) {
            $.easyAjax({
                url: url,
                container: '#save-customer-data-form',
                type: "POST",
                disableButton: true,
                blockUI: true,
