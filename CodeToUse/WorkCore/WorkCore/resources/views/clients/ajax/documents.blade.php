...
<div class="tab-pane fade show active mt-5" role="tabpanel" aria-labelledby="nav-email-tab">
    <x-cards.data :title="__('app.menu.documents')">
        ...
        @if ($addDocumentPermission == 'all')
            <div class="row">
                <div class="col-md-12">
                    <a class="f-15 f-w-500" href="javascript:;" id="add-customer-file"><i
                            class="icons icon-plus font-weight-bold mr-1"></i>
                        @lang('app.menu.addFile')</a>
                </div>
            </div>

            <x-form id="save-customer-file-data-form" class="d-none">
                ...
                <div class="row">
                    ...
                    <div class="col-md-12">
                        <x-forms.file :fieldLabel="__('modules.sites.uploadFile')" fieldName="file"
                            fieldRequired="true" fieldId="client_file" allowedFileExtensions="txt pdf doc xls xlsx docx rtf png jpg jpeg svg" :popover="__('team chat.fileFormat.multipleImageFile')" />
                    </div>
                    <div class="col-md-12">
        ...
        <div class="d-flex flex-wrap mt-3" id="service job-file-list">
            ...
            @endphp
            @forelse($customer->clientDocuments as $file)
                                    ...
                                    <div class="dropdown-menu dropdown-menu-right border-grey rounded b-shadow-4 p-0"
                                        ...
                                        || ($viewDocumentPermission == 'both' && ($file->added_by == user()->id || $file->user_id == user()->id)))
                                            ...
                                            <a class="cursor-pointer d-block text-dark-grey f-13 py-3 px-3 "
                                                href="{{ route('customer-docs.download', md5($file->id)) }}">@lang('app.download')</a>
                                        @endif
...
<script>
    $('#add-customer-file').click(function() {
        ...
        $('#save-customer-file-data-form').removeClass('d-none');
    });

    $('body').on('click', '.edit-file', function() {
        var fileId = $(this).data('file-id');
        var url = "{{ route('customer-docs.edit', ':id') }}";
    ...
    $('#cancel-document').click(function() {
        $('#save-customer-file-data-form').addClass('d-none');
        $('#add-customer-file').closest('.row').removeClass('d-none');
    });

    $('#submit-document').click(function() {
        var url = "{{ route('customer-docs.store') }}";

        $.easyAjax({
            url: url,
            container: '#save-customer-file-data-form',
            type: "POST",
            disableButton: true,
            ...
            success: function(response) {
                if (response.status == 'success') {
                    ...
                    $('#save-customer-file-data-form')[0].reset();
    ...
    $('body').on('click', '.delete-file', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                var url = "{{ route('customer-docs.destroy', ':id') }}";
