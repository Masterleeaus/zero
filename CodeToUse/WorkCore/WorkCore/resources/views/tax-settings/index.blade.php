                ...
                <div class="table-responsive">
                    <x-table class="table-bordered table-hover">
                        <x-slot name="thead">
                            ...
                            <th class="text-center">@lang('modules.invoices.toKeepUnderTotalPayment')</th>
                        ...
                        @forelse($taxes as $key => $tax)
                            <tr id="tax-{{ $tax->id }}">
                                ...
                                <td class="text-center">
                                    <input type="checkbox"
                                           class="tax-to-keep-under-total-payment"
                                           data-tax-id="{{ $tax->id }}"
                                           @checked($tax->to_keep_under_total_payment)
                                           title="@lang('modules.invoices.toKeepUnderTotalPayment')">
                                </td>
...
@push('scripts')
    ...
    <script>
        ...
        $('body').on('change', '.tax-to-keep-under-total-payment', function () {
            const id = $(this).data('tax-id');
            const checked = $(this).prop('checked');
            const url = "{{ route('taxes.update_to_keep_under_total_payment', ':id') }}".replace(':id', id);
            ...
            $.easyAjax({
                ...
                data: {
                    '_token': token,
                    '_method': 'PUT',
                    'to_keep_under_total_payment': checked ? '1' : '0'
                },
                ...
                error: function (response) {
                    $('.tax-to-keep-under-total-payment[data-tax-id="' + id + '"]').prop('checked', !checked);
                }
            });
        });
