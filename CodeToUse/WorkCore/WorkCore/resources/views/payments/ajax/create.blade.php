            ...
            <div class="add-customer bg-white rounded">
                    ...
                    <div class="col-md-3">
                        ...
                        @if (isset($site) || !is_null($site))
                            <input type="hidden" id="payment_project_id" name="project_id"
                                value="{{ !is_null($site) ? $site->id : '' }}">
                            <x-forms.text :fieldLabel="__('app.site')" fieldName="projectName" fieldId="projectName"
                                :fieldValue="!is_null($site) ? $site->project_name : ''" fieldReadOnly="true" />
                        @else
                            <x-forms.select fieldId="payment_project_id" :fieldLabel="__('app.site')"
                                fieldName="project_id" search="true">
                                <option value="">--</option>
                                @foreach ($sites as $data)
                                    <option data-currency-id="{{ $data->currency_id }}" data-currency-code="{{ $data->currency->currency_code }}" @if (isset($site) && $data->id == $site->id) selected @endif
                                        value="{{ $data->id }}">
                ...
                <div class="row pl-20 pr-20">
                    ...
                    <div class="col-md-3">
                        ...
                        fieldName="exchange_rate" fieldRequired="true" :fieldValue="(isset($projectName) ? $site->currency->exchange_rate : (isset($invoice) ? $invoice->exchange_rate : $companyCurrency->exchange_rate))"
                    ...
                    <div class="col-lg-12">
                        <x-forms.file allowedFileExtensions="txt pdf doc xls xlsx docx rtf png jpg jpeg svg" class="mr-0 mr-lg-2 mr-md-2" :fieldLabel="__('app.receipt')" fieldName="bill"
                            fieldId="bill" :popover="__('team chat.fileFormat.multipleImageFile')" />
                    </div>
...
<script>
    $(document).ready(function() {
        ...
        $('#payment_project_id').change(function() {
            ...
            var url = "{{ route('sites.invoice_list', ':id') }}";
