...
<aside class="{{ !user()->dark_theme ? 'sidebar-' . $appTheme->sidebar_theme : '' }}">
        ...
        <div class="sidebar-brand-box dropdown cursor-pointer {{ user()->dark_theme ? 'bg-dark' : '' }}">
            ...
            <div class="dropdown-menu dropdown-menu-right sidebar-brand-dropdown ml-3"
                ...
                <div class="d-flex justify-content-between align-items-center profile-box">
                    <a @if(in_array('customer', user_roles())) href="{{ route('profile-settings.index') }}" @elseif (user()->is_superadmin) href="{{ route('superadmin.settings.super-admin-profile.index') }}" @else href="{{ route('cleaners.show', user()->id) }}" @endif >
                        <div class="profileInfo d-flex align-items-center mr-1 flex-wrap">
                            ...
                            <div class="ProfileData">
                                ...
                                <p class="mb-0 f-12 text-dark-grey">{{ user()->employeeDetail->role->name ?? '' }}</p>
                            </div>
                        </div>
                    </a>
                ...
                @if (checkCompanyCanAddMoreEmployees(user()->company_id))
                    @if (!in_array('customer', user_roles()) && ($sidebarUserPermissions['add_employees'] == 4 || $sidebarUserPermissions['add_employees'] == 1) && in_array('cleaners', user_modules()))
    ...
    @include('sections.raise-support-issue / support-modal')
</aside>
<!-- SIDEBAR END -->
...
<script>
    $(document).ready(function() {
        ...
        $('.invite-member').click(function() {
            const url = "{{ route('cleaners.invite_member') }}";
