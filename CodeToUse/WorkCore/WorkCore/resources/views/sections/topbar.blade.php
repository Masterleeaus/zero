...
<header class="main-header clearfix bg-white" id="header">
    ...
    <div class="page-header-right float-right d-flex align-items-center justify-content-end flex-nowrap">
        ...
        @if(isWorksuiteSaas())
            @if(session('impersonate'))
                <a href="{{ route('superadmin.superadmin.stop_impersonate') }}" class='btn-primary btn btn-sm rounded mr-2 f-12 py-2 px-2 px-md-3 header-saas-btn' data-toggle="tooltip" data-original-title="{{ __('superadmin.stopImpersonationTooltip') }}" data-placement="left">
                    <i class="fa fa-stop"></i>
                    <span class="d-none d-xl-inline">@lang('superadmin.stopImpersonation')</span>
                </a>
            @endif

            {{-- SAAS --}}
            @if (in_array(company()->package->default, ['yes']) && in_array('admin', user_roles()))
                <a href="{{ route('billing.upgrade_plan') }}" class='btn-primary btn btn-sm rounded mr-2 f-12 py-2 px-2 px-md-3 header-saas-btn' data-toggle="tooltip" data-original-title="{{ __('superadmin.packages.upgradePlan') }}" >
                    <i class="bi bi-stars"></i>
                    @if (is_null($selfActiveTimer)) <span class="d-none d-xl-inline">{{ __('superadmin.packages.upgradePlan') }}</span> @endif
                </a>
            @elseif (in_array(company()->package->default, ['trial']) && in_array('admin', user_roles()))
                <a href="{{ route('billing.upgrade_plan') }}" class='btn-light border btn btn-sm rounded mr-2 f-12 py-2 px-2 px-md-3 font-weight-semibold header-saas-btn' data-toggle="tooltip" data-original-title="{{ __('superadmin.packages.upgradePlan') }}" >
                    @php
                    ...
                    @if (is_null($selfActiveTimer)) <span class="d-none d-xl-inline">{{ ($daysLeftInTrial > 0) ? $daysLeftInTrial . ' ' . __('superadmin.packages.daysLeftTrial') : __('superadmin.packages.trialExpired') }}</span> @endif
                </a>
            @endif
                ...
                <li data-toggle="tooltip" data-placement="top" title="{{__('app.createNew')}}">
                    <div class="add_box dropdown">
                        ...
                        <div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenuLink" tabindex="0">
                            @if (in_array('sites', user_modules()) && (add_project_permission() == 'all' || add_project_permission() == 'added'))
                            ...
                            @if (in_array('service jobs', user_modules()) && (add_tasks_permission() == 'all' || add_tasks_permission() == 'added'))
                                <a class="dropdown-item f-14 text-dark openRightModal"
                                   href="{{ route('service jobs.create') }}">
                                    <i class="fa fa-plus f-w-500 mr-2 f-11"></i>
                                    @lang('app.addTask')
                                </a>
                            @endif

                            @if (in_array('customers', user_modules()) && (add_clients_permission() == 'all' || add_clients_permission() == 'added'))
                                <a class="dropdown-item f-14 text-dark openRightModal"
                            ...
                            @if (in_array('cleaners', user_modules()) && (add_employees_permission() == 'all' || add_employees_permission() == 'added'))
                                <a class="dropdown-item f-14 text-dark openRightModal"
                                   href="{{ route('cleaners.create') }}">
                                    <i class="fa fa-plus f-w-500 mr-2 f-11"></i>
                            ...
                            @if (in_array('issues / support', user_modules()) && (add_tickets_permission() == 'all' || add_tickets_permission() == 'added'))
                                <a class="dropdown-item f-14 text-dark openRightModal"
                                   href="{{ route('issues / support.create') }}">
                                    <i class="fa fa-plus f-w-500 mr-2 f-11"></i>
