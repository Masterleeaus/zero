@if (!is_null($selfActiveTimer) && in_array('service jobs', user_modules()))
...
// Check for site time limit every minute
...
function stopTimerOnTimeLimit() {
    // Stop the timer by calling the stop timer endpoint
    ...
    $.easyAjax({
        ...
        data: {
            timeId: timeId,
            memo: 'Timer stopped due to site time limit reached',
            _token: '{{ csrf_token() }}'
        },
        success: function (response) {
            if (response.status == 'success') {
                // Show warning modal
                showProjectTimeLimitModal(response.message || '@lang("team chat.projectTimeReached")');
...
function showProjectTimeLimitModal(message) {
    var modalHtml = `
        <div class="modal fade" id="projectTimeLimitModal" tabindex="-1" role="dialog">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">@lang('team chat.projectTimeReached')</h5>
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-primary" data-dismiss="modal">@lang('app.ok')</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    $('#projectTimeLimitModal').remove();
    
    // Add modal to body
    $('body').append(modalHtml);
    
    // Show modal
    $('#projectTimeLimitModal').modal('show');
}
