...
@section('filter-section')
    <x-filters.filter-box>
        ...
        <div class="select-box d-flex py-2 px-lg-2 px-md-2 px-0 border-right-grey border-right-grey-sm-0">
            <p class="mb-0 pr-2 f-14 text-dark-grey d-flex align-items-center">@lang('app.menu.zone')</p>
            <div class="select-status">
...
@push('scripts')
    <script>
        ...
        $('#user_id, #zone, #view_type').on('change', function() {
            if ($('#user_id').val() != "all") {
        ...
        @if (canDataTableExport())
            $('#export-all').click(function() {
                ...
                var url =
                    "{{ route('shifts.export_all', [':year', ':month', ':userId', ':zone', ':startDate', ':viewType']) }}";
                url = url.replace(':year', year).replace(':month', month).replace(':userId', userId).replace(':zone', zone).replace(':startDate', startDate).replace(':viewType', viewType);
