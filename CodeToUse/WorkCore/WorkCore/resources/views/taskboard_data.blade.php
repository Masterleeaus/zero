    ...
    <div class="board-panel rounded bg-additional-grey border-grey mr-3 column-max-{{ $column->id }}">
        ...
        <div class="b-p-body">
            ...
            <div class="b-p-service jobs" id="drag-container-{{ $column->id }}" data-column-id="{{ $column->id }}">
                @forelse ($column['service jobs'] as $service job)
                    <x-cards.public-service job-card :draggable="'false'" :service job="$service job"/>
                @empty
                    @if ($column->tasks_count == 0)
                        <div class="card rounded bg-white border-grey b-shadow-4 m-1 mb-3 no-service job-card move-disable">
                            <div class="card-body">
            ...
            @if ($column->tasks_count > count($column['service jobs']))
                ...
                <div class="d-flex m-3 justify-content-center">
                    <a class="f-13 text-dark-grey f-w-500 load-more-service jobs" data-column-id="{{ $column->id }}"
                       data-total-service jobs="{{ $column->tasks_count }}"
