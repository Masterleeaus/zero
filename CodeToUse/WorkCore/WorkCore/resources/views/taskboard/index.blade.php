                    ...
                    <div class="select-others">
                        <select class="form-control select-picker" id="assignedTo" data-live-search="true" data-container="body" data-size="8">
                            ...
                            @foreach ($cleaners as $cleaner)
                                <x-user-option :user="$cleaner"></x-user-option>
                            @endforeach
...
@section('content')
    ...
    <div class="w-service job-board-box px-4 py-2 bg-white">
        <!-- Add Service Job Export Buttons Start -->
        <div class="d-grid d-lg-flex d-md-flex action-bar my-3">

            <x-alert type="warning" icon="info" class="d-lg-none">@lang('team chat.dragDropScreenInfo')</x-alert>

            <div id="table-actions" class="flex-grow-1 align-items-center mb-2 mb-lg-0 mb-md-0">
                @if ($addTaskPermission == 'all' || $addTaskPermission == 'added')
                    <x-forms.link-primary :link="route('service jobs.create')" class="mr-lg-3 mr-1 openRightModal float-left" icon="plus" data-redirect-url="{{ url()->full() }}">
                        @lang('app.add')
                        @lang('app.service job')
                    </x-forms.link-primary>
                @endif

                @if (user()->permission('add_status') == 'all')
                    <x-forms.button-secondary icon="plus" class="mr-lg-3 mr-1 float-left" id="add-column">
                        @lang('modules.service jobs.addBoardColumn')
                    </x-forms.button-secondary>
                @endif


                @if (!in_array('customer', user_roles()))
                    <x-forms.button-secondary id="filter-my-service job" icon="user">
            ...
            <div class="btn-group mt-2 mt-lg-0 mt-md-0 ml-0 ml-lg-3 ml-md-3" role="group">
                <a href="{{ route('service jobs.index') }}" class="btn btn-secondary f-14" data-toggle="tooltip"
        ...
        <x-alert type="warning">
            <div><b>Note:</b>@lang('modules.service jobs.taskboardNote') <a href="{{ route('service jobs.index') }}">@lang('app.service jobs')</a> @lang('modules.service jobs.menu') </div>
        </x-alert>

        <div class="w-service job-board-panel d-flex" id="taskboard-columns">
...
@push('scripts')
    ...
    <script>
        ...
        $('body').on('click', '.load-more-service jobs', function() {
            var columnId = $(this).data('column-id');
            var totalTasks = $(this).data('total-service jobs');
            var currentTotalTasks = $('#drag-container-' + columnId + ' .service job-card').length;
            ...
            $.easyAjax({
                ...
                success: function(response) {
                    ...
                    if (response.load_more == 'show') {
                        $('#drag-container-' + columnId).closest('.b-p-body').find('.load-more-service jobs');
        ...
        if ((pusher_setting.status === 1 && pusher_setting.taskboard === 1) || (pusher_setting.status == "1" && pusher_setting.taskboard == "1")) {
            ...
            var channel = pusher.subscribe('service job-updated-channel');
                channel.bind('service job-updated', function (data) {
                loadData()
            });
        }
