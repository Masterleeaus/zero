    ...
    @if ($column->userSetting && $column->userSetting->collapsed)
        ...
        <div class="minimized rounded bg-additional-grey border-grey mr-3">
            ...
            <div class="d-flex mt-4 mx-1 b-p-header align-items-center">
                ...
                <span class="b-p-badge bg-grey f-13 px-2 py-2 text-lightest font-weight-bold rounded d-inline-block" id="service job-column-count-{{ $column->id }}">{{ $column->tasks_count }}</span>
    ...
    @else
        ...
        <div class="board-panel rounded bg-additional-grey border-grey mr-3">
            ...
            <div class="d-flex m-3 b-p-header">
                ...
                <span
                    class="b-p-badge bg-grey f-13 px-2 text-lightest font-weight-bold rounded d-inline-block" id="service job-column-count-{{ $column->id }}">{{ $column->tasks_count }}</span>
                            ...
                            <div class="dropdown-menu dropdown-menu-right border-grey rounded b-shadow-4 p-0"
                                ...
                                @if (($addTaskPermission == 'all' || $addTaskPermission == 'added') && $column->slug != 'waiting_approval')
                                    <a class="dropdown-item openRightModal"
                                        href="{{ route('service jobs.create') }}?column_id={{ $column->id }}">@lang('app.addTask')
                                    </a>
                                @endif
            ...
            <div class="b-p-body">
                <!-- MAIN TASKS START -->
                <div class="b-p-service jobs" id="drag-container-{{ $column->id }}" data-column-id="{{ $column->id }}" data-status="{{ $column->slug }}">
                    <div
                        class="card rounded bg-white border-grey b-shadow-4 m-1 mb-3 no-service job-card move-disable {{ ($column->tasks_count > 0) ? 'd-none' : '' }}">
                        <div class="card-body">
                            <div class="d-flex justify-content-center py-3">
                                <p class="mb-0">
                                    @if ($addTaskPermission == 'all' || $addTaskPermission == 'added')
                                        @if (isset($site))
                                            @if($column->slug == 'waiting_approval')
                                                <div class="align-items-center d-flex flex-column text-lightest w-100">
                                                    <i class="fa fa-service jobs f-15 w-100"></i>
                                                    <div class="f-15 mt-4">
                                                        - @lang('team chat.noRecordFound') -
                                                    </div>
                                                </div>
                                            @else
                                                <a href="{{ route('service jobs.create') }}?column_id={{ $column->id }}&task_project_id={{ $site->id }}" class="text-dark-grey openRightModal"><i class="fa fa-plus mr-2"></i>@lang('app.add')
                                                @lang('app.service job')</a>
                                            @endif
                                        @elseif(isset($site) == false && $column->slug == 'waiting_approval')
                                            <div class="align-items-center d-flex flex-column text-lightest w-100">
                                                <i class="fa fa-service jobs f-15 w-100"></i>
                                                <div class="f-15 mt-4">
                                                    - @lang('team chat.noRecordFound') -
                                        ...
                                        @elseif(isset($site) == false && $column->slug != 'waiting_approval')
                                            <a href="{{ route('service jobs.create') }}?column_id={{ $column->id }}" class="text-dark-grey openRightModal"><i class="fa fa-plus mr-2"></i>@lang('app.add')
                                            @lang('app.service job')</a>
                                        @endif
                                    @else
                                        <div class="align-items-center d-flex flex-column text-lightest w-100">
                    ...
                    @foreach ($column['service jobs'] as $service job)
                        @php
                            $taskUsers = $service job->users ? $service job->users->pluck('id')->toArray() : [];
                        @endphp

                        <x-cards.service job-card :draggable="(($changeStatusPermission == 'all'
                        ...
                        || ($service job->site && $service job->site->project_admin == user()->id)) ? 'true' : 'false')"
                            :service job="$service job" :company="$company"/>
                    @endforeach
                </div>
                <!-- MAIN TASKS END -->

                @if ($column->tasks_count > count($column['service jobs']))
                    <!-- TASK BOARD FOOTER START -->
                    <div class="d-flex m-3 justify-content-center">
                        <a class="f-13 text-dark-grey f-w-500 load-more-service jobs" data-column-id="{{ $column->id }}"
                            data-total-service jobs="{{ $column->tasks_count }}" data-status="{{ $column->status }}"
                            href="javascript:;">@lang('modules.service jobs.loadMore')</a>
                    </div>
...
<script>
    var arraylike = document.getElementsByClassName('b-p-service jobs');
...
<script>
    drake.on('drop', function(element, target, source, sibling) {
        ...
        var movingTaskId = $('#' + element.id).data('service job-id');

        var sourceBoardColumnId = $('#' + source.id).data('column-id');
        var sourceColumnCount = parseInt($('#service job-column-count-' + sourceBoardColumnId).text());
        var targetColumnCount = parseInt($('#service job-column-count-' + boardColumnId).text());
        ...
        $children.each(function(ind, el) {
            taskIds.push($(el).data('service job-id'));
        ...
        if((sourceStatus == 'waiting_approval') || (targetStatus == 'waiting_approval')){
            drake.cancel(true);
            Swal.fire({
                title: "@lang('team chat.youCannotMoveTask')",
                icon: 'warning',
            ...
            $.easyAjax({
                ...
                success: function(response) {
                    ...
                    if ($('#' + source.id + ' .service job-card').length == 0) {
                        $('#' + source.id + ' .no-service job-card').removeClass('d-none');
                    }
                    if ($('#' + target.id + ' .service job-card').length > 0) {
                        $('#' + target.id + ' .no-service job-card').addClass('d-none');
                    }

                    $('#service job-column-count-' + sourceBoardColumnId).text(sourceColumnCount - 1);
                    $('#service job-column-count-' + boardColumnId).text(targetColumnCount + 1);
                }
            });
        }
