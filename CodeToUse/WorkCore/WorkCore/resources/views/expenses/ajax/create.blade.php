                ...
                <div class="row p-20">
                    ...
                    <div class="col-md-6 col-lg-3">
                        @if(isset($projectName))
                            <input type="hidden" id="currency_id" name="currency_id" value="{{ $site->currency_id}}">
                            <x-forms.text :fieldLabel="__('modules.invoices.currency')" fieldName="site-currency" fieldId="site-currency" :fieldValue="$site->currency->currency_name" fieldReadOnly="true" />
                        @else
                    ...
                    <div class="col-md-6 col-lg-3">
                        ...
                        fieldName="exchange_rate" fieldRequired="true" :fieldValue="(isset($projectName) ? $site->currency->exchange_rate : $companyCurrency->exchange_rate)" fieldReadOnly="true"
                        :fieldHelp="' '"/>
                    </div>
                    ...
                    @if (user()->permission('add_expenses') == 'all')
                        <div class="col-md-6 col-lg-4">
                            <x-forms.label class="mt-3" fieldId="user_id" :fieldLabel="__('app.cleaner')">
                            </x-forms.label>
                            <x-forms.input-group>
                    ...
                    <div class="col-md-6 col-lg-4">
                        @if(isset($projectName))
                            ...
                            <x-forms.text :fieldLabel="__('app.site')" fieldName="projectName" fieldId="projectName" :fieldValue="$projectName" fieldReadOnly="true" />
                        @else
                            <x-forms.select fieldId="project_id" fieldName="project_id" :fieldLabel="__('app.site')"
                                search="true">
                                <option value="">--</option>
                                @foreach ($sites as $site)
                                    <option data-currency-id="{{ $site->currency_id }}" @selected ($projectId == $site->id) value="{{ $site->id }}">
                                        {{ $site->project_name }}
                                    </option>
                                @endforeach
                            </x-forms.select>
                    ...
                    <div class="col-lg-12">
                        <x-forms.file :fieldLabel="__('app.bill')" fieldName="bill" fieldId="bill" allowedFileExtensions="txt pdf doc xls xlsx docx rtf png jpg jpeg svg" :popover="__('team chat.fileFormat.multipleImageFile')" />
                    </div>
                </div>
...
<script>
    $(document).ready(function() {
        ...
        $('body').on('change', '#expense_category_id', function(){
            ...
            $.easyAjax({
                ...
                success: function(response) {
                    $('#user_id').html('<option value="">--</option>'+response.cleaners);
    ...
    @if(isset($projectName))
        ...
        function setExchangeRateHelp(){
            ...
            var currentCurrencyName = `{{ $site->currency->currency_name }}` ;
