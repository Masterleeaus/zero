            ...
            <div class="border-0 card invoice">
                                    ...
                                    <table class="mt-3 inv-num-date text-dark f-13">
                                        <tr>
                                            <td class="bg-light-grey border-right-0 f-w-500">
                                                @lang('modules.quotes.estimatesNumber')</td>
                                            <td class="border-left-0">{{ $quote->estimate_number }}</td>
                                        </tr>
                                        <tr>
                                            <td class="bg-light-grey border-right-0 f-w-500">
                                                @lang('modules.quotes.validTill')</td>
                        ...
                        <table width="100%">
                            <tr class="inv-unpaid">
                                    ...
                                    @if (($quote->customer || $quote->clientDetails) && ($quote->customer->name || $quote->customer->email || $quote->customer->mobile || $quote->clientDetails->company_name || $quote->clientDetails->address) && ($invoiceSetting->show_client_name == 'yes' || $invoiceSetting->show_client_email == 'yes' || $invoiceSetting->show_client_phone == 'yes' || $invoiceSetting->show_client_company_name == 'yes' || $invoiceSetting->show_client_company_address == 'yes'))
                                        <p class="mb-0 text-left">
                                            ...
                                            @if ($quote->customer && $quote->customer->mobile && $invoiceSetting->show_client_phone == 'yes')
                                                {{ $quote->customer->mobile_with_phonecode }}
                                                <br>
                                            @endif
                                            ...
                                            @if ($quote->clientDetails && $quote->clientDetails->address && $invoiceSetting->show_client_company_address == 'yes')
                                                {!! nl2br($quote->clientDetails->address) !!}<br><br>
                                            @endif
                                            @if ($quote->clientDetails && $quote->clientDetails->gst_number && invoice_setting()->show_gst == 'yes')
                                                {{ $quote->clientDetails->tax_name }}: {{ $quote->clientDetails->gst_number }}<br>
                                            @endif
                                        </p>
                                    @endif
                                </td>
                                ...
                                <td align="right" class="mt-4 mt-lg-0 mt-md-0">
                                    ...
                                    @endif
                                    <span
                                        class="unpaid {{ $quote->status == 'draft' ? 'text-primary border-primary' : '' }} {{ $quote->status == 'accepted' ? 'text-success border-success' : '' }} rounded f-15 ">@lang('modules.quotes.'.$quote->status)</span>
                                </td>
                            </tr>
                            <tr>
                                            ...
                                            @if ($item->type == 'item')
                                                <tr class="text-dark font-weight-semibold f-13">
                                                    ...
                                                    <td align="right">{{ $item->quantity }}@if($item->unit)
                                                        ...
                                                        @endif</td>
                                                    <td align="right">
                                                        {{ currency_format($item->unit_price, $quote->currency_id, false) }}
                                                    </td>
                                                    <td align="right">{{ $item->tax_list }}</td>
                                                    <td align="right">
                                                        {{ currency_format($item->amount, $quote->currency_id, false) }}
                                                    </td>
                                                </tr>
                                                ...
                                                <table width="100%">
                                                    <tr class="text-dark-grey" align="right">
                                                        ...
                                                        <td class="border-top-0 border-right-0">
                                                            {{ currency_format($quote->sub_total, $quote->currency_id, false) }}
                                                        </td>
                                                    </tr>
                                                    @if ($discount != 0 && $discount != '')
                                                        <tr class="text-dark-grey" align="right">
                                                            ...
                                                            <td class="border-top-0 border-right-0">
                                                                {{ currency_format($discount, $quote->currency_id, false) }}
                                                            </td>
                                                        </tr>
                                                    @endif
                                                    @foreach ($taxes as $key => $tax)
                                                        <tr class="text-dark-grey" align="right">
                                                            <td class="border-top-0 border-left-0">
                                                                {{ $key }}</td>
                                                            <td class="border-top-0 border-right-0">
                                                                {{ currency_format($tax, $quote->currency_id, false) }}
                                                            </td>
                                                        </tr>
                                                    @endforeach
                            ...
                            @foreach ($quote->items->sortBy('field_order') as $item)
                                @if ($item->type == 'item')
                                    ...
                                    <tr>
                                        ...
                                        <td width="50%">{{ currency_format($item->amount, $quote->currency_id, false) }}
                                        </td>
                                    </tr>
                                    <tr>
                            ...
                            @foreach ($taxes as $key => $tax)
                                <tr>
                                    <th width="50%" class="text-dark-grey font-weight-normal">
                                        {{ $key }}</th>
                                    <td width="50%" class="text-dark-grey font-weight-normal">
                                        {{ currency_format($tax, $quote->currency_id, false) }}</td>
                                </tr>
                            @endforeach
                            <tr>
                                ...
                                <td width="50%" class="text-dark-grey font-weight-bold">
                                    {{ currency_format($quote->total, $quote->currency_id, false) }}</td>
                            </tr>
                        </table>
                            ...
                            <tr>
                                <td style="vertical-align: text-top">
                                    <table>
                                        <tr>@lang('app.note')</tr>
                                        <tr>
                                            <p class="text-dark-grey">{!! $quote->note ?? '--' !!}</p>
                                        </tr>
                                    </table>
                                </td>
                                <td align="right">
                    ...
                    @if ($quote->sign)
                        <div class="row">
                            <div class="mt-4 col-sm-12">
                                ...
                                <img src="{{ $quote->sign->signature }}" style="width: 200px;">
                ...
                <div
                    ...
                    <div class="d-flex">
                        ...
                        <x-forms.button-cancel :link="route('quotes.index')" class="mr-3 border-0">
                            @lang('app.cancel')
                        </x-forms.button-cancel>

                        <x-forms.link-secondary :link="route('front.quote.download', [$quote->hash])" class="mr-3"
                                                icon="download">@lang('app.download')
                        </x-forms.link-secondary>

                        @if ($quote->status == 'waiting')
                            <x-forms.link-secondary link="javascript:;" class="mr-3" icon="times"
                                                    id="decline-quote">@lang('app.decline')
                            </x-forms.link-secondary>
...
<script>
    ...
    $(body).on('click', '#download-invoice', function () {
        window.location.href = "{{ route('invoices.download', [$quote->id]) }}";
    })
</script>
...
<script>
    ...
    $('#decline-quote').click(function () {
        $.easyAjax({
            type: 'POST',
            url: "{{ route('front.quote.decline', $quote->id) }}",
            blockUI: true,
            data: {
                _token: '{{ csrf_token() }}'
            },
    ...
    $('#save-signature').click(function () {
        ...
        $.easyAjax({
            url: "{{ route('front.quote.accept', $quote->id) }}",
