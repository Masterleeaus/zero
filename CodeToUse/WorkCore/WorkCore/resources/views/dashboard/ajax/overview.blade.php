...
<div class="row">
    @if (in_array('customers', user_modules()) && in_array('total_clients', $activeWidgets))
    ...
    @if (in_array('cleaners', user_modules()) && in_array('total_employees', $activeWidgets))
        <div class="col-xl-3 col-lg-6 col-md-6 mb-3">
    ...
    @if (in_array('sites', user_modules()) && in_array('total_projects', $activeWidgets))
        <div class="col-xl-3 col-lg-6 col-md-6 mb-3">
            <a href="{{ route('sites.index'). '?sites=all' }}">
    ...
    @if (in_array('service jobs', user_modules()) && in_array('total_pending_tasks', $activeWidgets))
        <div class="col-xl-3 col-lg-6 col-md-6">
            <a href="{{ route('service jobs.index') }}?status=pending_task&type=public">
                <x-cards.widget :title="__('modules.dashboard.totalPendingTasks')" :value="$counts->totalPendingTasks"
                                icon="service jobs" :info="__('modules.dashboard.privateTaskInfo')">
                </x-cards.widget>
            </a>
        </div>
    @endif
    ...
    @if (in_array('issues / support', user_modules()) && in_array('total_unresolved_tickets', $activeWidgets))
        <div class="col-xl-3 col-lg-6 col-md-6">
            <a href="{{ route('issues / support.index') . '?status=open' }}">
                <x-cards.widget :title="__('modules.issues / support.totalUnresolvedTickets')"
                                :value="floor($counts->totalOpenTickets)" icon="issue / support-alt">
                </x-cards.widget>
            </a>
        </div>
    @endif
...
<div class="row">
    @if (in_array('payments', user_modules()) && in_array('recent_earnings', $activeWidgets))
        <div class="col-sm-12 col-lg-6 mt-3">
            <x-cards.data
                ...
                <x-bar-chart id="service job-chart1" :chartData="$earningChartData" height="300"></x-bar-chart>
            </x-cards.data>
        </div>
    @endif
    ...
    @if (in_array('timelogs', user_modules()) && in_array('timelogs', $activeWidgets))
        <div class="col-sm-12 col-lg-6 mt-3">
            <x-cards.data
                ...
                <x-line-chart id="service job-chart2" :chartData="$timlogChartData" height="300"></x-line-chart>
            </x-cards.data>
        </div>
    @endif
                        ...
                        <tr>
                            <td colspan="5" class="shadow-none">
                                <x-cards.no-record icon="calendar"
                                                   :message="__('team chat.noRecordFound')"></x-cards.no-record>
                            </td>
                        </tr>
                    @endforelse
                </x-table>
    ...
    @if (in_array('issues / support', user_modules()) && in_array('new_tickets', $activeWidgets))
        <div class="col-sm-12 col-lg-6 mt-3">
            <x-cards.data
                ...
                <x-table>
                    @forelse ($newTickets as $item)
                        <tr>
                            ...
                            <td><a href="{{ route('issues / support.show', $item->ticket_number) }}"
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="shadow-none">
                                <x-cards.no-record icon="issue / support-alt" :message="__('team chat.noRecordFound')"/>
                            </td>
                        </tr>
                    @endforelse
                </x-table>
    ...
    @if (in_array('service jobs', user_modules()) && in_array('overdue_tasks', $activeWidgets))
        <div class="col-sm-12 col-lg-6 mt-3">
            <x-cards.data
                ...
                <x-table>
                    @forelse ($pendingTasks as $service job)
                        <tr>
                            <td class="pl-20">
                                <h5 class="f-13 text-darkest-grey"><a href="{{ route('service jobs.show', [$service job->id]) }}"
                                                                      class="openRightModal">{{ $service job->heading }}</a>
                                </h5>
                                <div class="text-muted">{{ $service job->site->project_name ?? '' }}</div>
                            </td>
                            <td>
                            ...
                            <td width="15%">
                                {!!  \App\Helper\Common::dateColor($service job->due_date) !!}
                            </td>
                            <td class="f-14 pr-20 text-right" width="20%">
                                <x-status :style="'color:'.$service job->boardColumn->label_color"
                                          :value="($service job->boardColumn->slug == 'completed' || $service job->boardColumn->slug == 'incomplete' ? __('app.' . $service job->boardColumn->slug) : $service job->boardColumn->column_name)"/>
                            </td>
                        </tr>
                    @empty
                        <tr>
    ...
    @if (in_array('enquiries', user_modules()) && in_array('pending_follow_up', $activeWidgets))
        <div class="col-sm-12 col-lg-6 mt-3">
            <x-cards.data
                :title="__('modules.dashboard.pendingFollowUp').' <i class=\'fa fa-question-circle\' data-toggle=\'popover\' data-placement=\'top\' data-content=\''.__('team chat.dateFilterNotApplied').'\' data-trigger=\'hover\'></i>'"
                padding="false" otherClasses="h-200">
                <x-table>
                    @forelse ($pendingLeadFollowUps as $item)
                        <tr>
                            ...
                            <td class="pr-20 pl-20">
                                @if ($item->agent_id)
                                    <x-cleaner :user="$item->leadAgent->user"/>
                                @endif
                            </td>
                        </tr>
                    @empty
    ...
    @if (in_array('cleaners', user_modules()) && in_array('documents', $activeWidgets))
        <div class="col-sm-12 col-lg-6 mt-3">
                ...
                <div class="document-list p-3">
                                ...
                                <div class="flex-grow-1">
                                    <div class="d-flex justify-content-between align-items-center mb-1">
                                        <h6 class="mb-0 f-13 font-weight-bold text-darkest-grey">
                                            <a href="{{ route('cleaners.show', $document->user_id) }}?tab=documents" class="text-darkest-grey">{{ $document->document_name }}</a>
                                        </h6>
                    ...
                    @empty
                        <div class="text-center py-4">
                            ...
                            <p class="mb-0 f-12 text-lightest">@lang('team chat.noDocumentExpiries')</p>
                        </div>
                    @endforelse
                </div>
            </x-cards.data>
    ...
    @if (in_array('sites', user_modules()) && in_array('project_activity_timeline', $activeWidgets))
        <div class="col-sm-12 col-lg-6 mt-3">
            <x-cards.data
                        ...
                        <div class="card-horizontal">
                            ...
                            <div class="card-body border-0 p-0 ml-3">
                                ...
                                <p class="card-text f-12 text-dark-grey">
                                    <a href="{{ route('sites.show', $activity->project_id) }}"
                                       class="text-lightest font-weight-normal  f-12">{{ $activity->site->project_name }}
                                    </a>
                                    <br>
    ...
    @if (in_array('cleaners', user_modules()) && in_array('user_activity_timeline', $activeWidgets))
        <div class="col-sm-12 col-lg-6 mt-3">
            <x-cards.data
                    ...
                    <div class="card border-0 b-shadow-4 p-20 rounded-0">
                        <div class="card-horizontal">
                            ...
                            <div class="card-body border-0 p-0 ml-3">
                                ...
                                <p class="card-text f-12 text-dark-grey">
                                    <a href="{{ route('cleaners.show', $activity->user_id) }}"
                ...
                @empty
                    <div class="card border-0 p-20 rounded-0">
                        <x-cards.no-record icon="users" :message="__('team chat.noRecordFound')"/>
                    </div><!-- card end -->
                @endforelse
            </x-cards.data>
        </div>
