...
<div class="row">
    ...
    @if (in_array('enquiries', user_modules()) && in_array('total_leads', $activeWidgets))
        <div class="col-xl-4 col-lg-6 col-md-6 mb-3">
    ...
    @if (in_array('enquiries', user_modules()) && in_array('total_deals', $activeWidgets))
        <div class="col-xl-4 col-lg-6 col-md-6 mb-3">
            <a href="javascript:;" id="totalDeals">
                <x-cards.widget :title="__('modules.deal.totalDeals')" :value="$totalDeals" icon="file-service agreement"
                                :info="__('team chat.totalDealWidget')"/>
            </a>
        </div>
    @endif

    @if (in_array('enquiries', user_modules()) && in_array('total_lead_conversions', $activeWidgets))
        <div class="col-xl-4 col-lg-6 col-md-6 mb-3">
                    ...
                    <div class="d-block ">
                        <h5 class="f-15 f-w-500 mb-20 text-darkest-grey"> @lang('modules.deal.dealConversions')
                            <i class="fa fa-question-circle" data-toggle="popover" data-placement="top"
                               data-content="@lang('team chat.leadConversion')" data-html="true"
                               data-trigger="hover"></i>
                        </h5>
                        <div class="d-flex">
                            <p class="mb-0 f-15 font-weight-bold text-blue text-primary d-grid"><span
                                    id="total_lead_conversions">{{ $convertedDeals }}/{{ $totalLeadConversions->count() }}  <span
                                        class="text-dark-grey f-11 text-wrap ql-editor p-0"> {{ $convertDealPercentage }}%</span></span>
                            </p>
                        </div>
                    </div>
                    <div class="d-block">
    ...
    @if (in_array('service agreements', user_modules()) && in_array('total_contracts_generated', $activeWidgets))
        <div class="col-xl-4 col-lg-6 col-md-6 mb-3">
            <a href="javascript:;" id="totalContractsGenerated">
                <x-cards.widget :title="__('modules.dashboard.totalContractsGenerated')"
                                :value="$totalContractsGenerated" icon="file-service agreement"/>
            </a>
        </div>
    @endif

    @if (in_array('service agreements', user_modules()) && in_array('total_contracts_signed', $activeWidgets))
        <div class="col-xl-4 col-lg-6 col-md-6 mb-3">
...
<div class="row">
    @if (in_array('payments', user_modules()) && in_array('client_wise_earnings', $activeWidgets))
        <div class="col-sm-12 col-lg-6 mt-3">
            <x-cards.data
                :title="__('modules.dashboard.clientWiseEarnings').' <i class=\'fa fa-question-circle\' data-toggle=\'popover\' data-placement=\'top\' data-content=\''.__('team chat.earningChartNote').'\' data-trigger=\'hover\'></i>'">
                <x-bar-chart id="service job-chart1" :chartData="$clientEarningChart" height="300"/>
            </x-cards.data>
        </div>
    @endif
    ...
    @if (in_array('timelogs', user_modules()) && in_array('client_wise_timelogs', $activeWidgets))
        <div class="col-sm-12 col-lg-6 mt-3">
            <x-cards.data :title="__('modules.dashboard.clientWiseTimelogs')">
                <x-line-chart id="service job-chart2" :chartData="$clientTimelogChart" height="300"/>
            </x-cards.data>
        </div>
    @endif

    @if (in_array('enquiries', user_modules()) && in_array('lead_vs_status', $activeWidgets))
        <div class="col-sm-12 col-lg-6 mt-3">
            <div class="card bg-white border-0 b-shadow-4" id="lead_vs_status">
                ...
                <div class="card-body p-0" id="leadStageData">
                    @if (array_sum($leadStatusChart['values']) > 0)
                        <a href="javascript:;" class="text-darkest-grey f-w-500 piechart-full-screen" data-chart-id="service job-chart3" data-chart-data="{{ json_encode($leadStatusChart) }}"><i class="fas fa-expand float-right mr-3"></i></a>
                    @endif
                    <x-pie-chart id="service job-chart3" :labels="$leadStatusChart['labels']"
                                 :values="$leadStatusChart['values']"
                                 :colors="$leadStatusChart['colors']" height="310" width="300"/>
                </div>
    ...
    @if (in_array('enquiries', user_modules()) && in_array('lead_vs_source', $activeWidgets))
        <div class="col-sm-12 col-lg-6 mt-3">
            <x-cards.data :title="__('modules.dashboard.leadVsSource')">
                @if (array_sum($leadSourceChart['values']) > 0)
                    <a href="javascript:;" class="text-darkest-grey f-w-500 piechart-full-screen" data-chart-id="service job-chart4" data-chart-data="{{ json_encode($leadSourceChart) }}"><i class="fas fa-expand float-right"></i></a>
                @endif
                <x-pie-chart id="service job-chart4" :labels="$leadSourceChart['labels']" :values="$leadSourceChart['values']"
                             :colors="$leadSourceChart['colors']" height="300" width="300"/>
            </x-cards.data>
        </div>
    @endif

    @if (in_array('customers', user_modules()) && in_array('latest_client', $activeWidgets))
        <div class="col-sm-12 col-lg-6 mt-3">
            <x-cards.data :title="__('modules.dashboard.latestClient')" padding="false" otherClasses="h-200">
                <x-table class="border-0 pb-3 admin-dash-table table-hover">
                    <x-slot name="thead">
                        <th class="pl-20">@lang('app.customer')</th>
    ...
    @if (in_array('customers', user_modules()) && in_array('recent_login_activities', $activeWidgets))
        <div class="col-sm-12 col-lg-6 mt-3">
...
<script>
    $('#save-dashboard-widget').click(function () {
        $.easyAjax({
            url: "{{ route('dashboard.widget', 'admin-customer-dashboard') }}",
    ...
    $('#totalLeads').click(function () {
        var dateRange = getDateRange();
        var url = `{{ route('enquiry-contact.index') }}`;
    ...
    $('#totalContractsGenerated').click(function () {
        var dateRange = getDateRange();
        var url = `{{ route('service agreements.index') }}`;
