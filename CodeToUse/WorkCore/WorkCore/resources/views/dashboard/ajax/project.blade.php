...
<div class="row">
    @if (in_array('sites', user_modules()) && in_array('total_project', $activeWidgets))
    ...
    @if (in_array('sites', user_modules()) && in_array('total_overdue_project', $activeWidgets))
        <div class="col-xl-3 col-lg-6 col-md-6 mb-3">
            <a href="javascript:;" id="overDue">
                <x-cards.widget :title="__('modules.issues / support.overDueProjects')" :value="$totalOverdueProject"
                    icon="layer-group" />
            </a>
        </div>
    @endif
    ...
    @if (in_array('timelogs', user_modules()) && in_array('total_hours_logged', $activeWidgets))
        <div class="col-xl-3 col-lg-6 col-md-6 mb-3">
            <a href="{{ route('time-log-report.index') . '?site=required' }}">
...
<div class="row">
    @if (in_array('sites', user_modules()) && in_array('status_wise_project', $activeWidgets))
        <div class="col-sm-12 col-lg-6 mt-3">
            <x-cards.data :title="__('modules.dashboard.statusWiseProject')">
                @if (array_sum($statusWiseProject['values']) > 0)
                    <a href="javascript:;" class="text-darkest-grey f-w-500 piechart-full-screen" data-chart-id="status-wise-site" data-chart-data="{{ json_encode($statusWiseProject) }}"><i class="fas fa-expand float-right mr-3"></i></a>
                @endif
                <x-pie-chart id="service job-chart" :labels="$statusWiseProject['labels']"
                    :values="$statusWiseProject['values']" :colors="$statusWiseProject['colors']" height="310" width="300" />
            </x-cards.data>
        </div>
    @endif

    @if (in_array('sites', user_modules()) && in_array('pending_milestone', $activeWidgets))
        <div class="col-sm-12 col-lg-6 mt-3">
            <x-cards.data :title="__('modules.dashboard.pendingMilestone')" padding="false" otherClasses="h-200">
                <x-table class="border-0 pb-3 admin-dash-table table-hover">
                    ...
                    <x-slot name="thead">
                        ...
                        <th class="pr-20 text-right">@lang('app.site')</th>
                    </x-slot>
                    ...
                    @forelse($pendingMilestone as $key=>$item)
                        <tr id="row-{{ $item->id }}">
                            ...
                            <td class="pr-20 text-right">
                                <a href="{{ route('sites.show', [$item->project_id]) }}"
                                    class="text-darkest-grey">{{ $item->site?->project_name }}</a>
                            </td>
                        </tr>
                    @empty
                        <tr>
...
<script>
    ...
    $('#save-dashboard-widget').click(function() {
        $.easyAjax({
            url: "{{ route('dashboard.widget', 'admin-site-dashboard') }}",
