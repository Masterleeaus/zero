                ...
                <div class="col-md-12">
                    <x-alert type="info" icon="info-circle">
                        <a href="{{ route('leaves.show', $checkTodayLeave->id) }}" class="openRightModal text-dark-grey">
                            <u>@lang('team chat.youAreOnLeave')</u>
                        </a>
                    </x-alert>
                </div>
            </div>
                ...
                <div class="col-md-12">
                    <x-alert type="info" icon="info-circle">
                        <a href="{{ route('holidays.show', $checkTodayHoliday->id) }}" class="openRightModal text-dark-grey">
                            <u>@lang('team chat.holidayToday')</u>
                        </a>
                    </x-alert>
                </div>
            </div>
            ...
            <div
                ...
                @php
                    ...
                    if($leaveApplied != null && ($leaveApplied->duration === 'single' || $leaveApplied->duration === 'multiple') && $currentDateTime->between($start_time, $end_time)){
                        ...
                        $msg = __('team chat.leaveForFirstHalf');
                        ...
                        $msg = __('team chat.shiftNotAssigned');
                    }else if($cannotLogin == true){
                        $msg = __('team chat.notInOfficeHours');
                    }
                    /*
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-12">
                            <div class="card border-0 b-shadow-4 mb-3 e-d-info">
                                <a @if(!in_array('customer', user_roles())) href="{{ route('cleaners.show', user()->id) }}" @endif >
                                    <div class="card-horizontal align-items-center">
                                        ...
                                        <div class="card-body border-0 pl-0">
                                            ...
                                            <p class="f-13 font-weight-normal text-dark-grey mb-2">
                                                {{ $user->employeeDetail->role->name ?? '--' }}</p>
                                ...
                                <div class="card-footer bg-white border-top-grey py-3">
                                    <div class="d-flex flex-wrap justify-content-between">
                                        @if(in_array('service jobs', user_modules()))
                                            <span>
                                                ...
                                                <p class="mb-0 f-18 f-w-500">
                                                    <a href="{{ route('service jobs.index') . '?assignee=me' }}"
                                                        class="text-dark">
                                                        {{ $inProcessTasks }}
                                                    </a>
                                                </p>
                                        ...
                                        @if(in_array('sites', user_modules()))
                                            <span>
                                                <label class="f-12 text-dark-grey mb-12 " for="usr">
                                                    @lang('app.menu.sites') </label>
                                                <p class="mb-0 f-18 f-w-500">
                                                    <a href="{{ route('sites.index') . '?assignee=me&status=all' }}"
                                                        class="text-dark">{{ $totalProjects }}</a>
                                                </p>
                                            </span>
                                        @endif
                                        @if (isset($totalOpenTickets) && in_array('issues / support', user_modules()))
                                            <span>
                                                <label class="f-12 text-dark-grey mb-12 " for="usr">
                                                    @lang('modules.dashboard.totalOpenTickets') </label>
                                                <p class="mb-0 f-18 f-w-500">
                                                    <a href="{{ route('issues / support.index') . '?agent=me&status=open' }}"
                        ...
                        @if (!is_null($myActiveTimer) && in_array('service jobs', user_modules()))
                                    ...
                                    <div class="row">
                                        <div class="col-sm-12">
                                            ...
                                            <ul class="list-group">
                                                ...
                                                <li
                                                    class="list-group-item d-flex justify-content-between align-items-center f-12 text-dark-grey">
                                                    <span><i class="fa fa-briefcase"></i> @lang('app.service job')</span>
                                                    <a href="{{ route('service jobs.show', $myActiveTimer->service job->id) }}"
                                                        class="text-dark-grey openRightModal">{{ $myActiveTimer->service job->heading }}</a>
                                                </li>
                                        ...
                                        <div class="col-sm-12 pt-3 text-right">
                                            @if ($editTimelogPermission == 'all' || ($editTimelogPermission == 'added' && $myActiveTimer->added_by == user()->id) || ($editTimelogPermission == 'owned' && (($myActiveTimer->site && $myActiveTimer->site->client_id == user()->id) || $myActiveTimer->user_id == user()->id)) || ($editTimelogPermission == 'both' && (($myActiveTimer->site && $myActiveTimer->site->client_id == user()->id) || $myActiveTimer->user_id == user()->id || $myActiveTimer->added_by == user()->id)))
                        ...
                        @endif
                            ...
                            @include('dashboard.cleaner.widgets.shift_schedule')

                            @include('dashboard.cleaner.widgets.birthday')

                            @include('dashboard.cleaner.widgets.appreciation')

                            @include('dashboard.cleaner.widgets.leave')

                            @include('dashboard.cleaner.widgets.work_from_home')

                            @include('dashboard.cleaner.widgets.work_anniversary')

                            @include('dashboard.cleaner.widgets.notice-period')

                            @include('dashboard.cleaner.widgets.probation')

                            @include('dashboard.cleaner.widgets.internship')

                            @include('dashboard.cleaner.widgets.service agreement')
                    </div>
                </div>
            @endif
            <!-- EMP DASHBOARD INFO NOTICES END -->
            <!-- EMP DASHBOARD TASKS PROJECTS EVENTS START -->
            <div class="col-xl-7 col-lg-12 col-md-12 e-d-service jobs-sites-events">
                <!-- EMP DASHBOARD TASKS PROJECTS START -->
                <div class="row mb-3 mt-xl-0 mt-lg-4 mt-md-4 mt-4">
                    @if (in_array('service jobs', $activeWidgets) && (!is_null($viewTaskPermission) && $viewTaskPermission != 'none') && in_array('service jobs', user_modules()))
                        <div class="col-md-6 mb-3">
                            <div
                                class="bg-white p-20 rounded b-shadow-4 d-flex justify-content-between align-items-center mb-4 mb-md-0 mb-lg-0">
                                <div class="d-block ">
                                    <h5 class="f-15 f-w-500 mb-20 text-darkest-grey">@lang('app.menu.service jobs')</h5>
                                    <div class="d-flex">
                                        <a href="{{ route('service jobs.index') . '?assignee=me' }}">
                                        ...
                                        <a href="{{ route('service jobs.index') . '?assignee=me&overdue=yes' }}">
                    ...
                    @include('dashboard.cleaner.widgets.sites')
                    @include('dashboard.cleaner.widgets.follow_ups')
                    @include('dashboard.cleaner.widgets.enquiry')
                    @include('dashboard.cleaner.widgets.week_timelog')
                    @include('dashboard.cleaner.widgets.onboarding')
                    @include('dashboard.cleaner.widgets.documents')
                </div>
                <!-- EMP DASHBOARD TASKS PROJECTS END -->
                @include('dashboard.cleaner.widgets.my_tasks')
                @include('dashboard.cleaner.widgets.issues / support')
                @include('dashboard.cleaner.widgets.my_calendar')
                @include('dashboard.cleaner.widgets.notices')
...
@push('scripts')
    @if ((!is_null($viewEventPermission) && $viewEventPermission != 'none')
        ...
        <script>
            ...
            // Service Job Detail show in sidebar
            var getEventDetail = function(id,type) {
                if(type == 'issue / support')
                {
                    var url = "{{ route('issues / support.show', ':id') }}";
                ...
                switch (type) {
                    case 'service job':
                        var url = "{{ route('service jobs.show', ':id') }}";
                        break;
                    case 'event':
    ...
    <script>
        ...
        $('#view-shifts').click(function() {
            const url = "{{ route('cleaner-shifts.index') }}";
