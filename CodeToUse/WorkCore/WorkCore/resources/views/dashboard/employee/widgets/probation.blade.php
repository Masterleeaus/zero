@if (in_array('probation_date', $activeWidgets) && in_array('cleaners', user_modules()))
    @php
    ...
    <div class="col-sm-12">
        @if (in_array('admin', user_roles()))
            <x-cards.data class="e-d-info mb-2" :title="__('modules.dashboard.probationDate')" padding="false">
                <x-table>
                        @forelse ($probations as $probation)
                            ...
                            <tr>
                                ...
                                <td class="pr-20 text-right">
                                    ...
                                    <br>
                                    @if ($probationDate->setTimezone(company()->timezone)->isToday())
                                        <span class="text-success f-11">@lang('team chat.probationMessage')
                                            {{ $probationDate->translatedFormat($company->date_format) }}</span>
                                    @elseif($probationDate->diffInDays($currentDay) <= 7)
                                        <span class="text-warning f-11">@lang('team chat.probationMessage')
                                            {{ $probationDate->translatedFormat($company->date_format) }}</span>
                                    @else
                                        <span class="f-11">@lang('team chat.probationMessage')
                                            {{ $probationDate->translatedFormat($company->date_format) }}</span>
                                    @endif
                ...
                <x-cards.data class="e-d-info mb-2" :title="__('modules.dashboard.probationDate')" padding="false">
                    <x-table>
                        <tr>
                            ...
                            <td class="pl-20">
                                @if ($probationDate->setTimezone(company()->timezone)->isToday())
                                    <span class="text-success f-12">@lang('team chat.probationMessage')
                                        {{ $probationDate->translatedFormat($company->date_format) }}</span>
                                @elseif($probationDate->diffInDays($currentDay) <= 7)
                                    <span class="text-warning f-12">@lang('team chat.probationMessage')
                                        {{ $probationDate->translatedFormat($company->date_format) }}</span>
                                @else
                                    <span class="f-12">@lang('team chat.probationMessage')
                                        {{ $probationDate->translatedFormat($company->date_format) }}</span>
                                @endif
                            </td>
