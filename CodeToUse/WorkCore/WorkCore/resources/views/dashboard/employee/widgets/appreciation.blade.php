@if (in_array('appreciation', $activeWidgets) && isset($sidebarUserPermissions['view_appreciation']) && $sidebarUserPermissions['view_appreciation'] != 5 && in_array('cleaners', user_modules()))
    <!-- EMP DASHBOARD APPRECIATION START -->
    <div class="col-sm-12">
        <x-cards.data class="e-d-info mb-2" :title="__('app.cleaner').' '.__('modules.dashboard.appreciation')" padding="false" otherClasses="h-200">
            <x-table class="appreciation-table">
                @forelse ($appreciations as $appreciation)
                    <tr>
                        <td>
                            <x-cleaner :user="$appreciation->awardTo" />
                        </td>
                ...
                @empty
                    <tr>
                        <td colspan="3" class="shadow-none">
                            <x-cards.no-record icon="award" :message="__('team chat.noRecordFound')" />
                        </td>
                    </tr>
                @endforelse
            </x-table>
