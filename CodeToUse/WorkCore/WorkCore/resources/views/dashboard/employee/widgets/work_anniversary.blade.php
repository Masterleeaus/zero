@if (in_array('work_anniversary', $activeWidgets) && in_array('cleaners', user_modules()))
<!-- EMP DASHBOARD ANNIVERSARY START -->
<div class="col-sm-12">
    <x-cards.data class="e-d-info" :title="__('modules.cleaners.joineeAndWorkAnniversary')" padding="false" otherClasses="h-200">
        <x-table>
            @forelse ($upcomingAnniversaries as $upcomingAnniversary)
                <tr>
                    <td class="pl-20">
                        <x-cleaner :user="$upcomingAnniversary->user" />
                    </td>
