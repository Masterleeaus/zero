@if (in_array('leave', $activeWidgets) && $sidebarUserPermissions['view_leave'] != 5 && $sidebarUserPermissions['view_leave'] != 'none' && in_array('cleaners', user_modules()))
        ...
        <x-cards.data class="e-d-info mb-2" :title="__('modules.dashboard.leave')" padding="false" otherClasses="h-200">
            <x-table>
                @forelse ($leave as $totalLeave)
                    <tr>
                        <td class="pl-20">
                            <x-cleaner :user="$totalLeave->user"/>
                        </td>
                        <td class="pr-20">
