@if (in_array('my_calender', $activeWidgets) &&
                (in_array('service jobs', user_modules()) || in_array('events', user_modules()) || in_array('holidays', user_modules()) ||
                        ...
                        <div id="cal-drop" class="dropdown-menu dropdown-menu-right border-grey rounded b-shadow-4 p-2">
                            @if(in_array('service jobs', user_modules()))
                                <div class="custom-control custom-checkbox cal-filter">
                                    <input type="checkbox" value="service job"
                                           class="form-check-input filter-check" name="calendar[]"
                                           id="customCheck1" @if(in_array('service job',$event_filter)) checked @endif>
                                    <label
                                        ...
                                        for="customCheck1">@lang('app.menu.service jobs')</label>
                                </div>
                            @endif
                            ...
                            @if(in_array('issues / support', user_modules()))
                                <div class="custom-control custom-checkbox cal-filter">
                                    <input type="checkbox" value="issues / support"
                                           class="form-check-input filter-check" name="calendar[]"
                                           id="customCheck4" @if(in_array('issues / support',$event_filter)) checked @endif>
                                    <label
                                        class="form-check-label form_custom_label text-dark-grey pl-2 mr-3 justify-content-start cursor-pointer checkmark-20 pt-2 text-wrap"
                                        for="customCheck4">@lang('app.menu.issues / support')</label>
                                </div>
                            @endif
                            ...
                            @endif
                            @if(in_array('enquiries', user_modules()) && $leadAgent)
                                <div class="custom-control custom-checkbox cal-filter">
