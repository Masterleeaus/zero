@if (in_array('internship_date', $activeWidgets) && in_array('cleaners', user_modules()))
    @php
            ...
            <x-cards.data class="e-d-info mb-2" :title="__('modules.dashboard.internshipDate')" padding="false">
                <x-table>
                        @forelse ($internships as $internship)
                            ...
                            <tr>
                                <td class="pl-20">
                                    <x-cleaner :user="$internship->user"/>
                                </td>
                                ...
                                <td class="pr-20 text-right">
                                    ...
                                    @if ($internshipDate->setTimezone(company()->timezone)->isToday())
                                        <span
                                            class="text-success f-12">@lang('team chat.internshipMessage'){{ $internshipDate->translatedFormat($company->date_format) }}</span>
                                    @elseif($internshipDate->diffInDays($currentDay) <= 7)
                                        <span class="text-warning f-12">@lang('team chat.internshipMessage')
                                            {{ $internshipDate->translatedFormat($company->date_format) }}</span>
                                    @else
                                        <span class="f-12">@lang('team chat.internshipMessage')
                                            {{ $internshipDate->translatedFormat($company->date_format) }}</span>
                                    @endif
                                </td>
                            </tr>
