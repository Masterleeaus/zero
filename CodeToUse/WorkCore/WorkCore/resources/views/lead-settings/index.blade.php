                ...
                <div class="s-b-n-header" id="tabs">
                    <nav class="tabs px-4 border-bottom-grey">
                        <div class="nav" id="nav-tab" role="tablist">
                            <a class="nav-item nav-link f-15 active source" href="{{ route('enquiry-settings.index') }}"
                                role="tab" aria-controls="nav-leadSource" aria-selected="true">@lang('app.menu.leadSource')
                            </a>
                            <a class="nav-item nav-link f-15 pipeline" href="{{ route('enquiry-settings.index') }}?tab=pipeline"
                                role="tab" aria-controls="nav-pipeline" aria-selected="true">@lang('modules.deal.pipeline')
                            </a>
                            <a class="nav-item nav-link f-15 agent" href="{{ route('enquiry-settings.index') }}?tab=agent"
    ...
    <script>
       ...
       $("body").on("click", "#editSettings .nav a", function(event) {
        ...
        $('body').on('click', '#addAgent', function() {
            var url = '{{ route('enquiry-agent-settings.create') }}';
        ...
        $('body').on('click', '.delete-agent', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    var url = "{{ route('enquiry-agent-settings.destroy', ':id') }}";
        ...
        $('body').on('click', '.edit-source', function() {
            ...
            var url = "{{ route('enquiry-source-settings.edit', ':id ') }}";
        ...
        $('body').on('click', '.delete-source', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    var url = "{{ route('enquiry-source-settings.destroy', ':id') }}";
                    url = url.replace(':id', id);
        ...
        /* LEAD STATUS SCRIPTS */
           ...
           $('body').on('click', '.set_default_stage', function() {
            ...
            var url = "{{ route('enquiry-stage-setting.stageUpdate', ':id') }}";
            url = url.replace(':id', id);
           ...
           $('body').on('click', '.set_default_pipeline', function() {
            ...
            var url = "{{ route('enquiry-pipeline-update.stageUpdate', ':id') }}";
            url = url.replace(':id', id);
        ...
        $('body').on('click', '#addStage', function() {
            var url = "{{ route('enquiry-stage-setting.create') }}";
            $(MODAL_LG + ' ' + MODAL_HEADING).html('...');
            $.ajaxModal(MODAL_LG, url);
        });
        ...
        $('body').on('click', '#addPipeline', function() {
            var url = "{{ route('enquiry-pipeline-setting.create') }}";
            $(MODAL_LG + ' ' + MODAL_HEADING).html('...');
            $.ajaxModal(MODAL_LG, url);
        });
        ...
        $('body').on('click', '.edit-pipeline', function() {
            ...
            var url = "{{ route('enquiry-pipeline-setting.edit', ':id ') }}";
        ...
        $('body').on('click', '.delete-pipeline', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    var url = "{{ route('enquiry-pipeline-setting.destroy', ':id') }}";
                    url = url.replace(':id', id);
