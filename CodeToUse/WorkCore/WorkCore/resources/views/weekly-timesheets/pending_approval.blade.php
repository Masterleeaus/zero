...
<style>
    ...
    .cleaner-td:hover > .work-setting-icon {
...
@section('filter-section')
    @if(isset($cleaners) && $cleaners->count() > 0)
            ...
            <div class="select-status">
                <select class="form-control select-picker" name="employee_id" id="employee_id" data-live-search="true" data-size="8">
                    ...
                    @foreach ($cleaners as $cleaner)
                        <x-user-option :user="$cleaner" :selected="request('employee_id') == $cleaner->id"/>
                    @endforeach
                </select>
            </div>
        </div>
            ...
            <div class="table-responsive">
                <x-table class="table-bordered mt-3 table-hover" headType="thead-light" id="weekly-timesheet-table" >
                    ...
                    @forelse ($weeklyTimesheet as $timesheet)
                        <tr>
                            <td class="px-2"><x-cleaner :user="$timesheet->user" /></td>
                    ...
                    @empty
                        <tr>
                            <td colspan="3" class="text-center">@lang('team chat.noRecordFound')</td>
                        </tr>
                    @endforelse
