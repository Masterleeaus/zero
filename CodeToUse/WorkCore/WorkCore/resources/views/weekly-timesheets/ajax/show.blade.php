            ...
            <div class="table-responsive">
                ...
                <div class="d-flex">
                    <div class="text-muted f-12">
                       @lang('modules.timeLogs.submittedBy') <x-cleaner :user="$weeklyTimesheet->user" />
                    </div>
                    @if ($weeklyTimesheet->approvedBy)
                        <div class="ml-4 text-muted f-12">
                            @lang('modules.expenses.approvedBy') <x-cleaner :user="$weeklyTimesheet->approvedBy" />
                        </div>
                    @endif
                </div>
                ...
                <x-table class="table-bordered mt-3 table-hover" headType="thead-light" id="weekly-timesheet-table" >
                    ...
                    @if($weeklyTimesheet)
                        ...
                        <tr>
                            <td class="px-1 cleaner-td fixed-column bg-white">
                                <a href="{{ route('service jobs.show', $entries->first()->service job->id) }}" class="text-darkest-grey openRightModal">{{ $entries->first()->service job->heading }}</a>
                            </td>
    ...
    $('.change-timesheet-status').on('click', function() {
        ...
        if(status == 'draft'){
            ...
            Swal.fire({
                ...
                text: "@lang('team chat.changeWeeklyTimesheetStatusConfirmation')",
                icon: 'warning',
