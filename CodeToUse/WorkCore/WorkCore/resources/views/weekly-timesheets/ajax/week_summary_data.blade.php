...
<form action="{{ route('weekly-timesheets.store') }}" method="post" id="weekly-timesheet-form">
    ...
    <div class="table-responsive">
        <x-table class="table-bordered mt-3 table-hover" headType="thead-light" id="weekly-timesheet-table" >
            <x-slot name="thead">
                <th class="px-2 fixed-column font-weight-semibold f-16" style="vertical-align: middle;"><span class="f-16">@lang('app.service job')</span></th>
            ...
            @if($weekTimesheet)
                ...
                <tr>
                    <td class="px-1 cleaner-td fixed-column bg-white" style="position: relative;">
                        <div class="form-group d-flex justify-content-between">
                        <select class="form-control week-service job select-picker" name="task_ids[{{ $key }}]" data-container="body" data-live-search="true">
                            @foreach ($tasksForWeek as $service job)
                                <option value="{{ $service job->id }}" {{ $key == $service job->id ? 'selected' : '' }} 
                                    data-content="<h5 class='f-13 text-darkest-grey mb-0'>{{ $service job->heading }}</h5>
                                    @if($service job->site)
                                        <div class='text-muted f-11'>{{ $service job->site->project_name }}</div>
                                    @endif">
                                    {{ $service job->heading }}
                                </option>
                            @endforeach
                        ...
                        @if($weekTimesheet->status == 'draft')
                            <button class="btn btn-light btn-sm remove-service job" type="button">
                ...
                <tr>
                    <td class="px-1 cleaner-td fixed-column bg-white" style="position: relative;">
                        <div class="form-group d-flex justify-content-between">
                            <select class="form-control week-service job select-picker" name="task_ids[{{ $key }}]" data-container="body" data-live-search="true">
                                @foreach ($tasksForWeek as $service job)
                                    <option value="{{ $service job->id }}" data-content="<h5 class='f-13 text-darkest-grey mb-0'>{{ $service job->heading }}</h5>
                                    @if($service job->site)
                                        <div class='text-muted f-11'>{{ $service job->site->project_name }}</div>
                                    @endif">{{ $service job->heading }}</option>
                                @endforeach
                            </select>
        ...
        @if ($addTimelogPermission == 'all' || $addTimelogPermission == 'added')
            @if ($weekTimesheet && $weekTimesheet->status == 'draft' || !$weekTimesheet)
                <div class="mt-3">
                    <x-forms.button-secondary type="button" class="mr-2" id="add-more-service job" >@lang('app.addMore')</x-forms.button-secondary>
...
<script>
    $(document).ready(function () {
        ...
        $('#add-more-service job').click(function (e) {
            e.preventDefault();
            let $clone = $('#weekly-timesheet-table').find('tbody tr:last').clone();
            
            $clone.find('.week-service job').attr('name', 'task_ids[' + entryCount + ']');
        ...
        $('.submit-timesheet').click(function (e) {
            ...
            if($(this).data('status') == 'draft'){
                ...
                Swal.fire({
                    ...
                    text: "@lang('team chat.submitWeeklyTimesheetConfirmation')",
                    icon: 'warning',
                    showCancelButton: true,
                focusConfirm: false,
                confirmButtonText: "@lang('team chat.confirm')",
        ...
        $('#weekly-timesheet-table').on('click', '.remove-service job', function (e) {
            e.preventDefault();
            $(this).closest('tr').remove();
        });

        if($('#status').val() == 'draft') {
            $('.week-service job, .week-hours').prop('disabled', false);
        } else {
            $('.week-service job, .week-hours').prop('disabled', true);
            $('.week-service job').selectpicker('refresh');
        }
    });
</script>
