...
<x-alert class="mb-3" type="danger" icon="exclamation-circle"><span>@lang('team chat.enableClientModule')</span>
    <x-forms.link-secondary icon="arrow-left" :link="route('quote-request.index')">@lang('app.back')</x-forms.link-secondary>
</x-alert>
@else
    ...
    <x-form class="c-inv-form" id="saveInvoiceForm" method="PUT">
        ...
        <div class="row px-lg-4 px-md-4 px-3 py-3">
                ...
                <div class="col-md-6 col-lg-4">
                    <x-forms.select fieldId="project_id" fieldName="project_id" :fieldLabel="__('app.site')"
                        ...
                        @foreach ($sites as $site)
                            <option data-currency-id="{{ $site->currency_id }}" @selected($estimateRequest->project_id == $site->id)
                                value="{{ $site->id }}">
                                {{ $site->project_name }}
                            </option>
                        @endforeach
            ...
            @else
                <div class="col-md-6 col-lg-4 my-3" id="client_list_ids">
                    <x-customer-selection-dropdown :customers="$customers" :selected="$estimateRequest->client_id"/>
                </div>
                ...
                <div class="col-md-6 col-lg-4">
                    <x-forms.select fieldId="project_id" fieldName="project_id" :fieldLabel="__('app.site')"
                        ...
                        @foreach ($sites as $site)
                            <option data-currency-id="{{ $site->currency_id }}" @selected($estimateRequest->project_id == $site->id)
                                value="{{ $site->id }}">{{ $site->project_name }}
                            </option>
                        @endforeach
                    </x-forms.select>
                </div>
        ...
        <x-form-actions class="c-inv-btns">
            <div class="d-flex mb-3">
                <x-forms.button-primary id="save-quote-request" class="mr-3" icon="check">@lang('app.save')
...
<script>
    $(document).ready(function() {
        ...
        $('#save-quote-request').click(function() {
            ...
            $.easyAjax({
                url: "{{ route('quote-request.update', $estimateRequest->id) }}",
        ...
        $('body').on('change', "#client_list_id", function () {
            ...
            if (requesterType == 'customer' && clientId) {
