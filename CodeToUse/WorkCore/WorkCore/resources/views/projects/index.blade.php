...
@section('filter-section')
    ...
    <x-filters.filter-box>
        ...
        <!-- DATE END -->
        <div class="select-box d-flex py-2 {{ !in_array('customer', user_roles()) ? 'px-lg-2 px-md-2 px-0' : '' }}  border-right-grey border-right-grey-sm-0 pr-2 pl-2">
            ...
            <div class="select-status mw-120">
                <x-forms.input-group>
                    <select class="select-picker form-control" multiple name="progress[]" id="progress" data-live-search="true" data-size="8">
                        ...
                        <option value="100-100" {{ request()->sites == 'all' ? 'selected' : ''}}>100%</option>
                    </select>
                </x-forms.input-group>
            </div>
        </div>
        ...
        <x-filters.more-filter-box>
            ...
            <div class="more-filter-items @if(!in_array('customers', user_modules())) d-none @endif">
            ...
            <div class="more-filter-items">
                <label class="f-14 text-dark-grey mb-12 "
                    for="usr">@lang('modules.sites.projectCategory')</label>
                <div class="select-filter mb-4">
                    <div class="select-others">
                    ...
                    <div class="select-filter mb-4">
                        <div class="select-others">
                            <select class="form-control select-picker" name="employee_id" id="employee_id"
                                ...
                                @foreach ($allEmployees as $cleaner)
                                        <x-user-option :user="$cleaner" :selected="request('assignee') == 'me' && $cleaner->id == user()->id"/>
                                @endforeach
                            </select>
                        </div>
                    </div>
        ...
        <div class="d-grid d-lg-flex d-md-flex action-bar">
            <div id="table-actions" class="flex-grow-1 align-items-center mb-2 mb-lg-0 mb-md-0">
                @if ($addProjectPermission == 'all' || $addProjectPermission == 'added' || $addProjectPermission == 'both')
                    <x-forms.link-primary :link="route('sites.create')"
                ...
                @if ($viewProjectTemplatePermission == 'all' || in_array($manageProjectTemplatePermission , ['added', 'all']))
                    <x-forms.link-secondary :link="route('site-template.index')"
                ...
                @if ($addProjectPermission == 'all' || $addProjectPermission == 'added' || $addProjectPermission == 'both')
                    <x-forms.link-secondary :link="route('sites.import')" class="mr-3 float-left mb-2 mb-lg-0 mb-md-0 d-none d-lg-block" icon="file-upload">
            ...
            <div class="btn-group mt-2 mt-lg-0 mt-md-0 ml-0 ml-lg-3 ml-md-3" role="group">
                <a href="{{ route('sites.index') }}" class="btn btn-secondary f-14 btn-active sites" data-toggle="tooltip"
                    data-original-title="@lang('app.menu.sites')"><i class="side-icon bi bi-list-ul"></i></a>

                    @if ($deleteProjectPermission != 'none')
                        <a href="{{ route('sites.archive') }}" class="btn btn-secondary f-14" data-toggle="tooltip"
                            data-original-title="@lang('app.archive')"><i class="side-icon bi bi-archive"></i></a>
                    @endif
                    <a href="{{ route('site-calendar.index') }}" class="btn btn-secondary f-14" data-toggle="tooltip"
...
@push('scripts')
    ...
    <script>
        ...
        $('#sites-table').on('preXhr.dt', function(e, settings, data) {
        ...
        const showTable = () => {
            window.LaravelDataTables["sites-table"].draw(true);
        }
        ...
        $('#client_id, #status, #employee_id, #team_id, #label, #category_id, #pinned, #date_filter_on, #public, #progress').on('change keyup',
            function() {
        ...
        $('#sites-table').on('change', '.change-status', function() {
            var url = "{{ route('sites.change_status') }}";
        ...
        $('body').on('click', '.archive', function() {
            ...
            Swal.fire({
                title: "@lang('team chat.sweetAlertTitle')",
                text: "@lang('team chat.archiveMessage')",
                icon: 'warning',
                showCancelButton: true,
                focusConfirm: false,
                confirmButtonText: "@lang('team chat.confirmArchive')",
                cancelButtonText: "@lang('app.cancel')",
                customClass: {
                    confirmButton: 'btn btn-primary mr-3',
                    cancelButton: 'btn btn-secondary'
                ...
                if (result.isConfirmed) {
                    var url = "{{ route('sites.archive_delete', ':id') }}";
                    url = url.replace(':id', id);
        ...
        const applyQuickAction = () => {
            var rowdIds = $("#sites-table input:checkbox:checked").map(function() {
                return $(this).val();
            }).get();

            var url = "{{ route('sites.apply_quick_action') }}?row_ids=" + rowdIds;
        ...
        $('body').on('click', '.duplicateProject', function(){
            var id = $(this).data('site-id');
            var url = "{{ route('sites.duplicate_project', ':id') }}";
            url = url.replace(':id', id);
