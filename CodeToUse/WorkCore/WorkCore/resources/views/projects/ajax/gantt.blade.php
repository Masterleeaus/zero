        ...
        <div class="d-flex flex-column w-tables rounded mt-3 bg-white">
            ...
            <div class="d-flex">
                ...
                <div class="select-box py-2 px-2 mr-3">
                    <p class="mb-0 pr-2 f-14 text-dark-grey d-flex align-items-center">@lang('modules.service jobs.assignTo')
                    </p>
                ...
                <!-- ASSIGN END -->
                 ...
                 <!-- ASSIGN START -->
                 <div class="select-box py-2 px-2 mr-3">
                    <p class="mb-0 pr-2 f-14 text-dark-grey d-flex align-items-center">@lang('app.service job')
                    </p>
                    <div class="select-status mr-3">
                        <select class="form-control select-picker" id="projectTask" data-live-search="true"
                            data-size="8" multiple name="projectTask[]">
                            @foreach ($site->service jobs as $service job)
                                <option value="{{ $service job->id}}">{{ $service job->heading }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <!-- ASSIGN END -->
                 ...
                 <!-- ASSIGN START -->
                 <div class="select-box py-2 px-2 mr-3">
                    <p class="mb-0 pr-2 f-14 text-dark-grey d-flex align-items-center">@lang('modules.sites.milestones')
                    </p>
                    <div class="select-status mr-3">
...
<script>
    $(document).ready(function() {
        ...
        function loadData() {
            ...
            var url = "{{ route('sites.gantt_data') }}?assignedTo=" +
            ...
            $.easyAjax({
                ...
                success: function(response) {
                    if (!response.length) {
                        $("#gantt").html(
                            "<div class='d-flex justify-content-center p-20'>{{ __('team chat.noRecordFound') }}</div>"
                        );
                        return;
                    }
                    ...
                    var gantt = new Gantt("#gantt", response, {
                        ...
                        view_mode: viewMode,
                        on_click: function(service job) {
                            taskDetail(service job.taskid);
                        },
                        on_date_change: function(service job, start, end) {
                            var taskId = service job.taskid;
                            var token = '{{ csrf_token() }}';
                            var url =
                                "{{ route('service jobs.gantt_task_update', ':id') }}";
                        ...
                        on_progress_change: function(service job, progress) {
                        },
