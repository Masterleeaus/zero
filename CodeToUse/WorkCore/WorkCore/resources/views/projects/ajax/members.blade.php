...
<div class="row py-5">
    <div class="col-lg-12 col-md-12 mb-4 mb-xl-0 mb-lg-4">
        @if (($addProjectMemberPermission == 'all' || $addProjectMemberPermission == 'added' || $site->project_admin == user()->id) && !$site->trashed())
            <x-forms.button-primary icon="plus" id="add-site-member" class="type-btn mb-3">
                @lang('modules.sites.addMemberTitle')
            </x-forms.button-primary>
        @endif

        @if ($viewProjectMemberPermission == 'all')
            <x-cards.data :title="__('modules.sites.members')"
                ...
                <x-table class="border-0 pb-3 admin-dash-table table-hover">
                    ...
                    <x-slot name="thead">
                        ...
                        @if ($viewProjectHourlyRatePermission == 'all')
                            <th>@lang('modules.cleaners.hourlyRate')</th>
                        @endif
                        <th>@lang('app.role') <i class="fa fa-question-circle" data-toggle="popover" data-placement="top" data-content="@lang('modules.sites.projectAdminInfo')" data-html="true" data-trigger="hover"></i></th>
                        <th class="text-right pr-20">@lang('app.action')</th>
                    </x-slot>

                    @forelse($site->members as $key=>$member)
                        <tr id="row-{{ $member->id }}">
                            <td class="pl-20">{{ $key + 1 }}</td>
                            <td>
                                <x-cleaner :user="$member->user" :disabledLink="$isClient" />
                            </td>
                            @if($viewProjectHourlyRatePermission == 'all')
                                <td>
                            ...
                            <td>
                                @if ($editProjectMemberPermission == 'all')
                                    <x-forms.radio fieldId="project_admin_{{ $member->user->id }}"
                                        ...
                                        :checked="($member->user->id == $site->project_admin) ? 'checked' : ''">
                                    </x-forms.radio>
                                @elseif($member->user->id == $site->project_admin)
                            ...
                            <td class="text-right pr-20">
                                @if ($member->user->id == $site->project_admin
                    ...
                    @empty
                        <tr>
                            <td colspan="5">
                                <x-cards.no-record icon="user" :message="__('team chat.noMemberAddedToProject')" />
                            </td>
                        </tr>
                    @endforelse
                </x-table>
...
<script>
    ...
    $('#add-site-member').click(function() {
        const url = "{{ route('site-members.create') }}" + "?id={{ $site->id }}";
    ...
    $('.delete-row').click(function() {
        ...
        var url = "{{ route('site-members.destroy', ':id') }}";
    ...
    $('.change-hourly-rate').blur(function() {
        ...
        var url = "{{ route('site-members.update', ':id') }}";
        url = url.replace(':id', id);
    ...
    $('body').on('click', '.assign_role', function() {
        ...
        $.easyAjax({
            url: "{{ route('sites.assign_project_admin') }}",
            type: "POST",
            data: {
                userId: userId,
                projectId: projectId,
