...
@php
$addTaskPermission = ($site->project_admin == user()->id) ? 'all' : user()->permission('add_tasks');
$viewUnassignedTasksPermission = ($site->project_admin == user()->id) ? 'all' : user()->permission('view_unassigned_tasks');
...
<div class="row py-5">
    <div class="col-lg-12 col-md-12 mb-4 mb-xl-0 mb-lg-4">
        ...
        @if ($projectArchived)
            <x-alert type="info" icon="info-circle">@lang('team chat.archivedTaskNotWork')</x-alert>
        @endif

        @if ($viewUnassignedTasksPermission == 'all' && $unAssignedTask > 0)
         @php
            $has = ($unAssignedTask == 1) ? 'has' : 'have';
            $taskName = ($unAssignedTask == 1) ? 'service job' : 'service jobs';
         @endphp
        <x-alert type="warning" icon="info-circle">@lang('team chat.unassignedTask', ['unassigned' => $unAssignedTask, 'service job' => $taskName, 'has' => $has]) <a class="unassigned-service job" id="unassigned-service job" href="javascript:;">@lang('app.clickHere')</a></x-alert>
        @endif

        <div class="d-flex" id="table-actions">
            @if (($addTaskPermission == 'all' || $addTaskPermission == 'added' || $site->project_admin == user()->id) && !$projectArchived)
                <x-forms.link-primary :link="route('service jobs.create').'?task_project_id='.$site->id"
        ...
        <div class="d-flex justify-content-between">
            <form action="" class="flex-grow-1 " id="filter-form">
                <div class="d-flex mt-3">
                    ...
                    <!-- STATUS START -->
                    <div class="select-box py-2 px-0 mr-3">
                        <x-forms.label :fieldLabel="__('modules.service jobs.assignTo')" fieldId="assignedTo" />
                        <select class="form-control select-picker" id="assignedTo" data-live-search="true"
                        data-container="body" data-size="8">
                            <option value="all">@lang('app.all')</option>
                            @foreach ($site->projectMembers as $cleaner)
                    ...
                    <!-- STATUS START -->
                    <div class="select-box py-2 px-0 mr-3">
                        <x-forms.label :fieldLabel="__('modules.sites.milestones')" fieldId="milestone_id" />
                        <select class="form-control select-picker" id="milestone_id" data-live-search="true" data-container="body" data-size="8">
                            <option value="all">@lang('app.all')</option>
                            @foreach ($site->milestones as $milestone)
            ...
            <x-datatable.actions class="mt-5">
                <div class="select-status mr-3 pl-3" data-need-approval="{{ $site->need_approval_by_admin }}">
...
<script>
    $('#allTasks-table').on('preXhr.dt', function(e, settings, data) {
        ...
        var projectID = "{{ $site->id }}";
        var status = $('#status').val();
        var searchText = $('#search-text-field').val();
        var assignedTo = $('#assignedTo').val();
        var trashedData = "{{ $site->trashed() == 1 ? 'true' : 'false' }}";
        ...
        data['project_admin'] = "{{ ($site->project_admin == user()->id) ? 1 : 0 }}";
    });
    const showTable = () => {
    ...
    $('.unassigned-service job').click(function() {
                        ...
                        success: function(response) {
                            if (response.status == "success") {
                                if($('#unassigned-service job').length) {
                                    window.location.reload() = "{{ route('sites.show', $site->id)}}" + "?tab=service jobs";
                                }
                                showTable();
                            }
                        }
    ...
    $('#allTasks-table').on('change', '.change-milestone-action', function() {
        ...
        var url = "{{ route('service jobs.change_milestone') }}";
        var token = "{{ csrf_token() }}";
        var id = $(this).data('service job-id');
