...
$editTaskPermission = ($site->project_admin == user()->id) ? 'all' : user()->permission('edit_tasks');
@endphp
...
<div class="row py-3 py-lg-5 py-lg-5">
    <div class="col-lg-12 col-md-12 mb-4 mb-xl-0 mb-lg-4">
        ...
        <div class="d-flex align-items-center" id="table-actions">
            @if (($addTaskPermission == 'all' || $addTaskPermission == 'added') && !$site->trashed())
                <x-forms.link-primary :link="route('service jobs.create').'?task_project_id='.$site->id"
                    class="mr-3 openRightModal" icon="plus" data-redirect-url="{{ route('sites.show', $site->id).'?tab=gantt' }}">
                    @lang('app.addTask')
            ...
            {{-- <div>
                <x-forms.checkbox :checked="($hideCompleted == 1)"
                    :fieldLabel="__('modules.service jobs.hideCompletedTask')"
...
<div class="row ">
    <div class="col-lg-12 col-md-12 mb-4 mb-xl-0 mb-lg-4">
        <!-- Add Service Job Export Buttons Start -->
        <div class="d-lg-flex d-md-flex d-block flex-wrap customer-list-filter">
            ...
            <div class="select-box d-flex pr-2">
                <p class="mb-0 pr-2 f-14 text-dark-grey d-flex align-items-center">@lang('app.service job') @lang('app.status')</p>
                <div class="select-status">
...
<script>
    $(function() {
        ...
        function renderGantt() {
            ...
            let service jobs = [...ganttData.data];
            ...
            if (hasRange) {
                ...
                const tasksInRange = ganttData.data.filter(row =>
                    row.type === 'service job' && inRange(row.start_date)
                );
            ...
            const links = ganttData.links.filter(l => {
                return service jobs.find(t => t.id == l.source) &&
                    service jobs.find(t => t.id == l.target);
            });
            const tasksAreEmpty = service jobs.length === 0;
            ...
            gantt.clearAll();
            gantt.parse({ data: service jobs, links: links });
...
var ganttData = @json($ganttData);
    ...
    gantt.attachEvent("onTaskCreated", function(service job){
        service job[gantt.config.resource_property] = [];
        return true;
    });
    ...
    gantt.templates.tooltip_text = function(start,end,service job){
        return service job.view;
    };
    ...
    gantt.templates.timeline_cell_class = function(service job,date){
        if (date.getDay()==0||date.getDay()==6) {
            return "weekend" ;
        }
    };
    ...
    gantt.templates.task_text = function(start, end, service job){
        return service job.text;
    };

    gantt.templates.rightside_text = function(start, end, service job){
        if (service job.type == "milestone" || service job.type == "site") {
            return false;
        }

        return service job.text_user;
    };
...
<script>
    ...
    gantt.attachEvent("onLinkCreated", function(link){
        ...
        $.easyAjax({
            ...
            data: { source: link.source, target: link.target, type: link.type, site: '{{ $site->id }}', '_token': token},
    ...
    gantt.attachEvent("onAfterTaskDrag", function(id, mode, e){
        var service job = gantt.getTask(id);

        var url = "{{ route('gantt_link.task_update') }}";
        var token = "{{ csrf_token() }}";
        var start_date = new Date(service job.start_date);
        var end_date = new Date(service job.end_date);
    ...
    gantt.attachEvent("onBeforeTaskDrag", function(id, mode, e){
        ...
        if (service job.type == "milestone" || allowDrag == 'false') {
            return false;
        }
    ...
    {{-- $('#hide_completed_tasks').change(function () {
        if($(this).is(':checked')){
            window.location.href = "{{ route('sites.show', $site->id).'?tab=gantt' }}" + '&hide_completed=1';
        } else {
            window.location.href = "{{ route('sites.show', $site->id).'?tab=gantt' }}";
        }
    }); --}}
