...
<div class="row pb-5">
    <div class="col-lg-12 col-md-12 mb-4 mb-xl-0 mb-lg-4 mt-3 mt-lg-5 mt-md-5">
        ...
        <div class="d-flex" id="table-actions">
            @if (($addProjectNotePermission == 'all' || $addProjectNotePermission == 'added' || $site->project_admin == user()->id) && !$site->trashed())
                <x-forms.link-primary :link="route('site-notes.create').'?site='.$site->id"
...
<script>
    $('#site-notes-table').on('preXhr.dt', function(e, settings, data) {
        var projectID = "{{ $site->id }}";
        data['projectID'] = projectID;
    });
    const showTable = () => {
        window.LaravelDataTables["site-notes-table"].draw(true);
    }
    ...
    $('body').on('click', '.delete-table-row', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                var url = "{{ route('site-notes.destroy', ':id') }}";
    ...
    const applyQuickAction = () => {
        var rowdIds = $("#site-notes-table input:checkbox:checked").map(function() {
            return $(this).val();
        }).get();
    ...
    $('body').on('click', '.ask-for-password', function() {
        let projectNoteId = $(this).data('site-note-id');
    ...
    var getNoteDetail = function(id, formType) {
        ...
        if(formType == 'view') {
            var url = "{{ route('site-notes.show', ':id') }}";
        } else {
            var url = "{{ route('site-notes.edit', ':id') }}";
        }
