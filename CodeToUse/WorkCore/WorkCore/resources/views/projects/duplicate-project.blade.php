...
<div class="modal-body">
    ...
    <x-form id="createDuplicateProject">
        <input type="hidden" name="duplicateProjectID" value="{{$site->id}}">
        <input type="hidden" name="type" value="duplicateProject">
        <div class="row">
            <div class="col-sm-12 mt-2">
                <x-forms.checkbox fieldId="service job"
                    :fieldLabel="__('app.service job')"
                    fieldName="service job"/>
                    <div class="ml-3 mt-2 d-none" id="service job-options">
                        <x-forms.checkbox fieldId="sub_task"
                        :fieldLabel="__('app.copy').' '.__('modules.service jobs.subTask')"
                        fieldName="sub_task"/>
                        <x-forms.checkbox fieldId="same_assignee"
                        :fieldLabel="__('modules.sites.sameAssignee')"
        ...
        <div class="row border-top-grey mt-3 d-none" id="service job-status">
            <div class="col-md-12">
        ...
        <div class="row border-top-grey mt-3">
            ...
            <div class="col-md-12">
                <x-forms.text class="mr-0 mr-lg-2 mr-md-2" :fieldLabel="__('modules.sites.projectName')"
                            ...
                            :fieldValue="$site ? $site->project_name : '' "/>
            </div>
            ...
            @endif
            <div class="@if(!in_array('customers', user_modules())) d-none @endif col-md-12 @if (!isset($customer) && is_null($customer)) py-3 @endif">
                ...
                @else
                    <x-forms.select fieldName="client_id" fieldId="client_id"
                        :fieldLabel="__('app.customer')">
                        <option value="">--</option>
                        @foreach ($customers as $clientOpt)
                            <option data-content="<x-customer-search-option :user='$clientOpt' />"
        ...
        <div class="modal-footer">
            ...
            <x-forms.button-primary id="save-copy-site" icon="check">@lang('app.copyProject')</x-forms.button-primary>
        </div>
    </x-form>
</div>
...
<script>
    ...
    $('#service job').change(function () {
        if($(this).is(':checked')){
            $('#service job-options').removeClass('d-none');
            $('#service job-status').removeClass('d-none');
        } else {
            $('#service job-options').addClass('d-none');
            $('#service job-status').addClass('d-none');
        }
    });
    ...
    $('#save-copy-site').click(function() {
        var url = "{{ route('sites.store') }}";
        $.easyAjax({
            url: url,
