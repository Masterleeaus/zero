            ...
            <div class="select-status">
                <select class="form-control select-picker" name="project_id" id="project_id" data-live-search="true"
                    ...
                    @foreach ($sites as $site)
                        <option value="{{ $site->id }}">{{ $site->project_name }}
                        </option>
                    @endforeach
                </select>
            </div>
...
@section('content')
    ...
    <div class="content-wrapper">
        ...
        <div class="d-flex flex-column">
            <!-- TASK STATUS START -->
            <x-cards.data id="service job-chart-card" :title="__('app.menu.service jobs')" padding="false" class="py-3">
            </x-cards.data>
            <!-- TASK STATUS END -->
            ...
            <div class="d-grid d-lg-flex d-md-flex action-bar align-items-center mt-4">
                <div id="table-actions" class="flex-grow-1 align-items-center">
                    ...
                    <div class="btn-group mt-2 mt-lg-0 mt-md-0 ml-0 ml-lg-3 ml-md-3" role="group">
                        <a href="{{ route('service job-report.index') }}" class="btn btn-secondary f-14 btn-active" data-toggle="tooltip"
                            data-original-title="@lang('app.menu.taskReport')"><i class="side-icon bi bi-list-ul"></i></a>

                            <a href="{{ route('cleaner-wise-service job-report') }}" class="btn btn-secondary f-14" data-toggle="tooltip"
                            data-original-title="@lang('modules.service jobs.employeeWiseTaskReport')"><i class="side-icon bi bi-people-fill"></i></a>

                            <a href="{{ route('consolidated-service job-report') }}" class="btn btn-secondary f-14 " data-toggle="tooltip"
                             data-original-title="@lang('modules.service jobs.consolidatedTaskReport')"><i class="side-icon bi bi-calendar-fill"></i></a>
                    </div>
                </div>
...
@push('scripts')
    ...
    <script>
        ...
        function pieChart() {
            ...
            var url = "{{ route('service job-report.chart') }}";

            $.easyAjax({
                url: url,
                container: '#service job-chart-card',
                blockUI: true,
                type: "POST",
                data: {
                    clientID: clientID,
                ...
                success: function(response) {
                    $('#service job-chart-card').html(response.html);
                }
            });
        }
        pieChart();
