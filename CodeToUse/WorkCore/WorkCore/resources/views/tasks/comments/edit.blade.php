<div class="modal-header">
    <h5 class="modal-title" id="modelHeading">@lang('modules.service jobs.comment')</h5>
                ...
                <div class="media">
                    ...
                    <div class="media-body bg-white">
                        <div class="form-group">
                            <div id="service job-edit-comment">{!! $comment->comment !!}</div>
                            <textarea name="comment" class="form-control invisible d-none"
                                id="service job-edit-comment-text">{!!  $comment->comment !!}</textarea>
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    ...
    $(document).ready(function() {
            ...
            if (add_task_comments == "all" || add_task_comments == "added") {
                quillMention(atValues, '#service job-edit-comment');
            }

            $('#save-edit-comment').click(function() {
            var comment = document.getElementById('service job-edit-comment').children[0].innerHTML;
            document.getElementById('service job-edit-comment-text').value = comment;
            var mention_user_id = $('#service job-edit-comment span[data-id]').map(function(){
            ...
            $.easyAjax({
                ...
                data: {
                    ...
                    '_method': 'PUT',
                    taskId: '{{ $comment->service job->id }}'
                },
                success: function(response) {
                    if (response.status == "success") {
                        destory_editor('#service job-edit-comment');
