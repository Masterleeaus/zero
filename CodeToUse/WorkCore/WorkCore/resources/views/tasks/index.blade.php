...
@section('filter-section')
    ...
    <x-filters.filter-box>
        ...
        <div class="select-box d-flex py-2 px-lg-2 px-md-2 px-0 border-right-grey border-right-grey-sm-0">
            ...
            <div class="select-status">
                <select class="form-control select-picker" name="status" id="status" data-live-search="true" data-size="8">
                    <option value="not finished">@lang('modules.service jobs.hideCompletedTask')</option>
        ...
        <x-filters.more-filter-box>
            ...
            <div class="more-filter-items">
                <label class="f-14 text-dark-grey mb-12 " for="usr">@lang('modules.issues / support.type')</label>
                <div class="select-filter mb-4">
            ...
            @if (in_array('sites', user_modules()))
                <div class="more-filter-items">
                    <label class="f-14 text-dark-grey mb-12 " for="usr">@lang('app.site')</label>
                    <div class="select-filter mb-4">
            ...
            @endif
            <div class="more-filter-items">
                <label class="f-14 text-dark-grey mb-12 " for="usr">@lang('app.customer')</label>
                <div class="select-filter mb-4">
                    <div class="select-others">
                        <select class="form-control select-picker" id="clientID" data-live-search="true"
                            ...
                            @foreach ($customers as $customer)
                                <x-user-option :user="$customer"></x-user-option>
                            @endforeach
                        </select>
                    </div>
                </div>
            ...
            <div class="more-filter-items">
                <label class="f-14 text-dark-grey mb-12 " for="usr">@lang('modules.service jobs.assignTo')</label>
                <div class="select-filter mb-4">
                    <div class="select-others">
                        <select class="form-control select-picker" id="assignedTo" data-live-search="true"
                            ...
                            @if ($viewUnassignedTasksPermission == 'all')
                                <option value="unassigned">@lang('modules.service jobs.unassigned')</option>
                            @endif
                        </select>
                    </div>
                </div>
            ...
            <div class="more-filter-items">
                <label class="f-14 text-dark-grey mb-12 " for="usr">@lang('modules.service jobs.assignBy')</label>
                <div class="select-filter mb-4">
                    <div class="select-others">
            ...
            <div class="more-filter-items">
                <label class="f-14 text-dark-grey mb-12 " for="usr">@lang('modules.service jobs.priority')</label>
                <div class="select-filter mb-4">
                    <div class="select-others">
            ...
            @if (in_array('sites', user_modules()))
                <div class="more-filter-items">
                    <label class="f-14 text-dark-grey mb-12 " for="usr">@lang('modules.sites.milestones')</label>
                    <div class="select-filter mb-4">
                        <div class="select-others">
                            <select class="form-control select-picker" id="milestone_id" data-live-search="true"
                                ...
                                @foreach ($milestones as $milestone)
                                    <option value="{{ $milestone->id }}">{{ $milestone->milestone_title . ($milestone->site->project_short_code ? ' (' . $milestone->site->project_short_code . ')' : '')}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
        ...
        <div class="d-grid d-lg-flex d-md-flex action-bar">
            <div id="table-actions" class="flex-grow-1 align-items-center">
                @if ($addTaskPermission == 'all' || $addTaskPermission == 'added')
                    <x-forms.link-primary :link="route('service jobs.create')" class="mr-3 openRightModal float-left" icon="plus">
                        @lang('app.addTask')
                    </x-forms.link-primary>
                @endif

                @if (!in_array('customer', user_roles()) && !in_array($viewPermission, ['added', 'owned', 'both']))
                    <x-forms.button-secondary id="filter-my-service job" class="mr-3 float-left" icon="user">
                        @lang('modules.service jobs.myTask')
                    </x-forms.button-secondary>
                @endif

                @if ($recurringTaskPermission == 'all')
                     <x-forms.link-secondary :link="route('recurring-service job.index')" class="mr-3 float-left"
                        icon="sync">
            ...
            <x-datatable.actions>
                ...
                <div class="select-status mr-3 d-none quick-action-field" id="change-status-action">
                    <select name="status" class="form-control select-picker">
                        @foreach ($taskBoardStatus as $status)
                            @if ($status->slug != 'waiting_approval')
                                <option value="{{ $status->id }}">{{ $status->slug == 'completed' || $status->slug == 'incomplete' ? __('app.' . $status->slug) : $status->column_name }}</option>
                            @endif
                        @endforeach
                    </select>
            ...
            <div class="btn-group mt-2 mt-lg-0 mt-md-0 ml-0 ml-lg-3 ml-md-3" role="group">
                <a href="{{ route('service jobs.index') }}" class="btn btn-secondary f-14 btn-active service job" data-toggle="tooltip"
                    data-original-title="@lang('app.menu.service jobs')"><i class="side-icon bi bi-list-ul"></i></a>

                <a href="{{ route('taskboards.index') }}" class="btn btn-secondary f-14" data-toggle="tooltip"
                    data-original-title="@lang('modules.service jobs.taskBoard')"><i class="side-icon bi bi-kanban"></i></a>

                <a href="{{ route('service job-calendar.index') }}" class="btn btn-secondary f-14" data-toggle="tooltip"
                ...
                @if(in_array('admin', user_roles()) || in_array('cleaner', user_roles()))
                    <a href="{{ route('service jobs.waiting-approval') }}" class="btn btn-secondary f-14 show-waiting-approval-service job" data-toggle="tooltip"
...
@push('scripts')
    ...
    <script>
        ...
        $('.show-pinned').click(function() {
            $('.service job').removeClass('btn-active');
        ...
        $('#reset-filters,#reset-filters-2').click(function() {
            ...
            document.getElementById('filter-my-service job').style.setProperty('background-color', '#ffff', 'important');
            document.getElementById('filter-my-service job').style.setProperty('color', '#616e80', 'important');
            document.getElementById('filter-my-service job').style.setProperty('border', '1px solid #616e80', 'important');
        ...
        $('body').on('click', '.delete-table-row', function() {
            ...
            if (activeRunning == 1) {
                ...
                Swal.fire({
                    title: "@lang('team chat.taskTimerRunning')",
                    text: "@lang('team chat.stopTheTimer')",
                    icon: 'warning',
                    showConfirmButton: true,
                    confirmButtonText: "@lang('team chat.timerOkay')",
                    customClass: {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        var url = "{{ route('service jobs.destroy', ':id') }}";
        ...
        const applyQuickAction = () => {
            ...
            var url = "{{ route('service jobs.apply_quick_action') }}?row_ids=" + rowdIds;
        ...
        $('#allTasks-table').on('change', '.change-status', function() {
            ...
            var token = "{{ csrf_token() }}";
            var id = $(this).data('service job-id');
            var status = $(this).val();
            var needApproval = $(this).data('need-approval');
            var projectAdmin = $(this).data('site-admin');
        ...
        $('#filter-my-service job').click(function () {

            document.getElementById('filter-my-service job').style.setProperty('background-color', '#5a645a', 'important');
            document.getElementById('filter-my-service job').style.setProperty('color', 'white', 'important');
        ...
        $('#allTasks-table').on('click', '.start-timer', function() {
            ...
            var user_id = "{{ user()->id }}";
            var token = "{{ csrf_token() }}";
            var task_id = $(this).data('service job-id');
            var memo = "{{ __('app.service job') }}#" + $(this).data('service job-id');
