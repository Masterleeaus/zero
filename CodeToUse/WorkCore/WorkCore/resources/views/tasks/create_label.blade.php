...
<div class="modal-body">
    <x-table class="table-bordered" headType="thead-light">
        <x-slot name="thead">
            ...
            @if (in_array('sites', user_modules())) <th>@lang('app.site')</th> @endif
        ...
        @forelse($taskLabels as $key=>$item)
            <tr id="label-{{ $item->id }}">
                ...
                @if (in_array('sites', user_modules()))
                    <td data-row-id="{{ $item->id }}" data-column="site">
                        <select class="form-control select-picker change-site" name="site" id="project_id"
                        data-live-search="true" data-size="8" data-label-id="{{ $item->id }}">
                            <option value="">--</option>
                            @foreach ($sites as $site)
                                <option @selected($site->id == $item->project_id) value="{{ $site->id }}">
                                    {{ $site->project_name }}
                                </option>
                            @endforeach
                        </select>
    ...
    <x-form id="createTaskLabelForm">
        <div class="row border-top-grey ">
            ...
            @if (in_array('sites', user_modules()))
                <div class="col-md-6">
                    <x-forms.select fieldId="project_id" :fieldLabel="__('app.site')" fieldName="project_id"
                    search="true">
                        <option value="">--</option>
                        @foreach($sites as $site)
                            <option @selected($site->id == $projectId) value="{{ $site->id }}">{{ $site->project_name }}</option>
                        @endforeach
                    </x-forms.select>
                    <input type="hidden" name="parent_project_id" value="{{ $projectId }}">
                </div>
...
<script>
    ...
    $('.delete-label').click(function() {
        ...
        var url = "{{ route('service job-label.destroy', ':id') }}";
    ...
    $('#save-label').click(function() {
        var url = "{{ route('service job-label.store') }}";
        $.easyAjax({
            url: url,
    ...
    $('[contenteditable=true]').focus(function() {
        ...
        if ($(this).data("initialText") !== $(this).html()) {
            ...
            if(id){
                var url = "{{ route('service job-label.update', ':id') }}";
                url = url.replace(':id', id);
                var token = "{{ csrf_token() }}";
