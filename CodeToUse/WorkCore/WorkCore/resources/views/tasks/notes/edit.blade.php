                ...
                <div class="media">
                    ...
                    <div class="media-body bg-white">
                        <div class="form-group">
                            <div id="service job-edit-note">{!! $note->note !!}</div>
                            <textarea name="note" class="form-control invisible d-none"
                                id="service job-edit-note-text">{!!  $note->note !!}</textarea>
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        if (edit_task_notes == "all" || edit_task_notes == "added") {
            quillMention(atValues, '#service job-edit-note');
        }

        $('#save-edit-note').click(function() {
            var note = document.getElementById('service job-edit-note').children[0].innerHTML;
            document.getElementById('service job-edit-note-text').value = note;
            var mention_user_id = $('#service job-edit-note span[data-id]').map(function(){
                return $(this).attr('data-id')
            }).get();

            var token = '{{ csrf_token() }}';

            const url = "{{ route('service job-note.update', $note->id) }}";
