...
<div class="tab-pane fade show active" role="tabpanel" aria-labelledby="nav-email-tab">
    ...
    || ($addTaskNotePermission == 'added' && ($service job->added_by == user()->id || $service job->added_by == $userId || in_array($service job->added_by, $clientIds)))
    || ($addTaskNotePermission == 'owned' && in_array(user()->id, $taskUsers))
    || ($addTaskNotePermission == 'both' && (in_array(user()->id, $taskUsers) || $service job->added_by == user()->id || $service job->added_by == $userId || in_array($service job->added_by, $clientIds)))
    )

        <div class="row p-20">
            <div class="col-md-12">
                <a class="f-15 f-w-500" href="javascript:;" id="add-notes"><i
                        class="icons icon-plus font-weight-bold mr-1"></i>@lang('modules.customer.createNote')
                    </a>
            </div>
        </div>
        ...
        @if ($service job->approval_send == 1 && $isEmployee && !$isAdmin && $service job->site->need_approval_by_admin == 1 && $status->slug == 'waiting_approval')
                    ...
                    <div class="media">
                        ...
                        <div class="media-body bg-white">
                            <div class="form-group">
                                <div id="service job-note"></div>
                                <textarea name="note" class="form-control invisible d-none" id="service job-note-text"></textarea>
                            </div>
                        </div>
                    </div>
    ...
    <div class="d-flex flex-wrap justify-content-between p-20" id="note-list">
        @forelse($service job->notes as $note)
                ...
                <div class="card-horizontal">
                    ...
                    <div class="card-body border-0 pl-0 py-1">
                        <div class="d-flex flex-grow-1">
                            ...
                            $deleteTaskNotePermission == 'all' || ($deleteTaskNotePermission == 'added' && ($note->added_by == user()->id || $note->added_by == $userId || in_array($note->added_by, $clientIds))))
                         ...
                         @php
                            ...
                            $customer = \App\Models\User::where('id', $note->added_by)->where('is_client_contact', 1)->first();
                        @endphp
                        @if(($isClient == true) && $customer)
        ...
        @empty
            <x-cards.no-record :message="__('team chat.noNoteFound')" icon="clipboard"/>
        @endforelse
    </div>
...
<script>
    ...
    $(document).ready(function () {
        ...
        if (add_task_notes == "all" || add_task_notes == "added") {
            quillMention(atValues, '#service job-note');
        }


        $('#submit-note').click(function () {
            var note = document.getElementById('service job-note').children[0].innerHTML;
            document.getElementById('service job-note-text').value = note;
            var mention_user_id = $('#service job-note span[data-id]').map(function(){
                return $(this).attr('data-id')

            }).get();
            var token = '{{ csrf_token() }}';

            const url = "{{ route('service job-note.store') }}";
            ...
            $.easyAjax({
                ...
                success: function (response) {
                    if (response.status == "success") {
                        ...
                        document.getElementById('service job-note').children[0].innerHTML = "";
                        $('#service job-note-text').val('');
                    }
