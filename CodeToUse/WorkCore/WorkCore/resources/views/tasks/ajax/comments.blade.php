...
<div class="tab-pane fade show active" role="tabpanel" aria-labelledby="nav-email-tab">
    ...
    || ($addTaskCommentPermission == 'added' && ($service job->added_by == user()->id || $service job->added_by == $userId))
    || ($addTaskCommentPermission == 'owned' && in_array(user()->id, $taskUsers))
    || ($addTaskCommentPermission == 'both' && (in_array(user()->id, $taskUsers) || $service job->added_by == user()->id || $service job->added_by == $userId))
    )
        <div class="row p-20">
            <div class="col-md-12">
                <a class="f-15 f-w-500" href="javascript:;" id="add-comment"><i
                        class="icons icon-plus font-weight-bold mr-1"></i>@lang('modules.service agreements.addComment')
                </a>
            </div>
        </div>
                    ...
                    <div class="media">
                        ...
                        <div class="media-body bg-white">
                            <div class="form-group">
                                <div id="service job-comment"></div>
                                <textarea name="comment" class="form-control invisible d-none"
                                    id="service job-comment-text"></textarea>
                            </div>
                        </div>
                    </div>
    ...
    <div class="d-flex flex-wrap justify-content-between p-20" id="comment-list">
        @forelse ($service job->comments as $comment)
                        ...
                        @php
                            ...
                            if($likeUsers)
                            {
                                if(in_array(user()->name, $likeUsers)){
                                    ...
                                    array_splice( $likeUsers, 0, 0, __('modules.service jobs.you') );
                            ...
                            if($dislikeUsers)
                            {
                                if(in_array(user()->name, $dislikeUsers)){
                                    ...
                                    array_splice( $dislikeUsers, 0, 0, __('modules.service jobs.you') );
                            ...
                            $customer = \App\Models\User::where('id', $comment->added_by)->first();
                        @endphp
                        @if(($isClient == true) && ($comment->added_by != $comment->user->id) && $customer)
                            <div class="text-grey f-10 float-left mt-0">{{ __('(Added By : ') . $customer->name . ')' }}</div><br/>
                        @endif
                        <div class="card-text f-14 text-dark-grey ">
                            ...
                            <div id="emoji-{{$comment->id}}">
                                <button class="btn cursor-pointer comment-like mr-2 f-12 btn-sm" data-comment-id="{{ $comment->id }}" data-toggle="tooltip"
                                    data-emoji="thumbs-up" @if($comment->like->count() != 0) data-original-title="{{ trans('modules.service jobs.likeUser', [ 'user' => $likeUserList ]) }}" style="background-color: #f7f2f2;" @else data-original-title="@lang('modules.service jobs.like')" @endif>
                                ...
                                <button class="btn cursor-pointer comment-like f-12 btn-sm" data-comment-id="{{ $comment->id }}" data-toggle="tooltip"
                                    data-emoji="thumbs-down" @if($comment->dislike->count() != 0) data-original-title="{{ trans('modules.service jobs.dislikeUser', [ 'user' => $dislikeUserList ]) }}" style="background-color: #f7f2f2;" @else data-original-title="@lang('modules.service jobs.dislike')" @endif>
...
<script>
    ...
    $(document).ready(function() {
        if (add_task_comments == "all" || add_task_comments == "added") {
            quillMention(userValues, '#service job-comment');
        }
        });


        $('#submit-comment').click(function() {
            var comment = document.getElementById('service job-comment').children[0].innerHTML;
            document.getElementById('service job-comment-text').value = comment;
            var mention_user_id = $('#service job-comment span[data-id]').map(function(){
            ...
            $.easyAjax({
                ...
                success: function(response) {
                    if (response.status == "success") {
                        ...
                        document.getElementById('service job-comment').children[0].innerHTML = "";
                        $('#service job-comment-text').val('');
                    }
