...
<div class="tab-pane fade show active" role="tabpanel" aria-labelledby="nav-email-tab">
    ...
    || ($addTaskFilePermission == 'added' && ($service job->added_by == user()->id || $service job->added_by == $userId || in_array($file->added_by, $clientIds)))
    || ($addTaskFilePermission == 'owned' && in_array(user()->id, $taskUsers))
    || ($addTaskFilePermission == 'both' && (in_array(user()->id, $taskUsers) || $service job->added_by == user()->id || $service job->added_by == $userId || in_array($file->added_by, $clientIds)))
    )
        <div class="p-20">
            ...
            @else
                <x-form id="save-taskfile-data-form" class="d-none">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-12">
                            <x-forms.file-multiple fieldLabel="" fieldName="file[]" fieldId="service job-file-upload-dropzone"/>
                        </div>
                        <div class="col-md-12">
    ...
    <div class="d-flex flex-wrap p-20" id="service job-file-list">
        @php
        ...
        @endphp
        @forelse($service job->files as $file)
        ...
        @if ($filesShowCount == 0 && $service job->files->count() > 0)
            <x-cards.no-record :message="__('team chat.noFileUploaded')" icon="file"/>
        @endif
    </div>
    ...
    $(document).ready(function () {
        ...
        if (add_task_files == "all" || add_task_files == "added") {
            ...
            taskDropzone.on('completemultiple', function (file) {
                ...
                $('#service job-file-list').html(taskView);
            });
