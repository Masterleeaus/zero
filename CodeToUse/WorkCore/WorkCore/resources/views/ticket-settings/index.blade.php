                ...
                <div class="s-b-n-header" id="tabs">
                    <nav class="tabs px-6 border-bottom-grey">
                        <div class="nav" id="nav-tab" role="tablist">
                            ...
                            <a class="nav-item nav-link f-15 active agent" href="{{ route('issue / support-settings.index') }}"
                            ...
                            @if($manageGroupPermission == 'all')
                                <a class="nav-item nav-link f-15 group-manage"
                                    href="{{ route('issue / support-settings.index') }}?tab=group-manage" role="tab"
                                    aria-controls="nav-groupManage" aria-selected="true">@lang('app.menu.groupManage')
                                </a>
                            @endif

                            <a class="nav-item nav-link f-15 type" href="{{ route('issue / support-settings.index') }}?tab=type"
                                role="tab" aria-controls="nav-ticketTypes"
                                aria-selected="true">@lang('app.menu.ticketTypes')
                            </a>

                            <a class="nav-item nav-link f-15 channel"
                                href="{{ route('issue / support-settings.index') }}?tab=channel" role="tab"
                            ...
                            <a class="nav-item nav-link f-15 reply-template"
                                href="{{ route('issue / support-settings.index') }}?tab=reply-template" role="tab"
                                aria-controls="nav-replyTemplates" aria-selected="true">@lang('app.menu.replyTemplates')
                            </a>

                            <a class="nav-item nav-link f-15 method"
                                href="{{ route('issue / support-settings.index') }}?tab=round-robin" role="tab"
                                aria-controls="nav-leadAgent" aria-selected="true">@lang('modules.deal.dealMethod')
                            </a>

                            <a class="nav-item nav-link f-15 issue / support-agent-setting"
                                href="{{ route('issue / support-settings.index') }}?tab=issue / support-agent-setting" role="tab"
...
{{--
                            <a class="nav-item nav-link f-15 email-sync"
                                href="{{ route('issue / support-settings.index') }}?tab=email-sync" role="tab"
       ...
       $("body").on("click", "#editSettings .nav a", function(event) {
        ...
        $('body').on('click', '.delete-agents', function() {
            ...
            Swal.fire({
                ...
                text: "@lang('team chat.removeAgentText')",
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    var url = "{{ route('issue / support-agents.destroy', ':id') }}";
        ...
        $('body').on('change', '.change-agent-status', function() {
            ...
            var token = '{{ csrf_token() }}';
            var url = "{{ route('issue / support-agents.update', ':id') }}";
        ...
        $('body').on('click', '.delete-type', function() {
            ...
            Swal.fire({
                title: "@lang('team chat.sweetAlertTitle')",
                text: "@lang('team chat.removeTicketText')",
                icon: 'warning',
                showCancelButton: true,
                focusConfirm: false,
                confirmButtonText: "@lang('team chat.confirmDelete')",
        ...
        $('body').on('click', '.delete-channel', function() {
            ...
            Swal.fire({
                title: "@lang('team chat.sweetAlertTitle')",
                text: "@lang('team chat.removeChannelText')",
                icon: 'warning',
                showCancelButton: true,
                focusConfirm: false,
                confirmButtonText: "@lang('team chat.confirmDelete')",
        ...
        $('body').on('click', '.delete-template', function() {
            ...
            Swal.fire({
                title: "@lang('team chat.sweetAlertTitle')",
                text: "@lang('team chat.removeTemplateText')",
                icon: 'warning',
                showCancelButton: true,
                focusConfirm: false,
                confirmButtonText: "@lang('team chat.confirmDelete')",
        ...
        $('body').on('click', '.delete-group', function() {
            ...
            Swal.fire({
                title: "@lang('team chat.sweetAlertTitle')",
                text: "@lang('team chat.removeGroupText')",
                icon: 'warning',
                showCancelButton: true,
                focusConfirm: false,
                confirmButtonText: "@lang('team chat.confirmDelete')",
                ...
                if (result.isConfirmed) {
                    var url = "{{ route('issue / support-groups.destroy', ':id') }}";
                    url = url.replace(':id', id);
        ...
        $('body').on('click', '.edit-group', function() {
            ...
            var url = "{{ route('issue / support-groups.edit', ':id') }}";
