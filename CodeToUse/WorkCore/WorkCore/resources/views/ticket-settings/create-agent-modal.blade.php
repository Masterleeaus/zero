...
<div class="modal-body">
    <div class="portlet-body">
        <x-form id="createMethods" method="POST" class="ajax-form">
            <div class="row">
                <div class="col-md-6">
                    <x-forms.select fieldId="agent_id" :fieldLabel="__('modules.issues / support.chooseAgents')"
                        fieldName="user_id" search="true" fieldRequired="true">
                        @foreach ($cleaners as $emp)
                            <x-user-option :user="$emp" />
                        @endforeach
                    </x-forms.select>
                </div>
                <div class="col-md-6">
                    <x-forms.select fieldId="ticket_group_id" :fieldLabel="__('modules.issues / support.assignGroup')"
...
<script>
    ...
    $('#save-agent').click(function() {
        $.easyAjax({
            url: "{{ route('issue / support-agents.store') }}",
