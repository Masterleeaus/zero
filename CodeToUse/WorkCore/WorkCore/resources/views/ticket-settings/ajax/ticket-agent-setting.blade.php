...
<div class="p-20 w-100">
    ...
    <x-form id="save-issue / support-agent-setting-data-form">
        <div class="col-lg-12">
            <input type="hidden" name="company_id" id="company_id" value="{{$company->id}}" />
            <input type="hidden" name="user_id" id="user_id" value="{{$user->id}}" />

            <div class="form-group my-3">
                <x-forms.label fieldId="issue / support-visibility-scope" :fieldLabel="__('modules.ticketSetting.ticketVisibilityScope')">
                </x-forms.label>
                <div class="">
                    <div class="">
                        <x-forms.radio fieldId="issues / support-all" :fieldLabel="__('modules.ticketSetting.allTickets')"
                    ...
                    <div class="">
                        <x-forms.radio fieldId="issues / support-group" :fieldLabel="__('modules.ticketSetting.groupTickets')"
                    ...
                    <div class="">
                        <x-forms.radio fieldId="issues / support-assigned" :fieldLabel="__('modules.ticketSetting.assignedTickets')"
            ...
            <x-form-actions class="c-inv-btns">
                <div class="d-flex">
                    <x-forms.button-primary class="mr-3" id="save-issue / support-agent-setting-form" icon="check">@lang('app.save')</x-forms.button-primary>
                </div>
            </x-form-actions>
...
<script>
    $(document).ready(function() {
        ...
        // Event listener for changes in the issue / support scope radio buttons
        $('input[name="ticket_scope"]').change(function() {
            toggleGroupSelect();
        });

        $('#save-issue / support-agent-setting-form').click(function() {
            ...
            var url = "{{ route('issue / support-agent-settings.update', ':id') }}";
            url = url.replace(':id', companyID);

            $.easyAjax({
                url: url,
                container: '#save-issue / support-agent-setting-data-form',
                type: "POST",
                blockUI: true,
                data:  {
                    '_token': token,
