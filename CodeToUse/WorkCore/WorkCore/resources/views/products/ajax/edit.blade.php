...
<div class="row">
    <div class="col-sm-12">
        <x-form id="save-service / extra-data-form" method="PUT">
            <div class="add-customer bg-white rounded">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-4">
                                <x-forms.number class="mr-0 mr-lg-2 mr-md-2" :fieldLabel="__('app.price')"
                                    ...
                                    :fieldValue="$service / extra->price" />
                            </div>
                                ...
                                <x-forms.input-group>
                                    <select class="form-control select-picker height-35" name="category_id"
                                        ...
                                        @foreach ($categories as $category)
                                            <option value="{{ $category->id }}" @if ($category->id == $service / extra->category_id) selected @endif>{{ $category->category_name }}</option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="col-md-4">
                                ...
                                <x-forms.input-group>
                                    <select class="form-control select-picker" name="sub_category_id"
                                        ...
                                        <option value="">@lang('team chat.noProductSubCategoryAdded')</option>
                                        @if ($service / extra->category_id)
                                            @foreach ($service / extra->category->subCategories as $category)
                                                <option value="{{ $category->id }}" @if ($category->id == $service / extra->sub_category_id) selected @endif>{{ $category->category_name }}</option>
                                            @endforeach
                                        @endif
                                    </select>
                                ...
                                <x-forms.input-group>
                                    <select class="form-control select-picker" name="tax[]" id="multiselect"
                                        ...
                                        @foreach ($taxes as $tax)
                                            <option value="{{ $tax->id }}" @if (isset($service / extra->taxes) && array_search($tax->id, json_decode($service / extra->taxes)) !== false) selected @endif>
                            ...
                            <div class="col-md-4">
                                <x-forms.text fieldId="hsn_sac_code" :fieldLabel="__('app.hsnSac')"
                                    ...
                                    :fieldValue="$service / extra->hsn_sac_code">
                                </x-forms.text>
                            </div>
                                ...
                                <x-forms.input-group>
                                    <select class="form-control select-picker" name="unit_type" id="unit_type_id"
                                        ...
                                        @foreach ($unit_types as $unit_type)
                                            <option value="{{ $unit_type->id }}" @if ($unit_type->id == $service / extra->unit_id) selected @endif>{{ $unit_type->unit_type }}
                                            </option>
                                        @endforeach
                                    </select>
                                </x-forms.input-group>
                            ...
                            <div class="col-lg-4 col-md-6 mt-5">
                                <x-forms.checkbox class="mr-0 mr-lg-2 mr-md-2" :fieldLabel="__('app.purchaseAllow')"
                                    ...
                                    fieldRequired="true" :checked="$service / extra->allow_purchase == 1" />
                            </div>
                            <div class="col-lg-4 col-md-6 mt-5">
                                <x-forms.checkbox class="mr-0 mr-lg-2 mr-md-2" :fieldLabel="__('app.downloadable')"
                                    ...
                                    fieldRequired="true" :popover="__('team chat.downloadable')" :checked="$service / extra->downloadable == 1" />
                            </div>

                            <div class="col-lg-12 col-xl-12  mt-2 downloadable {{$service / extra->downloadable ? '' : 'd-none'}}">
                                <x-forms.file class="mr-0 mr-lg-2 mr-md-2" :fieldLabel="__('app.downloadableFile')"
                                    fieldName="downloadable_file" fieldId="downloadable_file" fieldRequired="true" :fieldValue="$service / extra->download_file_url" />
                            </div>

                            <div class="col-md-4">
                                <x-forms.text fieldId="sku" :fieldLabel="__('app.sku')"
                                    fieldName="sku" :fieldPlaceholder="__('placeholders.sku')"
                                    :fieldValue="$service / extra->sku">
                                </x-forms.text>
                            </div>
                            ...
                            <div class="col-md-12 mt-3">
                                <div class="form-group">
                                    ...
                                    <textarea name="description" id="description-text" rows="4" class="form-control f-14 w-100">{{ $service / extra->description }}</textarea>
                                </div>
                            </div>
                ...
                <x-forms.custom-field :fields="$fields" :model="$service / extra"></x-forms.custom-field>

                <x-form-actions>
                    <x-forms.button-primary id="save-service / extra-form" class="mr-3" icon="check">@lang('app.save')
                    </x-forms.button-primary>
                    <x-forms.button-cancel :link="route('services / extras.index')" class="border-0">@lang('app.cancel')
...
<script>
    $(document).ready(function() {
        ...
        productDropzone = new Dropzone("div#file-upload-dropzone", {
            ...
            url: "{{ route('service / extra-files.update_images') }}",
            headers: {
                ...
                if(typeof(file.id) != 'undefined') {
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            ...
                            var url = "{{ route('service / extra-files.destroy', ':id') }}";
                            ...
                            $.easyAjax({
                                ...
                                success: function(response) {
                                    ...
                                    if ('{{ $service / extra->default_image }}' == file.hashname) {
        ...
        productDropzone.on('sending', function(file, xhr, formData) {
            var productID = '{{ $service / extra->id }}';
        ...
        productDropzone.on('successmultiple', function() {
            window.location.href = '{{ route("services / extras.index") }}';
        });
        ...
        $('#save-service / extra-form').click(function() {
            const url = "{{ route('services / extras.update', [$service / extra->id]) }}";

            $.easyAjax({
                url: url,
                container: '#save-service / extra-data-form',
                type: "POST",
                disableButton: true,
                blockUI: true,
                buttonSelector: "#save-service / extra-form",
                file: true,
                data: $('#save-service / extra-data-form').serialize(),
                success: function(response) {
