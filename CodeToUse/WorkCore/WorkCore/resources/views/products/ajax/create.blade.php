...
<div class="row">
    <div class="col-sm-12">
        <x-form id="save-service / extra-form">
            ...
            <div class="add-customer bg-white rounded">
                        ...
                        <div class="row">
                            ...
                            <div class="col-lg-4 col-md-6 mt-5">
                                <x-forms.checkbox class="mr-0 mr-lg-2 mr-md-2" :fieldLabel="__('app.downloadable')"
                                                  ...
                                                  fieldRequired="true" :popover="__('team chat.downloadable')"/>
                            </div>
                            ...
                            <div class="col-lg-12">
                                <x-forms.file-multiple class="mr-0 mr-lg-2 mr-md-2" :fieldLabel="__('app.menu.addFile')" fieldName="file" fieldId="file-upload-dropzone-service / extra"/>
                            </div>
                ...
                <x-form-actions>
                    <x-forms.button-primary id="save-service / extra" class="mr-3" icon="check">@lang('app.save')
                    </x-forms.button-primary>
                    <x-forms.button-secondary class="mr-3" id="save-more-service / extra" icon="check-double">@lang('app.saveAddMore')
...
<script>
    $(document).ready(function () {
        ...
        //Dropzone class
        productDropzone = new Dropzone("div#file-upload-dropzone-service / extra", {
            dictDefaultMessage: "{{ __('app.dragDrop') }}",
            url: "{{ route('service / extra-files.store') }}",
            headers: {
        ...
        $('#save-more-service / extra').click(function () {

            $('#add_more').val(true);

            const url = "{{ route('services / extras.store') }}";
            var data = $('#save-service / extra-form').serialize();

            saveProduct(data, url, "#save-more-service / extra");

        });

        $('#save-service / extra').click(function() {

            const url = "{{ route('services / extras.store') }}";
            var data = $('#save-service / extra-form').serialize();

            saveProduct(data, url, "#save-service / extra");
        ...
        function saveProduct(data, url, buttonSelector) {
            $.easyAjax({
                url: url,
                container: '#save-service / extra-form',
                type: "POST",
                disableButton: true,
                blockUI: true,
        ...
        function getProductOptions() {
            $.easyAjax({
                url: "{{ route('services / extras.options') }}",
                type: "GET",
                success: function (response) {
                    $('#add-services / extras').html(response.services / extras);
                    $('#add-services / extras').val('');
