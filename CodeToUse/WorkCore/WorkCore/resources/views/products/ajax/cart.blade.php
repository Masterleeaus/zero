...
<div class="bg-white rounded b-shadow-4 create-inv">
    ...
    <div class="d-block d-lg-flex d-md-flex justify-content-between action-bar">
        ...
        <div class="px-lg-4 px-md-4 px-3 py-3 cart_empty">
            <x-forms.link-primary :link="route('services / extras.empty_cart')" class="empty-cart"
                icon="trash">
                @lang('app.emptyCart')
    ...
    <x-form class="c-inv-form" id="saveInvoiceForm">
        @if (count($services / extras) == 0)
            <div class="row px-lg-4 px-md-4 px-3 py-5">
                <div class="col-sm-12">
                    <x-alert type="danger">@lang('team chat.addItem')</x-alert>
                </div>
            </div>
             ...
             <x-form-actions class="c-inv-btns d-block d-lg-flex d-md-flex">
                <div class="d-flex mb-3 mb-lg-0 mb-md-0">
                    ...
                    <x-forms.button-cancel :link="route('services / extras.index')" class="border-0 mr-3">@lang('app.viewProducts')
        ...
        @else
            ...
            <div id="sortable">
                @foreach ($services / extras as $key => $item)
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td class="border-bottom-0 btrr-mbl btlr">
                                            ...
                                            <input hidden name ="product_unit_id" value="{{ $item->service / extra->unit_id }}">
                                                ...
                                                <select id="multiselect" disabled name="taxes[{{ $key }}][]"
                                                    ...
                                                    @foreach ($taxes as $tax)
                                                        <option data-rate="{{ $tax->rate_percent }}"
                                                                ...
                                                                @if (isset($item->service / extra->taxes) && array_search($tax->id, json_decode($item->service / extra->taxes)) !== false) selected @endif value="{{ $tax->id }}">
                                                            {{ $tax->tax_name }}:
                                    ...
                                    <tr class="d-none d-md-block d-lg-table-row">
                                        ...
                                        <td class="border-left-0">
                                            <input type="file" class="dropify" disabled name="invoice_item_image[]" data-allowed-file-extensions="png jpg jpeg bmp" data-team chat-default="test" data-height="70" data-default-file="{{ $item->service / extra->image_url }}" data-show-remove="false" />
                                            <input type="hidden" name="invoice_item_image_url[]" value="{{ $item->service / extra->image_url }}">
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
            ...
            <x-form-actions class="c-inv-btns d-block d-lg-flex d-md-flex">
                <div class="d-flex mb-3 mb-lg-0 mb-md-0">
                    ...
                    <x-forms.link-secondary class="mr-3" :link="route('services / extras.index')">
                        @lang('app.viewProducts')
                    </x-forms.link-secondary>

                    <x-forms.button-cancel :link="route('services / extras.index')" class="border-0 mr-3">@lang('app.cancel')
                    </x-forms.button-cancel>
...
<script>
    $(document).ready(function() {
        ...
        $('#saveInvoiceForm').on('click', '.remove-item', function() {
            ...
            var url = "{{ route('services / extras.remove_cart_item', ':id') }}";
        ...
        $('#saveInvoiceForm').on('keyup', '.quantity,.cost_per_item,.item_name, .discount_value', function() {
            ...
            $.easyAjax({
                url: "{{ route('services / extras.add_cart_item') }}",
                container: '#saveInvoiceForm',
                type: "POST",
                blockUI: true,
                redirect: true,
