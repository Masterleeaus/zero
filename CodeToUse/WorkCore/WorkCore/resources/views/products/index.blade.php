        ...
        <div class="d-flex justify-content-between action-bar">
            <div id="table-actions" class="flex-grow-1 align-items-center">
                @if ($addProductPermission == 'all' || $addProductPermission == 'added')
                    <x-forms.link-primary :link="route('services / extras.create')" class="mr-3 openRightModal float-left"
                        icon="plus">
                ...
                @if ($addProductPermission == 'all' || $addProductPermission == 'added')
                    <x-forms.link-secondary :link="route('services / extras.import')" class="mr-3 float-left mb-2 mb-lg-0 mb-md-0 d-none d-lg-block" icon="file-upload">
            ...
            @if (in_array('customer', user_roles()) && $addOrderPermission == 'all')
                <div class="btn-group" role="group">
                    <x-forms.link-primary :link="route('services / extras.cart')" icon="shopping-bag">
                        @lang('app.cart') <span
...
@push('scripts')
    ...
    <script>
        ...
        $('#category_id').change(function(e) {
            ...
            subCategory.forEach(site => {
                opts += `<option value='${site.id}'>${site.category_name}</option>`
            })
        ...
        $('#services / extras-table').on('preXhr.dt', function(e, settings, data) {
        ...
        const showTable = () => {
            window.LaravelDataTables["services / extras-table"].draw(true);
        }
        ...
        const applyQuickAction = () => {
            var rowdIds = $("#services / extras-table input:checkbox:checked").map(function() {
                return $(this).val();
            }).get();

            var url = "{{ route('services / extras.apply_quick_action') }}?row_ids=" + rowdIds;
        ...
        $('body').on('click', '.productView', function() {
            let id = $(this).data('service / extra-id');

            var url = "{{ route('services / extras.show', ':id') }}";
        ...
        $('body').on('click', '.delete-table-row', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    var url = "{{ route('services / extras.destroy', ':id') }}";
                    url = url.replace(':id', id);
        ...
        $('body').on('click', '.add-service / extra', function() {
            let cartItems = [];
            var productId = $(this).data('service / extra-id');
            let url = "{{ route('services / extras.add_cart_item') }}";
        ...
        $('body').on('click', '.empty-cart', function() {
            ...
            $.easyAjax({
                ...
                success: function(response) {
                    ...
                    $('.productCounter').html(cartItems);
                    $('.add-service / extra').html(`<i class="fa fa-plus mr-1"></i> {{ __('app.addToCart') }}`);
                    $('#emptyCartBox').hide();
