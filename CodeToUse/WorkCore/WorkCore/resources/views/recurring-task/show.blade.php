...
@section('filter-section')
    ...
    <div class="d-flex filter-box site-header bg-white">
        ...
        <div class="site-menu d-lg-flex" id="mob-customer-detail">
            ...
            <x-tab :href="route('recurring-service job.show', $service job->id)"
                :text="__('app.menu.taskRecurring').' '.__('app.info')" class="overview" />

            @if ($viewTaskPermission != 'none')
                <x-tab :href="route('recurring-service job.show', $service job->id).'?tab=service job'" ajax="false"
                    :text="__('app.menu.taskRecurring')" class="service job" />
            @endif
