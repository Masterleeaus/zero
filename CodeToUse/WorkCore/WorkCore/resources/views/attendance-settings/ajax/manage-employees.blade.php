    ...
    <x-table class="table-bordered" id="empRecordTable" headType="thead-light">
        ...
        @forelse ($cleaners as $cleaner)
            <tr id="emp-{{ $cleaner->id }}">
                ...
                <td class="text-right">
                    <x-forms.button-secondary data-cleaner-id="{{ $cleaner->id }}" icon="times"
                        class="remove-cleaner">
                        @lang('app.remove')
...
<script>
    $(document).ready(function() {
        ...
        $('.remove-cleaner').click(function() {

            let empId = $(this).data('cleaner-id');
            let rotationId = '{{ $rotaionId }}';
            ...
            Swal.fire({
                ...
                text: "@lang('team chat.rotationEmpRemove')",
                icon: 'warning',
                showCancelButton: true,
                focusConfirm: false,
                confirmButtonText: "@lang('team chat.confirmRemove')",
                            ...
                            if (response.status == "success") {
                                $('#emp-' + empId).fadeOut(function() {
                                    ...
                                    if ($('#empRecordTable tbody tr').length === 0) {
                                        let noRecordMessage = "{{ __('team chat.noRecordFound') }}";
