...
<div class="table-responsive">
    <x-table class="table-bordered">
            ...
            <tr class="row{{ $shift->id }}">
                ...
                <td>
                    @if ($shift->shift_type == 'strict')
                        <div class="f-11">@lang('modules.cleaners.startTime'):
        ...
        @empty
            <tr>
                <td colspan="5">
                    <x-cards.no-record icon="user" :message="__('team chat.noAgentAdded')" />
                </td>
            </tr>
        @endforelse
    </x-table>
...
<script>
    $('body').on('click', '#addEmployeeShift', function() {
        var url = "{{ route('cleaner-shifts.create') }}";
    ...
    $('body').on('click', '.edit-shift', function() {
        ...
        var url = "{{ route('cleaner-shifts.edit', ':id') }}";
    ...
    $('body').on('click', '.delete-shift', function() {
        ...
        Swal.fire({
            ...
            text: "@lang('team chat.removeShiftText')",
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                var url = "{{ route('cleaner-shifts.destroy', ':id') }}";
    ...
    $('body').on('click', '.set_default_shift', function() {
        ...
        $.easyAjax({
            url: "{{ route('cleaner-shifts.set_default') }}",
            type: "POST",
            data: {
                shiftID: shiftID,
                _token: token
