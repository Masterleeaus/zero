@php
$addProjectMilestonePermission = ($site->project_admin == user()->id) ? 'all' : user()->permission('add_project_milestones');
$viewProjectMilestonePermission = ($site->project_admin == user()->id) ? 'all' : user()->permission('view_project_milestones');
$editProjectMilestonePermission = ($site->project_admin == user()->id) ? 'all' : user()->permission('edit_project_milestones');
$deleteProjectMilestonePermission = ($site->project_admin == user()->id) ? 'all' : user()->permission('delete_project_milestones');
...
<div class="row py-5">
    <div class="col-lg-12 col-md-12 mb-4 mb-xl-0 mb-lg-4">
        @if (($addProjectMilestonePermission == 'all' || $site->project_admin == user()->id))
            <x-forms.button-primary icon="plus" id="add-site-milestone" class="type-btn mb-3">
                @lang('modules.sites.createMilestone')
            </x-forms.button-primary>
        @endif

        @if ($viewProjectMilestonePermission == 'all' || $viewProjectMilestonePermission == 'added' || ($viewProjectMilestonePermission == 'owned' && $userId == $site->client_id))
            <x-cards.data :title="__('modules.sites.milestones')"
                ...
                <x-table class="border-0 pb-3 admin-dash-table table-hover">
                    ...
                    <x-slot name="thead">
                        ...
                        <th>@lang('modules.sites.milestoneTitle')</th>
                        <th>@lang('modules.sites.milestoneCost')</th>
                        {{-- <th>@lang('modules.sites.taskCount')</th> --}}
                        <th>@lang('app.status')</th>
                        <th class="text-right pr-20">@lang('app.action')</th>
                    </x-slot>

                    @forelse($site->milestones as $key=>$item)
                        <tr id="row-{{ $item->id }}">
        ...
        @else
        <x-cards.no-record :message="__('team chat.noRecordFound')" icon="flag" />
        @endif
    </div>
...
<script>
    $('#add-site-milestone').click(function() {
        const url = "{{ route('site-template-milestone.create') }}" + "?id={{ $site->id }}";
    ...
    $('body').on('click', '.edit-milestone', function() {
        ...
        var url = "{{ route('site-template-milestone.edit', ':id') }}";
    ...
    $('body').on('click', '.milestone-detail', function() {
        ...
        var url = "{{ route('site-template-milestone.show', ':id') }}";
        url = url.replace(':id', id);
    ...
    $('.delete-row').click(function() {
        ...
        var id = $(this).data('row-id');
        var url = "{{ route('site-template-milestone.destroy', ':id') }}";
        url = url.replace(':id', id);
    ...
    $(document).ready(function() {
        $('.change-milestone-status').on('change', function() {
            ...
            var url = "{{ route('site-template-milestone.updateStatus', ':id') }}";
