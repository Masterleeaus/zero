...
@section('filter-section')
    ...
    <x-filters.filter-box>
        ...
        <div class="service job-search d-flex  py-1 px-0 border-right-grey align-items-center">
        ...
        <div class="select-box d-flex py-2 {{ !in_array('customer', user_roles()) ? 'px-lg-2 px-md-2 px-0' : '' }}  border-right-grey border-right-grey-sm-0 pr-2 pl-2">
            <p class="mb-0 pr-2 f-14 text-dark-grey d-flex align-items-center">@lang('modules.sites.projectCategory')</p>
        ...
        <div class="select-box d-flex py-2 {{ !in_array('customer', user_roles()) ? 'px-lg-2 px-md-2 px-0' : '' }}  border-right-grey border-right-grey-sm-0 pr-2 pl-2">
            <p class="mb-0 pr-2 f-14 text-dark-grey d-flex align-items-center">@lang('modules.customer.projectSubCategory')</p>
            <div class="select-status">
                <select class="form-control select-picker" name="project_template_sub_category_id" id="project_template_sub_category_id" data-live-search="true" data-size="8">
                    <option value="all" >Select Site Category</option>
                </select>
            </div>
        </div>
        ...
        <div class="d-block d-lg-flex d-md-flex justify-content-between action-bar">
            ...
            <div class="d-flex" id="table-actions">
                @if(in_array($manageProjectTemplatePermission, ['added', 'all']))
                    <x-forms.link-primary :link="route('site-template.create')" class="mr-3 openRightModal" icon="plus">
...
@push('scripts')
    ...
    <script>
        $('#sites-template-table').on('preXhr.dt', function(e, settings, data) {
        ...
        const showTable = () => {
            window.LaravelDataTables["sites-template-table"].draw(true);
        }
        ...
        $('#project_template_category_id').change(function(e) {
            ...
            var url = "{{ route('site.get_project_sub_category', ':id') }}";
        ...
        $('body').on('click', '.delete-table-row', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    var url = "{{ route('site-template.destroy', ':id') }}";
        ...
        const applyQuickAction = () => {
            var rowdIds = $("#sites-template-table input:checkbox:checked").map(function() {
                return $(this).val();
            }).get();
