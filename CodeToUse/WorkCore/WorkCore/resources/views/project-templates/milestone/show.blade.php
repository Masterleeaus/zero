...
<div class="modal-body bg-additional-grey">
    <x-cards.data>
        <x-cards.data-row :label="__('modules.sites.milestoneTitle')" :value="$milestone->milestone_title" />

        <x-cards.data-row :label="__('modules.sites.milestoneCost')"
        ...
        <x-cards.data-row :label="__('modules.sites.milestoneSummary')" :value="$milestone->summary" html="true" />

        {{-- <x-cards.data-row :label="__('modules.timeLogs.totalHours')" :value="$timeLog" /> --}}

        <x-cards.data-row :label="__('modules.sites.milestoneStartDate')" :value="is_null($milestone->start_date) ? '--':$milestone->start_date->translatedFormat(company()->date_format)" />

        <x-cards.data-row :label="__('modules.sites.milestoneEndDate')" :value="is_null($milestone->end_date) ? '--':$milestone->end_date->translatedFormat(company()->date_format)" />

    </x-cards.data>

    <x-cards.data :title="__('app.menu.service jobs')" class="mt-4">
        <x-table class="border-0 pb-3 admin-dash-table table-hover">

            <x-slot name="thead">
                <th class="pl-20">#</th>
                <th>@lang('app.service job')</th>
                {{-- <th>@lang('modules.service jobs.assignTo')</th> --}}
                {{-- <th>@lang('modules.service jobs.assignBy')</th> --}}
                {{-- <th>@lang('app.dueDate')</th> --}}
                {{-- <th>@lang('modules.timeLogs.totalHours')</th> --}}
                <th class="pr-20">@lang('modules.service jobs.priority')</th>
            </x-slot>

            @forelse ($milestone->service jobs as $key=>$item)
            {{-- @php
                    ...
                    {{-- <td>
                        @foreach ($item->users as $member)
                            <div class="taskEmployeeImg rounded-circle"><a
                                    href="{{ route('cleaners.show', $member->id) }}">
