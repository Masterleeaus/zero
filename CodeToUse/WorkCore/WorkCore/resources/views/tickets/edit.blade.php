...
@push('styles')
    ...
    <style>
        ...
        .issue / support-left .card:hover .message-action {
            visibility: visible;
        }
        ...
        .issue / support-activity .recent-issue / support-inner:before {
...
$canEditTicket = ($editTicketPermission == 'all' || ($editTicketPermission == 'owned' && $issue / support->agent_id == user()->id));
@endphp
...
@section('filter-section')
    ...
    <div class="d-flex px-4 filter-box bg-white">
        ...
        @if ($editTicketPermission == 'all'
                        || ($editTicketPermission == 'added' && user()->id == $issue / support->added_by)
                        || ($editTicketPermission == 'owned' && (user()->id == $issue / support->user_id || $issue / support->agent_id == user()->id))
                        || ($editTicketPermission == 'both' && (user()->id == $issue / support->user_id || $issue / support->agent_id == user()->id || $issue / support->added_by == user()->id)))
            <div id="issue / support-closed" @if ($issue / support->status == 'closed') style="display:none" @endif>
                <a href="javascript:;" data-status="closed"
                    class="d-flex align-items-center height-44 text-dark-grey  border-right-grey px-3 submit-issue / support"><i
        ...
        @if ($deleteTicketPermission == 'all' || ($deleteTicketPermission == 'owned' && $issue / support->agent_id == user()->id))
            <a href="javascript:;"
                class="d-flex align-items-center height-44 text-dark-grey  border-right-grey px-3 delete-issue / support"><i
...
@section('content')
    ...
    <!-- TICKET START -->
    <div class="issue / support-wrapper bg-white border-top-0 d-lg-flex">

        <!-- TICKET LEFT START -->
        <div class="issue / support-left w-100">
            <x-form id="updateTicket2" method="PUT">
                <input type="hidden" name="status" id="status" value="{{ $issue / support->status }}">
                <input type="hidden" name="type" id="type" value="{{ $issue / support->type }}">
                <input type="hidden" id="ticket_reply_id" value="">

                <!-- START -->
                <div id="issue / support-info-bar" class="d-flex justify-content-between align-items-center p-3 border-right-grey border-bottom-grey">
                    <span>
                        <p class="f-15 f-w-500 mb-0">{{ $issue / support->subject }}</p>
                        <p class="f-11 text-lightest mb-0">@lang('modules.issues / support.requestedOn')
                            {{ $issue / support->created_at->timezone(company()->timezone)->translatedFormat(company()->date_format . ' ' . company()->time_format) }}
                        </p>
                    </span>
                    <span id="ticketStatusBadge">
                        @if ($issue / support->status == 'open')
                            @php
                                $statusColor = 'red';
                            @endphp
                        @elseif($issue / support->status == 'pending')
                            @php
                                $statusColor = 'yellow';
                            @endphp
                        @elseif($issue / support->status == 'resolved')
                            @php
                                $statusColor = 'dark-green';
                            @endphp
                        @elseif($issue / support->status == 'closed')
                            @php
                                $statusColor = 'blue';
                            @endphp
                        @endif
                        {{-- <p class="mb-0  issue / support-status">
                           {!! $issue / support->badge('span') !!}
                            <x-status :color="$statusColor" :value="__('app.'. $issue / support->status)" />

                        </p> --}}

                        <div class="d-flex align-items-center justify-content-between issue / support-status">
                            ...
                            @if ($canEditTicket || $isEditable)
                                <div class="ml-2">
                                    <div class="dropdown">
                                        ...
                                        <div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdownMenuLink-{{ $issue / support->id }}" tabindex="0">
                                            <a class="dropdown-item edit-issue / support" href="javascript:;" id="edit-issue / support" data-issue / support-id="{{ $issue / support->id }}">
                ...
                <div class="issue / support-msg border-right-grey" data-menu-vertical="1" data-menu-scroll="1"
                    data-menu-dropdown-timeout="500" id="ticketMsg">
                    @foreach ($issue / support->reply as $reply)
                    @if (!in_array('customer', user_roles()) || $reply->type != 'note')
                        <x-cards.issue / support :message="$reply" :user="$reply->user" />
                    @endif
                    @endforeach
                ...
                <div class="border-right-grey" id="ticketMsgBottom">
                    <div class="col-md-12 border-top d-none mb-5" id="reply-section">
                        <div class="form-group my-3">
                            @if ($issue / support->requester->id != user()->id || (!is_null($issue / support->agent_id) && $issue / support->agent_id != user()->id))
                            <p class="f-w-500">
                                @lang('app.to'): {{ ($issue / support->requester->id != user()->id) ? $issue / support->requester->name : $issue / support->agent->name }}
                            </p>
                            @endif
                        ...
                        <div class="my-3">
                            <a class="f-15 f-w-500" href="javascript:;" id="add-file"><i
                                    class="fa fa-paperclip font-weight-bold mr-1"></i>@lang('modules.sites.uploadFile')</a>
                        </div>
                        <x-forms.file-multiple class="mr-0 mr-lg-2 mr-md-2 upload-section d-none"
                            fieldLabel=""
                            fieldName="file[]" fieldId="issue / support-file-upload-dropzone" />
                    </div>
                    ...
                    <div class="col-md-12 border-top d-none mb-5" id="note-section">
                        ...
                        <div class="my-3 form-group">
                            ...
                            <x-forms.file-multiple class="mr-0 mr-lg-2 mr-md-2 upload-note-section d-none"
                                fieldLabel=""
                                fieldName="file[]" fieldId="issue / support-note-file-upload-dropzone"/>
                        </div>
                    </div>

                    <div class="issue / support-reply-back justify-content-start px-lg-4 px-md-4 px-3 py-3  d-flex bg-white border-top-grey"
                        ...
                        <x-forms.link-secondary :link="route('issues / support.index')" icon="arrow-left">@lang('app.back')
                        </x-forms.link-secondary>

                    </div>
                    <div class="issue / support-reply-back flex-row justify-content-start px-lg-4 px-md-4 px-3 py-3 c-inv-btns bg-white border-top-grey d-none"
                        ...
                        || ($editTicketPermission == 'both' && (user()->id == $issue / support->user_id || $issue / support->agent_id == user()->id || $issue / support->added_by == user()->id)))
                            <div class="inv-action dropup mr-3">
                                ...
                                <ul class="dropdown-menu" aria-labelledby="dropdownMenuBtn" tabindex="0">
                                    <li>
                                        <a class="dropdown-item f-14 text-dark submit-issue / support" href="javascript:;"
                                            data-status="open" data-type="reply">
                                            <x-status color="red" :value="__('modules.issues / support.submitOpen')" />
                                        </a>
                                    </li>
                                    <li>
                                        <a class="dropdown-item f-14 text-dark submit-issue / support" href="javascript:;"
                                            data-status="pending" data-type="reply">
                                            <x-status color="yellow" :value="__('modules.issues / support.submitPending')" />
                                        </a>
                                    </li>
                                    <li>
                                        <a class="dropdown-item f-14 text-dark submit-issue / support" href="javascript:;"
                                            data-status="resolved" data-type="reply">
                                            <x-status color="dark-green" :value="__('modules.issues / support.submitResolved')" />
                                        </a>
                                    </li>
                                    <li>
                                        <a class="dropdown-item f-14 text-dark submit-issue / support" href="javascript:;"
                                            data-status="closed" data-type="reply">
                                            <x-status color="blue" :value="__('modules.issues / support.submitClosed')" />
                                        </a>
                                    </li>
                        ...
                        @else
                            <x-forms.button-primary icon="check" data-status="open" class="submit-issue / support mr-3" data-type="reply">
                                @lang('app.submit')
                            </x-forms.button-primary>
                        @endif
                        ...
                        @if (!in_array('customer', user_roles()))
                            <div class="inv-action dropup mr-3">
                                <button class="btn-secondary dropdown-toggle" type="button" data-toggle="dropdown"
                                    ...
                                    @lang('modules.issues / support.applyTemplate')
                                ...
                                <ul class="dropdown-menu" aria-labelledby="dropdownMenuBtn" tabindex="0">
                                    ...
                                    @empty
                                        <li><a class="dropdown-item f-14 text-dark">@lang('team chat.noTemplateFound')</a></li>
                                    @endforelse
                                </ul>
                            </div>
                        @endif
                    ...
                    <div class="issue / support-reply-back flex-row justify-content-start px-lg-4 px-md-4 px-3 py-3 c-inv-btns bg-white border-top-grey d-none"
                                ...
                                <ul class="dropdown-menu" aria-labelledby="dropdownMenuBtn" tabindex="0">
                                    <li>
                                        <a class="dropdown-item f-14 text-dark submit-issue / support" href="javascript:;"
                                            ...
                                            <x-status color="red" :value="__('modules.issues / support.noteOpen')" />
                                        </a>
                                    </li>
                                    <li>
                                        <a class="dropdown-item f-14 text-dark submit-issue / support" href="javascript:;"
                                            data-status="pending" data-type="note">
                                            <x-status color="yellow" :value="__('modules.issues / support.notePending')" />
                                        </a>
                                    </li>
                                    <li>
                                        <a class="dropdown-item f-14 text-dark submit-issue / support" href="javascript:;"
                                            data-status="resolved" data-type="note">
                                            <x-status color="dark-green" :value="__('modules.issues / support.noteResolved')" />
                                        </a>
                                    </li>
                                    <li>
                                        <a class="dropdown-item f-14 text-dark submit-issue / support" href="javascript:;"
                                            data-status="closed" data-type="note">
                                            <x-status color="blue" :value="__('modules.issues / support.noteClosed')" />
                                        </a>
                                    </li>
                        ...
                        @else
                            <x-forms.button-primary icon="edit" data-status="open" class="submit-issue / support mr-3">
                                @lang('app.addNote')
                            </x-forms.button-primary>
                        @endif
        ...
        <!-- TICKET RIGHT START -->
        <div class="mobile-close-overlay w-100 h-100" id="close-issues / support-overlay"></div>
        <div class="issue / support-right bg-white issue / support-sidebar" id="issue / support-detail-contact">
            <a class="d-none" id="close-issues / support"><i class="fa fa-times"></i></a>
            <div id="tabs">
            ...
            <div class="tab-content h-100" id="nav-tabContent">
                ...
                <div class="tab-pane fade h-100 show active" id="nav-details" role="tabpanel"
                    ...
                    <x-form id="updateTicket1">
                        ...
                        <div id="updateTicketForm" class="issue / support-overflow p-4 w-100 position-relative border-bottom">
                            <div class="more-filter-items mb-4">
                                @foreach ($groups as $group)

                                        @endforeach
                                <x-forms.label class="my-3" fieldId="group_id"
                                    :fieldLabel="__('modules.issues / support.assignGroup')">
                                </x-forms.label>
                                <x-forms.input-group>
                                    <select class="form-control select-picker " name="group_id" id="group_id"
                                        data-live-search="true" data-container="body" data-size="8">
                                        @foreach ($groups as $group)
                                            <option @selected($group->id == $issue / support->group_id) value="{{ $group->id }}">{{ $group->group_name }}</option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="more-filter-items mb-4">
                                <x-forms.label class="my-3" fieldId="agent_id"
                                    :fieldLabel="__('modules.issues / support.agent')">
                                </x-forms.label>
                                <x-forms.input-group>
                                ...
                                <x-forms.input-group>
                                    <select class="form-control select-picker" name="type_id" id="ticket_type_id"
                                        ...
                                        @foreach ($types as $type)
                                            <option @selected($type->id == $issue / support->type_id) value="{{ $type->id }}">
                                                {{ $type->type }}</option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="more-filter-items mb-4">
                                <x-forms.label class="my-3" fieldId="ticket_channel_id"
                                    :fieldLabel="__('modules.issues / support.channelName')">
                                </x-forms.label>
                                <x-forms.input-group>
                                    <select class="form-control select-picker" name="channel_id" id="ticket_channel_id"
                                        ...
                                        @foreach ($channels as $channel)
                                            <option @selected($channel->id == $issue / support->channel_id) value="{{ $channel->id }}">
                            ...
                            <div class="more-filter-items">
                                <x-forms.select fieldId="issue / support-status" :fieldLabel="__('app.status')"
                                    fieldName="status" data-container="body">
                                    <option @selected($issue / support->status == 'open') value="open"
                                        data-content="<i class='fa fa-circle mr-2 text-red'></i>{{ __('app.open') }}">
                                        @lang('app.open')
                                    </option>
                                    <option @selected($issue / support->status == 'pending') value="pending"
                                        data-content="<i class='fa fa-circle mr-2 text-yellow'></i>{{ __('app.pending') }}">
                                        @lang('app.pending')</option>
                                    <option @selected($issue / support->status == 'resolved') value="resolved"
                                        data-content="<i class='fa fa-circle mr-2 text-dark-green'></i>{{ __('app.resolved') }}">
                                        @lang("app.resolved")</option>
                                    <option @selected($issue / support->status == 'closed') value="closed"
                            ...
                            <div class="more-filter-items">
                                <x-forms.label class="my-3" fieldId="tags"
                                    :fieldLabel="__('modules.issues / support.tags')">
                                </x-forms.label>
                                <input type="text" name="tags" id="tags" class="rounded f-14"
                                    value="{{ implode(',', $issue / support->ticketTags->pluck('tag_name')->toArray()) }}">
                            </div>
                        </div>
                        <!-- TICKET FILTERS END -->
                        <!-- TICKET UPDATE START -->
                        <div id="updateTicketFormSubmit" class="issue / support-update bg-white px-4 py-3">
                            <x-forms.button-primary class="ml-none d-flex submit-issue / support-2 fixed-bottom">
                                @lang('app.update')
                            </x-forms.button-primary>
                        </div>
                        <!-- TICKET UPDATE END -->
                ...
                class="tab-pane fade" id="nav-contact" role="tabpanel" aria-labelledby="nav-contact-tab">
                    <!-- CONTACT OWNER START  -->
                    <div class="card-horizontal bg-white-shade issue / support-contact-owner p-4 rounded-0">
                        <div class="card-img mr-3">
                            <img class="___class_+?88___" src="{{ $issue / support->requester->image_url }}"
                                alt="{{ $issue / support->requester->name }}">
                        </div>
                        <div class="card-body border-0 p-0 w-100">
                            <h4 class="card-title f-14 font-weight-normal mb-0">
                                <a class="text-dark-grey" @if ($issue / support->requester->hasRole('cleaner'))
                                    href="{{ route('cleaners.show', $issue / support->requester->id) }}"
                                @else
                                    href="{{ route('customers.show', $issue / support->requester->id) }}"
                                @endif>
                                {{ $issue / support->requester->name }}
                                </a>
                            </h4>
                            @if ($issue / support->requester->country_id)
                                <span class="card-text f-12 text-dark-grey  d-flex align-items-center">
                                    <span class='flag-icon flag-icon-{{ strtolower($issue / support->requester->country->iso) }} mr-2'></span>
                                    {{ $issue / support->requester->country->nicename }}
                                </span>
                            @endif
                            @if ($issue / support->requester->mobile)
                                <span class="card-text f-12 text-dark-grey d-flex align-items-center mt-1">
                                    <i class="fa fa-mobile-alt mr-2"></i>
                                    {{ $issue / support->requester->mobile }}
                                </span>
                            @endif

                            @if ($issue / support->requester->clientDetails)
                                @php
                                    $addressParts = array_filter([
                                        $issue / support->requester->clientDetails->address ?? null,
                                        $issue / support->requester->clientDetails->city ?? null,
                                        $issue / support->requester->clientDetails->state ?? null,
                                    ]);
                                @endphp
                    ...
                    <x-cards.data :title="__('app.menu.issues / support')" padding="false" class="pb-3">
                        ...
                        @endif
                        <x-pie-chart id="issue / support-chart" :labels="$ticketChart['labels']" :values="$ticketChart['values']"
                    ...
                    <div class="card pt-4 px-4 border-grey border-left-0 border-right-0 rounded-0">
                        <div class="card-title">
                            <h4 class="f-18 f-w-500  mb-3">@lang('modules.issues / support.recentTickets')</h4>
                        </div>
                        <!-- CHART START -->
                        <div class="card-body p-0">
                            <div class="recent-issue / support position-relative" data-menu-vertical="1" data-menu-scroll="1"
                                data-menu-dropdown-timeout="500" id="recentTickets">
                                <div class="recent-issue / support-inner position-relative">
                                    @foreach ($issue / support->requester->issues / support as $item)
                                        <div class="r-t-items d-flex">
                                            <div class="r-t-items-left text-lightest f-21">
                                                <i class="fa fa-issue / support-alt"></i>
                                            </div>
                                            <div class="r-t-items-right ">
                                                <h3 class="f-14 font-weight-bold">
                                                    <a class="text-dark"
                                                        href="{{ route('issues / support.show', $item->ticket_number) }}">{{ $item->subject }}</a>
                                                </h3>
            ...
            <!-- CONTACT END -->
                ...
                <div class="tab-pane fade" id="nav-other" role="tabpanel" aria-labelledby="nav-other-tab">
                    @if ($issue / support->site)
                        <div class="p-4 w-100 position-relative border-bottom">
                            @lang('app.site') : <a
                            href="{{ route('sites.show', $issue / support->project_id) }}">{{ $issue / support->site->project_name }}</a>
                        </div>
                    @endif

                    <x-form id="updateOther">
                        <!-- TICKET FILTERS START -->
                        <div class="issue / support-filters p-4 w-100 position-relative border-bottom">
                            <x-forms.custom-field-show :fields="$fields" :model="$issue / support"></x-forms.custom-field-show>
                        </div>
                        <!-- TICKET FILTERS END -->
                    </x-form>
                </div>
                ...
                <div class="tab-pane fade h-100" id="nav-activity" role="tabpanel" aria-labelledby="nav-activity-tab">
                    ...
                    <div class="card pt-4 pl-4 border-grey border-left-0 border-right-0 rounded-0 h-100">
                        ...
                        <!-- CHART START -->
                        <div class="card-body p-0">
                            <div class="issue / support-activity issue / support-overflow position-relative h-100" data-menu-vertical="1" data-menu-scroll="1"
                                data-menu-dropdown-timeout="500" id="ticketActivity">
                                <div class="recent-issue / support-inner position-relative">
                                    @foreach ($issue / support->activities as $activity)
                                    @if (!in_array('customer', user_roles()) || $activity->type != 'note')
                                        <div class="r-t-items d-flex">
                                            <div class="r-t-items-left text-lightest f-21">
                                                <i class="fa fa-issue / support-alt"></i>
                                            </div>
...
@push('scripts')
    ...
    <script>
        ...
        $('body').on('click', '.edit-issue / support', function() {
            var ticketId = $(this).data('issue / support-id');
            var url = "{{ route('issues / support.edit_detail', ':id') }}";
        ...
        //Dropzone class
        ticketDropzone = new Dropzone("div#issue / support-file-upload-dropzone", {
            dictDefaultMessage: "{{ __('app.dragDrop') }}",
            url: "{{ route('issue / support-files.store') }}",
            headers: {
        ...
        ticketDropzone.on('sending', function(file, xhr, formData) {
            ...
            formData.append('ticket_id', '{{ $issue / support->id }}');
            $.easyBlockUI();
        });
        ...
        ticketDropzone.on('queuecomplete', function() {
            var msgs = "@lang('team chat.addDiscussion')";
            window.location.href = "{{ route('issues / support.show', $issue / support->ticket_number) }}";
        });
        ...
        ticketNoteDropzone = new Dropzone("div#issue / support-note-file-upload-dropzone", {
            dictDefaultMessage: "{{ __('app.dragDrop') }}",
            url: "{{ route('issue / support-files.store') }}",
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}'
        ...
        $('.submit-issue / support').click(function() {
            ...
            const url = "{{ route('issues / support.update', $issue / support->id) }}";
        ...
        $('.submit-issue / support-2').click(function() {

            $.easyAjax({
                url: "{{ route('issues / support.update_other_data', $issue / support->id) }}",
                container: '#updateTicket1',
                type: "POST",
                blockUI: true,
                disableButton: true,
                buttonSelector: ".submit-issue / support-2",
                data: $('#updateTicket1').serialize(),
                success: function(response) {
                    if (response.status == 'success') {
                        var status = $('#issue / support-status').val();

                        ($('#issue / support-status').val() != 'closed') ? $('#issue / support-closed').show() :  $('#issue / support-closed').hide();
        ...
        $('body').on('click', '#edit-note-file', function() {
            ...
            $('.edit-issue / support-file').addClass('d-none');
        });
        ...
        $('body').on('click', '#save-notes', function() {
            ...
            url = '{{ route("issue / support-replies.edit_note", ":id") }}';
        ...
        $('body').on('click', '.delete-file', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    var url = "{{ route('issue / support-files.destroy', ':id') }}";
                    url = url.replace(':id', id);
        ...
        $('body').on('click', '.delete-issue / support', function() {
            Swal.fire({
                title: "@lang('team chat.sweetAlertTitle')",
                text: "@lang('team chat.recoverRecord')",
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    var url = "{{ route('issues / support.destroy', $issue / support->id) }}";
        ...
        $('body').on('click', '.delete-message', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    var url = "{{ route('issue / support-replies.destroy', ':id') }}";
                    url = url.replace(':id', id);
        ...
        $('body').on('click', '#addAgent', function() {
            var url = "{{ route('issue / support-agents.create') }}";
        ...
        function getAgents(groupId,exceptThis=null ){
            var url = "{{ route('issues / support.agent_group', [':id', ':exceptThis']).'?ticketNumber='.$issue / support->ticket_number}}";
        ...
        $('#manage-groups').click(function() {
            var url = "{{ route('issue / support-groups.create') }}";
            $(MODAL_LG + ' ' + MODAL_HEADING).html('...');
            $.ajaxModal(MODAL_LG, url);
        });
        ...
        function setViewHeight() {
            ...
            document.getElementById("issue / support-detail-contact").style.height = "calc(100vh - " + (
                $('#tabs').outerHeight() + otherBodyHeight
            ) + "px)";
            document.getElementById("ticketMsg").style.height = "calc(100vh - " + (
                otherBodyHeight + $('#issue / support-info-bar').outerHeight() +
