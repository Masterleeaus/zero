...
<div class="row">
    <div class="col-sm-12">
        <x-form id="save-issue / support-data-form">
            <input type="hidden" id="replyID">
            <div class="add-customer bg-white rounded">
                <h4 class="mb-0 p-20 f-21 font-weight-normal  border-bottom-grey">
                    @lang('modules.issues / support.ticketDetail')</h4>
                <div class="row p-20">
                    @if(!isset($defaultAssign))
                        @if (!in_array('customer', user_roles()))
                            @if ($addPermission == 'all')
                                @if (!(isset($customer)) && !(isset($cleaner)) && in_array('customers', user_modules()) && in_array('cleaners', user_modules()))
                                    <div class="col-md-4">
                                        <div class="form-group my-3">
                                            <x-forms.label fieldId="requester-customer"
                                                        :fieldLabel="__('modules.issues / support.requester')"/>
                                            <div class="d-flex">
                                                <x-forms.radio fieldId="requester-customer" :fieldLabel="__('app.customer')"
                                                            fieldName="requester_type" fieldValue="customer"
                                                            checked="true">
                                                </x-forms.radio>
                                                <x-forms.radio fieldId="requester-cleaner" :fieldLabel="__('app.cleaner')"
                                                            fieldValue="cleaner"
                                                            fieldName="requester_type"></x-forms.radio>
                                            </div>
                                        </div>
                                    </div>
                                @elseif(!in_array('cleaners', user_modules()))
                                    <input type="hidden" name="requester_type" id="requester_type" value="customer">
                                @elseif(!in_array('customers', user_modules()))
                                    <input type="hidden" name="requester_type" id="requester_type" value="cleaner">
                                @endif
                                <input type = "hidden" name = "mention_user_ids" id = "mentionUserId" class ="mention_user_ids">

                                <div class="col-md-4  @if (isset($cleaner) || !in_array('customers', user_modules())) d-none @endif " id="customer-requester">
                                    @if (isset($customer) && !is_null($customer))
                                        <x-forms.label fieldId="requester-customer" class="mt-3"
                                                    :fieldLabel="__('modules.issues / support.requesterName')"/>
                                        <input type="hidden" name="requester_type" id="requester_type" value="customer">
                                    ...
                                    @else
                                        <x-forms.select fieldId="client_id"
                                            ...
                                            @foreach ($customers as $customer)
                                                <x-user-option :user="$customer" :additionalText="$customer->clientDetails->company_name" />
                                            @endforeach
                                        </x-forms.select>
                                    @endif
                                </div>

                                <div class="col-md-4 @if (!(isset($cleaner)) && in_array('customers', user_modules())) d-none @endif" id="cleaner-requester">
                                    @if(isset($cleaner) && !is_null($cleaner))
                                        <x-forms.label class="my-3" fieldId="requestuser_id"
                                                    :fieldLabel="__('modules.issues / support.requesterName')"
                                                    fieldRequired="true">
                                        </x-forms.label>
                                        <input type="hidden" name="requester_type" id="requester_type" value="cleaner">
                                        <input type="hidden" name="user_id" id="user_id" value="{{ $cleaner->id }}">
                                        <input type="text" value="{{ $cleaner->name }}"
                                            class="form-control height-35 f-15 readonly-background" readonly>
                                    @else
                                        <x-forms.label class="my-3" fieldId="requestuser_id"
                                                    :fieldLabel="__('modules.issues / support.requesterName')"
                            ...
                            @else
                                <input type="hidden" name="requester_type" value="cleaner">
                                <input type="hidden" id="user_id" name="user_id" value="{{ user()->id }}">
                            @endif
                        @else
                            <input type="hidden" name="requester_type" value="customer">
                            <input type="hidden" id="client_id" name="client_id" value="{{ user()->id }}">
                        @endif
                    @else
                        <div class="col-md-4" id="cleaner-requester">
                            <x-forms.label class="my-3" fieldId="requestuser_id"
                                        :fieldLabel="__('modules.issues / support.requesterName')"
                                        fieldRequired="true">
                            </x-forms.label>
                    ...
                    @if (!in_array('customer', user_roles()))
                        <div class="col-md-6 col-lg-4">
                            <x-forms.label class="mt-3" fieldId="ticket_agent_id" :fieldLabel="__('modules.issues / support.agent')">
                            </x-forms.label>
                            <x-forms.input-group>
                                ...
                                @if ($manageAgentPermission == 'all')
                                    <x-slot name="append">
                                        <button id="add-agent" type="button"
                                                ...
                                                data-toggle="tooltip" data-original-title="{{ __('app.addNew').' '.__('modules.issues / support.agent') }}">@lang('app.add')</button>
                                    </x-slot>
                                @endif
                            </x-forms.input-group>
                        </div>
                    ...
                    @endif
                    <div class="col-md-4">
                        <x-forms.select fieldId="project_id" :fieldLabel="__('app.site')"
                            ...
                            @if ($manageTypePermission == 'all')
                                <x-slot name="append">
                                    <button id="add-type" type="button"
                                            class="btn btn-outline-secondary border-grey"
                                            data-toggle="tooltip" data-original-title="{{ __('app.addNew').' '.__('modules.issues / support.ticketType') }}">@lang('app.add')</button>
                                </x-slot>
                            @endif
                        </x-forms.input-group>
                    </div>
                    ...
                    <div class="col-md-12">
                        <x-forms.text :fieldLabel="__('modules.issues / support.ticketSubject')" fieldName="subject"
                ...
                <div class="row p-20 d-none" id="other-details">
                            ...
                            @if ($manageChannelPermission == 'all')
                                <x-slot name="append">
                                    <button id="add-channel" type="button"
                                            class="btn btn-outline-secondary border-grey"
                                            data-toggle="tooltip" data-original-title="{{ __('app.add').' '.__('modules.issues / support.ticketChannel') }}">@lang('app.add')</button>
                                </x-slot>
                            @endif
                        </x-forms.input-group>
                    </div>
                    ...
                    <div class="col-md-12">
                        <x-forms.text fieldId="tags" :fieldLabel="__('modules.issues / support.tags')" fieldName="tags"/>
                    </div>
                ...
                <x-form-actions>
                    <x-forms.button-primary id="save-issue / support-form" class="mr-3" icon="check">@lang('app.save')
                    </x-forms.button-primary>
                    <x-forms.button-cancel :link="route('issues / support.index')" class="border-0">@lang('app.cancel')
...
<script>
    $(document).ready(function () {
        ...
        function getAgents(groupId, exceptThis = null){
            var url = "{{ route('issues / support.agent_group', [':id', ':exceptThis'])}}";
        ...
        //Dropzone class
        var ticketDropzone = new Dropzone("div#issue / support-file-upload-dropzone", {
        ...
        ticketDropzone.on('queuecomplete', function () {
            ...
            window.location.href = "{{ route('issues / support.index') }}";
        });
        ...
        $('body').on('change', "input[name=requester_type]", function () {
            ...
            if (value == 'customer')
            {
                $('#customer-requester').removeClass('d-none');
                $('#cleaner-requester').addClass('d-none');
            } else {
                $('#customer-requester').addClass('d-none');
                $('#cleaner-requester').removeClass('d-none');
            }
        });
        ...
        $('#save-issue / support-form').click(function () {
            ...
            const url = "{{ route('issues / support.store') }}";

            $.easyAjax({
                url: url,
                container: '#save-issue / support-data-form',
                type: "POST",
                disableButton: true,
                blockUI: true,
                file: true,
                buttonSelector: "#save-issue / support-form",
                data: $('#save-issue / support-data-form').serialize(),
                success: function (response) {
        ...
        let isProjectModuleEnabled = {{ in_array('sites', user_modules()) ? 'true' : 'false' }};
        ...
        function getProjects() {
            ...
            if ((requester_type == 'customer' && client_id) || (requester_type == 'cleaner' && user_id)) {
                let url = "{{ route('get.sites') }}";
                $.easyAjax({
                    url: url,
                    type: "GET",
                    data: {
        ...
        function getContacts() {
            ...
            if ((requester_type == 'customer' && client_id)) {
