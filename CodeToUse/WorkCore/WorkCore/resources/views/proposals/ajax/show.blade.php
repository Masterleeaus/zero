    ...
    <div class="card-body">
        <div class="invoice-table-wrapper">
            <table width="100%" class="">
                <tr class="inv-logo-heading">
                    ...
                    <td align="right" class="font-weight-bold f-21 text-dark text-uppercase mt-4 mt-lg-0 mt-md-0">
                        @lang('modules.enquiry.proposal')</td>
                </tr>
                <tr class="inv-num">
            ...
            <table width="100%">
                <tr class="inv-unpaid">
                    <td class="f-14 text-dark">
                        @if ($invoice->enquiry && ($invoice->enquiry->contact->client_name || $invoice->enquiry->contact->client_email || $invoice->enquiry->contact->mobile || $invoice->enquiry->contact->company_name || $invoice->enquiry->contact->address) && (invoice_setting()->show_client_name == 'yes' || invoice_setting()->show_client_email == 'yes' || invoice_setting()->show_client_phone == 'yes' || invoice_setting()->show_client_company_name == 'yes' || invoice_setting()->show_client_company_address == 'yes'))
                        <p class="mb-0 text-left">
                            ...
                            @endif
                            @if ($invoice->enquiry->contact && $invoice->enquiry->contact->client_name && invoice_setting()->show_client_name == 'yes')
                                {{ $invoice->enquiry->contact->client_name_salutation }}<br>
                            @endif
                            @if ($invoice->enquiry->contact && $invoice->enquiry->contact->client_email && invoice_setting()->show_client_email == 'yes')
                                {{ $invoice->enquiry->contact->client_email }}<br>
                            @endif
                            @if ($invoice->enquiry->contact && $invoice->enquiry->contact->mobile && invoice_setting()->show_client_phone == 'yes')
                                {{ $invoice->enquiry->contact->mobile }}<br>
                            @endif
                            @if ($invoice->enquiry->contact && $invoice->enquiry->contact->company_name && invoice_setting()->show_client_company_name == 'yes')
                                {{ $invoice->enquiry->contact->company_name }}<br>
                            @endif
                            @if ($invoice->enquiry->contact && $invoice->enquiry->contact->address && invoice_setting()->show_client_company_address == 'yes')
                                {!! nl2br($invoice->enquiry->contact->address) !!}
                            @endif
                        </p>
                        @endif
                    </td>
