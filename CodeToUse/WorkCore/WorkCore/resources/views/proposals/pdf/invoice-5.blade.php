...
<head>
    ...
    <!-- Template CSS -->
    <title>@lang('modules.enquiry.proposal') - {{ $proposal->proposal_number }}</title>
...
<table class="bg-white" border="0" cellpadding="0" cellspacing="0" width="100%" role="presentation">
    ...
    <tr>
        <td><img src="{{ $invoiceSetting->logo_url }}" alt="{{ $company->company_name }}"
                 id="logo"/></td>
        <td align="right" class="f-21 text-black font-weight-700 text-uppercase">@lang('modules.enquiry.proposal')
        </td>
    </tr>
    <!-- Table Row End -->
    <!-- Table Row Start -->
    <tr>
        ...
        <td>
            <table class="text-black mt-1 f-13 b-collapse rightaligned">
                <tr>
                    <td class="heading-table-left">@lang('modules.enquiry.proposal') @lang('app.number')</td>
                    <td class="heading-table-right">{{ $proposal->proposal_number }}</td>
                </tr>
                <tr>
                ...
                <tr>
                    ...
                    <td class="heading-table-right">{{ $proposal->created_at->translatedFormat($company->date_format) }}
                    </td>
                </tr>
            </table>
        </td>
                    ...
                    <td class="f-14 text-black">
                        @if ($proposal->enquiry->contact && ($proposal->enquiry->contact->client_name || $proposal->enquiry->contact->client_email || $proposal->enquiry->contact->mobile || $proposal->enquiry->contact->company_name || $proposal->enquiry->contact->address) && ($invoiceSetting->show_client_name == 'yes' || $invoiceSetting->show_client_email == 'yes' || $invoiceSetting->show_client_phone == 'yes' || $invoiceSetting->show_client_company_name == 'yes' || $invoiceSetting->show_client_company_address == 'yes'))
                            <p class="line-height mb-0">
                                <span class="text-grey ">@lang("modules.invoices.billedTo")</span><br>
        ...
        @foreach ($proposal->items->sortBy('field_order') as $item)
            @if ($item->type == 'item')
                ...
                <tr class="main-table-items text-black">
                    ...
                    <td align="right"
                        class="border-bottom-0">{{ currency_format($item->amount, $proposal->currency_id, false) }}</td>
                </tr>
