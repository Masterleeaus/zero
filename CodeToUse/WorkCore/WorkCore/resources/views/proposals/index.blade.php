    ...
    <x-filters.filter-box>
        ...
        @if (!in_array('customer', user_roles()))
            ...
            <div class="select-box d-flex py-2 px-lg-2 px-md-2 px-0 border-right-grey border-right-grey-sm-0">
                <p class="mb-0 pr-2 f-14 text-dark-grey d-flex align-items-center">@lang('app.enquiry')</p>
                <div class="select-status">
                    <select class="form-control select-picker" id="clientID" data-live-search="true" data-size="8">
                        @if (!in_array('customer', user_roles()))
                            <option value="all">@lang('app.all')</option>
                        @endif
                        @foreach ($enquiries as $customer)
                            <option value="{{ $customer->id }}">{{ $customer->client_name_salutation }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
