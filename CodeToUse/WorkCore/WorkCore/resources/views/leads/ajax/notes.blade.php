...
<div class="tab-pane fade show active" role="tabpanel" aria-labelledby="nav-email-tab">
    @if ($addDealNotePermission == 'all' || $addDealNotePermission == 'added' || $addDealNotePermission == 'both')
        <div class="row p-20">
            <div class="col-md-12">
                <a class="f-15 f-w-500 openRightModal" href="{{ route('deal-notes.create').'?enquiry='.$deal->id }}" id="add-notes"><i
    ...
    <div class="d-flex flex-wrap p-20" id="service job-file-list">
        @if ($viewLeadNotePermission == 'all' || $viewLeadNotePermission == 'added' || $viewLeadNotePermission == 'both')
            <x-table headType="thead-light">
                <x-slot name="thead">
                    <th>@lang('modules.customer.noteDetail')</th>
                            ...
                            <div class="dropdown ml-auto note-action">
                                ...
                                <div class="dropdown-menu dropdown-menu-right border-grey rounded b-shadow-4 p-0"
                                    ...
                                    @if ($deleteLeadNotePermission == 'all' || ($deleteLeadNotePermission == 'added' && $note->added_by == user()->id))
                                        <a class="cursor-pointer d-block text-dark-grey f-13 py-1 px-3 delete-note-enquiry"
                                           data-id="{{ $note->id }}"
                ...
                @empty
                    <tr>
                        <td colspan="4">
                            <x-cards.no-record :message="__('team chat.noRecordFound')" icon="clipboard" />
                        </td>
                    </tr>
                @endforelse
            </x-table>
