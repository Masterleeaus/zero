<div class="tab-pane fade show active" role="tabpanel" aria-labelled@lang('app.by')="nav-email-tab">
    ...
    <div class="d-flex flex-wrap p-20">
                            ...
                            <h4 class="card-title f-12 font-weight-normal text-dark mr-3 mb-1">
                                @switch($history->event_type)
                                    @case("file-added")
                                        ...
                                        <a href="{{ route('deals.show', $deal->id) . '?tab=files' }}"> {{ __('modules.customer.viewDetails') }}</a>
                                        @break

                                    @case("proposal-created")
                                        {{ __(ucfirst($history->event_type)) }} @lang('app.by') <span class="text-darkest-grey">{{ $history->user->name }}</span>
                                        <a href="{{ route('deals.show', $deal->id) . '?tab=proposals' }}"> {{ __('modules.customer.viewDetails') }}</a>
                                        @break

                                    @case("note-added")
                                        {{ __(ucfirst($history->event_type)) }} @lang('app.by') <span class="text-darkest-grey">{{ $history->user->name }}</span>
                                        <a href="{{ route('deals.show', $deal->id) . '?tab=notes' }}"> {{ __('modules.customer.viewDetails') }}</a>
                                        @break

                                    @case("followup-created")
                                        {{ __(ucfirst($history->event_type)) }} @lang('app.by') <span class="text-darkest-grey">{{ $history->user->name }}</span>
                                        <a href="{{ route('deals.show', $deal->id) . '?tab=follow-up' }}"> {{ __('modules.customer.viewDetails') }}</a>
                                        @break
                                    ...
                                    @case("stage-updated")
                                        @if ($history->deal_stage_from_id && $history->deal_stage_to_id)
                                            ...
                                            <a href="{{ route('deals.show', $history->deal_id) }}">
                                                {{ __('modules.customer.viewDetails') }}
                                            </a>
                                        @else
                                            @lang('app.stageUpdated') @lang('app.by') <span class="text-darkest-grey">{{ $history->user->name }}</span>
                                            <a href="{{ route('deals.show', $history->deal_id) }}"> {{ __('modules.customer.viewDetails') }}</a>
                                        @endif
                                    @break
        ...
        @empty
            <x-cards.no-record icon="history" :message="__('team chat.noRecordFound')"/>
        @endforelse
