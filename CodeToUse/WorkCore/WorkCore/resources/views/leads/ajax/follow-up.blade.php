...
<div class="tab-pane fade show active" role="tabpanel" aria-labelledby="nav-email-tab">
    <div class="d-flex p-20">
        @if ($deal->leadStage->slug == 'win' || $deal->leadStage->slug == 'lost')
            <x-alert type="info" icon="info-circle">@lang('team chat.cantAddFollowup') </x-alert>
        @endif
        @if ($deal->leadStage->slug != 'win' && $deal->leadStage->slug != 'lost' && ($addLeadFollowUpPermission == 'all' || $addLeadFollowUpPermission == 'added'))
            <div class="row">
                <div class="col-md-12">
                    <a class="f-15 f-w-500" href="javascript:;" id="add-enquiry-followup"><i
        ...
        @if ($viewLeadFollowUpPermission == 'all' || $viewLeadFollowUpPermission == 'added')
            <x-table headType="thead-light">
                <x-slot name="thead">
                    ...
                    <th>@lang('modules.enquiry.nextFollowUp')</th>
                            ...
                            <div class="dropdown ml-auto file-action">
                                ...
                                <div class="dropdown-menu dropdown-menu-right border-grey rounded b-shadow-4 p-0"
                                    ...
                                    @if ($editLeadFollowUpPermission == 'all' || ($editLeadFollowUpPermission == 'added' && $folllowUp->added_by == user()->id))
                                        <a class="cursor-pointer d-block text-dark-grey f-13 py-3 px-3 edit-table-row-enquiry"
                                    ...
                                    @if ($deleteLeadFollowUpPermission == 'all' || ($deleteLeadFollowUpPermission == 'added' && user()->id == $folllowUp->added_by))
                                        <a class="cursor-pointer d-block text-dark-grey f-13 pb-3 px-3 delete-table-row-enquiry"
                ...
                @empty
                    <tr>
                        <td colspan="7">
                            <x-cards.no-record :message="__('team chat.noRecordFound')" icon="clock" />
                        </td>
                    </tr>
                @endforelse
            </x-table>
