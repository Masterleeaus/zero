...
<div id="service job-detail-section">
    ...
    <div class="row">
        ...
        <div class="col-sm-9 mb-4 mb-xl-0 mb-lg-4 mb-md-0">
            ...
            <x-cards.data :title="__('modules.deal.dealInfo')">
                ...
                <x-cards.data-row :label="__('modules.enquiry.companyName')"
                ...
                <div class="col-12 px-0 pb-3 d-flex">
                    ...
                    <p class="mb-0 text-dark-grey f-14">
                        @if (!is_null($deal->leadAgent))
                            <x-cleaner :user="$deal->leadAgent->user"/>
                        @else
                            --
                        @endif
                    </p>
                ...
                <div class="col-12 px-0 pb-3 d-flex">
                    ...
                    <p class="mb-0 text-dark-grey f-14">
                        @if (!is_null($deal->dealWatcher))
                            <x-cleaner :user="$deal->dealWatcher"/>
                        @else
                            --
                        @endif
                    </p>
                ...
                <x-cards.data-row :label="__('modules.enquiry.services / extras')"
                    ...
                    <x-tab-section class="deal-tabs">
                        @if($viewLeadFilePermission != 'none')
                            <x-tab-item class="ajax-tab files" :active="(request('tab') === 'files' || !request('tab'))"
                                            :link="route('deals.show', $deal->id).'?tab=files'">@lang('modules.enquiry.file')</x-tab-item>
                        @endif
                        @if($viewLeadFollowupPermission != 'none')
                            <x-tab-item class="ajax-tab follow-up" :active="request('tab') === 'follow-up'"
                                            :link="route('deals.show', $deal->id).'?tab=follow-up'">@lang('modules.enquiry.followUp')</x-tab-item>
                        @endif


                        @if($viewProposalPermission != 'none')
                            <x-tab-item class="ajax-tab proposals" :active="request('tab') === 'proposals'"
                                            :link="route('deals.show', $deal->id).'?tab=proposals'">@lang('modules.enquiry.proposal')</x-tab-item>
                        @endif
                        ...
                        <x-tab-item class="ajax-tab history" :active="request('tab') === 'history'"
                                    :link="route('deals.show', $deal->id).'?tab=history'">@lang('modules.service jobs.history')</x-tab-item>
        ...
        <div class="col-sm-3">
            ...
            <x-cards.data :title="__('modules.leadContact.leadDetails')">
                ...
                <x-cards.data-row :label="__('modules.leadContact.leadContact')" otherClasses="pr-1" labelClasses="pr-1"
                                    value="<a href='{{ route('enquiry-contact.show', $deal->contact->id) }}' class='text-darkest-grey'> {{ $deal->contact->client_name_salutation }}</a>"/>

                <x-cards.data-row :label="__('app.email')" :value="$deal->contact->client_email ?? '--'" otherClasses="pr-1" labelClasses="pr-1"/>
                <x-cards.data-row :label="__('modules.enquiry.mobile')" :value="$deal->contact->mobile ?? '--'" otherClasses="pr-1" labelClasses="pr-1"/>
    ...
    <script>
        ...
        $('body').on('click', '#add-enquiry-followup', function() {
            const url = "{{ route('deals.follow_up', $deal->id) }}";
            $(MODAL_LG + ' ' + MODAL_HEADING).html('...');
            $.ajaxModal(MODAL_LG, url);
        })

        $('body').on('click', '.edit-table-row-enquiry', function() {
        ...
        $('body').on('click', '.delete-table-row-enquiry', function() {
            var id = $(this).data('followup-id');
            Swal.fire({
                title: "@lang('team chat.sweetAlertTitle')",
        ...
        $('body').on('click', '.delete-note-enquiry', function() {
            var id = $(this).data('id');
            Swal.fire({
                title: "@lang('team chat.sweetAlertTitle')",
                text: "@lang('team chat.recoverRecord')",
