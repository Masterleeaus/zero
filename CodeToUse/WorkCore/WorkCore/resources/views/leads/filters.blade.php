<x-filters.filter-box>
    ...
    <div class="select-box d-flex py-2 px-lg-2 px-md-2 px-0 border-right-grey border-right-grey-sm-0">
        ...
        <div class="select-status">
            <select class="form-control select-picker pipelineFilter" name="pipeline" id="pipeline">
                {{-- <option value="all">@lang('modules.enquiry.all')</option> --}}
    ...
    <!-- PRODUCT START -->
    <div class="select-box d-flex py-2 px-lg-2 px-md-2 px-0 border-right-grey border-right-grey-sm-0">
        <p class="mb-0 pr-2 f-14 text-dark-grey d-flex align-items-center">@lang('modules.deal.service / extra')</p>
        <div class="select-status">
            <select class="form-control select-picker productFilter" name="service / extra" id="service / extra">
                <option value="all">@lang('app.all')</option>
                @foreach($services / extras as $service / extra)
                    <option value="{{ $service / extra->id }}">{{ $service / extra->name }}</option>
                @endforeach
            </select>
        </div>
    </div>
    ...
    <x-filters.more-filter-box>
        ...
        <div class="more-filter-items">
            ...
            <div class="more-filter-items  ">
                <label class="f-14 text-dark-grey mb-12 " for="usr">@lang('modules.issues / support.agent')</label>
                <div class="select-filter mb-4">
                    <div class="select-others">
            ...
            <div class="more-filter-items  ">
                <label class="f-14 text-dark-grey mb-12 " for="usr">@lang('app.enquiry')</label>
                <div class="select-filter mb-4">
                    <div class="select-others">
...
@push('scripts')
    <script>
        $('#type, #pipeline, #category,#service / extra, #filter_category_id, #filter_source_id, #filter_status_id, #date_filter_on, #min, #max, #agent_id, #deal_watcher_agent_id, #lead_agent_id')
            ...
            $.easyAjax({
                ...
                success: function (response) {
                    if (response.status == 'success') {
                        ...
                        $('#filter_status_id').html('<option value="all">@lang('modules.enquiry.all')</option>' +
                            options);
