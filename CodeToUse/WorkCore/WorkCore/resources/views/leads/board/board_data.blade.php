...
@foreach ($result['boardColumns'] as $key => $column)
     @if ($column->userSetting && $column->userSetting->collapsed)
         ...
         <div class="minimized rounded bg-additional-grey border-grey mr-3">
             ...
             <div class="d-flex mt-4 mx-1 b-p-header align-items-center">
                 ...
                 <span class="b-p-badge bg-grey f-13 px-2 py-2 text-lightest font-weight-bold rounded d-inline-block" id="enquiry-column-count-{{ $column->id }}">{{ $column->deals_count }}</span>
         ...
         <div class="board-panel rounded bg-additional-grey border-grey mr-3">
             ...
             <div class="mx-3 mt-3 mb-1 b-p-header">
                <div class="d-flex">
                 ...
                 <span
                     class="b-p-badge bg-grey f-13 px-2 text-lightest font-weight-bold rounded d-inline-block ml-1" id="enquiry-column-count-{{ $column->id }}">{{ $column->deals_count }}</span>
             ...
             <div class="b-p-body">
                 <!-- MAIN TASKS START -->
                 <div class="b-p-service jobs" id="drag-container-{{ $column->id }}" data-column-id="{{ $column->id }}">
                    <div class="card rounded bg-white border-grey b-shadow-4 m-1 mb-3 no-service job-card move-disable {{ (count($column['deals']) > 0) ? 'd-none' : '' }}">
                        <div class="card-body">
                    ...
                    @foreach ($column['deals'] as $enquiry)

                         <x-cards.enquiry-card :draggable="($changeStatusPermission == 'all') ? 'true' : 'false'" :enquiry="$enquiry" />
                    @endforeach
                </div>
     <!-- MAIN TASKS END -->
     @if ($column->deals_count > count($column['deals']))
         ...
         <div class="d-flex m-3 justify-content-center">
             <a class="f-13 text-dark-grey f-w-500 load-more-service jobs" data-column-id="{{ $column->id }}"
                 data-total-service jobs="{{ $column->deals_count }}"
 ...
 <script>
     drake.on('drop', function(element, target, source, sibling) {
         ...
         var sourceColumnCount = parseInt($('#enquiry-column-count-' + sourceBoardColumnId).text());
         var targetColumnCount = parseInt($('#enquiry-column-count-' + boardColumnId).text());
         ...
         // update values for all service jobs
         $.easyAjax({
             ...
             success: function() {
                ...
                $('#enquiry-column-count-' + sourceBoardColumnId).text(sourceColumnCount - 1);
                $('#enquiry-column-count-' + boardColumnId).text(targetColumnCount + 1);
