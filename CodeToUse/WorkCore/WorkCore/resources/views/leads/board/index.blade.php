        ...
        <div class="d-grid d-lg-flex d-md-flex action-bar my-3">
            ...
            <div class="btn-group mt-2 mt-lg-0 mt-md-0 ml-0 ml-lg-3 ml-md-3" role="group">
                ...
                <a href="{{ route('leadboards.index') }}" class="btn btn-secondary f-14 btn-active" data-toggle="tooltip"
                    data-original-title="@lang('modules.enquiry.kanbanboard')"><i class="side-icon bi bi-kanban"></i></a>
...
@push('scripts')
    ...
    <script>
        function showTable() {
            ...
            var service / extra = $('#service / extra').val();
            var min = $('#min').val();
            var max = $('#max').val();
            var type = $('#type').val();
            ...
            var url = "{{ route('leadboards.index') }}?startDate=" + encodeURIComponent(startDate) + '&endDate=' +
                ...
                '&searchText=' + searchText  + '&min=' + min + '&max=' + max + '&date_filter_on=' + date_filter_on + '&status_id=' + status_id + '&pipeline=' + pipeline+ '&service / extra=' + service / extra;
        ...
        $('body').on('click', '.load-more-service jobs', function() {
            ...
            var url = "{{ route('leadboards.load_more') }}?startDate=" + encodeURIComponent(startDate) +
                ...
                '&totalTasks=' + totalTasks + '&min=' + min + '&max=' + max + '&date_filter_on=' + date_filter_on + '&pipeline=' + pipeline+ '&service / extra=' + service / extra;
        ...
        $('#add-column').click(function() {
            const url = "{{ route('enquiry-stage-setting.create') }}";
        ...
        $('body').on('click', '.edit-column', function() {
            ...
            var url = "{{ route('enquiry-stage-setting.edit', ':id ') }}";
        ...
        $('body').on('click', '.delete-column', function() {
            ...
            var url = "{{ route('enquiry-stage-setting.destroy', ':id') }}";
            url = url.replace(':id', id);

            Swal.fire({
                title: "@lang('team chat.sweetAlertTitle')",
                text: "@lang('team chat.deal.deleteStage')",
                icon: 'warning',
