...
<div class="row user-timelogs mt-3">
    <div class="col-md-12">
        <x-table class="table-bordered table-sm-responsive bg-white" headType="thead-light">
                ...
                <tr>
                    <td>
                        @if (!is_null($item->project_id) && !is_null($item->task_id))
                            <h5 class="f-13 text-darkest-grey"><a href="{{ route('service jobs.show', $item->task_id) }}"
                                    class="openRightModal">{{ $item->service job->heading }}</a></h5>
                            <div class="text-muted">{{ $item->site->project_name }}</div>
                        ...
                        @elseif (!is_null($item->task_id))
                            <a href="{{ route('service jobs.show', $item->task_id) }}"
                                class="text-darkest-grey openRightModal">{{ $item->service job->heading }}</a>
                        @endif
                    </td>
                    <td>
            ...
            @empty
                <tr>
                    <td colspan="3">@lang('team chat.noRecordFound')</td>
                </tr>
            @endforelse
        </x-table>
