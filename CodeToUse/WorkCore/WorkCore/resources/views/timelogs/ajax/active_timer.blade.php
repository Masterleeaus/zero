...
<div class="modal-header">
    <h5 class="modal-title" id="modelHeading">@lang('modules.sites.activeTimers')</h5>
                        ...
                        <div class="col-sm-12">
                            ...
                            <ul class="list-group">
                                ...
                                <li class="list-group-item d-flex justify-content-between align-items-center f-12 text-dark-grey">
                                    ...
                                    {{ $myActiveTimer->service job->heading }}
                                </li>
                        ...
                        <div class="col-sm-12 pt-3 text-right">
                            @if (
                                    ...
                                    || ($editTimelogPermission == 'owned'
                                        && (($myActiveTimer->site && $myActiveTimer->site->client_id == user()->id) || $myActiveTimer->user_id == user()->id)
                                        )
                                    || ($editTimelogPermission == 'both' && (($myActiveTimer->site && $myActiveTimer->site->client_id == user()->id) || $myActiveTimer->user_id == user()->id || $myActiveTimer->added_by == user()->id))
                                )
                    ...
                    <x-form id="startTimerForm">
                        ...
                        <div class="row">
                            <div class="col" id="task_div">
                                <x-service job-selection-dropdown :service jobs="$service jobs" />
                            </div>
                        </div>
                                ...
                                <div class="col">
                                    <div class="form-group">
                                        <div class="d-flex mt-3">
                                            <x-forms.checkbox :fieldLabel="__('app.create') . ' ' . __('modules.service jobs.newTask')"
            ...
            <div class="table-responsive">
                <x-table class="table-bordered table-hover rounded" id="active-timer-table" width="100%" headType="thead-light">
                    <x-slot name="thead">
                        ...
                        <th id="cleaner-header" width="5%" style="width: 20px !important;">@lang('app.cleaner')</th>
                    ...
                    @forelse ($activeTimers as $key => $item)
                        ...
                        <tr id="timer-{{ $item->id }}">
                            <td>{{ $key + 1 }}</td>
                            <td>
                                <a href="{{ route('service jobs.show', $item->task_id) }}" class="text-darkest-grey">
                                    {{ $item->service job->heading }}
                                </a>
                                @if ($item->service job->project_id)
                                    <p class="text-lightest mb-0 site-name">{{ $item->service job->site->project_name }}</p>
                                @endif
                            </td>
                            <td class="text-right cleaner-user"  width="5%">
                               <div style="display: none"> {{ $item->user->name }} </div>
                                <x-cleaner-image :user="$item->user" />
                            </td>
                            <td class="text-right">
                            ...
                            <td class="text-right">
                                @if (
                                    ...
                                    || ($editTimelogPermission == 'owned'
                                        && (($item->site && $item->site->client_id == user()->id) || $item->user_id == user()->id)
                                        )
                                    || ($editTimelogPermission == 'both' && (($item->site && $item->site->client_id == user()->id) || $item->user_id == user()->id || $item->added_by == user()->id))
                                )
                    ...
                    @empty
                        <tr>
                            <td colspan="5">
                                <x-cards.no-record icon="clock" :message="__('team chat.noRecordFound')" />
                            </td>
                        </tr>
                    @endforelse
...
<script>
    ...
    $('#startTimerForm').on('change', '#project_id', function () {
        ...
        let url = "{{ route('sites.pendingTasks', ':id') }}";
