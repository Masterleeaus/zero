                        ...
                        <div class="row">
                            <div class="col-md-6 col-lg-4">
                                <x-forms.select fieldId="project_id2" fieldName="project_id"
                                    :fieldLabel="__('app.site')" search="true">
                                    <option value="">--</option>
                                    @foreach ($sites as $site)
                                        <option @if ($site->id == $timeLog->project_id) selected @endif value="{{ $site->id }}">
                                            {{ $site->project_name }}
                                        </option>
                                    @endforeach
                                </x-forms.select>
                            ...
                            <div class="col-md-6 col-lg-4">
                                <x-forms.select fieldId="task_id2" fieldName="task_id" :fieldLabel="__('app.service job')"
                                    fieldRequired="true" search="true">
                                    <option value="">--</option>
                                    @if ($timeLog->task_id)
                                        <option selected value="{{ $timeLog->task_id }}">
                                            {{ $timeLog->service job->heading }}</option>
                                    @endif
                            ...
                            @if ($editTimelogPermission == 'all')
                                <div class="col-md-6 col-lg-4">
                                    <x-forms.label class="mt-3" fieldId="user_id2" :fieldLabel="__('app.cleaner')"
                                        fieldRequired="true">
                                    </x-forms.label>
                                    <x-forms.input-group>
...
<script>
    $(document).ready(function() {
        ...
        $('#project_id2').change(function() {
            ...
            var url = "{{ route('service jobs.project_tasks', ':id').'?for_timelogs=true' }}";
        ...
        $('#task_id2').change(function() {
            ...
            var url = "{{ route('service jobs.members', ':id') }}";
            url = url.replace(':id', id);
            $.easyAjax({
                url: url,
                type: "GET",
