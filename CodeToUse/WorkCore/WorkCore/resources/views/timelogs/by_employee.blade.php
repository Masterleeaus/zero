...
<script src="{{ asset('vendor/jquery/daterangepicker.min.js') }}"></script>
    ...
    <script>
        ...
        $('#project_id, #cleaner, #status, #invoice_generate').on('change keyup',
            function() {
                if ($('#status').val() != "all") {
        ...
        $('body').on('click', '.show-user-timelogs', function() {
            // Close all other expanded entries first
            $('.user-timelogs').remove();
            $('.show-user-timelogs').removeClass('d-none');
            $('.hide-user-timelogs').addClass('d-none');

            var cleaner = $(this).data('user-id');
            ...
            $.easyAjax({
                ...
                url: url,
                blockUI: true,
                container: '.timelog-user-' + cleaner + ' .card-body',
                data: {
                    '_token': token,
                    startDate: startDate,
                    endDate: endDate,
                ...
                success: function(response) {
                    $('.user-timelogs').remove();
                    $('.timelog-user-' + cleaner + ' .card-body').append(response.html);
                    $('.timelog-user-' + cleaner).find('.hide-user-timelogs').removeClass('d-none');
                    $('.timelog-user-' + cleaner).find('.show-user-timelogs').addClass('d-none');
                    $("body").tooltip({
        ...
        $('body').on('click', '.hide-user-timelogs', function() {
            var cleaner = $(this).data('user-id');
            $('.user-timelogs').remove();
            $('.timelog-user-' + cleaner).find('.show-user-timelogs').removeClass('d-none');
        ...
        $('body').on('click', '.revert-timelog-to-pending', function() {
            ...
            Swal.fire({
                title: '@lang("team chat.sweetAlertTitle")',
                text: '@lang("team chat.revertTimelogToPending")',
                icon: 'warning',
                showCancelButton: true,
                focusConfirm: false,
        ...
        @if (canDataTableExport())
            $('.export-excel').click(function() {
                ...
                window.location = url + '?startDate=' + encodeURIComponent(startDate) + '&endDate=' + encodeURIComponent(endDate) + '&projectID=' + projectID + '&cleaner=' + cleaner;
            });
        @endif
