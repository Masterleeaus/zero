        ...
        <x-form id="save-enquiry-data-form" method="PUT">
            <div class="add-customer bg-white rounded">
                            ...
                            <x-forms.input-group>
                                <select class="form-control select-picker" name="parent_id" id="parent_id"
                                    ...
                                    @foreach($roles as $roles)
                                        @if($role->id != $roles->id)
                                            <option value="{{ $roles->id }}" @if($role->parent_id == $roles->id) selected @endif>{{ $roles->name }}</option>
                                        @endif
                                    @endforeach
                                </select>
                            </x-forms.input-group>
                ...
                <x-form-actions>
                    ...
                    <x-forms.button-cancel :link="route('roles.index')" class="border-0">@lang('app.cancel')
