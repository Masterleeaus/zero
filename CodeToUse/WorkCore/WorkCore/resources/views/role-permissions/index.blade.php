                ...
                @forelse($roles as $role)
                    <div class="d-flex justify-content-between border rounded my-3 px-4 py-2 align-items-center">
                        <div>
                            ...
                            <div class="simple-text text-lightest mt-1">{{ $role->name == 'cleaner' ? ($role->users_count - $otherRoleUserCount) : $role->users_count }} @lang('app.member')
                            </div>
                        </div>
                        <div>
                            @if ($role->name == 'admin')
                                <span class="text-lightest">@lang('team chat.adminPermissionsCantChange')</span>
                            @else
    ...
    <script>
        ...
        $('body').on('click', '.reset-permission', function() {
            ...
            Swal.fire({
                ...
                text: "@lang('team chat.confirmResetPermission')",
                icon: 'warning',
