...
@section('filter-section')
    ...
    <x-filters.filter-box>
        ...
        @if ($viewAttendancePermission == 'owned')
            <input type="hidden" name="zone" id="zone" value="all">
            <input type="hidden" name="role" id="role" value="all">
        @else
            <div class="select-box d-flex py-2 px-lg-2 px-md-2 px-0 border-right-grey border-right-grey-sm-0">
                <p class="mb-0 pr-2 f-14 text-dark-grey d-flex align-items-center">@lang('app.zone')</p>
                <div class="select-status">
                    <select class="form-control select-picker" name="zone" id="zone" data-live-search="true"
                        data-size="8">
                        <option value="all">@lang('app.all')</option>
    ...
    <div class="content-wrapper px-4">
        ...
        <div class="d-grid d-lg-flex d-md-flex action-bar">
            ...
            <div class="btn-group mt-2 mt-lg-0 mt-md-0 ml-0 ml-lg-3 ml-md-3" role="group">
                ...
                @if (isset($attendanceRegularisation) && $attendanceRegularisation->adjust_attendance_logs != 'not_allowed')
                    <a href="{{ route('attendances.by_request') }}" class="btn btn-secondary f-14" data-toggle="tooltip"
                    data-original-title="@lang('clan.attendance.attendanceRegularisation') ({{$pendingRequestRegularisation ?? 0}})"><i
...
@push('scripts')
    <script>
        $('#user_id, #zone, #month, #year, #role').on('change', function() {
            if ($('#user_id').val() != "all") {
        ...
        @if (canDataTableExport())
            $('#export-all').click(function() {
                ...
                var url =
                    "{{ route('attendances.export_all_attendance', [':year', ':month', ':userId', ':zone', ':role']) }}";
                url = url.replace(':year', year).replace(':month', month).replace(':userId', userId).replace(':zone', zone).replace(':role', role);
