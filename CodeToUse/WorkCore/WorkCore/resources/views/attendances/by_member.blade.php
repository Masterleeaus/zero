...
@push('scripts')
    ...
    <script>
        ...
        $('#attendance-data').on('click', '.request-regularise', function() {
            var date =  $(this).data('date');
            var userId = $('#userId').val();

            regulariseRequest(userId, date);
        });

        function regulariseRequest(userID, date) {
            ...
            var url = "{{ route('attendances.request-regulisation', [':userid', ':day', ':month', ':year']) }}";
            url = url.replace(':userid', userID);
        ...
        $('#attendance-data').on('click', '.request-pending', function() {
            var date =  $(this).data('date');
            var userId = $('#userId').val();

            pendingRequest(userId, date);

        });

        function pendingRequest(userID, date) {
            const attendanceDate = date.split("-");
            let dayTime = attendanceDate[2];
            dayTime = dayTime.split(' ');
            let day = dayTime[0];
            ...
            var url = "{{ route('attendances.request-pending', [':userid', ':day', ':month', ':year']) }}";
            url = url.replace(':userid', userID);
            url = url.replace(':day', day);
            url = url.replace(':month', month);
            url = url.replace(':year', year);
