...
<div class="modal-body">
    <div class="row">
        <div class="col-md-12 mb-4">
            <x-cleaner :user="$attendanceUser" />
        </div>
    </div>

    <div class="row">
        <div class="col-sm-12">
            <h5 class="f-w-500 f-15 d-flex justify-content-between">{{ __('app.date').' - '.\Carbon\Carbon::parse($date)->translatedFormat(company()->date_format) }} <span class="badge badge-info f-14" style="background-color: {{ $attendanceSettings->color ?? '#ccc'}}">{{ $attendanceSettings->shift_name ?? 'No Shift'}}</span></h5>
            ...
            <x-form id="attendance-container">
                ...
                <div class="row">
                    ...
                    <div class="col-lg-6 col-md-6">
                        <div class="bootstrap-timepicker timepicker">
                            <x-forms.text class="a-timepicker"
                                          ...
                                          fieldId="clock-in-time"
                                          fieldRequired="true"
                                          :fieldValue="isset($row) && $row->clock_in_time
                                              ? \Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $row->clock_in_time)
                    ...
                    <div class="col-lg-6 col-md-6">
                        ...
                        search="true" >
                        <option value="office" selected>@lang('modules.attendance.office')</option>
                    ...
                    <div class="col-lg-6 col-md-6" id="other_place" style="display:none">
                        <x-forms.text fieldId="other_location" :fieldLabel="__('modules.attendance.otherPlace')" fieldName="other_location" fieldRequired="true" >
                        </x-forms.text>
                    </div>
                </div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#save-attendance').click(function () {
            ...
            var url = "{{route('attendances.regularisationStore')}}";
            $.easyAjax({
                url: url,
                type: "POST",
                ...
                success: function (response) {
                    if(response.status == 'success'){
                        $(MODAL_XL).modal('hide');
                        $(MODAL_LG).modal('hide');
                        var pendingRequest = response.pendingRequest;
                        var total = "{{__('clan.attendance.attendanceRegularisation')}} "+ ' ('+pendingRequest+')';
                        $('#request-tab').attr('data-original-title', total);
                        showTable();
                    }
                }
            })
