            ...
            <div class="add-customer bg-white rounded">
                    ...
                    <div class="col-lg-3 col-md-6">
                        @if (isset($defaultAssign))
                            <div class="form-group">
                                <x-forms.label class="mt-3" fieldId=department_id :fieldLabel="__('app.zone')" fieldRequired="true">
                                </x-forms.label>
                                <input type="hidden" name="zone" id="department_id" value="{{ $defaultAssign->employeeDetail->department_id }}">
                                <input type="text" value="{{ $defaultAssign->employeeDetail->zone->team_name }}"
                                    class="form-control height-35 f-15 readonly-background" readonly>
                            </div>
                        @else
                            <x-forms.select fieldId="department_id" :fieldLabel="__('app.zone')"
                    ...
                    <div class="col-md-9">
                        ...
                        @else
                            <div class="form-group my-3">
                                <x-forms.label fieldId="selectEmployee" :fieldLabel="__('app.menu.cleaners')"
                                fieldRequired="true">
                                </x-forms.label>
                                <x-forms.input-group>
                ...
                <div class="row px-4 pb-4">
                    ...
                    <div class="col-lg-4 col-md-6 d-none multi_date_div">
                        <x-forms.text :fieldLabel="__('team chat.selectMultipleDates')" fieldName="multi_date"
                            fieldId="multi_date" :fieldPlaceholder="__('team chat.selectMultipleDates')"
                ...
                <div class="row p-20">
                    <div class="col-lg-4 col-md-6">
                        <x-forms.checkbox class="mr-0 mr-lg-2 mr-md-2" :fieldLabel="__('app.overwriteAttendance')"
                                          ...
                                          fieldRequired="true" :popover="__('team chat.overwriteAttendanceTooltip')"/>
                    </div>
                </div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#department_id').change(function() {
            var id = $(this).val();
            var url = "{{ route('cleaners.by_department', ':id') }}";
        ...
        const saveAttendanceForm = () => {
            // Validate cleaner selection before submitting
            var selectEmployeeField = $('#selectEmployee');
            if (selectEmployeeField.is('select')) {
                var selectedEmployees = selectEmployeeField.val();
                if (!selectedEmployees || selectedEmployees.length === 0) {
                    Swal.fire({
                        title: "@lang('team chat.sweetAlertTitle')",
                        text: "@lang('team chat.atleastOneValidation')",
                        icon: 'error',
            ...
            // Handle date range only if it exists (for date mode)
        ...
        {
            // Validate cleaner selection before submitting (only if it's a select field, not hidden input)
            var selectEmployeeField = $('#selectEmployee');
            if (selectEmployeeField.is('select')) {
                var selectedEmployees = selectEmployeeField.val();
                if (!selectedEmployees || selectedEmployees.length === 0) {
            ...
            $.easyAjax({
                ...
                success: function(response) {
                        ...
                        (response.attendanceDuration === response.leaveDuration)
                            ...
                            Swal.fire({
                                title: "@lang('team chat.sweetAlertTitle')",
                                text: "@lang('team chat.halfDayAlreadyApplied')",
                                icon: 'warning',
                                showCancelButton: true,
                                focusConfirm: false,
                                confirmButtonText: "@lang('team chat.rejectIt')",
                        ...
                        (response.halfDayExist == false && response.requestedHalfDay == 'yes')) {
                            Swal.fire({
                                title: "@lang('team chat.sweetAlertTitle')",
                                text: "@lang('team chat.fullDayAlreadyApplied')",
                                icon: 'warning',
                                showCancelButton: true,
                                focusConfirm: false,
                                confirmButtonText: "@lang('team chat.rejectIt')",
                        ...
                        (response.fullDayExist == true && response.requestedFullDay == 'no')){
                            Swal.fire({
                                title: "@lang('team chat.sweetAlertTitle')",
                                text: "@lang('team chat.leaveAlreadyApplied')",
                                icon: 'warning',
                                showCancelButton: true,
                                focusConfirm: false,
                                confirmButtonText: "@lang('team chat.rejectIt')",
