...
<div class="tab-pane fade show active" role="tabpanel" aria-labelledby="nav-email-tab">
    ...
    <x-cards.data :title="__('modules.service agreements.discussion')">
            ...
            <div class="row">
                <div class="col-md-12">
                    <a class="f-15 f-w-500" href="javascript:;" id="add-comment"><i
                        ...
                        @lang('modules.service agreements.addComment')</a>
                </div>
            </div>
        ...
        <div class="d-flex flex-wrap justify-content-between mt-3" id="comment-list">
            @if ($viewDiscussionPermission != 'none')
                @forelse ($service agreement->discussion as $discussion)
                    <div class="card w-100 rounded-0 border-0 comment">
                        <div class="card-horizontal">
                            <div class="card-img my-1 ml-0">
                                <a href="{{ route('cleaners.show', $discussion->user->id) }}">
                            ...
                            <div class="card-body border-0 pl-0 py-1">
                                <div class="d-flex flex-grow-1">
                                    <h4 class="card-title f-15 f-w-500 mr-3"><a class="text-dark"
                                            href="{{ route('cleaners.show', $discussion->user->id) }}">{{ $discussion->user->name }}</a>
                                    </h4>
                                    ...
                                    <div class="dropdown ml-auto comment-action">
                                        ...
                                        <div class="dropdown-menu dropdown-menu-right border-grey rounded b-shadow-4 p-0"
                                            ...
                                            @if ($editContractDiscussionPermission == 'all' || ($editContractDiscussionPermission == 'added' && ($discussion->added_by == user()->id || $discussion->added_by == $userId || in_array($discussion->added_by, $id))))
                                                <a class="cursor-pointer d-block text-dark-grey f-13 py-3 px-3 edit-comment"
                                                    href="javascript:;"
                                                    data-row-id="{{ $discussion->id }}">@lang('app.edit')</a>
                                            @endif

                                            @if ($deleteContractDiscussionPermission == 'all' || ($deleteContractDiscussionPermission == 'added' && ($discussion->added_by == user()->id || $discussion->added_by == $userId || in_array($discussion->added_by, $id))))
                                ...
                                @php
                                    $isClient = \App\Models\User::isClient($discussion->user->id);
                                    $customer = \App\Models\User::where('id', $discussion->added_by)->where('is_client_contact', 1)->first();
                                @endphp
...
<script>
    ...
    $(document).ready(function() {
        if (add_task_comments == "all" || add_task_comments == "added") {
              quillMention(null, '#service job-comment');
        }
        ...
        $('#submit-comment').click(function() {
            ...
            $.easyAjax({
                ...
                data: {
                    '_token': token,
                    comment: comment,
                    contract_id: '{{ $service agreement->id }}'
                },
