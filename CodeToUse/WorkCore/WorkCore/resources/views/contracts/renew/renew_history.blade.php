...
@endphp
@forelse ($service agreement->renewHistory as $history)
    <div class="card w-100 rounded-0 border-0 comment mb-2">
        <div class="card-horizontal">
            <div class="card-img my-1 ml-0">
                <a href="{{ route('cleaners.show', $history->renewedBy->id) }}">
            ...
            <div class="card-body border-0 pl-0 py-1">
                <div class="d-flex flex-grow-1">
                    <h4 class="card-title f-15 f-w-500"><a class="text-dark"
                        href="{{ route('cleaners.show', $history->renewedBy->id) }}">{{ $history->renewedBy->name }}</a>
                        <br>
                ...
                <div class="card-text f-14 text-dark-grey text-justify">
                    <x-table class="table-bordered my-3 rounded">
                        <x-slot name="thead">
                            ...
                            <th class="text-right">@lang('modules.service agreements.newAmount')</th>
                        </x-slot>
                        <tr>
                            ...
                            <td class="text-right">{{ currency_format($history->amount, $service agreement->currency_id) }}</td>
                        </tr>
                    </x-table>
                </div>
            </div>
...
@empty
    <x-cards.no-record icon="redo" :message="__('team chat.noRecordFound')" />
