...
<div id="container">
    <div class="invoice-top">
        ...
        @if(!is_null($creditNote->site) && !is_null($creditNote->site->customer))
            <section id="customer-info">
                @if(!is_null($creditNote->site->customer))
                    <span>@lang('modules.credit-notes.billedTo'):</span>
                    <div>
                        <span class="bold">{{ $creditNote->site->customer->name_salutation }}</span>
                    </div>

                    <div>
                        <span>{{ $creditNote->site->clientDetails->company_name }}</span>
                    </div>

                    <div>
                        <span>{{ $creditNote->site->customer->email }}</span>
                    </div>

                    <div>
                        <b>@lang('app.address') :</b>
                        <span>{!! nl2br($creditNote->site->clientDetails->address) !!}</span>
                    </div>

                    @if($creditNoteSetting->show_gst == 'yes' && !is_null($creditNote->site->clientDetails->gst_number))
                        <div>
                            <span> @lang('app.gstIn'): {{ $creditNote->site->clientDetails->gst_number }} </span>
                        </div>
                    @endif
                @endif
                @if (($invoiceSetting->show_project == 1) && (isset($creditNote->site)))
                <br>
                <span class="text-dark-grey ">@lang("modules.invoices.projectName"):</span>
                {{ $creditNote->site->project_name }}
                @endif
            </section>
        @elseif(is_null($creditNote->site) && !is_null($creditNote->customer->clientDetails))
            <section id="customer-info">
                @if(!is_null($creditNote->customer))
                    <span>@lang('modules.credit-notes.billedTo'):</span>
                    <div>
                        <span class="bold">{{ $creditNote->customer->name_salutation }}</span>
                    </div>

                    <div>
                        <span>{{ $creditNote->customer->clientDetails->company_name }}</span>
                    </div>
                    ...
                    <div>
                        <b>@lang('app.address') :</b>
                        <span>{!! nl2br($creditNote->customer->clientDetails->address) !!}</span>
                    </div>
                    @if($creditNoteSetting->show_gst == 'yes' && !is_null($creditNote->customer->clientDetails->gst_number))
                        <div>
                            <span> @lang('app.gstIn'): {{ $creditNote->customer->clientDetails->gst_number }} </span>
                        </div>
                    @endif
                @endif
                @if (($invoiceSetting->show_project == 1) && (isset($creditNote->customer)))
                    <br>
                    <span class="text-dark-grey ">@lang("modules.invoices.projectName"):</span>
                    {{ $creditNote->site->project_name }}
                @endif
    ...
    <div class="invoice-body">
        <section id="items">
            ...
            <table cellpadding="0" cellspacing="0">
                ...
                <tr data-iterate="tax">
                    <td colspan="{{ $creditNoteSetting->hsn_sac_code_show ? '5': '4' }}">@lang("modules.credit-notes.discount"):
                ...
                @php
                    ...
                    $totalPaymentAmount = $creditNote->sub_total - $discount + array_sum($taxesIncluded);
                    $deductedSum = array_sum(array_map('abs', $taxesDeducted));
                    $netPayableBeforeAdjustment = $hasDeducted
                        ? ($totalPaymentAmount - $deductedSum)
                        : ($creditNote->sub_total - $discount + array_sum($taxes ?? []));
                    $displayTotal = $netPayableBeforeAdjustment + (float) ($creditNote->adjustment_amount ?? 0);
                @endphp
                ...
                @if ($hasDeducted)
                    ...
                    <tr data-iterate="tax">
                        <td colspan="{{ $creditNoteSetting->hsn_sac_code_show ? '5': '4' }}">@lang('modules.invoices.totalPayment'):</td>
                        <td>{{ currency_format($totalPaymentAmount, $creditNote->currency_id, false) }}</td>
                    </tr>

                    @foreach($taxesDeducted as $key => $tax)
                        <tr data-iterate="tax">
                            <td colspan="{{ $creditNoteSetting->hsn_sac_code_show ? '5': '4' }}">{{ $key }} (Deducted):</td>
                            <td>-{{ currency_format(abs($tax), $creditNote->currency_id, false) }}</td>
                        </tr>
                    @endforeach

                    <tr data-iterate="tax">
                        <td colspan="{{ $creditNoteSetting->hsn_sac_code_show ? '5': '4' }}">@lang('modules.invoices.netPayable'):</td>
                        <td>{{ currency_format($netPayableBeforeAdjustment, $creditNote->currency_id, false) }}</td>
                    </tr>
                @else
                ...
                <tr class="amount-total">
                    ...
                    <td>{{ currency_format($displayTotal, $creditNote->currency_id, false) }}</td>
                </tr>
                <tr>
                ...
                <tr>
                    ...
                    <td>
                        {{ currency_format($displayTotal - $creditNote->creditAmountUsed(), $creditNote->currency_id, false) }}</td>
                </tr>
