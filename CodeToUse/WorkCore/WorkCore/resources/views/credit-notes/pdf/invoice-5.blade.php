...
<table class="bg-white" border="0" cellpadding="0" cellspacing="0" width="100%" role="presentation">
    ...
    <tr>
        <td colspan="2">
            @if (!is_null($creditNote->project_id) && !is_null($creditNote->site) && !is_null($creditNote->site->clientDetails))
                @php
            ...
            <table border="0" cellpadding="0" cellspacing="0" width="100%">
                <tr>
                    <td class="f-14 text-black">
                        ...
                        @if ($creditNoteSetting->show_gst == 'yes' && !is_null($customer->clientDetails->gst_number))
                            <br>@lang('app.gstIn'):
                            {{ $customer->clientDetails->gst_number }}
                        @endif
                    </td>
                    <td align="right">
                        <div
    ...
    <tr>
        ...
        <td class="total-box" align="right" width="{{ $invoiceSetting->hsn_sac_code_show ? '17%' : '20%' }}">
            ...
            @if ($hasDeducted)
                ...
                <!-- Table Row Start -->
                <tr align="right" class="text-black">
                    <td class="subtotal-amt">{{ currency_format($totalPaymentAmount, $creditNote->currency_id, false) }}</td>
                </tr>
                <!-- Table Row End -->
                ...
                @foreach ($taxesDeducted as $key => $tax)
                    <!-- Table Row Start -->
                    <tr align="right" class="text-grey">
                        <td class="subtotal-amt">-{{ currency_format(abs($tax), $creditNote->currency_id, false) }}</td>
                    </tr>
                    <!-- Table Row End -->
                @endforeach
                ...
                <!-- Table Row Start -->
                <tr align="right" class="text-black">
                    <td class="subtotal-amt">{{ currency_format($netPayableBeforeAdjustment, $creditNote->currency_id, false) }}</td>
                </tr>
                <!-- Table Row End -->
            @else
                @foreach ($taxes as $key => $tax)
            ...
            <!-- Table Row Start -->
                ...
                <tr align="right" class="balance text-black">
                    <td class="balance-right">
                        {{ currency_format($displayTotal - $creditNote->creditAmountUsed(), $creditNote->currency_id, false) }}
