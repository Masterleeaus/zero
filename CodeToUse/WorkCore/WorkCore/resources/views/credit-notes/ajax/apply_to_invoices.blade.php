...
<div class="row">
    ...
    <div class="col-sm-12 mb-4">
        <x-alert type="warning" icon="info-circle">@lang('team chat.sameCurrencyInvoiceNote')</x-alert>
    </div>
        ...
        <div class="col-lg-12 col-md-12 mb-4 mb-xl-0 mb-lg-4 mb-md-0">
            ...
            <div class="row">
                ...
                <div class="col-xl-6 col-sm-12 mb-4">
                    <x-cards.user :image="$creditNote->customer->image_url">
                        <div class="row">
                            <div class="col-10">
                        ...
                        <p class="f-13 font-weight-normal text-dark-grey mb-0">
                            {{ $creditNote->customer->clientDetails->company_name }}
                        </p>
                        <p class="card-text f-12 text-lightest">@lang('app.lastLogin')

                            @if (!is_null($creditNote->customer->last_login))
                                {{ $creditNote->customer->last_login->timezone(company()->timezone)->translatedFormat(company()->date_format . ' ' . company()->time_format) }}
                            @else
                                --
                            @endif
                        </p>
            ...
            <x-cards.data padding="false">
                <div class="table-responsive">
                    <x-table class="table-hover">
                        ...
                        @empty
                            <td colspan="3">
                                <x-cards.no-record icon="file-invoice-dollar" :message="__('team chat.noRecordFound')" />
                            </td>
                        @endforelse
                    </x-table>
                </div>
    ...
    @else
        <x-cards.no-record icon="file-invoice-dollar" :message="__('team chat.noUnpaidInvoiceFound')" />
    @endif
