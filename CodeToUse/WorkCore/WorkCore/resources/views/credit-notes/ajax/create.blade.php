        ...
        <div class="row px-lg-4 px-md-4 px-3 pt-3">
            ...
            <div class="col-md-3">
                <div class="form-group c-inv-select mb-4">
                    ...
                    <p>
                        {{ $creditNote->customer->name_salutation }}
                    </p>
                </div>
            </div>
            <!-- CLIENT END -->
                                        ...
                                        <div class="select-others height-35 rounded border-0">
                                            <select id="multiselect"
                                                ...
                                                @foreach ($taxes as $tax)
                                                    <option data-rate="{{ $tax->rate_percent }}" data-tax-text="{{ $tax->tax_name .':'. $tax->rate_percent }}%" data-include-in-total="{{ $tax->to_keep_under_total_payment ? '1' : '0' }}" @if (isset($item->taxes) && array_search($tax->id, json_decode($item->taxes)) !== false)
                                                    selected @endif
                                                    value="{{ $tax->id }}">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td colspan="2" class="p-0 border-0">
                                    <table width="100%" id="invoice-taxes" data-total-payment-label="{{ __('modules.invoices.totalPayment') }}" data-net-payable-label="{{ __('modules.invoices.netPayable') }}">
                                        <tr>
                            ...
                            <tr>
                                ...
                                <td colspan="2" class="p-0 border-0">
                                    <table width="100%" id="credit-note-adjustment">
                                        <tr>
                                            <td colspan="2">
                                                <input type="number"
...
<script>
    // Total Payment (included taxes) and Net Payable (after deducted taxes) - credit note version (same as invoices)
    function calculateTotalAmount() {
        if (!$("#saveInvoiceForm").length || !$("#invoice-taxes").length) return;

        var totalPaymentLabel = $("#invoice-taxes").data("total-payment-label") || "@lang('modules.invoices.totalPayment')";
        var netPayableLabel = $("#invoice-taxes").data("net-payable-label") || "@lang('modules.invoices.netPayable')";
        ...
        var calculateTax = $("#calculate_tax").length ? $("#calculate_tax").val() : "after_discount";
        ...
        if (Object.keys(taxListDeducted).length > 0) {
            taxHtml += '<tr class="font-weight-semibold"><td class="text-dark-grey">' + totalPaymentLabel + '</td><td><span class="total-payment-val">' + totalPaymentAmount.toFixed(2) + '</span></td></tr>';
        }
        $.each(taxListDeducted, function (key, value) {
            if (!isNaN(value)) {
                taxHtml += '<tr><td class="text-dark-grey">' + key + ' (Deducted)</td><td><span class="tax-percent">-' + Math.abs(decimalupto2(value)).toFixed(2) + '</span></td></tr>';
            }
        });
        if (Object.keys(taxListDeducted).length > 0) {
            taxHtml += '<tr class="font-weight-bold"><td class="text-dark-grey">' + netPayableLabel + '</td><td><span class="net-payable-val">' + netPayable.toFixed(2) + '</span></td></tr>';
        }
    ...
    function recalculateCreditNoteTotals() {
        calculateTotalAmount();
        // base total used by adjustment handler
        $('#total-field').val($('.total-field').val());
        $('#adjustment_amount').trigger('change');
    }
