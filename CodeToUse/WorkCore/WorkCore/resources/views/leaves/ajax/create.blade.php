            ...
            <div class="add-customer bg-white rounded">
                ...
                <div class="row p-20">
                    ...
                    <div class="col-lg-4 col-md-6">
                        ...
                        @else
                            <x-forms.select fieldId="user_id" :fieldLabel="__('modules.team chat.chooseMember')" fieldName="user_id" search="true"
                                fieldRequired="true">
                                <option value="">--</option>
                                @foreach ($cleaners as $cleaner)
                                    <x-user-option :user="$cleaner" :selected="request()->has('default_assign') &&
                                        request('default_assign') == $cleaner->id" />
                                @endforeach
                            </x-forms.select>
                        @endif
                    </div>
                    ...
                    <div class="col-lg-4 col-md-6">
                        ...
                        <x-forms.input-group>
                            ...
                            @if ($addLeadAgentPermission == 'all' || $addLeadAgentPermission == 'added')
                                <x-slot name="append">
                                    <button type="button"
                                        class="btn btn-outline-secondary border-grey add-enquiry-type2"
                    ...
                    <div class="col-lg-4 col-md-6 d-none multi_date_div">
                        <x-forms.text :fieldLabel="__('team chat.selectMultipleDates')" fieldName="multi_date" fieldId="multi_date" :fieldPlaceholder="__('team chat.selectMultipleDates')"
                            :fieldValue="now(company()->timezone)->translatedFormat(company()->date_format)" />
                    </div>
                    ...
                    <div class="col-lg-12">
                        <x-forms.file-multiple class="mr-0 mr-lg-2 mr-md-2" :fieldLabel="__('app.menu.addFile')" fieldName="file" :popover="__('team chat.leaveFileMessage')" fieldId="leave-file-upload-dropzone" />
...
<script>
    $(document).ready(function() {
        ...
        // Function to get joining date for selected cleaner
        function getJoiningDate(employeeID) {
            ...
            var cleaner = cleaners.filter(function(item) {
                return item.id == employeeID;
            });

            if (cleaner.length > 0 && cleaner[0] !== undefined && cleaner[0].employee_detail && cleaner[0].employee_detail.joining_date) {
                // Parse the joining date - it comes as a string from JSON
                var joiningDateStr = cleaner[0].employee_detail.joining_date;
        ...
        // Function to set minimum date based on cleaner joining date
        ...
        // Set min date on page load if cleaner is pre-selected
        if (initialEmployeeId) {
            setMinDate(initialEmployeeId);
        }

        // Update min date when cleaner changes
        ...
        $('#save-leave-form').click(function() {
            ...
            function sendAjaxRequest(){
                $.easyAjax({
                    ...
                    data: $('#save-enquiry-data-form').serialize()+'&multiStartDate='+startDate + '&multiEndDate='+endDate + '&markLeave='+markleave,
                    success: function(response) {
                        ...
                        if (response.status == 'attendanceMarked') {
                            ...
                            Swal.fire({
                                ...
                                text: "@lang('team chat.attendanceIsMarked')",
                                icon: 'warning',
        ...
        $('body').on('click', '.add-enquiry-type2', function() {
        ...
        $('#user_id').change(function() {
            ...
            var url = "{{ route('cleaner-leaves.employee_leave_types', ':id') }}";
