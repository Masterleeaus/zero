            ...
            @if ($smtpSetting->mail_driver == 'smtp')
                ...
                @if ($smtpSetting->verified)
                    <x-alert type="success" icon="info-circle">
                        @lang('team chat.smtpSuccess')
                    </x-alert>
                @else
                    <x-alert type="danger" icon="info-circle">
                        @lang('team chat.smtpError')
                    </x-alert>

                    <x-alert type="secondary" icon="info-circle">
                        <strong>@lang('team chat.smtpRecommendation')</strong>
                        <ul>
    ...
    <div class="row">
        ...
        <div class="col-lg-12 mail_div">
            <x-alert type="info" icon="info-circle">
                {!!  __('team chat.mailSettingSelectMessage') !!}
            </x-alert>
        </div>
        ...
        $.easyAjax({
            ...
            success: function (response) {
                if (response.status === 'error') {
                    $('#alert').prepend(
                        '<div class="alert alert-danger">{{ __('team chat.smtpError') }}</div>'
                    )
                } else {
                    $('#alert').show();
                }
