...
@if (user()->is_superadmin)
    <div class="col-xl-4 col-lg-12 col-md-12 ntfcn-tab-content-right border-left-grey p-4">
        ...
        <div class="mb-3 d-flex">
            <x-forms.checkbox :checked="$pusherSettings->taskboard" :fieldLabel="__('modules.service jobs.taskBoard')"
                fieldName="taskboard" fieldId="taskboard_broadcast" fieldValue="1" />
        </div>
        <div class="mb-3 d-flex">
            <x-forms.checkbox :checked="$pusherSettings->team chat" :fieldLabel="__('app.menu.team chat')"
                fieldName="team chat" fieldId="messages_broadcast" fieldValue="1" />
        </div>
    </div>
@endif
    ...
    $('body').on('click', '#save-pusher-form', function() {
        ...
        $.easyAjax({
            ...
            success: function (response) {
                ...
                {
                    $('#alert').prepend(
                        '<div class="alert alert-danger">{{ __('team chat.pusherError') }}</div>'
                    )
                }
                ...
                if(response.status == 1){
                    ...
                    $('#alert').prepend(
                        '<div class="alert alert-success">{{ __('team chat.pusherSuccess') }}</div>'
                    )
                }
                else
                {
