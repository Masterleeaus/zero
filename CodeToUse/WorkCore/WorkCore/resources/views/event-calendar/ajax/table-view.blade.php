        ...
        <div class="d-grid d-lg-flex d-md-flex action-bar">
            <div id="table-actions" class="flex-grow-1 align-items-center">
                @if ($addPermission == 'all' || $addPermission == 'added')
                    <x-forms.link-primary :link="route('events.create', ['type' => 'table-view'])"
            ...
            <div class="btn-group mt-2 mt-lg-0 mt-md-0 ml-0 ml-lg-3 ml-md-3" role="group" aria-label="Basic example">
                <a href="{{ route('events.index') }}" class="btn btn-secondary f-14" data-toggle="tooltip"
                   data-original-title="@lang('app.menu.calendar')"><i class="side-icon bi bi-calendar"></i></a>

                <a href="{{ route('events.table_view') }}" class="btn btn-secondary f-14 btn-active"
                   data-toggle="tooltip"
...
@push('scripts')
    ...
    <script>
        $('#event-table').on('preXhr.dt', function (e, settings, data) {
            var month = $('#month').val();
            var year = $('#year').val();
        ...
        const showTable = () => {
            window.LaravelDataTables["event-table"].draw(true);
        }

        $('#month, #year, #customer, #cleaner, #status').on('change keyup',
            function () {
                if ($('#month').val() != "") {
        ...
        const applyQuickAction = () => {
            var rowdIds = $("#event-table input:checkbox:checked").map(function () {
                return $(this).val();
            }).get();
