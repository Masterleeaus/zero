            ...
            <div class="border-0 card invoice">
                ...
                <div class="card-body">
                    <div class="invoice-table-wrapper">
                                    ...
                                    <table class="mt-3 inv-num-date text-dark f-13">
                                        <tr>
                                            <td class="bg-light-grey border-right-0 f-w-500">
                                                @lang('app.menu.proposal')</td>
                                            <td class="border-left-0">{{ $proposal->proposal_number }}</td>
                                        </tr>
                                        <tr>
                                            <td class="bg-light-grey border-right-0 f-w-500">
                        ...
                        <table width="100%">
                            <tr class="inv-unpaid">
                                <td class="f-14 text-dark">
                                    @if ($proposal->enquiry->contact && ($proposal->enquiry->contact->client_name || $proposal->enquiry->contact->client_email || $proposal->enquiry->contact->mobile || $proposal->enquiry->contact->company_name || $proposal->enquiry->contact->address) && ($invoiceSetting->show_client_name == 'yes' || $invoiceSetting->show_client_email == 'yes' || $invoiceSetting->show_client_phone == 'yes' || $invoiceSetting->show_client_company_name == 'yes' || $invoiceSetting->show_client_company_address == 'yes'))
                                        <p class="mb-0 text-left">
                                            <span class="text-dark-grey ">
                                                @lang("modules.invoices.billedTo")
                                            </span><br>
                                            @if ($proposal->deal && !empty($proposal->deal->name))
                                                {{ $proposal->deal->name }}<br>
                                            @endif
                                            @if ($proposal->enquiry->contact && $proposal->enquiry->contact->client_name && $invoiceSetting->show_client_name == 'yes')
                                                {{ $proposal->enquiry->contact->client_name_salutation }}<br>
                                            @endif
                                            @if ($proposal->enquiry->contact && $proposal->enquiry->contact->client_email && $invoiceSetting->show_client_email == 'yes')
                                                {{ $proposal->enquiry->contact->client_email }}<br>
                                            @endif
                                            @if ($proposal->enquiry->contact && $proposal->enquiry->contact->mobile && $invoiceSetting->show_client_phone == 'yes')
                                                {{ $proposal->enquiry->contact->mobile }}<br>
                                            @endif
                                            @if ($proposal->enquiry->contact && $proposal->enquiry->contact->company_name && $invoiceSetting->show_client_company_name == 'yes')
                                                {{ $proposal->enquiry->contact->company_name }}<br>
                                            @endif
                                            @if ($proposal->enquiry->contact && $proposal->enquiry->contact->address && $invoiceSetting->show_client_company_address == 'yes')
                                                {!! nl2br($proposal->enquiry->contact->address) !!}
                                            @endif
                                        </p>
                                    @endif
                                </td>
                                ...
                                <td align="right" class="mt-4 mt-lg-0 mt-md-0">
                                        <span
                                            class="unpaid {{ $proposal->status == 'waiting' ? 'text-warning border-warning' : '' }} {{ $proposal->status == 'accepted' ? 'text-success border-success' : '' }} rounded f-15 ">@lang('modules.quotes.'.$proposal->status)</span>
                                </td>
                            </tr>
                            <tr>
                                                ...
                                                @if ($item->type == 'item')
                                                    ...
                                                    @if ($item->item_summary || $item->proposalItemImage)
                                                        <tr class="text-dark f-12">
                                                            <td colspan="{{ ($invoiceSetting->hsn_sac_code_show == 1) ? '6' : '5' }}"
                                                                class="border-bottom-0">
                                                                {!! nl2br(strip_tags($item->item_summary)) !!}
                                                                @if ($item->proposalItemImage)
                                                                    <p class="mt-2">
                                                                        <a href="javascript:;" class="img-lightbox"
                                                                           data-image-url="{{ $item->proposalItemImage->file_url }}">
                                                                            <img
                                                                                src="{{ $item->proposalItemImage->file_url }}"
                                                                                width="80" height="80"
                                                                                class="img-thumbnail">
                                                                        </a>
                                                                    </p>
                    ...
                    @if ($proposal->signature)
                        <div class="mt-4 col-sm-12">
                            @if (!is_null($proposal->signature->signature))
                                <h6>@lang('modules.quotes.signature')</h6>
                                <img src="{{ $proposal->signature->signature }}" style="width: 200px;">
                            @else
                                <h6>@lang('modules.quotes.signedBy')</h6>
                            @endif
                ...
                <div
                    ...
                    <div class="d-flex">
                        ...
                        <x-forms.link-secondary :link="route('front.download_proposal', $proposal->hash)" class="mr-3"
    ...
    <div class="modal-dialog d-flex justify-content-center align-items-center modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <x-forms.button-cancel data-dismiss="modal" class="mr-3 border-0">@lang('app.cancel')
...
<script>
    ...
    const RIGHT_MODAL = '#service job-detail-1';
...
<script>
    ...
    $('#toggle-pad-uploader').click(function () {
        const text = $('.signature').hasClass('d-none') ? '{{ __("modules.quotes.uploadSignature") }}' : '{{ __("app.sign") }}';
