...
@if (in_array('team chat', user_modules()))
    <script>
        ...
        function checkNewMessage() {
            var url = "{{ route('team chat.check_new_message') }}";
    ...
    @if(!user()->is_superadmin)
        // if (message_setting.send_sound_notification == 1 && !(pusher_setting.status === 1 && pusher_setting.team chat === 1)) {
        //     window.setInterval(function () {
        //         checkNewMessage()
        //     }, 10000); // Check team chat every 10 seconds
        // }
    @endif
