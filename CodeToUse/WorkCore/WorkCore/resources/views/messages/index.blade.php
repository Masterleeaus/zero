...
@push('styles')
    <style>
        ...
        div#msgContentRight .chat-box .tab-content {
            overflow-y: scroll !important;
            overflow-x: hidden !important;
        }
        
        /* Apply padding only on tablet and mobile devices */
                            ...
                            <form class="mb-0">
                                <div class="input-group rounded py-1">
                                    ...
                                    <input type="text" id="user-search" class="form-control border-0 f-14"
                                           placeholder="@lang('modules.team chat.searchContact')">
                                </div>
                            </form>
                        </div>
            ...
            <div class="msg-content-right" id="msgContentRight">
                <div class="msg-header d-none d-lg-flex align-items-center">
                    <div class="msg-header-right w-100 d-flex justify-content-between align-items-center">
                        ...
                        <x-forms.button-primary id="new-chat" icon="plus">@lang('modules.team chat.startConversation')
                        </x-forms.button-primary>
                    </div>
                </div>
                ...
                <div class="chat-box">
                    ...
                    <div data-menu-vertical="1" data-menu-scroll="1" data-menu-dropdown-timeout="500" id="chatBox"
                        ...
                        <div id="tab1" class="tabcontent" style="display: block;">
                            <x-cards.no-record icon="comment-alt" :message="__('team chat.selectConversation')"/>
                        </div><!-- TAB END -->
                    ...
                    <div class="row">
                        <div class="w-100 col-md-12">
                            <div class="form-group">
                             ...
                             <div id="submitTexts" class="form-control rounded-0 f-14 p-3 border-left-0 border-right-0 border-bottom-0" data-text="@lang('team chat.enterText')"></div>
...
@push('scripts')
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('body').on('click', '.delete-all-message', function () {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        var url = "{{ route('team chat.destroy_all', ':id') }}";
        ...
        history.replaceState(null, null, "{{route('team chat.index')}}");
        ...
        $('#new-chat, #new-chat-mbl').click(function () {
            const url = "{{ route('team chat.create') }}";
            $(MODAL_LG + ' ' + MODAL_HEADING).html('...');
            $.ajaxModal(MODAL_LG, url);
        });

        $('#user-search').keyup(function () {
            var url = "{{ route('team chat.index') }}";
            var term = $(this).val();
        ...
        $('body').on('click', '.show-user-team chat', function () {
            var id = $(this).data('user-id');
            ...
            var url = "{{ route('team chat.show', ':id') }}";
            url = url.replace(':id', id);
        ...
        function scrollChat(params) {
            // Add extra scroll offset only on mobile and tablet devices
            var extraOffset = window.innerWidth <= 991 ? 50 : 0;
            $('#chatBox').stop().animate({
                scrollTop: $("#chatBox")[0].scrollHeight + extraOffset
            }, 800);
        }
        ...
        $('body').on('click', '.delete-message', function () {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    var url = "{{ route('team chat.destroy', ':id') }}";
                    url = url.replace(':id', id);
        ...
        function fetchUserList() {
            var url = "{{ route('team chat.fetch_user_list') }}";
        ...
        function fetchUserMessages(scrollChatBox = true) {
            ...
            var url = "{{ route('team chat.fetch_messages', ':id') }}";
        ...
        if ((pusher_setting.status === 1 && pusher_setting.team chat === 1) || (pusher_setting.status == "1" && pusher_setting.team chat == "1")) {
            var channel = pusher.subscribe('team chat-channel');
            channel.bind('team chat.received', function (data) {
                fetchUserMessages()
            ...
            }, 10000); // Fetch team chat every 10 seconds
        }
