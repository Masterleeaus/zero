                    ...
                    <div class="card-body">
                                ...
                                <div class="dropdown-menu dropdown-menu-right">
                                    ...
                                    || ($viewDocumentPermission == 'both' && ($document->added_by == user()->id || $document->user_id == user()->id)))
                                        @if($document->hashname)
                                            ...
                                            <a class="dropdown-item" href="{{ route('cleaner-document-expiries.download', md5($document->id)) }}">
                        ...
                        @if($document->document_number)
                            <p class="text-muted small mb-2">
                                <strong>@lang('modules.cleaners.documentNumber'):</strong> {{ $document->document_number }}
                            </p>
                        @endif

                        <div class="row mb-3">
                            <div class="col-6">
                                <small class="text-muted d-block">@lang('modules.cleaners.issueDate')</small>
                                <strong>{{ $document->issue_date->translatedFormat(company()->date_format) }}</strong>
                            </div>
                            <div class="col-6">
                                <small class="text-muted d-block">@lang('modules.cleaners.expiryDate')</small>
                        ...
                        <div class="row mb-3">
                            <div class="col-6">
                                <small class="text-muted d-block">@lang('modules.cleaners.alertBefore')</small>
                                <strong>{{ $document->alert_before_days }} @lang('modules.cleaners.days')</strong>
                            </div>
                            <div class="col-6">
                                <span class="badge badge-{{ $document->alert_enabled == 1 ? 'success' : 'secondary' }}">
                                    @lang('modules.cleaners.alert') {{ $document->alert_enabled ? __('app.on') : __('app.off') }}
                                </span>
                            </div>
                        </div>
                        ...
                        @if($document->is_expired)
                            <div class="alert alert-danger py-2 mb-2">
                                ...
                                @lang('modules.cleaners.documentExpired')
                            </div>
                        @elseif($document->is_expiring_soon)
                            <div class="alert alert-warning py-2 mb-2">
                                <i class="fa fa-clock mr-1"></i>
                                @lang('modules.cleaners.documentExpiringSoon', ['days' => $document->days_until_expiry])
                            </div>
                        @endif

                        <div class="text-muted small">
                            @lang('modules.cleaners.uploaded') {{ $document->created_at->diffForHumans() }}
                        </div>
                    </div>
                </div>
            </div>
    ...
    @empty
        <div class="col-12">
            <div class="align-items-center d-flex flex-column text-lightest p-20 w-100">
                ...
                <div class="f-15 mt-4">
                    - @lang('modules.cleaners.noDocumentExpiryUploaded')
                </div>
            </div>
        </div>
    @endforelse
