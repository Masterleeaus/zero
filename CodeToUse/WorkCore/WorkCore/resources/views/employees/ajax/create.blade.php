...
<div class="row">
    <div class="col-sm-12">
        <x-form id="save-cleaner-data-form">
            ...
            <div class="bg-white rounded add-customer">
                ...
                <div class="p-20 row">
                        ...
                        <div class="row">
                            <div class="col-md-3">
                                <x-forms.text fieldId="employee_id" :fieldLabel="__('modules.cleaners.employeeId')"
                                    ...
                                    :fieldPlaceholder="__('modules.cleaners.employeeIdInfo')" :popover="__('modules.cleaners.employeeIdHelp')">
                                </x-forms.text>
                            </div>
                            ...
                            <div class="col-lg-4 col-md-6">
                                ...
                                <x-forms.input-group>
                                    ...
                                    @if ($addDesignationPermission == 'all')
                                        <x-slot name="append">
                                            <button id="role-setting-add" type="button"
                            ...
                            <div class="col-lg-4 col-md-6">
                                ...
                                <x-forms.input-group>
                                    ...
                                    @if ($addDepartmentPermission == 'all')
                                        <x-slot name="append">
                                            <button id="zone-add" type="button"
                                                class="btn btn-outline-secondary border-grey">@lang('app.add')</button>
                                        </x-slot>
                                    @endif
                                </x-forms.input-group>
                    ...
                    <div class="col-lg-3">
                        <x-forms.file allowedFileExtensions="png jpg jpeg svg bmp" class="mr-0 mr-lg-2 mr-md-2 cropper"
                            ...
                            fieldHeight="119" :popover="__('team chat.fileFormat.ImageFile')" />
                    </div>
                    <div class="col-lg-3 col-md-6">
                        <x-forms.select fieldId="country" :fieldLabel="__('app.country')" fieldName="country"
                            ...
                            @foreach ($countries as $item)
                                <option data-tokens="{{ $item->iso3 }}" data-phonecode = "{{$item->phonecode}}" data-iso="{{ $item->iso }}"
                                    ...
                                    value="{{ $item->id }}" @selected($item->id == user()->country_id)>{{ $item->nicename }}</option>
                            @endforeach
                        </x-forms.select>
                    </div>
                    <div class="col-lg-3 col-md-6">
                    ...
                    <div class="col-lg-3 col-md-6">
                        <x-forms.select fieldId="role" :fieldLabel="__('app.role')" fieldName="role">
                            @foreach ($roles as $role)
                                <option {{ ($role->name == 'cleaner') ? 'selected' : '' }} value="{{ $role->id }}">{{ $role->display_name }}</option>
                            @endforeach
                        </x-forms.select>
                    </div>
                    <div class="col-md-12">
                ...
                <x-form-actions>
                    <x-forms.button-primary id="save-cleaner-form" class="mr-3" icon="check">
                        @lang('app.save')
                    </x-forms.button-primary>
                    <x-forms.button-secondary class="mr-3" id="save-more-cleaner-form" icon="check-double">@lang('app.saveAddMore')
...
<script>
    $(document).ready(function() {
        ...
        $('#save-more-cleaner-form').click(function() {

            $('#add_more').val(true);

            const url = "{{ route('cleaners.store') }}";
            var data = $('#save-cleaner-data-form').serialize();
            saveEmployee(data, url, "#save-more-cleaner-form");


        });

        $('#save-cleaner-form').click(function() {

            const url = "{{ route('cleaners.store') }}";
            var data = $('#save-cleaner-data-form').serialize();
            saveEmployee(data, url, "#save-cleaner-form");
        ...
        function saveEmployee(data, url, buttonSelector) {
            $.easyAjax({
                url: url,
                container: '#save-cleaner-data-form',
                type: "POST",
                disableButton: true,
                blockUI: true,
        ...
        $('#role-setting-add').click(function() {
