...
<div class="tab-pane fade show active mt-5" role="tabpanel" aria-labelledby="nav-email-tab">
    <x-cards.data :title="__('app.menu.documents')">
        ...
        @if ($addDocumentPermission == 'all' || $addDocumentPermission == 'added')
            ...
            <div class="row">
                <div class="col-md-12">
                    <a class="f-15 f-w-500" href="javascript:;" id="add-service job-file"><i
                            class="icons icon-plus font-weight-bold mr-1"></i>@lang('modules.enquiry.addFile')
                        </a>
                </div>
            </div>

            <x-form id="save-taskfile-data-form" class="d-none">
                <input type="hidden" name="user_id" id="doc_user_id" value="{{ $cleaner->id }}">
                <div class="row">
                    <div class="col-md-12">
                    ...
                    <div class="col-md-12">
                        <x-forms.file :fieldLabel="__('modules.sites.uploadFile')" fieldName="file"
                            ...
                            :popover="__('team chat.fileFormat.multipleImageFile')" />
                    </div>
                    <div class="col-md-12">
        ...
        <div class="d-flex flex-wrap mt-3" id="service job-file-list">
            @php
            ...
            @endphp
            @forelse($cleaner->documents as $file)
                                ...
                                <div class="dropdown-menu dropdown-menu-right border-grey rounded b-shadow-4 p-0"
                                    ...
                                    || ($viewDocumentPermission == 'both' && ($file->added_by == user()->id || $file->user_id == user()->id)))
                                        ...
                                        <a class="cursor-pointer d-block text-dark-grey f-13 py-3 px-3 "
                                            href="{{ route('cleaner-docs.download', md5($file->id)) }}">@lang('app.download')</a>
                                    @endif
    ...
    <!-- Cleaner Document Expiry Section -->
    <x-cards.data :title="__('modules.cleaners.employeeDocumentExpiry')" class="mt-4">
        @if ($addDocumentPermission == 'all' || $addDocumentPermission == 'added')
            <div class="row">
                <div class="col-md-12">
                    <a class="f-15 f-w-500" href="javascript:;" id="add-document-expiry">
                        <i class="icons icon-plus font-weight-bold mr-1"></i>@lang('modules.cleaners.addDocumentExpiry')
                    </a>
                </div>
            </div>
        ...
        <div id="document-expiry-list-container">
            @php
                ...
                // Try to get the cleaner from the controller's data
                if (isset($cleaner) && $cleaner->relationLoaded('documentExpiries')) {
                    $documentExpiries = $cleaner->documentExpiries;
            ...
            @endphp
            @include('cleaners.ajax.document-expiry.show', ['documents' => $documentExpiries])
        </div>
    </x-cards.data>
</div>
<!-- TAB CONTENT END -->
...
<script>
    $('#add-service job-file').click(function() {
    ...
    $('body').on('click', '.edit-file', function() {
        ...
        var url = "{{ route('cleaner-docs.edit', ':id') }}";
    ...
    $('#cancel-document').click(function() {
        ...
        $('#add-service job-file').closest('.row').removeClass('d-none');
    });

    $('#submit-document').click(function() {
        var url = "{{ route('cleaner-docs.store') }}";
    ...
    $('body').off('click', '#add-document-expiry').on('click', '#add-document-expiry', function() {
        var url = "{{ route('cleaner-document-expiries.create') }}?user_id=:id";
    ...
    $('body').off('click', '#submit-document-expiry').on('click', '#submit-document-expiry', function(e) {
        ...
        var url = "{{ route('cleaner-document-expiries.store') }}";
        var userId = $('#doc_user_id').val();
    ...
    $('body').off('click', '#update-document-expiry').on('click', '#update-document-expiry', function(e) {
        ...
        var url = "{{ route('cleaner-document-expiries.update', ':id') }}";
    ...
    $('body').on('click', '.edit-document-expiry', function() {
        ...
        docId = documentId;
        var url = "{{ route('cleaner-document-expiries.edit', ':id') }}";
        url = url.replace(':id', documentId);
    ...
    $('body').on('click', '.delete-document-expiry', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                var url = "{{ route('cleaner-document-expiries.destroy', ':id') }}";
                url = url.replace(':id', documentId);
    ...
    $('body').on('click', '.delete-file', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                var url = "{{ route('cleaner-docs.destroy', ':id') }}";
