...
<x-alert type="warning" @class(['mt-4', 'd-none' => !($cleaner->customised_permissions)])>
    <div class="d-flex justify-content-between">
        <div class="pt-2">
            <i class="fa fa-exclamation-triangle"></i> @lang('team chat.customPermissionError')
        </div>
        <x-forms.button-secondary id="reset-user-permissions" icon="sync">@lang('app.resetPermissions')</x-forms.button-secondary>
    </div>
</x-alert>

@if ($cleaner->hasRole('admin'))
    <x-alert type="danger" class="mt-5" icon="exclamation-triangle">
        @lang('team chat.adminPermissionError')
    </x-alert>
@else
        ...
        @foreach ($modulesData as $moduleData)
            ...
            <tr>
                ...
                <td>@lang('modules.module.'.$moduleData->module_name)
                    @if($notPermited)
                        <i class="fa fa-info-circle" data-toggle="popover" data-placement="top" data-content="@lang('team chat.moduleDisabled')" data-html="true" data-trigger="hover"></i>
                    @endif
                </td>
                @foreach ($moduleData->permissions as $permission)
                    @php
                        $permissionType = $cleaner->customised_permissions === 1 ? $cleaner->permissionTypeId($permission->name) : $role->permissionType($permission->id);
                        ...
                        <select class="role-permission-select border-0" data-permission-id="{{ $permission->id }}" {{ $notPermited }}>
                            @if (!is_null($allowedPermissions))
                                @foreach ($allowedPermissions as $key => $item)
                                    @php
                                        $type = $cleaner->customised_permissions === 1 ? $key : $item;
                                    @endphp
    ...
    <script>
        $('body').on('click', '.show-custom-permission', function() {
            ...
            var url = "{{ route('user-permissions.custom_permissions', $cleaner->id) }}";
        ...
        $('body').on('change', '.role-permission-select', function() {
            ...
            var url = "{{ route('user-permissions.update', $cleaner->id) }}";
        ...
        $('body').on('click', '#reset-user-permissions', function() {
            var url = "{{ route('user-permissions.reset_permissions', $cleaner->id) }}";
