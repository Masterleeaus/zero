    ...
    <div class="col-xl-12 col-lg-12 col-md-12 mb-4 mb-xl-0 mb-lg-4 mb-md-0 mt-5">
        @if(is_null($passport))
            @if ($addImmigrationPermission == 'all'
                || ($addImmigrationPermission == 'owned' && ($cleaner->id == user()->id))
                )
            ...
            @endif
        @endif
        <x-cards.data :title="__('modules.cleaners.passportDetails')">
            ...
            @if($passport)
                ...
                <x-cards.data-row :label="__('modules.cleaners.passportNumber')" :value="$passport->passport_number ?? '--'" />
                ...
                <x-cards.data-row :label="__('modules.cleaners.issueDate')" :value=" $passport->issue_date ? $passport->issue_date->format(company()->date_format) : '--'" />
                <x-cards.data-row :label="__('modules.cleaners.expiryDate')" :value=" $passport->expiry_date  ? $passport->expiry_date->format(company()->date_format) : '--'" />
                <x-cards.data-row :label="__('modules.cleaners.alertBeforeMonths')" :value="$passport->alert_before_months > 0 ? $passport->alert_before_months . ' ' . ($passport->alert_before_months == 1 ? __('app.month') : __('app.monthPlural')) : __('app.noAlert')" />
                <div class="col-12 px-0 pb-3 d-block d-lg-flex d-md-flex">
                    <p class="mb-0 text-lightest f-14 w-30 d-inline-block ">
                        @lang('modules.cleaners.scanCopy')</p>
                    <p class="mb-0 text-dark-grey f-14 w-70">
                        @if($passport->file)
...
<div class="row">
    ...
    <div class="col-xl-12 col-lg-12 col-md-12 mb-4 mb-xl-0 mb-lg-4 mb-md-0 mt-5">
        ...
        @endif
            ...
            <x-cards.data :title="__('modules.cleaners.visaDetails')"
                otherClasses="border-0 p-0 d-flex justify-content-between align-items-center table-responsive-sm">
                <x-table class="border-0 pb-3 admin-dash-table table-hover">

                    <x-slot name="thead">
                        <th class="pl-20">#</th>
                        <th>@lang('modules.cleaners.visaNumber')</th>
                        <th>@lang('app.country')</th>
                        <th>@lang('modules.cleaners.issueDate')</th>
                        <th>@lang('modules.cleaners.expiryDate')</th>
                        <th>@lang('modules.cleaners.alertBeforeMonths')</th>
                    ...
                    @forelse($visa as $key => $visaValue)
                        <tr id="row-{{ $visaValue->id }}">
                            ...
                            <td>
                                {{ $visaValue->alert_before_months > 0 ? $visaValue->alert_before_months . ' ' . ($visaValue->alert_before_months == 1 ? __('app.month') : __('app.monthPlural')) : __('app.noAlert') }}
                            </td>
                                    ...
                                    <div class="dropdown">
                                        ...
                                        <div class="dropdown-menu dropdown-menu-right">
                                            ...
                                            || ($viewImmigrationPermission == 'both' && ($visaValue->added_by == user()->id || $visaValue->user_id == user()->id)))
                                                <a class="dropdown-item openRightModal" href="{{ route('cleaner-visa.show', $visaValue->id ) }}"
...
<script>
    ...
    $('body').on('click', '.delete-visa', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                var url = "{{ route('cleaner-visa.destroy', ':id') }}";
    ...
    $('#add-visa').click(function() {
        var url = "{{ route('cleaner-visa.create').'?empid='.$cleaner->id }}";
    ...
    $('.edit-visa').click(function() {
        var id = $(this).data('id');
        var url = "{{ route('cleaner-visa.edit', ':id') }}";
        url = url.replace(':id', id);
        $(MODAL_LG + ' ' + MODAL_HEADING).html('...');
        $.ajaxModal(MODAL_LG, url);
    });
    ...
    $('.add-passport').click(function(){
        var url = "{{ route('passport.create').'?empid='.$cleaner->id }}";
            $(MODAL_LG + ' ' + MODAL_HEADING).html('...');
            $.ajaxModal(MODAL_LG, url);
        });
    ...
    $('.edit-passport').click(function(){
        var id = $(this).data('id');
        var url = "{{ route('passport.edit', ':id').'?empid='.$cleaner->id }}";
        url = url.replace(':id', id);
            $(MODAL_LG + ' ' + MODAL_HEADING).html('...');
            $.ajaxModal(MODAL_LG, url);
    });
