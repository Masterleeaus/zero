        ...
        <div class="invoice-table-wrapper">
            ...
            <table width="100%">
                @if ($order->site)
                    <tr>
                        <td class="f-14 text-dark">
                            @lang('modules.invoices.site'):
                            <p class="mb-3">
                                @if ($order->site)
                                    {{$order->site->project_name}}
                                @endif
                            </p>
                        </td>
                    </tr>
                ...
                <tr class="inv-unpaid">
                    ...
                    <td class="f-14 text-dark">
                        ...
                        <p class="mt-3 mb-0">
                        @if ($order->customer->name && $invoiceSetting->show_client_name == 'yes')
                            {{ $order->customer->name_salutation }}<br>
                        @endif

                        @if ($order->customer->email && $invoiceSetting->show_client_email == 'yes')
                            {{ $order->customer->email }}<br>
                        @endif

                        @if ($order->customer->mobile && $invoiceSetting->show_client_phone == 'yes')
                            {{ $order->customer->mobile_with_phonecode }}<br>
                        @endif

                        @if ($order->customer->clientDetails->company_name && $invoiceSetting->show_client_company_name == 'yes')
                            {{ $order->customer->clientDetails->company_name }}<br>
                        @endif

                        @if ($order->customer->clientDetails->address && $invoiceSetting->show_client_company_address == 'yes')
                            {!! nl2br($order->customer->clientDetails->address) !!}
                        @endif
                        </p>
                    </td>
                    ...
                    <td colspan="2">
                        <table class="inv-detail f-14 table-responsive-sm" width="100%">
                            ...
                            @foreach ($order->items->sortBy('field_order') as $item)
                                ...
                                @endif
                                @if ($item->has('service / extra') && $item->service / extra && $item->service / extra->downloadable && $item->service / extra->download_file_url && $order->status == 'completed')
                                    <tr class="text-dark">
                                        <td colspan="{{ $invoiceSetting->hsn_sac_code_show ? '7' : '6' }}" class="border-bottom-0">
                                            <p class="mt-2">
                                                <x-forms.link-secondary icon="download" :link="$item->service / extra->download_file_url" class="mr-3" download="{{ $item->service / extra->name }}">@lang('app.download')</x-forms.link-secondary>
                                            </p>
                                        </td>
                                    </tr>
        ...
        <div class="d-flex">
                ...
                <div class="inv-action mr-3 mr-lg-3 mr-md-3 dropup">
                    ...
                    <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton" tabindex="0">
                        ...
                        @if(
                            (in_array('admin', user_roles()) || in_array('cleaner', user_roles()))
                        )
                        ...
                        @if (!in_array($order->status, ['completed', 'canceled', 'refunded']) && ($editOrderPermission == 'all'|| ($editOrderPermission == 'both' && ($order->added_by == user()->id || $order->client_id == user()->id)) || ($editOrderPermission == 'added' && $order->added_by == user()->id) || ($editOrderPermission == 'owned' && $order->client_id == user()->id)) && (!is_null($order->site) && is_null($order->site->deleted_at)) )
                            <li>
...
<script>
    ...
    $('body').on('click', '.orderStatus', function() {
        ...
        switch (status) {
            case 'pending':
                statusMessage = "@lang('team chat.orderStatus.pending')";
                break;
            case 'on-hold':
                statusMessage = "@lang('team chat.orderStatus.onHold')";
                break;
            case 'failed':
                statusMessage = "@lang('team chat.orderStatus.failed')";
                break;
            case 'processing':
                statusMessage = "@lang('team chat.orderStatus.processing')";
                break;
            case 'completed':
                statusMessage = "@lang('team chat.orderStatus.completed')";
                break;
            case 'canceled':
                statusMessage = "@lang('team chat.orderStatus.canceled')";
                break;
            case 'refunded':
                statusMessage = "@lang('team chat.orderStatus.refunded')";
                break;
        ...
        Swal.fire({
            title: "@lang('team chat.confirmation.orderStatusChange')",
            text: statusMessage,
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                if (status === 'completed' && Object.keys(bankAccounts).length > 0) {
                    Swal.fire({
                        title: "@lang('team chat.selectBank')",
                        input: 'select',
                        inputOptions: bankAccounts,
                        inputPlaceholder: "@lang('team chat.selectBank')",
                        showCancelButton: true,
