...
<div class="col-lg-12 col-md-12 ntfcn-tab-content-left w-100 p-4">
    ...
    <div class="table-responsive">
        <x-table class="table-bordered">
            ...
            @forelse($archiveleaveTypes as $key=>$leaveType)
                <tr id="archivetype-{{ $leaveType->id }}">
                    <td>
                    ...
                    <td>
                        <ol class="pl-3">
                            @foreach ($zones as $zone)
                                @if(!is_null($leaveType->zone) && in_array($zone->id, json_decode($leaveType->zone)))
                                    <li>{{$zone->team_name}}</li>
                                @endif
                            @endforeach
                        </ol>
                    </td>
                    <td>
                        <ol class="pl-3">
                            @foreach ($roles as $role)
                                @if(!is_null($leaveType->role) && in_array($role->id, json_decode($leaveType->role)))
                                    <li>{{$role->name}}</li>
                                @endif
                            @endforeach
                        </ol>
            ...
            @empty
                <tr>
                    <td colspan="8">
                        <x-cards.no-record icon="list" :message="__('team chat.noLeaveTypeAdded')" />
                    </td>
                </tr>
            @endforelse
        </x-table>
...
<script>
    $('body').on('click', '.delete-category', function() {
        ...
        var force_delete = $(this).data('leave-restore');

        Swal.fire({
            title: "@lang('team chat.sweetAlertTitle')",
            text: "@lang('team chat.deleteLeaveType')",
            icon: 'warning',
    ...
    $('body').on('click', '.restore-category', function() {
        ...
        var id = $(this).data('leave-id');
        var restore = $(this).data('leave-restore');

        Swal.fire({
            title: "@lang('team chat.sweetAlertTitle')",
            text: "@lang('team chat.unArchiveMessageLeave')",
            icon: 'warning',
            showCancelButton: true,
            focusConfirm: false,
