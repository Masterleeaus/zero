...
@if (!in_array('customer', user_roles()))
    @if (!is_null($invoice->last_viewed))
        <x-alert type="info">
            {{ $invoice->customer->name_salutation }} @lang('app.viewedOn')
...
<div class="card border-0 invoice">
            ...
            <table width="100%">
                <tr class="inv-unpaid">
                    ...
                    <td align="right" class="mt-2 mt-lg-0 mt-md-0">
                        ...
                        @endif
                        <span
                            class="unpaid {{ $invoice->status == 'draft' ? 'text-primary border-primary' : '' }} {{ $invoice->status == 'accepted' ? 'text-success border-success' : '' }} rounded f-15 ">@lang('modules.quotes.' . $invoice->status)</span>
                    </td>
                </tr>
                <tr>
    ...
    <div class="card-footer bg-white border-0 d-flex justify-content-start py-0 py-lg-4 py-md-4 mb-4 mb-lg-3 mb-md-3 ">
        ...
        <div class="d-flex">
            <div class="inv-action dropup">
                ...
                <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton" tabindex="0">
                    @if ($invoice->status == 'waiting' && $invoice->client_id == user()->id)
                        ...
                        <li>
                            <a class="dropdown-item f-14 text-dark" id="decline-quote" href="javascript:;">
                    ...
                    @if ($invoice->status == 'waiting' || $invoice->status == 'draft')
                        @if (
                            ...
                            <li>
                                <a class="dropdown-item openRightModal"
                                    href="{{ route('quotes.edit', [$invoice->id]) }}">
                        ...
                        @if($invoice->send_status)
                            <li>
                                <a class="dropdown-item btn-copy"
                                    data-clipboard-text="{{ url()->temporarySignedRoute('front.quote.show', now()->addDays(\App\Models\GlobalSetting::SIGNED_ROUTE_EXPIRY), $invoice->hash) }}">
                                    <i class="fa fa-copy mr-2"></i> @lang('modules.quotes.copyLink')</a>
                            </li>
                        @endif
                        @if ($invoice->status != 'canceled' && $invoice->status != 'accepted' && !in_array('customer', user_roles()))
                            <li>
                                <a href="javascript:;" data-toggle="tooltip" data-quote-id="{{ $invoice->id }}"
                        ...
                        <li>
                            ...
                            <a class="dropdown-item f-14 text-dark"
                                href="{{ route('quotes.download', [$invoice->id]) }}">
                            ...
                            @if ($addInvoicePermission == 'all' || $addInvoicePermission == 'added')
                                <li>
                                    <a class="dropdown-item"
                                        href="{{ route('invoices.create') . '?quote=' . $invoice->id }}">
                    ...
                    @if ($addEstimatePermission == 'all' || $addEstimatePermission == 'added')
                        <li>
                            <a href="{{ route('quotes.create') . '?quote=' . $invoice->id }}"
            ...
            <x-forms.button-cancel :link="route('quotes.index')" class="border-0 ml-3">@lang('app.cancel')
...
@push('scripts')
    ...
    <script>
        ...
        $('#decline-quote').click(function() {
            $.easyAjax({
                type: 'POST',
                url: "{{ route('quotes.decline', $invoice->id) }}",
                blockUI: true,
                data: {
                    _token: '{{ csrf_token() }}'
                },
        ...
        $('#toggle-pad-uploader').click(function() {
            var text = $('.signature').hasClass('d-none') ? '{{ __('modules.quotes.uploadSignature') }}' :
                '{{ __('app.sign') }}';
        ...
        $('#save-signature').click(function() {
            ...
            $.easyAjax({
                url: "{{ route('quotes.accept', $invoice->id) }}",
