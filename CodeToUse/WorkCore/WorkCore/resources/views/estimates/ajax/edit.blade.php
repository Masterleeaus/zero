...
@if ((!in_array('customer', user_roles()) && !in_array('customers', user_modules())) || (in_array('customer', user_roles()) && !in_array('quotes', user_modules())))
    <x-alert class="mb-3" type="danger" icon="exclamation-circle"><span>@lang('team chat.enableClientModule')</span>
        <x-forms.link-secondary icon="arrow-left" :link="route('quotes.index')">@lang('app.back')</x-forms.link-secondary>
    </x-alert>
@else
...
<div class="bg-white rounded b-shadow-4 create-inv">
    ...
    <x-form class="c-inv-form" id="saveInvoiceForm">
        ...
        <div class="row px-lg-4 px-md-4 px-3 py-3">
            ...
            <div class="col-md-6 col-lg-4">
                <div class="form-group mb-lg-0 mb-md-0 mb-4">
                    <label class="f-14 text-dark-grey mb-12 "
                        for="usr">@lang('modules.quotes.estimatesNumber')</label>
                    <x-forms.input-group>
            ...
            <div class="col-md-6 col-lg-4">
                <div class="form-group mb-lg-0 mb-md-0 mb-4">
                    <x-forms.label fieldId="due_date" :fieldLabel="__('modules.quotes.validTill')"
                        fieldRequired="true">
                    </x-forms.label>
                    <div class="input-group">
                        <input type="text" id="valid_till" name="valid_till"
                            ...
                            value="{{ $quote->valid_till->format(company()->date_format) }}">
                    </div>
                </div>
            </div>
            <!-- INVOICE DATE END -->
                    ...
                    <div class="select-others height-35 rounded">
                        <select class="form-control select-picker" name="currency_id" id="currency_id">
                            @foreach ($currencies as $currency)
                                <option @if ($quote->currency_id == $currency->id) selected @endif value="{{ $currency->id }}">
        ...
        <div class="row px-lg-4 px-md-4 px-3 pt-3">
            ...
            <!-- CLIENT START -->
            <div class="col-md-4">
                <x-forms.label fieldId="client_id" :fieldLabel="__('app.customer')" fieldRequired="true">
                </x-forms.label>
                <div class="form-group c-inv-select mb-4">
                    @if(isset($isClient) && $isClient == true)
                        <div class="input-group">
                            <input type="hidden" name="client_id" id="client_id" value="{{ $quote->client_id }}">
                            <input type="text" value="{{ $quote->customer->name_salutation }}"
                                class="form-control height-35 f-15 readonly-background" readonly>
                        </div>
                    @else
                        <select class="form-control select-picker" data-live-search="true" data-size="8" name="client_id"
                            ...
                            @foreach ($customers as $customer)
                                <x-user-option :user="$customer" :selected="$customer->id == $quote->client_id"/>
                            @endforeach
                        </select>
                    @endif
                </div>
                ...
                <div class="form-group c-inv-select mb-4">
                    <div class="select-others height-35 rounded">
                        <select class="form-control select-picker" data-live-search="true" data-size="8"
                            ...
                            <option value="">--</option>
                            @if($quote?->customer?->sites)
                                @foreach ($quote->customer->sites as $item)
                                    <option @if ($quote->project_id == $item->id) selected @endif value="{{ $item->id }}">
                ...
                <div class="form-group c-inv-select mb-4">
                    ...
                    <div class="select-others height-35 rounded">
                        <select class="form-control select-picker" data-live-search="true" data-size="8"
                            ...
                            <option value="before_discount" @if ($quote->calculate_tax == 'before_discount') selected @endif>
            ...
            <div class="col-md-12 my-3">
                <div class="form-group">
                    ...
                    <div id="description">{!! $quote->description !!}</div>
            ...
            <div class="col-md-4">
                ...
                <div class="form-group c-inv-select mb-4">
                    <select class="form-control select-picker" name="status" id="status">
                        <option @if ($quote->status == 'accepted') selected @endif value="accepted">@lang('modules.quotes.accepted')
                        </option>
                        <option @if ($quote->status == 'waiting') selected @endif value="waiting">@lang('modules.quotes.waiting')
                        </option>
                        <option @if ($quote->status == 'declined') selected @endif value="declined">@lang('modules.quotes.declined')
                        </option>
                        @if ($quote->status == 'draft')
                            <option @if ($quote->status == 'draft') selected @endif value="draft">@lang('modules.invoices.draft')
                            </option>
                        @endif
                    </select>
                </div>
        ...
        <!-- INVOICE NUMBER, DATE, DUE DATE, FREQUENCY END -->
            <x-forms.custom-field :fields="$fields" :model="$quote"></x-forms.custom-field>
        ...
        <div class="row px-lg-4 px-md-4 px-3 py-3">
            ...
            @if(in_array('services / extras', user_modules()) || in_array('purchase', user_modules()))
                <div class="col-md-3">
                    <div class="form-group c-inv-select mb-4">
                    <x-forms.input-group>
                        <select class="form-control select-picker" data-live-search="true" data-size="8" id="add-services / extras" title="{{ __('app.menu.selectProduct') }}">
        ...
        <div id="sortable">
            @foreach ($quote->items->sortBy('field_order') as $key => $item)
                                ...
                                <tbody>
                                    <tr>
                                        ...
                                        <td width="30%" class="border-top-0 sub-total">
                                            {{ number_format((float) $quote->sub_total, 2, '.', '') }}</td>
                                    ...
                                    <tr>
                                                    ...
                                                    <tr>
                                                        <td width="70%" class="c-inv-sub-padding">
                                                            <input type="number" min="0" name="discount_value"
                                                                ...
                                                                placeholder="0" value="{{ $quote->discount }}">
                                                        </td>
                                                        <td width="30%" align="left" class="c-inv-sub-padding">
                                                            <div
                                                                ...
                                                                <select class="form-control select-picker"
                                                                    ...
                                                                    <option @if ($quote->discount_type == 'percent') selected @endif value="percent">%
                                                                    </option>
                                                                    <option @if ($quote->discount_type == 'fixed') selected @endif value="fixed">
                                        ...
                                        <td><span
                                                id="discount_amount">{{ number_format((float) $quote->discount, 2, '.', '') }}</span>
                                        </td>
                                    </tr>
                                    <tr>
                                    ...
                                    <tr class="bg-amt-grey f-16 f-w-500">
                                        ...
                                        <td><span
                                                class="total">{{ number_format((float) $quote->total, 2, '.', '') }}</span>
                                        </td>
        ...
        <div class="d-flex flex-wrap px-lg-4 px-md-4 px-3 py-3">
            <div class="col-md-6 col-sm-12 c-inv-note-terms p-0 mb-lg-0 mb-md-0 mb-3">
                ...
                <textarea class="form-control" name="note" id="note" rows="4"
                    placeholder="@lang('placeholders.invoices.note')">{{ $quote->note }}</textarea>
            </div>
        ...
        <x-form-actions class="c-inv-btns">
            ...
            <x-forms.button-cancel :link="route('quotes.index')" class="border-0">@lang('app.cancel')
...
<script>
    ...
    $(document).ready(function() {
        ...
        file.on('dropify.beforeClear', function(event, element) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    const url = "{{ route('quotes.delete_image') }}";
        ...
        const dp1 = datepicker('#valid_till', {
            position: 'bl',
            dateSelected: new Date("{{ str_replace('-', '/', $quote->valid_till) }}"),
            ...datepickerConfig
        });
        ...
        $(document).on('click', '#add-item', function() {
            ...
            @endforeach
                ...
                '<input type="file" class="dropify" id="dropify'+i+'" name="invoice_item_image[]" data-allowed-file-extensions="png jpg jpeg bmp" data-team chat-default="test" data-height="70""/><input type="hidden" name="invoice_item_image_url[]">' +
                '</td>' +
                '</tr>' +
                '</tbody>' +
                '</table>' +
