...
<table class="bg-white" border="0" cellpadding="0" cellspacing="0" width="100%" role="presentation">
    ...
    <tr>
        ...
        <td class="f-12 text-black" align="right">
            ...
            @if (($invoiceSetting->show_client_name == 'yes' || $invoiceSetting->show_client_email == 'yes' || $invoiceSetting->show_client_phone == 'yes' || $invoiceSetting->show_client_company_name == 'yes' || $invoiceSetting->show_client_company_address == 'yes') && $customer)
                <p class="line-height mb-0">
                            <span class="text-grey ">
                                @lang('modules.invoices.billedTo')</span><br>

                    @if ($customer->name && $invoiceSetting->show_client_name == 'yes')
                        {{ $customer->name_salutation }}<br>
                    @endif

                    @if ($customer->email && $invoiceSetting->show_client_email == 'yes')
                        {{ $customer->email }}<br>
                    @endif

                    @if ($customer->mobile && $invoiceSetting->show_client_phone == 'yes')
                        {{ $customer->mobile_with_phonecode }}<br>
                    @endif

                    @if ($customer->clientDetails->company_name && $invoiceSetting->show_client_company_name == 'yes')
                        {{ $customer->clientDetails->company_name }}<br>
                    @endif

                    @if ($customer->clientDetails->address && $invoiceSetting->show_client_company_address == 'yes')
    ...
    <tr>
        <td class="total-box" align="right" colspan="{{ $invoiceSetting->hsn_sac_code_show ? '5' : '4' }}">
            ...
            @if ($hasDeducted)
                ...
                <!-- Table Row Start -->
                <tr align="right" class="text-grey">
                    <td width="50%" class="subtotal">@lang('modules.invoices.totalPayment')</td>
                </tr>
                <!-- Table Row End -->
                @foreach ($taxesDeducted as $key => $tax)
                    <!-- Table Row Start -->
                    <tr align="right" class="text-grey">
                        <td width="50%" class="subtotal">{{ $key }} (Deducted)</td>
                    </tr>
                    <!-- Table Row End -->
                @endforeach
                <!-- Table Row Start -->
                <tr align="right" class="text-grey">
                    <td width="50%" class="subtotal">@lang('modules.invoices.netPayable')</td>
                </tr>
                <!-- Table Row End -->
            @else
        ...
        <td class="total-box" align="right"
            ...
            @if ($hasDeducted)
                @foreach ($taxesIncluded as $key => $tax)
                    <!-- Table Row Start -->
                    <tr align="right" class="text-grey">
                        <td class="subtotal-amt">{{ currency_format($tax, $invoice->currency_id, false) }}</td>
                    </tr>
                    <!-- Table Row End -->
                @endforeach
                <!-- Table Row Start -->
                <tr align="right" class="text-grey">
                    <td class="subtotal-amt">{{ currency_format($totalPaymentAmount, $invoice->currency_id, false) }}</td>
                </tr>
                <!-- Table Row End -->
                @foreach ($taxesDeducted as $key => $tax)
                    <!-- Table Row Start -->
                    <tr align="right" class="text-grey">
                        <td class="subtotal-amt">-{{ currency_format(abs($tax), $invoice->currency_id, false) }}</td>
                    </tr>
                    <!-- Table Row End -->
                @endforeach
                <!-- Table Row Start -->
                <tr align="right" class="text-grey">
                    <td class="subtotal-amt">{{ currency_format($netPayableBeforeAdjustment, $invoice->currency_id, false) }}</td>
                </tr>
                <!-- Table Row End -->
            @else
                @foreach ($taxes as $key => $tax)
...
@if (count($payments) > 0)
    ...
    <table class="f-14 b-collapse" width="100%">
        ...
        @empty
            <tr>
                <td colspan="4">@lang('team chat.noRecordFound') </td>
            </tr>
        @endforelse
    </table>
@endif
