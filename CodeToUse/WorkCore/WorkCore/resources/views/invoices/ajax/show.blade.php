...
@if (!in_array('customer', user_roles()))
    @if (!is_null($invoice->last_viewed))
        <x-alert type="info">
            {{$invoice->customer->name_salutation}} @lang('app.viewedOn') {{$invoice->last_viewed->timezone($settings->timezone)->translatedFormat($settings->date_format)}}
...
@if(!is_null($invoice->client_id) && !is_null($invoice->clientDetails))
    ...
    @endphp
@elseif (
    !is_null($invoice->site) &&
        !is_null($invoice->site->customer) &&
        !is_null($invoice->site->customer->clientDetails))
    @php
...
@if (!$invoice->send_status && $invoice->status != 'canceled' && $invoice->amountDue() > 0)
    <x-alert icon="info-circle" type="warning">
        @lang('team chat.unsentInvoiceInfo')
    </x-alert>
@endif
        ...
        <div class="invoice-table-wrapper">
                ...
                <tr class="inv-unpaid">
                    <td class="f-14 text-dark">
                        <p class="mb-0 text-left">
                            @if ($invoice->customer || $invoice->clientDetails)
                                <span class="text-dark-grey ">@lang('modules.invoices.billedTo')</span>
                                <br>

                                @if ($invoice->customer && $invoice->customer->name && invoice_setting()->show_client_name == 'yes')
                                    {{ $invoice->customer->name_salutation }}<br>
                                @endif

                                @if ($invoice->customer && $invoice->customer->email && invoice_setting()->show_client_email == 'yes')
                                    {{ $invoice->customer->email }}<br>
                                @endif

                                @if ($invoice->customer && $invoice->customer->mobile && invoice_setting()->show_client_phone == 'yes')
                            ...
                            @if ($invoiceSetting->show_project == 1 && isset($invoice->site))
                                <br><br>
                                <span
                            ...
                            @if ($invoiceSetting->show_gst == 'yes' && !is_null($customer->clientDetails->gst_number))
                                @if ($customer->clientDetails->tax_name)
                                    <br>{{$customer->clientDetails->tax_name}}: {{$customer->clientDetails->gst_number}}
                                @else
                                    <br>@lang('app.gstIn'): {{ $customer->clientDetails->gst_number }}
                                @endif
                            @endif
                        </p>
                    </td>
                    @if ($invoice->show_shipping_address == 'yes')
                        <td class="f-14 text-black">
                            <p class="mb-0 text-left"><span
                                ...
                                {!! nl2br($customer->clientDetails->shipping_address) !!}</p>
                        </td>
                    @endif
                                    ...
                                    <table width="100%">
                                        ...
                                        @if ($hasDeducted)
                                            @foreach ($taxesIncluded as $key => $tax)
                                                <tr class="text-dark-grey" align="right">
                                                    <td class="w-50 border-top-0 border-left-0">{{ $key }}</td>
                                                </tr>
                                            @endforeach
                                            <tr class="text-dark-grey font-weight-semibold" align="right">
                                                <td class="w-50 border-top-0 border-left-0">@lang('modules.invoices.totalPayment')</td>
                                            </tr>
                                            @foreach ($taxesDeducted as $key => $tax)
                                                <tr class="text-dark-grey" align="right">
                                                    <td class="w-50 border-top-0 border-left-0">{{ $key }} (Deducted)</td>
                                                </tr>
                                            @endforeach
                                            <tr class=" text-dark-grey font-weight-bold" align="right">
                                                <td class="w-50 border-bottom-0 border-left-0">@lang('modules.invoices.netPayable')</td>
                                            </tr>
                                        @else
                                ...
                                <td class="p-0 border-left-0" align="right">
                                    <table width="100%">
                                        ...
                                        @if ($hasDeducted)
                                            ...
                                            <tr class="text-dark-grey font-weight-semibold" align="right">
                                                <td class="border-top-0 border-right-0">
                                                    {{ currency_format($totalPaymentAmount, $invoice->currency_id, false) }}</td>
                                            </tr>
                                            @foreach ($taxesDeducted as $key => $tax)
                                                <tr class="text-dark-grey" align="right">
                                                    <td class="border-top-0 border-right-0">
                                                        -{{ currency_format(abs($tax), $invoice->currency_id, false) }}</td>
                                                </tr>
                                            @endforeach
                                            <tr class=" text-dark-grey font-weight-bold" align="right">
            ...
            <table width="100%" class="inv-desc-mob d-block d-lg-none d-md-none">
                ...
                @if ($hasDeducted)
                    ...
                    @endforeach
                    <tr>
                        <th width="50%" class="text-dark-grey font-weight-semibold">@lang('modules.invoices.totalPayment')</th>
                        <td width="50%" class="text-dark-grey font-weight-semibold">
                            {{ currency_format($totalPaymentAmount, $invoice->currency_id, false) }}</td>
                    </tr>
                    @foreach ($taxesDeducted as $key => $tax)
                        <tr>
                            <th width="50%" class="text-dark-grey font-weight-normal">{{ $key }} (Deducted)</th>
                            <td width="50%" class="text-dark-grey font-weight-normal">
                                -{{ currency_format(abs($tax), $invoice->currency_id, false) }}</td>
                        </tr>
                    @endforeach
                    <tr>
                        <th width="50%" class="text-dark-grey font-weight-bold">@lang('modules.invoices.netPayable')</th>
        ...
        <div class="d-flex">
            <div class="inv-action mr-3 mr-lg-3 mr-md-3 dropup">
                ...
                <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton" tabindex="0">
                    ...
                    @if ($invoice->status == 'paid' && !in_array('customer', user_roles()) && $invoice->amountPaid() == 0)
                        <li>
                    ...
                    @php
                        $trashBtn = (!is_null($invoice->site) && is_null($invoice->site->deleted_at)) ? true : (is_null($invoice->site) ? true : false) ;
                    @endphp
                    ...
                    @if (
                        ($firstInvoice->id == $invoice->id && $invoice->status == 'unpaid' && $deleteInvoicePermission == 'all') ||
                            ($deleteInvoicePermission == 'added' && ($invoice->added_by == user()->id || $invoice->added_by == $userId) && $firstInvoice->id == $invoice->id))
                        <li>
                    ...
                    @if ($invoice->status != 'canceled' && !$invoice->credit_note && !in_array('customer', user_roles()))
                        <li>
                        ...
                        @if ($invoice->send_status == 0)
                            <li>
                                <a class="dropdown-item f-14 text-dark sendButton" href="javascript:;"
                                   data-toggle="tooltip" data-original-title="@lang('team chat.markSentInfo')"
                    ...
                    @if ($invoice->status == 'pending-confirmation' && !in_array('customer', user_roles()) && !empty($invoice->payment))
                    ...
                    @if ($invoice->status == 'paid' && !in_array('customer', user_roles()) && $invoice->credit_note == 0)
                        <a class="dropdown-item invoice-upload"  data-toggle="tooltip" data-original-title="@lang('team chat.uploadOtherInvoice')" href="javascript:;" data-toggle="tooltip"
                           data-invoice-id="{{ $invoice->id }}">
                    ...
                    @if ($invoice->status != 'canceled')
                        @if ($invoice->clientDetails)
                            ...
                            @endif
                        @else
                            @if ($invoice->site->clientDetails)
                                @if (!is_null($invoice->site->clientDetails->shipping_address))
                                    @if ($invoice->show_shipping_address == 'yes')
                                        <li>
                                            <a class="dropdown-item f-14 text-dark toggle-shipping-address"
                                               href="javascript:;" data-invoice-id="{{ $invoice->id }}">
...
@if (count($invoice->files) > 0)
    <div class="bg-white mt-4 pl-3 pt-3">
        ...
        <div class="d-flex flex-wrap" id="invoice-file-list">
            ...
            @empty
                <x-cards.no-record :message="__('team chat.noFileUploaded')" icon="file"/>
            @endforelse
...
<script>
    ...
    $('body').on('click', '.cancel-invoice', function () {
        ...
        Swal.fire({
            ...
            text: "@lang('team chat.invoiceText')",
            icon: 'warning',
    ...
    $('body').on('click', '.unpaidAndPartialPaidCreditNote', function () {
        ...
        Swal.fire({
            title: "@lang('team chat.confirmation.createCreditNotes')",
            text: "@lang('team chat.creditText')",
            icon: 'warning',
            showCancelButton: true,
            focusConfirm: false,
            confirmButtonText: "@lang('app.yes')",
