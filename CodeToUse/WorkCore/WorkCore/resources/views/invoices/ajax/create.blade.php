...
@if ((!in_array('customer', user_roles()) && !in_array('customers', user_modules())) || (in_array('customer', user_roles()) && !in_array('invoices', user_modules())))
...
<div class="bg-white rounded b-shadow-4 create-inv">
    ...
    <x-form class="c-inv-form" id="saveInvoiceForm">
                    ...
                    <div class="select-others height-35 rounded">
                        <select class="form-control select-picker" name="currency_id" id="currency_id">
                            @foreach ($currencies as $currency)
                                <option @if (isset($quote))
                                    @selected($currency->id == $quote->currency_id)
            ...
            <div class="col-md-4 mb-4 ">
                @if (isset($customer) && !is_null($customer))
                    <div class="form-group">
                        <x-forms.label fieldId="due_date" :fieldLabel="__('app.customer')">
                        </x-forms.label>
                        <div class="input-group">
                ...
                @else
                    <x-customer-selection-dropdown :customers="$customers" :selected="(isset($invoice) ? $invoice->client_id : (request()->has('default_client') ? request()->has('default_client') : (isset($quote) ? $quote->client_id : null)))" />
                @endif
            </div>
            <!-- CLIENT END -->
            ...
            <div class="col-md-4">
                @if (isset($site) && !is_null($site))
                    <div class="form-group mb-4">
                        <x-forms.label fieldId="due_date" :fieldLabel="__('app.site')">
                        </x-forms.label>
                        <div class="input-group">
                            <input type="hidden" name="project_id" id="project_id" value="{{ $site->id }}">
                            <input type="text" value="{{ $site->project_name }}"
                                class="form-control height-35 f-15 readonly-background" readonly>
                        </div>
                    </div>
                @else
                        ...
                        <div class="select-others height-35 rounded">
                            <select class="form-control select-picker" data-live-search="true" data-size="8"
                                ...
                                @if (isset($invoice) && $invoice->customer)
                                    @foreach ($invoice->customer->sites as $item)
                                    ...
                                    @endforeach
                                @elseif (isset($quote) && $quote->customer)
                                    @foreach ($quote->customer->sites as $item)
                                        <option @selected($quote->project_id == $item->id) value="{{ $item->id }}"
                                                data-content="{!! '<strong>'.$item->project_short_code."</strong> ".$item->project_name !!}"
                                        >
                                            {{ $item->project_name }}</option>
                                    @endforeach
                                @elseif (isset($isClient) && $isClient == true)
                                    @foreach ($customer->sites as $item)
                                        <option @selected($customer->project_id == $item->id) value="{{ $item->id }}"
                                                data-content="{!! '<strong>'.$item->project_short_code."</strong> ".$item->project_name !!}"
                                        >
                                            {{ $item->project_name }}</option>
                                    @endforeach
                ...
                <div class="form-group c-inv-select mb-4">
                    ...
                    <div class="select-others height-35 rounded">
                        <select class="form-control select-picker" data-live-search="true" data-size="8"
                            ...
                            <option value="after_discount" @if (isset($invoice) && $invoice->calculate_tax == 'after_discount') selected @elseif(isset($quote) && $quote->calculate_tax == 'after_discount') selected @endif>
                                @lang('modules.invoices.afterDiscount')</option>
                            <option value="before_discount" @if (isset($invoice) && $invoice->calculate_tax == 'before_discount') selected @elseif(isset($quote) && $quote->calculate_tax == 'before_discount') selected @endif>
            ...
            <div class="col-md-4">
                <div class="form-group c-inv-select mb-0">
                    ...
                    <p class="f-15" id="client_billing_address">
                        @if (isset($invoice) && $invoice->customer)
                            {!! nl2br($invoice->customer->clientDetails->address) !!}
                        @elseif (isset($invoice) && isset($customer))
                            {!! nl2br($customer->clientDetails->address) !!}
                        @elseif (isset($quote) && $quote->customer)
                            {!! nl2br($quote->customer->clientDetails->address) !!}
                        @else
                            <span class="text-lightest">@lang('team chat.selectCustomerForBillingAddress')</span>
                        @endif
                    </p>
            ...
            <div class="col-md-4">
                <div class="form-group c-inv-select mb-lg-0 mb-md-0 mb-4">
                    ...
                    <p class="f-15" id="client_shipping_address">
                        @if (isset($invoice) && $invoice->customer && $invoice->customer->clientDetails->shipping_address)
                            {!! nl2br($invoice->customer->clientDetails->shipping_address) !!}
                        @elseif(isset($customer) && $customer->clientDetails &&
                            $customer->clientDetails->shipping_address)
                            {!! nl2br($customer->clientDetails->shipping_address) !!}
                        @else
                    ...
                    <p class="d-none" id="add-shipping-field">
                        <textarea class="form-control f-14 pt-2" rows="3" placeholder="@lang('placeholders.address')"
                            name="shipping_address" id="shipping_address">@if (isset($invoice) && $invoice->customer) {!! nl2br($invoice->customer->clientDetails->shipping_address) !!} @endif</textarea>
                    </p>
                </div>
            </div>
        ...
        <div class="row px-lg-4 px-md-4 px-3 py-3">
            <div class="col-md-3 d-none service / extra-category-filter">
                <div class="form-group c-inv-select mb-4">
                    <x-forms.input-group>
                        <select class="form-control select-picker" name="category_id"
                                id="product_category_id" data-live-search="true">
                            <option value="">{{ __('app.select') . ' ' . __('app.service / extra') . ' ' . __('app.category')  }}</option>
            ...
            @if(in_array('services / extras', user_modules()) || in_array('purchase', user_modules()))
                <div class="col-md-3">
                    ...
                    <x-forms.input-group>
                        <select class="form-control select-picker" data-live-search="true" data-size="8"
                            id="add-services / extras" title="{{ __('app.menu.selectProduct') }}">
                        ...
                        <x-slot name="preappend">
                            <a href="javascript:;"
                                class="btn btn-outline-secondary border-grey toggle-service / extra-category"
                                data-toggle="tooltip"
                        ...
                        @if ($addProductPermission == 'all' || $addProductPermission == 'added')
                            @if (module_enabled('Purchase') && in_array('purchase', user_modules()))
                                <x-slot name="append">
                                    <a href="{{ route('purchase-services / extras.create') }}" data-redirect-url="no"
                        ...
                        <table width="100%">
                            <tbody>
                                ...
                                <tr class="d-none d-md-table-row d-lg-table-row">
                                    ...
                                    <td class="border-left-0">
                                        <input type="file" class="dropify" name="invoice_item_image[]" data-allowed-file-extensions="png jpg jpeg bmp" data-team chat-default="test" data-height="70" />
                                                    ...
                                                    <tr>
                                                        <td width="70%" class="c-inv-sub-padding">
                                                            <input type="number" min="0" name="discount_value"
                                                                ...
                                                                placeholder="0"
                                                                value= "{{ isset($quote) ? $quote->discount : (isset($invoice) ? $invoice->discount : '0') }}">
                                                        </td>
        ...
        <div class="d-flex flex-wrap px-lg-4 px-md-4 px-3 py-3">
            <div class="col-md-6 col-sm-12 c-inv-note-terms p-0 mb-lg-0 mb-md-0 mb-3">
                ...
                <textarea class="form-control" name="note" id="note" rows="4"
                    placeholder="@lang('placeholders.invoices.note')">{{ isset($quote) ? $quote->note : (isset($invoice) ? $invoice->note : '') }}</textarea>
            </div>
            ...
            <div class="d-flex mb-3 mb-lg-0 mb-md-0">
                ...
                <div class="inv-action dropup mr-3">
                    ...
                    <ul class="dropdown-menu" aria-labelledby="dropdownMenuBtn" tabindex="0">
                        ...
                        <li>
                            <a class="dropdown-item f-14 text-dark save-form" href="javascript:void(0);"
                                data-type="mark_as_send" data-toggle="tooltip" data-original-title="@lang('team chat.markSentInfo')">
...
<script>
    ...
    // Total Payment (included taxes) and Net Payable (after deducted taxes) - must be in global scope for saveForm()
    ...
    $(document).ready(function() {
        ...
        // Override calculateTotal for invoice create: show TOTAL (included taxes) and NET PAYABLE (after deducted taxes)
        ...
        $('.toggle-service / extra-category').click(function() {
            $('.service / extra-category-filter').toggleClass('d-none');
            ...
            $.easyAjax({
                ...
                success: function (response) {
                    if (response.status == 'success') {
                        ...
                        $('#add-services / extras').html(
                            '<option value="" class="form-control" >{{  __('app.menu.selectProduct') }}</option>' +
                            options);
                        $('#add-services / extras').selectpicker('refresh');
                    }
                }
            });
        }
        ...
        const isClient = userRoles.some(role => role.name === "customer");
        ...
        function changeClient(id) {
            ...
            var url = "{{ route('customers.project_list', ':id') }}";
            ...
            var url = "{{ route('customers.ajax_details', ':id') }}";
            url = url.replace(':id', id);
        ...
        const resetAddProductButton = () => {
            $("#add-services / extras").val('').selectpicker("refresh");
        };

        $('#add-services / extras').on('changed.bs.select', function(e, clickedIndex, isSelected, previousValue) {
        ...
        $(document).on('click', '#add-item', function() {
            ...
            @endforeach
                ...
                <tr class="d-none d-md-table-row d-lg-table-row">
                    ...
                    <td class="border-left-0">
                        <input type="file" class="dropify" id="dropify${i}" name="invoice_item_image[]" data-allowed-file-extensions="png jpg jpeg bmp" data-team chat-default="test" data-height="70" />
                        <input type="hidden" name="invoice_item_image_url[]">
                    </td>
                </tr>
                </tbody>
        ...
        $('.save-form').click(function() {
            ...
            if((type == 'send' || type == 'mark_as_send') && totalAmt == 0) {
                Swal.fire({
                    ...
                    text: "@lang('team chat.markAsPaid')",
                    icon: 'warning',
    ...
    function saveForm(type, exceed){
            ...
            if (parseFloat(discount) > parseFloat(total)) {
                Swal.fire({
                    icon: 'error',
                    text: "{{ __('team chat.discountExceed') }}",
            ...
            $.easyAjax({
                ...
                success: function(response) {
                    ...
                    if (response.status == 'error' && response.showValue === true && exceed == 'direct') {
                        ...
                        const url = "{{ route('invoices.committed_modal') }}" + "?services / extras=" + productIDs + "&type=" + type ;
