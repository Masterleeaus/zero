    ...
    <div class="border-0 card invoice">
        ...
        <div class="card-body">
            ...
            <div class="d-flex flex-column">
                ...
                <div class="p-0 pb-3 ql-editor">{!! $service agreement->contract_detail !!}</div>

                @if ($service agreement->amount != 0)
                    <div class="pt-3 text-right border-top">
            ...
            @if ($service agreement->signature)
                <div class="d-flex flex-column float-right margin-top: 20px;">
                    ...
                    <p>@lang('app.client_name'):- {{ $service agreement->signature->full_name }}<br>
                        @lang('app.place'):- {{ $service agreement->signature->place }}<br>
                        @lang('app.date'):- {{ $service agreement->signature->date->translatedFormat($company->date_format) }}
                    </p>
                </div>
            @endif
            ...
            @if ($service agreement->company_sign)
                <div class="d-flex flex-column">
                    ...
                    <p>@lang('app.date'):- {{ $service agreement->sign_date->translatedFormat($company->date_format) }}</p>
        ...
        <div
            ...
            <x-forms.button-cancel :link="route('service agreements.index')" class="mb-2 mr-3 border-0">@lang('app.cancel')
            </x-forms.button-cancel>

            <x-forms.link-secondary :link="route('front.service agreement.download', $service agreement->hash)" class="mb-2 mr-3"
...
<script>
    ...
    $('#save-signature').click(function () {
        ...
        $.easyAjax({
            url: "{{ route('front.service agreement.sign', $service agreement->id) }}",
