    ...
    <x-form id="process-{{ $importClassName }}-data-form">
        <div class="add-{{ $importClassName }} bg-white rounded">
            <h4 class="mb-0 p-20 f-21 font-weight-normal  border-bottom-grey">
                {{ $headingTitle }}</h4>
                <div class="col-12">
                    <p class="mt-3">@lang("team chat.matchColumnMessage")</p>
                    <div class="alert alert-success" id="getUnMatchedSuccess" style="display:none;">
                        @lang("team chat.columnMatchSuccess")
                    </div>
                    <div class="alert alert-warning" id="requiredColumnsUnmatched" style="display:none;">
                        @lang("team chat.requiredColumnsUnmatched")
                    </div>
                    <div class="row">
                    <div class="col-md-6">
                        <b>@lang("team chat.unmatchedColumns", ["unmatchCount" => (!empty($heading) ? collect($columns)->where('required','Yes')->whereNotIn('id', $heading)->count() : 0)])</b>
    ...
    $('.select-picker').selectpicker();
        ...
        function requiredMatchAction() {
            ...
            if (requiredMatched.length == 0) {
                ...
                let msg = "@lang("team chat.requiredColumnsUnmatched")";
        ...
        function saveColumnBox(columnID) {
            ...
            if (selectedColumnID == "") {
                Swal.fire({
                    icon: 'error',
                    text: "@lang("team chat.pleaseSelectAColumn")",
