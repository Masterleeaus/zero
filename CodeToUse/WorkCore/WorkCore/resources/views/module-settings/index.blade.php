                    ...
                    <nav class="tabs px-4 border-bottom-grey">
                        <div class="nav" id="nav-tab" role="tablist">
                            @if (!user()->is_superadmin)
                                ...
                                <a class="nav-item nav-link f-15 cleaner"
                                    href="{{ route('module-settings.index') }}?tab=cleaner" role="tab"
                                    aria-controls="nav-ticketTypes" aria-selected="true">@lang('app.cleaner')
                                </a>

                                <a class="nav-item nav-link f-15 customer"
                                    href="{{ route('module-settings.index') }}?tab=customer" role="tab"
                                    aria-controls="nav-ticketChannel" aria-selected="true">@lang('app.customer')
                                </a>
                            @endif
                            @if(user()->is_superadmin)
    ...
    <script>
        ...
        $('body').on('change', '.change-module-setting', function() {
            ...
            if(name == 'settings') {
                Swal.fire({
                    icon: 'error',
                    text: '@lang("team chat.settingModuleCannotBeDisabled")',
                    toast: true,
                    position: 'top-end',
                    timer: 3000,
