...
Route::group(['middleware' => ['auth', 'multi-company-select', 'email_verified'], 'prefix' => 'account'], function () {
    ...
    Route::get('dashboard/enquiry-data/{id}', [DashboardController::class, 'getLeadStage'])->name('dashboard.deal-stage-data');
    ...
    Route::post('approve/{id}', [ClientController::class, 'approve'])->name('customers.approve');
    Route::post('save-consent-purpose-data/{customer}', [ClientController::class, 'saveConsentLeadData'])->name('customers.save_consent_purpose_data');
    Route::get('customers/gdpr-consent', [ClientController::class, 'consent'])->name('customers.gdpr_consent');
    Route::post('customers/save-customer-consent/{enquiry}', [ClientController::class, 'saveClientConsent'])->name('customers.save_client_consent');
    Route::post('customers/ajax-details/{id}', [ClientController::class, 'ajaxDetails'])->name('customers.ajax_details');
    Route::get('customers/customer-details/{id}', [ClientController::class, 'clientDetails'])->name('customers.client_details');
    Route::post('customers/site-list/{id}', [ClientController::class, 'projectList'])->name('customers.project_list');
    Route::post('customers/apply-quick-action', [ClientController::class, 'applyQuickAction'])->name('customers.apply_quick_action');
    Route::get('customers/import', [ClientController::class, 'importClient'])->name('customers.import');
    Route::post('customers/import', [ClientController::class, 'importStore'])->name('customers.import.store');
    Route::post('customers/import/process', [ClientController::class, 'importProcess'])->name('customers.import.process');
    Route::get('customers/finance-count/{id}', [ClientController::class, 'financeCount'])->name('customers.finance_count');
    Route::resource('customers', ClientController::class);

    Route::post('customer-contacts/apply-quick-action', [ClientContactController::class, 'applyQuickAction'])->name('customer-contacts.apply_quick_action');
    Route::get('get-contacts', [TicketController::class, 'getContacts'])->name('get.contacts');
    Route::resource('customer-contacts', ClientContactController::class);

    Route::get('customer-notes/ask-for-password/{id}', [ClientNoteController::class, 'askForPassword'])->name('client_notes.ask_for_password');
    Route::post('customer-notes/check-password', [ClientNoteController::class, 'checkPassword'])->name('client_notes.check_password');
    Route::post('customer-notes/apply-quick-action', [ClientNoteController::class, 'applyQuickAction'])->name('customer-notes.apply_quick_action');
    Route::post('customer-notes/showVerified/{id}', [ClientNoteController::class, 'showVerified'])->name('customer-notes.show_verified');
    Route::resource('customer-notes', ClientNoteController::class);

    Route::get('customer-docs/download/{id}', [ClientDocController::class, 'download'])->name('customer-docs.download');
    Route::resource('customer-docs', ClientDocController::class);
    ...
    // cleaner routes
    Route::post('cleaners/apply-quick-action', [EmployeeController::class, 'applyQuickAction'])->name('cleaners.apply_quick_action');
    Route::post('cleaners/assignRole', [EmployeeController::class, 'assignRole'])->name('cleaners.assign_role');
    Route::get('cleaners/byDepartment/{id}', [EmployeeController::class, 'byDepartment'])->name('cleaners.by_department');
    Route::get('cleaners/invite-member', [EmployeeController::class, 'inviteMember'])->name('cleaners.invite_member');
    Route::get('cleaners/import', [EmployeeController::class, 'importMember'])->name('cleaners.import');
    Route::post('cleaners/import', [EmployeeController::class, 'importStore'])->name('cleaners.import.store');
    Route::post('cleaners/import/process', [EmployeeController::class, 'importProcess'])->name('cleaners.import.process');
    Route::get('import/process/{name}/{id}', [ImportController::class, 'getImportProgress'])->name('import.process.progress');

    Route::get('cleaners/import/exception/{name}', [ImportController::class, 'getQueueException'])->name('import.process.exception');
    Route::post('cleaners/send-invite', [EmployeeController::class, 'sendInvite'])->name('cleaners.send_invite');
    Route::post('cleaners/create-link', [EmployeeController::class, 'createLink'])->name('cleaners.create_link');
    Route::post('/get-exit-date-message', [EmployeeController::class, 'getExitDateMessage'])->name('getExitDateMessage');
    Route::resource('cleaners', EmployeeController::class);
    Route::resource('passport', PassportController::class);
    Route::resource('cleaner-visa', EmployeeVisaController::class);

    Route::resource('emergency-contacts', EmergencyContactController::class);

    Route::get('cleaner-docs/download/{id}', [EmployeeDocController::class, 'download'])->name('cleaner-docs.download');
    Route::resource('cleaner-docs', EmployeeDocController::class);

    Route::get('cleaner-document-expiries/download/{id}', [EmployeeDocumentExpiryController::class, 'download'])->name('cleaner-document-expiries.download');
    Route::resource('cleaner-document-expiries', EmployeeDocumentExpiryController::class);

    Route::get('cleaner-leaves/employeeLeaveTypes/{id}', [LeavesQuotaController::class, 'employeeLeaveTypes'])->name('cleaner-leaves.employee_leave_types');
    Route::resource('cleaner-leaves', LeavesQuotaController::class);

    Route::get('roles/role-hierarchy', [DesignationController::class, 'hierarchyData'])->name('role.hierarchy');
    Route::post('roles/changeParent', [DesignationController::class, 'changeParent'])->name('role.changeParent');
    Route::post('roles/search-filter', [DesignationController::class, 'searchFilter'])->name('role.srchFilter');
    Route::post('roles/apply-quick-action', [DesignationController::class, 'applyQuickAction'])->name('roles.apply_quick_action');
    Route::resource('roles', DesignationController::class);

    Route::post('zones/apply-quick-action', [DepartmentController::class, 'applyQuickAction'])->name('zones.apply_quick_action');
    Route::get('zones/zone-hierarchy', [DepartmentController::class, 'hierarchyData'])->name('zone.hierarchy');
    Route::post('zone/changeParent', [DepartmentController::class, 'changeParent'])->name('zone.changeParent');
    Route::get('zone/search', [DepartmentController::class, 'searchDepartment'])->name('zones.search');
    Route::get('zone/{id}', [DepartmentController::class, 'getMembers'])->name('zones.members');
    Route::resource('zones', DepartmentController::class);
    ...
    Route::post('sites/change-status', [ProjectController::class, 'changeProjectStatus'])->name('sites.change_status');

    Route::resource('ProjectSubCategory', ProjectSubCategoryController::class);
    Route::get('get_project_sub_category/{id}', [ProjectSubCategoryController::class, 'getSubCategories'])->name('site.get_project_sub_category');
    ...
    Route::group(
        ...
        function () {
            ...
            Route::get('import', [ProjectController::class, 'importProject'])->name('sites.import');
            Route::post('import', [ProjectController::class, 'importStore'])->name('sites.import.store');
            Route::post('import/process', [ProjectController::class, 'importProcess'])->name('sites.import.process');

            Route::post('assignProjectAdmin', [ProjectController::class, 'assignProjectAdmin'])->name('sites.assign_project_admin');
            Route::post('archive-restore/{id}', [ProjectController::class, 'archiveRestore'])->name('sites.archive_restore');
            Route::post('archive-delete/{id}', [ProjectController::class, 'archiveDestroy'])->name('sites.archive_delete');
            Route::get('archive', [ProjectController::class, 'archive'])->name('sites.archive');
            Route::post('apply-quick-action', [ProjectController::class, 'applyQuickAction'])->name('sites.apply_quick_action');
            Route::post('updateStatus/{id}', [ProjectController::class, 'updateStatus'])->name('sites.update_status');
            Route::post('store-pin', [ProjectController::class, 'storePin'])->name('sites.store_pin');
            Route::post('destroy-pin/{id}', [ProjectController::class, 'destroyPin'])->name('sites.destroy_pin');
            Route::post('gantt-data', [ProjectController::class, 'ganttData'])->name('sites.gantt_data');
            Route::post('invoiceList/{id}', [ProjectController::class, 'invoiceList'])->name('sites.invoice_list');
            Route::get('duplicate-site/{id}', [ProjectController::class, 'duplicateProject'])->name('sites.duplicate_project');

            Route::get('members/{id}', [ProjectController::class, 'members'])->name('sites.members');
            Route::get('pendingTasks/{id}', [ProjectController::class, 'pendingTasks'])->name('sites.pendingTasks');
            Route::get('labels/{id}', [TaskLabelController::class, 'labels'])->name('sites.labels');
            Route::resource('site-label', ProjectLabelController::class);

            Route::post('site-members/save-group', [ProjectMemberController::class, 'storeGroup'])->name('site-members.store_group');
            Route::resource('site-members', ProjectMemberController::class);
            ...
            // Rating routes
            Route::resource('site-ratings', ProjectRatingController::class);

            Route::get('sites/burndown/{projectId?}', [ProjectController::class, 'burndown'])->name('sites.burndown');

            /* PROJECT TEMPLATE */
            Route::post('site-template/apply-quick-action', [ProjectTemplateController::class, 'applyQuickAction'])->name('project_template.apply_quick_action');
            Route::resource('site-template', ProjectTemplateController::class);
            Route::post('site-template-members/save-group', [ProjectTemplateMemberController::class, 'storeGroup'])->name('project_template_members.store_group');
            Route::resource('site-template-member', ProjectTemplateMemberController::class);
            Route::get('site-template-service job/data/{templateId?}', [ProjectTemplateTaskController::class, 'data'])->name('project_template_task.data');
            Route::get('site-template-milestones/byProject/{id}', [ProjectTemplateMilestoneController::class, 'byProject'])->name('site-template-milestone.by_project');
            Route::post('/site-template-milestones/{id}/update-status', [ProjectTemplateMilestoneController::class, 'updateStatus'])->name('site-template-milestone.updateStatus');
            Route::resource('site-template-service job', ProjectTemplateTaskController::class);
            Route::resource('site-template-milestone', ProjectTemplateMilestoneController::class);
            Route::resource('site-template-sub-service job', ProjectTemplateSubTaskController::class);
            Route::resource('site-calendar', ProjectCalendarController::class);
        }
    );

    Route::get('site-notes/ask-for-password/{id}', [ProjectNoteController::class, 'askForPassword'])->name('project_notes.ask_for_password');
    Route::post('site-notes/check-password', [ProjectNoteController::class, 'checkPassword'])->name('project_notes.check_password');
    Route::post('site-notes/apply-quick-action', [ProjectNoteController::class, 'applyQuickAction'])->name('project_notes.apply_quick_action');
    Route::resource('site-notes', ProjectNoteController::class);
    Route::get('sites-ajax', [ProjectController::class, 'ajaxLoadProject'])->name('get.sites-ajax');
    Route::get('get-sites', [ProjectController::class, 'getProjects'])->name('get.sites');
    Route::resource('sites', ProjectController::class);

    /* PRODUCTS */
    Route::post('services / extras/apply-quick-action', [ProductController::class, 'applyQuickAction'])->name('services / extras.apply_quick_action');
    Route::post('services / extras/remove-cart-item/{id}', [ProductController::class, 'removeCartItem'])->name('services / extras.remove_cart_item');
    Route::get('services / extras/options', [ProductController::class, 'allProductOption'])->name('services / extras.options');


    Route::post('services / extras/add-cart-item', [ProductController::class, 'addCartItem'])->name('services / extras.add_cart_item');
    Route::get('services / extras/cart', [ProductController::class, 'cart'])->name('services / extras.cart');
    Route::get('services / extras/empty-cart', [ProductController::class, 'emptyCart'])->name('services / extras.empty_cart');

    /* Service / Extra Import */
    Route::group(
        ['prefix' => 'services / extras'],
        function () {
            Route::get('import', [ProductController::class, 'importProduct'])->name('services / extras.import');
            Route::post('import', [ProductController::class, 'importStore'])->name('services / extras.import.store');
            Route::post('import/process', [ProductController::class, 'importProcess'])->name('services / extras.import.process');
        }
    );

    Route::resource('services / extras', ProductController::class);
    ...
    /* PRODUCT FILES */
    Route::get('service / extra-files/download/{id}', [ProductFileController::class, 'download'])->name('service / extra-files.download');
    Route::post('service / extra-files/delete-image/{id}', [ProductFileController::class, 'deleteImage'])->name('service / extra-files.delete_image');
    Route::post('service / extra-files/update-images', [ProductFileController::class, 'updateImages'])->name('service / extra-files.update_images');
    Route::resource('service / extra-files', ProductFileController::class);
    ...
    /* TASKS */
    Route::get('service jobs/customer-detail', [TaskController::class, 'clientDetail'])->name('service jobs.clientDetail');
    Route::post('service jobs/change-status', [TaskController::class, 'changeStatus'])->name('service jobs.change_status');
    Route::post('service jobs/change-milestone', [TaskController::class, 'milestoneChange'])->name('service jobs.change_milestone');

    Route::post('service jobs/apply-quick-action', [TaskController::class, 'applyQuickAction'])->name('service jobs.apply_quick_action');
    Route::post('service jobs/store-pin', [TaskController::class, 'storePin'])->name('service jobs.store_pin');
    Route::post('service jobs/reminder', [TaskController::class, 'reminder'])->name('service jobs.reminder');
    Route::post('service jobs/destroy-pin/{id}', [TaskController::class, 'destroyPin'])->name('service jobs.destroy_pin');
    Route::post('service jobs/check-service job/{taskID}', [TaskController::class, 'checkTask'])->name('service jobs.check_task');
    Route::post('service jobs/send-approval', [TaskController::class, 'sendApproval'])->name('service jobs.send_approval');
    Route::post('service jobs/gantt-service job-update/{id}', [TaskController::class, 'updateTaskDuration'])->name('service jobs.gantt_task_update');
    Route::get('service jobs/members/{id}', [TaskController::class, 'members'])->name('service jobs.members');
    Route::get('service jobs/project_tasks/{id}', [TaskController::class, 'projectTasks'])->name('service jobs.project_tasks');
    Route::get('service jobs/check-leaves', [TaskController::class, 'checkLeaves'])->name('service jobs.checkLeaves');
    Route::get('service jobs/waiting-approval', [TaskController::class, 'waitingApproval'])->name('service jobs.waiting-approval');
    Route::get('service jobs/show-waiting-approval-change-status-modal', [TaskController::class, 'statusReason'])->name('service jobs.show_status_reason_modal');
    Route::post('service jobs/store-status-reason', [TaskController::class, 'storeStatusReason'])->name('service jobs.store_comment_on_change_status');

    Route::group(['prefix' => 'service jobs'], function () {

        Route::post('recurring-service job/apply-quick-action', [RecurringTaskController::class, 'applyQuickAction'])->name('recurring-service job.apply_quick_action');
        Route::resource('recurring-service job', RecurringTaskController::class);

        Route::resource('service job-label', TaskLabelController::class);
        ...
        Route::resource('service job-note', TaskNoteController::class);

        // service job files routes
        Route::get('service job-files/download/{id}', [TaskFileController::class, 'download'])->name('task_files.download');
        Route::resource('service job-files', TaskFileController::class);

        // Sub service job routes
        Route::post('sub-service job/change-status', [SubTaskController::class, 'changeStatus'])->name('sub_tasks.change_status');
        Route::resource('sub-service jobs', SubTaskController::class);

        // Service Job files routes
        Route::get('sub-service job-files/download/{id}', [SubTaskFileController::class, 'download'])->name('sub-service job-files.download');
        Route::resource('sub-service job-files', SubTaskFileController::class);
        ...
        Route::resource('service job-calendar', TaskCalendarController::class);
    });

    Route::resource('service jobs', TaskController::class);
    ...
    // Enquiry Category
    Route::post('/update-enquiry-category', [LeadCategoryController::class, 'updateLeadCategory'])->name('category.updateDefault');
    Route::resource('leadCategory', LeadCategoryController::class);

    // Enquiry Note
    Route::get('enquiry-notes/ask-for-password/{id}', [LeadNoteController::class, 'askForPassword'])->name('enquiry-notes.ask_for_password');
    Route::post('enquiry-notes/check-password', [LeadNoteController::class, 'checkPassword'])->name('enquiry-notes.check_password');
    Route::post('enquiry-notes/apply-quick-action', [LeadNoteController::class, 'applyQuickAction'])->name('enquiry-notes.apply_quick_action');

    Route::resource('enquiry-notes', LeadNoteController::class);
    ...
    Route::post('enquiry-form/sortFields', [LeadCustomFormController::class, 'sortFields'])->name('enquiry-form.sortFields');
    Route::resource('enquiry-form', LeadCustomFormController::class);
    ...
    Route::group(['prefix' => 'enquiry-contact'], function () {
        Route::get('import', [LeadContactController::class, 'importLead'])->name('enquiry-contact.import');
        Route::post('import', [LeadContactController::class, 'importStore'])->name('enquiry-contact.import.store');
        Route::post('import/process', [LeadContactController::class, 'importProcess'])->name('enquiry-contact.import.process');
    });

    // deals route

    Route::resource('enquiry-contact', LeadContactController::class);
    Route::post('enquiry-contact/apply-quick-action', [LeadContactController::class, 'applyQuickAction'])->name('enquiry-contact.apply_quick_action');
    ...
    // Team Chat
    Route::get('team chat/fetch-user-list', [MessageController::class, 'fetchUserListView'])->name('team chat.fetch_user_list');
    Route::post('team chat/fetch_messages/{id}', [MessageController::class, 'fetchUserMessages'])->name('team chat.fetch_messages');
    Route::post('team chat/check_messages', [MessageController::class, 'checkNewMessages'])->name('team chat.check_new_message');
    Route::delete('team chat/destroy-chat/{id}', [MessageController::class, 'destroyAll'])->name('team chat.destroy_all');
    Route::resource('team chat', MessageController::class);
    ...
    Route::get('invoices/get-customer-company/{projectID?}', [InvoiceController::class, 'getClientOrCompanyName'])->name('invoices.get_client_company');
    ...
    Route::get('invoices/service / extra-category/{id}', [InvoiceController::class, 'productCategory'])->name('invoices.product_category');
    ...
    Route::group(['prefix' => 'invoices'], function () {
        ...
        Route::get('recurring-invoice/export/{startDate}/{endDate}/{status}/{cleaner}', [RecurringInvoiceController::class, 'export'])->name('recurring_invoice.export');
    ...
    // Quotes
    Route::get('quotes/delete-image', [EstimateController::class, 'deleteEstimateItemImage'])->name('quotes.delete_image');
    Route::get('quotes/download/{id}', [EstimateController::class, 'download'])->name('quotes.download');
    Route::post('quotes/send-quote/{id}', [EstimateController::class, 'sendEstimate'])->name('quotes.send_estimate');
    Route::get('quotes/change-status/{id}', [EstimateController::class, 'changeStatus'])->name('quotes.change_status');
    Route::post('quotes/accept/{id}', [EstimateController::class, 'accept'])->name('quotes.accept');
    Route::post('quotes/decline/{id}', [EstimateController::class, 'decline'])->name('quotes.decline');
    Route::get('quotes/add-item', [EstimateController::class, 'addItem'])->name('quotes.add_item');
    Route::resource('quotes', EstimateController::class);
    ...
    Route::group(['prefix' => 'timelogs'], function () {
        ...
        Route::get('by-cleaner', [TimelogController::class, 'byEmployee'])->name('timelogs.by_employee');
        ...
        Route::get('check-site-time-limit/{projectId}', [TimelogController::class, 'checkProjectTimeLimit'])->name('timelogs.check_project_time_limit');
    ...
    // Service Agreements
    Route::post('service agreements/apply-quick-action', [ContractController::class, 'applyQuickAction'])->name('service agreements.apply_quick_action');
    Route::get('service agreements/download/{id}', [ContractController::class, 'download'])->name('service agreements.download');
    Route::post('service agreements/sign/{id}', [ContractController::class, 'sign'])->name('service agreements.sign');
    ...
    Route::post('service agreements/site-detail/{id}', [ContractController::class, 'projectDetail'])->name('service agreements.project_detail');
    Route::get('service agreements/company-sig/{id}', [ContractController::class, 'companySig'])->name('service agreements.company_sig');


    Route::group(['prefix' => 'service agreements'], function () {
    ...
    Route::resource('service agreements', ContractController::class);
    Route::resource('service agreement-renew', ContractRenewController::class);

    // Service Agreement template
    Route::post('service agreement-template/apply-quick-action', [ContractTemplateController::class, 'applyQuickAction'])->name('contract_template.apply_quick_action');
    Route::resource('service agreement-template', ContractTemplateController::class);
    Route::get('service agreement-template/download/{id}', [ContractTemplateController::class, 'download'])->name('service agreement-template.download');
    ...
    Route::get('attendances/export-all-attendance/{year}/{month}/{id}/{zone}/{role}', [AttendanceController::class, 'exportAllAttendance'])->name('attendances.export_all_attendance');
    Route::post('attendances/cleaner-data', [AttendanceController::class, 'employeeData'])->name('attendances.employee_data');
    ...
    Route::get('shifts/export-all/{year}/{month}/{id}/{zone}/{startDate}/{viewType}', [EmployeeShiftScheduleController::class, 'exportAllShift'])->name('shifts.export_all');

    Route::get('shifts/cleaner-shift-calendar', [EmployeeShiftScheduleController::class, 'employeeShiftCalendar'])->name('shifts.employee_shift_calendar');
    ...
    // Issues / Support
    Route::post('issues / support/apply-quick-action', [TicketController::class, 'applyQuickAction'])->name('issues / support.apply_quick_action');
    Route::post('issues / support/updateOtherData/{id}', [TicketController::class, 'updateOtherData'])->name('issues / support.update_other_data');
    Route::post('issues / support/change-status', [TicketController::class, 'changeStatus'])->name('issues / support.change-status');
    Route::post('issues / support/refreshCount', [TicketController::class, 'refreshCount'])->name('issues / support.refresh_count');
    Route::get('issues / support/agent-group/{id}/{exceptThis?}', [TicketController::class, 'agentGroup'])->name('issues / support.agent_group');
    Route::get('issues / support/edit-details/{id}', [TicketController::class, 'editDetail'])->name('issues / support.edit_detail');
    Route::put('issues / support/update-details/{id}', [TicketController::class, 'updateDetail'])->name('issues / support.update_detail');
    Route::resource('issues / support', TicketController::class);

    // Issue / Support Custom Embed From
    Route::post('issue / support-form/sort-fields', [TicketCustomFormController::class, 'sortFields'])->name('issue / support-form.sort_fields');
    Route::resource('issue / support-form', TicketCustomFormController::class);

    Route::get('issue / support-files/download/{id}', [TicketFileController::class, 'download'])->name('issue / support-files.download');
    Route::resource('issue / support-files', TicketFileController::class);

    Route::resource('issue / support-replies', TicketReplyController::class);
    Route::post('issue / support-replies/edit-note/{id}', [TicketReplyController::class, 'editNote'])->name('issue / support-replies.edit_note');

    Route::post('service job-report-chart', [TaskReportController::class, 'taskChartData'])->name('service job-report.chart');
    Route::get('service job-report/cleaner-wise-service job-report', [TaskReportController::class, 'employeeWiseTaskReport'])->name('cleaner-wise-service job-report');
    Route::get('service job-report/consolidated-service job-report', [TaskReportController::class, 'consolidatedTaskReport'])->name('consolidated-service job-report');

    Route::resource('service job-report', TaskReportController::class);
    ...
    Route::get('time-log-site-wise-report', [TimelogReportController::class, 'projectWiseTimelog'])->name('site-wise-timelog.report');
    Route::get('site-wise-timelog/report/export', [TimelogReportController::class, 'exportProjectWiseTimeLog'])->name('site-wise-timelog.export');
    ...
    Route::get('leave-report/leave-quota/{id}/{year}/{month}', [LeaveReportController::class, 'employeeLeaveQuota'])->name('leave-report.cleaner-leave-quota');
    ...
    Route::get('deal-report/enquiry', [LeadReportController::class, 'leadContact'])->name('enquiry-report.lead_contact');
    Route::get('enquiry-report/total', [LeadReportController::class, 'totalContact'])->name('enquiry-report.total_contact');

    Route::get('deal-report/chart', [LeadReportController::class, 'averageDealSizeReport'])->name('enquiry-report.chart');
    Route::get('deal-report/profile', [LeadReportController::class, 'profile'])->name('enquiry-report.profile');
    Route::get('deal-report/export/{year}/{pipeline}/{category}', [LeadReportController::class, 'exportDealReport'])->name('deal-report.export');



    Route::resource('enquiry-report', LeadReportController::class);
    ...
    Route::get('gdpr/enquiry/approve-reject/{id}/{type}', [GdprSettingsController::class, 'approveRejectLead'])->name('gdpr.enquiry.approve_reject');
    ...
    Route::post('gdpr/update-customer-consent', [GdprController::class, 'updateClientConsent'])->name('gdpr.update_client_consent');
    ...
    // Quote Template
    Route::get('quote-template/add-item', [EstimateTemplateController::class, 'addItem'])->name('quote-template.add_item');
    Route::resource('quote-template', EstimateTemplateController::class);
    Route::get('quotes-template/delete-image', [EstimateTemplateController::class, 'deleteEstimateItemImage'])->name('quote-template.delete_image');
    Route::get('quote-template/download/{id}', [EstimateTemplateController::class, 'download'])->name('quote-template.download');
    ...
    // Quote Request
    Route::post('quote-request/apply-quick-action', [EstimateRequestController::class, 'applyQuickAction'])->name('quote-request.apply_quick_action');
    Route::post('quote-request/change-status/', [EstimateRequestController::class, 'changeStatus'])->name('quote-request.change_status');
    Route::get('quote-request-confirm-rejected/{id}', [EstimateRequestController::class, 'rejectConfirmation'])->name('quote-request.confirm_rejected');
    Route::get('quote-request/send-quote-request', [EstimateRequestController::class, 'sendEstimateRequest'])->name('quote-request.send_estimate_request');
    Route::post('quote-request/send_estimate_mail', [EstimateRequestController::class, 'sendEstimateMail'])->name('quote-request.send_estimate_mail');
    Route::resource('quote-request', EstimateRequestController::class);
