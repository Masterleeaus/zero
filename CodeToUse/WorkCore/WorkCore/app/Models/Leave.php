...
class Leave extends BaseModel
{
    ...
    public function getLeavesTakenCountAttribute()
    {
        ...
        $currentYearJoiningDate = Carbon::parse($user->cleaner[0]->joining_date->format((now(company()->timezone)->year) . '-m-d'));
    ...
    public static function byUserCount($user, $year = null)
    {
        ...
        if ($setting->leaves_start_from == 'joining_date' && isset($user->cleaner[0])) {
            $currentYearJoiningDate = Carbon::parse($user->cleaner[0]->joining_date->format((now(company()->timezone)->year) . '-m-d'));
    ...
    public static function checkOverUtilizationMonthlyLimit($userId, $leaveTypeId, $monthlyLimit, $numberOfLeaves)
    {
        ...
        return ($monthlyLimit > 0 && ($totalLeaves + $numberOfLeaves) > $monthlyLimit) ? true : false;
    }
