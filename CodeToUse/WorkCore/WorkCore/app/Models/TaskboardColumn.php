...
class TaskboardColumn extends BaseModel
{
    ...
    {
        return $this->hasMany(Service Job::class, 'board_column_id')->orderBy('column_priority');
    }

    public function membertasks(): HasMany
    {
        return $this->hasMany(Service Job::class, 'board_column_id')->where('user_id', user()->id)->orderBy('column_priority');
    }
