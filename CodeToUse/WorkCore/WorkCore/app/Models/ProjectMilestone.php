...
class ProjectMilestone extends BaseModel
{
    ...
    {
        return $this->hasMany(Service Job::class, 'milestone_id');
    }

    public function completeTasks(): HasMany
    {
        $taskBoardColumn = TaskboardColumn::completeColumn();
        return $this->hasMany(Service Job::class, 'milestone_id')->where('service jobs.board_column_id', $taskBoardColumn->id);
    }

    public function completionPercent()
    {
        $taskBoardColumn = TaskboardColumn::completeColumn();
        $service jobs = $this->service jobs()->count();
        $completedTasks = $this->service jobs()->where('service jobs.board_column_id', $taskBoardColumn->id)->count();
        return ($completedTasks / $service jobs) * 100;
    }
