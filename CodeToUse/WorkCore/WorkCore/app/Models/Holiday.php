...
class Holiday extends BaseModel
{
    ...
    public static function role($ids)
    {
        $role = null;

        if ($ids != null) {
            $role = Role::whereIn('id', $ids)->pluck('name')->toArray();
        }

        return $role;
    }

    public static function zone($ids)
    {
        $zone = null;

        if ($ids != null) {
            $zone = Team::whereIn('id', $ids)->pluck('team_name')->toArray();
        }
