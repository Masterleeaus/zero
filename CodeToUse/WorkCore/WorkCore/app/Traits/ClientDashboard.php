...
trait ClientDashboard
{
    ...
    public function clientDashboard()
    {
        ...
        $this->totalLead = Enquiry::whereBetween(DB::raw('DATE(`created_at`)'), [$startDate, $endDate])
            ->count();
        ...
        $this->totalContractsGenerated = Service Agreement::whereBetween(DB::raw('DATE(service agreements.`end_date`)'), [$startDate, $endDate])->orWhereBetween(DB::raw('DATE(service agreements.`start_date`)'), [$startDate, $endDate])->count();
        ...
        $this->widgets = DashboardWidget::where('dashboard_type', 'admin-customer-dashboard')->get();
        $this->activeWidgets = $this->widgets->filter(function ($value, $key) {
            return $value->status == '1';
        })->pluck('widget_name')->toArray();

        $this->view = 'dashboard.ajax.customer';
    }

    public function clientEarningChart($startDate, $endDate)
    {
        $payments = Payment::with('site', 'site.customer', 'invoice', 'invoice.customer')
            ->join('currencies', 'currencies.id', '=', 'payments.currency_id')
            ->leftJoin('invoices', 'invoices.id', '=', 'payments.invoice_id')
            ->leftJoin('sites', 'sites.id', '=', 'payments.project_id')
            ->select('payments.amount', 'currencies.id as currency_id', 'payments.exchange_rate', 'sites.client_id', 'invoices.client_id as invoice_client_id', 'payments.invoice_id', 'payments.project_id')
            ->where('payments.status', 'complete');
        $payments = $payments->where(function ($query) {
            $query->whereNotNull('sites.client_id')
        ...
        foreach ($payments as $chart) {
            if (is_null($chart->client_id)) {
                $chartName = $chart->invoice->customer->name;
            }
            else {
                $chartName = $chart->site->customer->name;
            }
    ...
    {
        $allTimelogs = ProjectTimeLog::leftJoin('service jobs', 'service jobs.id', 'project_time_logs.task_id')
            ->leftJoin('sites as proj', 'proj.id', 'project_time_logs.project_id')
            ->leftJoin('sites', 'sites.id', 'service jobs.project_id')
            ->leftJoin('users', 'users.id', 'sites.client_id')
            ->leftJoin('users as customer', 'customer.id', 'proj.client_id')
            ->where('project_time_logs.approved', 1)
            ->whereBetween(DB::raw('DATE(project_time_logs.`created_at`)'), [$startDate, $endDate])
            ->select('project_time_logs.*', 'customer.name')
            ->get();
    ...
    {
        $leadStatus = LeadSource::withCount(['enquiries' => function ($query) use ($startDate, $endDate) {
            return $query->whereBetween(DB::raw('DATE(`created_at`)'), [$startDate, $endDate]);
        }])->get();
