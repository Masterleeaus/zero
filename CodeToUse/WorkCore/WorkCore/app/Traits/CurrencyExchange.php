        ...
        foreach ($currencies as $currency) {
            try {
                ...
                if ($currency->is_cryptocurrency == 'no') {
                    ...
                    $res = $customer->request('GET', $apiUrl . $currency->currency_code . '_' . $baseCurrency->currency_code . '&compact=ultra&apiKey=' . $currencyApiKey);
                }
                else {
                    // Get exchange rate for cryptocurrency
                    $res = $customer->request('GET', $apiUrl . $currency->currency_code . '_USD&compact=ultra&apiKey=' . $currencyApiKey);
                }
