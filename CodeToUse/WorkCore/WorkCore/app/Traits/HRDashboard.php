...
trait HRDashboard
{
    ...
    public function hrDashboard()
    {
        ...
        $this->leavesTaken = User::with('employeeDetail', 'employeeDetail.role')
        ...
        $this->lateAttendanceMarks = User::with('employeeDetail', 'employeeDetail.role')
    ...
    {
        $zones = Team::withCount(['teamMembers' => function ($query) {
            $query->join('users', 'users.id', '=', 'employee_details.user_id');
            $query->where('users.status', '=', 'active');
        }])->get();
        $data['labels'] = $zones->pluck('team_name')->toArray();

        foreach ($data['labels'] as $key => $value) {
            $data['colors'][] = '#' . substr(md5($value), 0, 6);
        }

        $data['values'] = $zones->pluck('team_members_count')->toArray();
    ...
    {
        $zones = Role::withCount(['members' => function ($query) {
            $query->join('users', 'users.id', '=', 'employee_details.user_id');
            $query->where('users.status', '=', 'active');
        }])->get();

        $data['labels'] = $zones->pluck('name')->toArray();

        foreach ($data['labels'] as $key => $value) {
            $data['colors'][] = '#' . substr(md5($value), 0, 6);
        }

        $data['values'] = $zones->pluck('members_count')->toArray();
    ...
    public function roleWiseChart()
    {
        ...
        foreach ($roleWiseChart as $key => $value) {
            if ($value->name == 'admin' || $value->name == 'cleaner') {
    ...
    public function joiningVsAttritionChart()
    {
        ...
        $attritionDates = EmployeeDetails::join('users', 'employee_details.user_id', '=', 'users.id')
            ...
            ->whereDate('last_date', '>=', $startDate)->whereDate('last_date', '<=', $endDate)
