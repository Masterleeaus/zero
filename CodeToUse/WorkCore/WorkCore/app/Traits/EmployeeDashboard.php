...
trait EmployeeDashboard
{
    ...
    public function employeeDashboard()
    {
        ...
        $this->totalProjects = Site::select('sites.id')
            ->where('completion_percent', '<>', 100)
            ->join('project_members', 'project_members.project_id', '=', 'sites.id')
        ...
        $this->counts = User::without(['clientDetails', 'employeeDetail', 'roles', 'session'])->withCount([
            ...
            'service jobs as totalCompletedTasks' => function ($query) use ($completedTaskColumn) {
                $query->where('service jobs.board_column_id', $completedTaskColumn->id)
        ...
        if (!is_null($this->viewNoticePermission) && $this->viewNoticePermission != 'none') {
            if ($this->viewNoticePermission == 'added') {
                ...
                $this->notices = Notice::with('member')->latest()
                    ...
                    ->orWhere(function ($q) {
                        $q->where(['to' => 'cleaner', 'department_id' => null])
                            ->orWhere(['department_id' => $this->user->employeeDetail->department_id]);
                    })
                    ->whereHas('member', function ($query) {
                        $query->where('user_id', auth()->id());
        ...
        $service jobs = $this->pendingTasks = Service Job::with(['activeProject', 'boardColumn', 'labels'])
            ->join('task_users', 'task_users.task_id', '=', 'service jobs.id')
            ->where('task_users.user_id', $this->user->id)
            ->where('service jobs.board_column_id', '<>', $completedTaskColumn->id)
            ->select('service jobs.*')
            ->distinct()
            ->orderBy('service jobs.id', 'desc')
            ->get();

        $this->inProcessTasks = $service jobs->count();

        $this->dueTasks = $service jobs->filter(function ($item) {
            return !is_null($item->due_date) && $item->due_date->endOfDay()->isPast();
        })->count();

        $sites = Site::with('members')
            ->where('completion_percent', '<>', '100')
            ->leftJoin('project_members', 'project_members.project_id', 'sites.id')
            ->leftJoin('users', 'project_members.user_id', 'users.id')
            ->selectRaw('project_members.user_id, sites.deadline as due_date, sites.id')
            ->where('project_members.user_id', $this->user->id)
            ->groupBy('sites.id')
            ->get();

        $sites = $sites->whereNotNull('due_date');

        $this->dueProjects = $sites->filter(function ($value) {
        ...
        $this->myActiveTimer = ProjectTimeLog::with('service job', 'user', 'site', 'breaks', 'activeBreak')
            ->where('user_id', user()->id)
            ->whereNull('end_time')
            ->first();
        ...
        $this->upcomingBirthdays = EmployeeDetails::whereHas('user', function ($query) {
            ...
            ->with('user', 'user.employeeDetail.role:id,name')
        ...
        if (!is_null($this->leadAgent)) {
            ...
            $this->dealFollowUps = DealFollowUp::with('enquiry')->whereHas('enquiry', function ($q) {
        ...
        $this->appreciations = Appreciation::with(['award', 'award.awardIcon'])
            ->with(['awardTo.employeeDetail', 'awardTo.employeeDetail.role:id,name'])
        ...
        $this->cleaners = EmployeeDetails::with('user')
        ...
        if (in_array('admin', user_roles())) {
            ...
            $this->noticePeriod = $cleaners->filter(function ($cleaner) {
                return $cleaner->notice_period_end_date !== null;
            })->sortBy('notice_period_end_date')->values();

            $this->probations = $cleaners->filter(function ($cleaner) {
                return $cleaner->probation_end_date !== null;
            })->sortBy('probation_end_date')->values();

            $this->internships = $cleaners->filter(function ($cleaner) {
                return $cleaner->internship_end_date !== null;
            })->sortBy('internship_end_date')->values();

            $this->service agreements = $cleaners->filter(function ($cleaner) {
                return $cleaner->contract_end_date !== null;
            })->sortBy('contract_end_date')->values();
        } else {
            $userId = user()->id;
            $cleaner = $this->cleaners->clone()
            ...
            $this->noticePeriod = $cleaner && $cleaner->notice_period_end_date && $cleaner->notice_period_end_date >= $currentDay ? $cleaner : null;

            $this->probation = $cleaner && $cleaner->probation_end_date && $cleaner->probation_end_date >= $currentDay ? $cleaner : null;

            $this->internship = $cleaner && $cleaner->internship_end_date && $cleaner->internship_end_date >= $currentDay ? $cleaner : null;

            $this->service agreement = $cleaner && $cleaner->contract_end_date && $cleaner->contract_end_date >= $currentDay ? $cleaner : null;
        }



        return view('dashboard.cleaner.index', $this->data);
    }
    ...
    public function clockInModal()
    {
        ...
        return view('dashboard.cleaner.clock_in_modal', $this->data);
    }
    ...
    public function storeClockIn(ClockInRequest $request)
    {
        ...
        if ($request->work_from_type !== 'home') {
            ...
            if (attendance_setting()->ip_check == 'yes') {
                ...
                if (!in_array($request->ip(), $ips)) {
                    return Reply::error(__('team chat.notAnAuthorisedDevice'));
                }
            }
            ...
            if (attendance_setting()->radius_check == 'yes') {
                ...
                if (attendance_setting()->auto_clock_in_location !== 'home') {
                    if (!$checkRadius) {
                        return Reply::error(__('team chat.notAnValidLocation'));
                    }
                }
            }
        }
        ...
        if ($clockInCount < $this->attendanceSettings->clockin_in_day) {
            ...
            return Reply::successWithData(__('team chat.attendanceSaveSuccess'), ['time' => $now->format('h:i A'), 'ip' => $attendance->clock_in_ip, 'working_from' => $attendance->working_from]);
        }
    ...
    public function showClockedHours()
    {
        ...
        return view('dashboard.cleaner.show_clocked_hours', $this->data);
    }
}
