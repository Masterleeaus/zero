...
trait ProjectDashboard
{
    ...
    public function projectDashboard()
    {
        ...
        $this->totalProject = Site::whereBetween(DB::raw('DATE(`start_date`)'), [$startDate, $endDate])->count();
        ...
        if ($todayDate >= $startDate && $todayDate <= $endDate) {
            $this->totalOverdueProject = Site::whereNotNull('deadline')
                ->whereRaw('Date(sites.deadline) >= ?', [$startDate])
                ->whereRaw('Date(sites.deadline) < ?', [$todayDate])->count();
        }else{
            $this->totalOverdueProject = Site::whereNotNull('deadline')->whereBetween(DB::raw('DATE(`deadline`)'), [$startDate, $endDate])->count();
        }

        $this->widgets = DashboardWidget::where('dashboard_type', 'admin-site-dashboard')->get();
    ...
    public function statusChartData($startDate, $endDate)
    {
        ...
        foreach ($labels as $label) {
            $data['values'][] = Site::whereBetween(DB::raw('DATE(`created_at`)'), [$startDate, $endDate])->where('status', $label)->count();
        }
