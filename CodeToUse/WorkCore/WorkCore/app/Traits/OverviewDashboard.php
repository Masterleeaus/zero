...
trait OverviewDashboard
{
    ...
    public function overviewDashboard()
    {
        ...
        $this->counts = DB::table('users')
            ->select(
                DB::raw('(select count(users.id) from `users` inner join role_user on role_user.user_id=users.id inner join roles on roles.id=role_user.role_id WHERE roles.name = "customer" AND users.company_id = ' . company()->id . ') as totalClients'),
                DB::raw('(select count(users.id) from `users` inner join role_user on role_user.user_id=users.id inner join roles on roles.id=role_user.role_id WHERE roles.name = "cleaner" and users.status = "active" AND users.company_id = ' . company()->id . ') as totalEmployees'),
                DB::raw('(select count(sites.id) from `sites` WHERE sites.company_id = ' . company()->id . ') as totalProjects'),
                ...
                DB::raw('(select count(service jobs.id) from `service jobs` where service jobs.board_column_id=' . $completedTaskColumn->id . ' and is_private = "0" AND service jobs.company_id = ' . company()->id . ') as totalCompletedTasks'),
                DB::raw('(select count(service jobs.id) from `service jobs` where service jobs.board_column_id != ' . $completedTaskColumn->id . ' and is_private = "0" and service jobs.deleted_at IS NULL AND service jobs.company_id = ' . company()->id . ') as totalPendingTasks'),
                DB::raw('(select count(distinct(attendances.user_id)) from `attendances` inner join users as atd_user on atd_user.id=attendances.user_id inner join role_user on role_user.user_id=atd_user.id inner join roles on roles.id=role_user.role_id WHERE roles.name = "cleaner" and attendances.clock_in_time >= "' . today(company()->timezone)->setTimezone('UTC')->toDateTimeString() . '" and atd_user.status = "active" AND attendances.company_id = ' . company()->id . ') as totalTodayAttendance'),
        ...
        $this->pendingTasks = Service Job::with('site', 'users', 'boardColumn')
            ->where('service jobs.board_column_id', '<>', $completedTaskColumn->id)
            ->where('service jobs.is_private', 0)
        ...
        $this->pendingLeadFollowUps = Deal::with('followup', 'leadAgent', 'leadAgent.user', 'leadAgent.user.employeeDetail', 'leadAgent.user.employeeDetail.role')
            ->selectRaw('deals.id,enquiries.company_name, enquiries.client_name as client_name, deals.agent_id, ( select lead_follow_up.next_follow_up_date from lead_follow_up where lead_follow_up.deal_id = deals.id and DATE(lead_follow_up.next_follow_up_date) < "' . $currentDate . '" ORDER BY lead_follow_up.created_at DESC Limit 1) as follow_up_date_past,
        ...
        $this->projectActivities = ProjectActivity::with('site')
            ->join('sites', 'sites.id', '=', 'project_activity.project_id')
        ...
        // Document Expiries for current year - All cleaners
