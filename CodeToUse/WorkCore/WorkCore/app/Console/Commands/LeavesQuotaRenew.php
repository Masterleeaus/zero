...
class LeavesQuotaRenew extends Command
{
     ...
     * @var string
     */
    protected $description = 'Renew leaves quota for all cleaners';
    ...
    public function renewLeavesQuotaForCompany(Company $company)
    {
        ...
        $cleaners = User::withoutGlobalScopes()
        ...
        foreach ($cleaners as $cleaner) {
            $this->renewLeavesQuota($cleaner);
        }
    }

    /**
     * Renew leaves quota for the cleaner
     */
    public function renewLeavesQuota(User $cleaner)
    {
        $employeeLeaveTypes = $cleaner->leaveTypes;
        ...
        foreach ($employeeLeaveTypes as $employeeLeaveType) {
            ...
            $this->info('Renewing leaves quota for ' . $cleaner->name . ' for ' . $leaveType->type_name . ' with ' . $noOfLeaves . ' leaves');
