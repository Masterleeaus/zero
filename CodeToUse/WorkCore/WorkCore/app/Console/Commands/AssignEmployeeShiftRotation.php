...
class AssignEmployeeShiftRotation extends Command
{
     ...
     * @var string
     */
    protected $signature = 'assign-cleaner-shift-rotation {--rotation_ids=*}';
     ...
     * @var string
     */
    protected $description = 'Assign shift rotation to cleaners';
    ...
    {
        EmployeeShiftScheduleObserver::$isShiftRotation = true;

        $rotationIds = $this->option('rotation_ids');
        ...
        Company::active()->select('id')->chunk(50, function ($companies) use ($currentDate, $dayOfWeekMap, $dayNameWeekMap, $rotationIds) {
            foreach ($companies as $company) {

                $dayOff = EmployeeShift::where('company_id', $company->id)->where('shift_name', 'Day Off')->first();
                $automateShifts = AutomateShift::whereHas('rotation', function ($query) use ($company, $rotationIds) {
                ...
                foreach ($automateShifts as $automateShift) {
                    if ($automateShift->rotation->status == 'active'){
                        ...
                        if ($nextShiftSequence && $sequences->isNotEmpty()) {
                            ...
                            $employeeData = User::with('employeeDetail')->find($automateShift->user->id);
                            ...
                            $holidaysForUser = $this->retrieveHolidaysForUser($employeeData);
                            ...
                            $scheduleOn = $automateShift->rotation->schedule_on;
                            $previousScheduleDate = $this->getPreviousScheduleDate($scheduleOn, $dayNameWeekMap);

                            // Weekly
                            if ($isWeekly) {
                                $reqFrom = 'weekly';
                                $currentDates = $this->getOfficeOpenDates($reqFrom, $dayOfWeekMap, $officeOpenDays, $holidaysForUser, $previousScheduleDate);
                                $weekOffDays = $this->getWeekendDays($reqFrom, $dayOfWeekMap, $officeOpenDays, $previousScheduleDate);
                            }
                            ...
                            if (!empty($currentDates) && (($bothWeekly ) || ($isMonthly))) {
                                foreach ($currentDates as $date) {
                                    $date = Carbon::parse($date);
                                    if ($date->greaterThanOrEqualTo($employeeData->employeeDetail->joining_date) &&
                                        (is_null($officeOpenDays) || (is_array($officeOpenDays) && in_array($date->dayOfWeek, $officeOpenDays)))) {
    ...
    private function getFullDateForCurrentMonth($dayOfMonth)
    {
        ...
        return $fullDate->format('Y-m-d'); // Return the formatted date
    }
    ...
    private function getWeekendDays($reqFrom, $dayOfWeekMap, $officeOpenDays, $rotationDate = null)
    {
        ...
        $weekendDates = [];
        $officeDays = [];
        $rotationDate = $rotationDate ? Carbon::parse($rotationDate) : null;

        $startDate = $today->copy();
        $endDate = match ($reqFrom) {
            'bi-weekly' => $rotationDate->copy()->addDays(14),
            default => $rotationDate->copy()->addDays(7),
        };
        if ($reqFrom == 'monthly') {
            $currentDate = $today;
            ...
            if ($daysInNextMonth < 30) {
                ...
                if ($daysInNextMonth == 28) {
                    ...
                    $endDate = $rotationDate->copy()->addDays($daysInNextMonth);
                    $endDate = $endDate->addDay(1);
                }
            }
                ...
                if ($currentDate->daysInMonth == 29) {
                    ...
                    $endDate = $rotationDate->copy()->addDays($daysInNextMonth)->subDays($subDays);
                    $endDate = $endDate->addDay(1);
                }
                ...
                $endDate = $rotationDate->copy()->addMonth()->subDay(); // End date is a day before the same date of next month
                $endDate = $endDate->addDay(1);
            }
        }
    ...
    private function getOfficeOpenDates($reqFrom, $dayOfWeekMap, $officeOpenDays, $holidaysForUser, $rotationDate = null)
    {
        ...
        $rotationDate = Carbon::parse($rotationDate);

        if ($reqFrom === 'bi-weekly') {
            $endDate = $rotationDate->copy()->addDays(14); // End date after 14 days from today
        }
        elseif ($reqFrom === 'monthly') {
            ...
            $endDate = $rotationDate->copy()->addDays(7); // End date after 7 days from today
        }
    ...
    private function getPreviousScheduleDate($scheduleOn, $dayNameWeekMap)
    {
        $dayName = strtolower(str_replace('every-', '', $scheduleOn));
        if (isset($dayNameWeekMap[$dayName])) {
            $dayOfWeek = $dayNameWeekMap[$dayName];
            $today = Carbon::now();
            if ($today->dayOfWeek == $dayOfWeek) {
                return $today;
            }
    
            return $today->previous($dayOfWeek);
        }
