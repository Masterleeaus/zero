...
class UpdateExchangeRates extends Command
{
    ...
    public function handle()
    {
        ...
        if ($currencyApiKey && $currencyApiKeyVersion) {
            ...
            try {
                $response = $customer->request('GET', 'https://' . $currencyApiKeyVersion . '.currconv.com/api/v7/convert?q=USD_USD&compact=ultra&apiKey=' . $currencyApiKey);
            ...
            Company::with(['currencies', 'currency'])
                ->chunk(50, function ($companies) use ($currencyApiKey, $currencyApiKeyVersion, $customer, $progressBar, &$exchangeRateCache) {
                    foreach ($companies as $company) {
                        $company->currencies->each(function ($currency) use ($currencyApiKey, $currencyApiKeyVersion, $company, $customer, &$exchangeRateCache) {
                            ...
                            try {
                                ...
                                if (!isset($exchangeRateCache[$conversionKey])) {
                                    $response = $customer->request('GET', 'https://' . $currencyApiKeyVersion . '.currconv.com/api/v7/convert?q=' . $conversionKey . '&compact=ultra&apiKey=' . $currencyApiKey);
                                    $response = json_decode($response->getBody(), true);
