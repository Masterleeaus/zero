...
class SendAutoFollowUpReminder extends Command
{
     ...
     * @var string
     */
    protected $description = 'Send notification of followup to cleaner or added by user';
    ...
    {
        $followups = DealFollowUp::with('enquiry', 'enquiry.leadAgent', 'enquiry.leadAgent.user')
            ->where('next_follow_up_date', '>=', now($company->timezone))
            ->whereHas('enquiry', function ($query) use ($company) {
        ...
        $dealFollowUps = DealFollowUp::with('enquiry', 'enquiry.leadAgent', 'enquiry.leadAgent.user')
            ->whereHas('enquiry', function ($query) use ($company) {
                $query->where('company_id', $company->id);
            })
