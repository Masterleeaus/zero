...
class SendTaskReminder extends Command
{
     ...
     * @var string
     */
    protected $signature = 'send-service job-reminder';
     ...
     * @var string
     */
    protected $description = 'Send service job reminders';
    ...
    /**
     * Send service job reminders for the given date and company.
     *
    ...
    private function sendReminders(string $dueDate, Company $company)
    {
        ...
        // Get all service jobs for the given date and company that are not in the "completed" column
        $service jobs = Service Job::where('due_date', $dueDate)
            ->where('company_id', $company->id)
            ->where('board_column_id', '<>', $completedTaskColumn->id)
            ->get();

        // Loop through each service job
        foreach ($service jobs as $service job) {
            // Fire a TaskReminderEvent for the service job
