...
class SendAutoTaskReminder extends Command
{
     ...
     * @var string
     */
    protected $signature = 'send-auto-service job-reminder';
        ...
        Company::active()->select(['id', 'before_days', 'after_days', 'timezone'])->chunk(50, function ($companies) {
            ...
            foreach ($companies as $company) {
                ...
                if ($company->before_days > 0) {
                    ...
                    $service jobs = Service Job::where('due_date', $beforeDeadline)
                        ->where('company_id', $company->id)
                        ->where('board_column_id', '<>', $completedTaskColumn->id)
                        ->get();

                    foreach ($service jobs as $service job) {
                        event(new AutoTaskReminderEvent($service job));
                    }
                }

                if ($company->after_days > 0) {
                    $afterDeadline = $now->clone()->addDays($company->after_days)->format('Y-m-d');
                    $service jobs = Service Job::where('due_date', $afterDeadline)
                        ->where('company_id', $company->id)
                        ->where('board_column_id', '<>', $completedTaskColumn->id)
                        ->get();
