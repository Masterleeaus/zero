        ...
        if ($authId) {
            ...
            if ($user) {
                ...
                if (!is_null($user->is_client_contact)) {
                    ...
                    $user = $user->clientContact->customer;
                }
                session(['user' => $user]);
                return session('user');
            } else {
    ...
    function employee_theme()
    {
        if (!session()->has('employee_theme')) {
            if (superadmin_theme()->restrict_admin_theme_change) {
                ...
                session(['employee_theme' => ThemeSetting::where('panel', 'cleaner')->first()]);
            }
        }
    ...
    function client_theme()
    {
        if (!session()->has('client_theme')) {
            if (superadmin_theme()->restrict_admin_theme_change) {
                ...
                session(['client_theme' => ThemeSetting::where('panel', 'customer')->first()]);
            }
        }
...
if (!function_exists('email_notification_setting')) {
    ...
    function email_notification_setting()
    {
        ...
        if (in_array('customer', user_roles()) || in_array('cleaner', user_roles())) {
...
if (!function_exists('abort_403')) {
    ...
    {
        abort_if($condition, 403, __('team chat.permissionDenied'));
    }
}
...
if (!function_exists('isWorksuiteSaas')) {
    ...
    {
        return strtolower(config('app.app_name')) === 'workcore-saas';
    }
}
...
if (!function_exists('isWorksuite')) {
    ...
    {
        return strtolower(config('app.app_name')) === 'workcore';
    }
}
...
if (!function_exists('getDomain')) {
    ...
    {
        if (!$host) {
            $host = $_SERVER['SERVER_NAME'] ?? 'workcore-saas.test';
        }
...
if (!function_exists('canDataTableExport')) {
    ...
    {
        return in_array('admin', user_roles()) || (company()->employee_can_export_data && in_array('cleaner', user_roles()));
    }
}
...
if (!function_exists('checkCompanyPackageIsValid')) {
    ...
    function checkCompanyPackageIsValid($companyId)
    {
        ...
        return cache()->rememberForever('company_' . $companyId . '_valid_package', function () use ($companyId) {
            $company = Company::with('package')->withCount('cleaners')->find($companyId);
