    ...
    public function created(Holiday $holiday)
    {
        if (request()->notification_sent == 'yes') {
            $users = User::join('employee_details', 'employee_details.user_id', '=', 'users.id')
                ...
                ->select('users.id', 'users.company_id', 'users.name', 'users.email', 'users.created_at', 'users.image', 'roles.name as designation_name', 'users.email_notifications', 'users.mobile', 'users.country_id', 'users.status');
