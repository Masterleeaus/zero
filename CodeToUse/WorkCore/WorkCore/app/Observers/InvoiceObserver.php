...
class InvoiceObserver
{
    ...
    public function creating(Invoice $invoice)
    {
        ...
        if (!isRunningInConsoleOrSeeding()) {
            ...
            if (!is_null($invoice->estimate_id)) {
                $quote = Quote::findOrFail($invoice->estimate_id);
            ...
            /* If it is a order invoice, then send_status will be always 1 so that it could be visible to customers */
    ...
    public function created(Invoice $invoice)
    {
        if (!isRunningInConsoleOrSeeding()) {
                    ...
                    if (!is_null($item)) {
                        $invoiceItem = InvoiceItems::create(
                            [
                                ...
                                'product_id' => (isset($service / extra[$key])) ? $service / extra[$key] : null,
            ...
            if (($invoice->site && $invoice->site->client_id != null) || $invoice->client_id != null) {
                $clientId = ($invoice->site && $invoice->site->client_id != null) ? $invoice->site->client_id : $invoice->client_id;
                // Notify customer
    ...
    public function updated(Invoice $invoice)
    {
        if (!isRunningInConsoleOrSeeding()) {
            ...
            */
            ...
            if (empty($item_ids)) {
                $item_ids = InvoiceItems::where('invoice_id', $invoice->id)->pluck('id')->toArray();
            }
    ...
    protected function googleCalendarEvent($event)
    {
        ...
        if ($company->google_calendar_status == 'active' && $googleAccount->google_calendar_verification_status == 'verified' && $googleAccount->token && $module->invoice_status == 1) {
            ...
            $description = __('team chat.invoiceDueOn');
