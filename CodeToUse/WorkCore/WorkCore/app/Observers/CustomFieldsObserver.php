...
class CustomFieldsObserver
{
    ...
    {
        $this->enquiry($customField);
        $this->ticket($customField);
    }

    private function enquiry($customField)
    {
        $enquiry = CustomFieldGroup::where('name', 'Enquiry')->first();

        if ($customField->custom_field_group_id != $enquiry->id) {
            return false;
        }
