...
class EmployeeShiftScheduleObserver
{
    ...
    public function created(EmployeeShiftSchedule $employeeShiftSchedule)
    {
        ...
        if (request()->hasFile('file')) {
            Files::deleteFile(request()->file, 'cleaner-shift-file/' . $employeeShiftSchedule->id);
            $fileName = Files::uploadLocalOrS3(request()->file, 'cleaner-shift-file/' . $employeeShiftSchedule->id);
    ...
    public function deleting(EmployeeShiftSchedule $employeeShiftSchedule)
    {
        if ($employeeShiftSchedule->file) {
            Files::deleteFile($employeeShiftSchedule->file, 'cleaner-shift-file/' . $employeeShiftSchedule->id);
            Files::deleteDirectory('cleaner-shift-file/' . $employeeShiftSchedule->id);
        }
    }
