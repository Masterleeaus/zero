        ...
        if ($message != '') {
            if (count($ticketReply->ticket->reply) > 1) {
                ...
                if (!is_null($ticketReply->ticket->agent)) {
                    ...
                    // Don't notify the customer if they are the one who replied
                    if ($ticketReply->type != 'note' && $ticketReply->ticket->customer->id != $ticketReply->user_id) {
                        event(new TicketReplyEvent($ticketReply, $ticketReply->ticket->customer, null));
                    }
                }
                    ...
                    // Don't notify the customer if they are the one who replied
                    if ($ticketReply->ticket->customer->id != $ticketReply->user_id) {
                        event(new TicketReplyEvent($ticketReply, $ticketReply->ticket->customer, null));
                    }
                }
                else {
