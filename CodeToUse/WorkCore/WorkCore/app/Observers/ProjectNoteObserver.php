...
class ProjectNoteObserver
{
    ...
    public function created(ProjectNote $projectNote)
    {
        ...
        if (request()->mention_user_id != null && request()->mention_user_id != '') {
            ...
            if ($mentionUserId != null && $mentionUserId != '') {
                ...
                event(new ProjectNoteMentionEvent($site, $projectNote->created_at, $mentionUserId));
            ...
            if ($unmentionIds != null && $unmentionIds != '') {
                ...
                event(new ProjectNoteEvent($site, $projectNote->created_at, $projectNoteUsers));
            ...
            if ($projectNote->type == 0) {
                event(new ProjectNoteEvent($site, $projectNote->created_at, $projectNote->site->projectMembers));
            } else {
    ...
    public function updating(ProjectNote $projectNote)
    {
        ...
        if (!empty($newMention) && $projectNote->type == '1') {
            event(new ProjectNoteMentionEvent($site, $projectNote->created_at, $newMention));
        }
        ...
        if (!empty($changes)) {
            ...
            if ($projectNote->type == 0) {
                // Public note - notify all site members
                $notifyUsers = $site->projectMembers;
            } else {
            ...
            if ($notifyUsers->isNotEmpty()) {
                event(new ProjectNoteUpdateEvent($site, $projectNote, $notifyUsers));
            }
        }
    }
