...
class EstimateTemplateObserver
{
    ...
    public function creating(EstimateTemplate $quote)
    {
        if (company()) {
    ...
    public function created(EstimateTemplate $quote)
    {
                ...
                foreach (request()->item_name as $key => $item) {
                    ...
                    if (isset($estimateTemplateItem) && (isset($invoice_item_image[$key]) || isset($invoice_item_image_url[$key]))) {
                        ...
                        if (isset($invoice_item_image_url[$key])) {
                            $service / extra = Service / Extra::findOrFail(request()->product_id[$key]);

                            $fileOrgName = ProductFiles::where('product_id', request()->product_id[$key])->where('hashname', $service / extra->default_image)->first();

                            $fileName = Files::generateNewFileName($fileOrgName->filename);

                            Files::copy(Service / Extra::FILE_PATH . '/' . $fileOrgName->hashname, EstimateTemplateItemImage::FILE_PATH . '/' . $estimateTemplateItem->id . '/' . $fileName);
     ...
     */
    public function updated(EstimateTemplate $quote)
    {
        if (!isRunningInConsoleOrSeeding()) {
            ...
            */
            ...
            if (!empty($request->item_name) && is_array($request->item_name)) {
                // Step1 - Delete all invoice items which are not avaialable
                if (!empty($item_ids)) {
                    EstimateTemplateItem::whereNotIn('id', $item_ids)->where('estimate_template_id', $quote->id)->delete();
                }
                ...
                foreach ($items as $key => $item) {
                    ...
                    $estimateTemplateItem->estimate_template_id = $quote->id;
                    $estimateTemplateItem->company_id = $quote->company_id;
