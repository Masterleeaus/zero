...
class CompanyObserver
{
    ...
    public function leaveType($company)
    {
        ...
        $roles = Role::where('name', '<>', 'customer')->where('company_id', $company->id)->pluck('id')->toArray();
    ...
    public function roles($company): void
    {
        ...
        $employeeRole->name = 'cleaner';
        $employeeRole->company_id = $company->id;
        $employeeRole->display_name = 'Cleaner'; // optional
        $employeeRole->description = 'Cleaner can see service jobs and sites assigned to him.'; // optional
        $employeeRole->saveQuietly();

        $clientRole = new Role();
        $clientRole->name = 'customer';
        $clientRole->company_id = $company->id;
        $clientRole->display_name = 'Customer'; // optional
        $clientRole->description = 'Customer can see own service jobs and sites.'; // optional
        ...
        $rolePermissionController->permissionRole($allPermissions, 'cleaner', $company->id);
        $rolePermissionController->rolePermissionInsert($allPermissions, $adminRole->id, 'all');
        $rolePermissionController->permissionRole($allPermissions, 'customer', $company->id);
    }
    ...
    public function themeSetting($company): void
    {
        ...
        $themeSettings = [
            ...
            ['panel' => 'cleaner', 'company_id' => $company->id, 'header_color' => $headerColor, 'sidebar_color' => $sidebarColor, 'sidebar_text_color' => $sidebarTextColor, 'link_color' => $linkColor],
            ['panel' => 'customer', 'company_id' => $company->id, 'header_color' => $headerColor, 'sidebar_color' => $sidebarColor, 'sidebar_text_color' => $sidebarTextColor, 'link_color' => $linkColor],
        ];
