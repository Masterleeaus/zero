...
class ProjectTimelogObserver
{
    ...
    public function saving(ProjectTimeLog $projectTimeLog)
    {
        ...
        if (!isRunningInConsoleOrSeeding()) {
            ...
            if ($projectId != '') {
                if($projectTimeLog->site->public == 1){
                ...
                $service job = $projectTimeLog->service job;

                if (!is_null($service job) && !is_null($service job->project_id)) {
                    $projectId = $service job->project_id;
                    $projectTimeLog->project_id = $service job->project_id;
                }
            ...
            if ($urlDuplicateTask && $projectId != '') {
                $projectTimeLog->project_id = $projectTimeLog->service job->project_id;
            }
