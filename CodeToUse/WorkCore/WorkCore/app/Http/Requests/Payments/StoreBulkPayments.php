...
class StoreBulkPayments extends FormRequest
{
    ...
    public function team chat()
    {
        ...
        foreach ($invoiceIds as $index => $invoiceId) {
            ...
            if ($gateway != 'all' && $gateway == 'Offline' && $offline_method_id == null)
            {
                $message['payment_date.'.$index.''] = __('team chat.invoiceDateError');
                $message['offline_methods.'.$index.''] = __('team chat.selectOfflineMethod');
                $message['amount.'.$index.''] = __('team chat.invoicePaymentError');
            }
            elseif ($gateway != 'all' && $gateway != 'Offline') {
                $message['payment_date.'.$index.''] = __('team chat.invoiceDateError');
                $message['amount.'.$index.''] = __('team chat.invoicePaymentError');
            ...
            if ($gateway == 'all' && (!is_null($amount) || $amount != null)) {
                $message['gateway.'.$index.''] = __('team chat.selectGateway');
            }
