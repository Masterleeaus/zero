...
class RenewRequest extends CoreRequest
{
    ...
    public function rules()
    {
        ...
        if (request()->has('contract_id')) {
            $service agreement = Service Agreement::findOrFail(request()->contract_id);
            $startDate = $service agreement->end_date ? $service agreement->end_date->format($setting->date_format) : $service agreement->start_date->format($setting->date_format);
