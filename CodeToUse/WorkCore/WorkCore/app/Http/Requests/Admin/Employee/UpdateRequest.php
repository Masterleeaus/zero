...
namespace App\Http\Requests\Admin\Cleaner;
...
class UpdateRequest extends CoreRequest
{
    ...
    {
        $detailID = EmployeeDetails::where('user_id', $this->route('cleaner'))->first();
        $setting = company();
        $rules = [
        ...
        if (isWorksuite()) {
            $rules['email'] = 'required|max:100|email:rfc,strict|unique:users,email,'.$this->route('cleaner').',id,company_id,' . company()->id;
        }
