...
class UpdateProject extends CoreRequest
{
    ...
    public function rules()
    {
        $rules = [
            ...
            'project_code' => $this->project_code != '' ? 'unique:sites,project_short_code,' . $this->project_id . ',id,company_id,' . company()->id : '',
        ];
        ...
        if (request()->private && in_array('cleaner', user_roles()))  {
            $rules['user_id.0'] = 'required';
        }

        if (!request()->has('private') && $site->public == 0 && !request()->has('public')) {
    ...
    {
        return [
            'user_id.0.required' => __('team chat.atleastOneValidation'),
            'project_code.required' => __('team chat.projectCodeRequired'),
            'member_id.0.required' => __('team chat.atleastOneValidation')
        ];
    }
