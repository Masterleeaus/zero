...
class CreditNoteController extends AccountBaseController
{
    ...
    public function create()
    {
        ...
        $this->creditNote = Invoice::with(['items', 'site', 'customer'])->findOrFail($id);
    ...
    {
        $this->invoiceSetting = invoice_setting();
        $this->creditNote = CreditNotes::with('customer')->findOrFail($id);
        App::setLocale($this->invoiceSetting->locale ?? 'en');
        Carbon::setLocale($this->invoiceSetting->locale ?? 'en');
        ...
        foreach ($items as $item) {
            ...
            foreach (json_decode($item->taxes) as $tax) {
                ...
                if ($this->creditNote->calculate_tax == 'after_discount' && $this->discount > 0) {
                    $taxAmount = ($item->amount - ($item->amount / $this->creditNote->sub_total) * $this->discount) * ($this->tax->rate_percent / 100);
                } else {
    ...
    public function update(UpdateCreditNote $request, $id)
    {
        ...
        return Reply::redirect(route('creditnotes.index'), __('team chat.updateSuccess'));
    }
    ...
    public function show($id)
    {
        ...
        abort_403(!((
            ...
            || ($this->viewPermission == 'both' && ($this->creditNote->client_id == $userId || $this->creditNote->invoice->added_by == $userId))
        ));
    ...
    public function destroy($id)
    {
        ...
        if ($firstCreditNote->id == $id) {
            ...
            return Reply::error(__('team chat.creditNoteCanNotDeleted'));
        }
    ...
    /**
     * @param mixed $id
     * @return \Illuminate\Service Agreements\Foundation\Application|\Illuminate\Service Agreements\View\Factory|\Illuminate\Service Agreements\View\View|mixed
     */
    ...
    public function applyInvoiceCredit(Request $request, $id)
    {
        ...
        if ($totalCreditAmount == 0) {
            return Reply::error(__('team chat.pleaseEnterCreditAmount'));
        }
        ...
        return Reply::redirect(route('creditnotes.show', $creditNote->id), __('team chat.creditNoteAppliedSuccessfully'));
    }
    ...
    public function deleteCreditedInvoice(Request $request, $id)
    {
        ...
        if ($this->payments->count() > 0) {
            ...
            return Reply::successWithData(__('team chat.deleteSuccess'), ['view' => $view, 'remainingAmount' => number_format((float)$this->creditNote->creditAmountRemaining(), 2, '.', '')]);
        }

        return Reply::redirect(route('creditnotes.show', [$this->creditNote->id]), __('team chat.deleteSuccess'));
    }
