...
use App\Http\Requests\Service Jobs\StoreTaskComment;
...
class TaskCommentController extends AccountBaseController
{
    ...
    public function store(StoreTaskComment $request)
    {
        ...
        $this->comments = TaskComment::with('user', 'service job', 'like', 'dislike', 'likeUsers', 'dislikeUsers')->where('task_id', $request->taskId)->orderByDesc('id')->get();

        $view = view('service jobs.comments.show', $this->data)->render();
    ...
    public function destroy($id)
    {
        ...
        $this->comments = TaskComment::with('user', 'service job', 'like', 'dislike', 'likeUsers', 'dislikeUsers')->where('task_id', $comment_task_id)->orderByDesc('id')->get();
    ...
    {
        $this->comment = TaskComment::with('user', 'service job')->findOrFail($id);
        $taskuserData = [];
        $usersData = $this->comment->service job->users;
        $userId = UserService::getUserId();
        ...
        return view('service jobs.comments.edit', $this->data);
    ...
    public function update(StoreTaskComment $request, $id)
    {
        ...
        $this->comments = TaskComment::with('user', 'service job', 'like', 'dislike', 'likeUsers', 'dislikeUsers')->where('task_id', $comment->task_id)->orderByDesc('id')->get();
    ...
    public function saveCommentLike(Request $request)
    {
        ...
        $view = view('service jobs.comments.comment-emoji', $this->data)->render();
        return Reply::dataOnly(['status' => 'success', 'view' => $view]);
    }
