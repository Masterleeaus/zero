...
use App\Http\Requests\Admin\Cleaner\StoreRequest;
use App\Http\Requests\Admin\Cleaner\UpdateRequest;
...
class EmployeeController extends AccountBaseController
{
    ...
    {
        parent::__construct();
        $this->pageTitle = 'app.menu.cleaners';
    ...
    public function index(EmployeesDataTable $dataTable)
    {
        ...
        if (!request()->ajax()) {
            ...
            $this->totalEmployees = count($this->cleaners);
            $this->roles = Role::where('name', '<>', 'customer')->orderBy('id')->get();
        }

        return $dataTable->render('cleaners.index', $this->data);
    }
    ...
    public function create()
    {
        ...
        if (in_array('admin', $userRoles)) {
            ...
            $this->roles = Role::whereNotIn('name', ['admin', 'customer'])->get();
        }

        $cleaner = new EmployeeDetails();
        $getCustomFieldGroupsWithFields = $cleaner->getCustomFieldGroupsWithFields();

        if ($getCustomFieldGroupsWithFields) {
            $this->fields = $getCustomFieldGroupsWithFields->fields;
        }

        $this->view = 'cleaners.ajax.create';
        ...
        return view('cleaners.create', $this->data);
    ...
    public function getExitDateMessage(Request $request)
    {
        ...
        if ($lastDate < $today) {
            $message = __('team chat.exitDateErrorForPast', ['date' => $tomorrow->format(company()->date_format)]);
            $showMessage = true;
        } else {
            $message = __('team chat.exitDateErrorForFuture', ['date' => $lastDate->format(company()->date_format)]);
    ...
    public function store(StoreRequest $request)
    {
        ...
        if (!is_null($company->cleaners) && $company->cleaners->count() >= $company->package->max_employees) {
        ...
        try {
            ...
            if ($user->id) {
                ...
                $this->employeeData($request, $cleaner);
                $cleaner->save();

                // To add custom fields data
                if ($request->custom_fields_data) {
                    $cleaner->updateCustomFieldData($request->custom_fields_data);
                }
            }
            ...
            return Reply::error('Please configure SMTP details to add cleaner. Visit Settings -> notification setting to set smtp ' . $e->getMessage(), 'smtp_error');
        } catch (\Exception $e) {
            logger($e->getMessage());
            // Rollback Transaction
            DB::rollback();
        ...
        return Reply::successWithData(__('team chat.recordSaved'), ['redirectUrl' => route('cleaners.index')]);
    }
    ...
    public function applyQuickAction(Request $request)
    {
        ...
        case 'change-status':
            $company = Company::with(['package', 'cleaners'])->where('id', user()->company_id)->first();

            $updateIds = explode(',', str_replace('on,', '', $request->row_ids));

            if ($request->status == 'active') {
                // Count only active cleaners for package limit check
    ...
    private function deleteEmployee(User $user)
    {
        ...
        $universalSearches = UniversalSearch::where('searchable_id', $user->id)->where('module_type', 'cleaner')->get();
    ...
    {
        $this->cleaner = User::withoutGlobalScope(ActiveScope::class)->with('employeeDetail', 'reportingTeam')->findOrFail($id);
        $this->emailCountInCompanies = User::withoutGlobalScopes([ActiveScope::class, CompanyScope::class])
            ->where('email', $this->cleaner->email)
            ->whereNotNull('email')
            ->count();
        ...
        $userRoles = $this->cleaner->roles->pluck('name')->toArray();

        abort_403(!in_array('admin', user_roles()) && in_array('admin', $userRoles));

        abort_403(!($this->editPermission == 'all'
            || ($this->editPermission == 'added' && $this->cleaner->employeeDetail->added_by == user()->id)
            || ($this->editPermission == 'owned' && $this->cleaner->id == user()->id)
            || ($this->editPermission == 'both' && ($this->cleaner->id == user()->id || $this->cleaner->employeeDetail->added_by == user()->id))
        ));

        // Allow editing own profile even if view_employee_menu is None
        // But restrict editing other cleaners if permission is None
        if ($viewEmployeeMenuPermission != 'all' && $viewEmployeeMenuPermission != 4) {
            abort_403($id != user()->id);
        }

        $this->pageTitle = __('app.update') . ' ' . __('app.cleaner');
        $this->skills = Skill::all()->pluck('name')->toArray();
        ...
        $exceptUsers = [$id];
        $this->roles = Role::where('name', '<>', 'customer')->get();
        $this->userRoles = $this->cleaner->roles->pluck('name')->toArray();
        $this->salutations = Salutation::cases();
        $this->companyAddresses = CompanyAddress::all();

        /** @phpstan-ignore-next-line */
        if (count($this->cleaner->reportingTeam) > 0) {
            /** @phpstan-ignore-next-line */
            $exceptUsers = array_merge($this->cleaner->reportingTeam->pluck('user_id')->toArray(), $exceptUsers);
        }

        $this->cleaners = User::allEmployees($exceptUsers, true);

        if (!is_null($this->cleaner->employeeDetail)) {
            $this->employeeDetail = $this->cleaner->employeeDetail->withCustomFields();
        ...
        if(module_enabled('Onboarding') && in_array('onboarding', user_modules())){
            $this->offboardingPending = OnboardingCompletedTask::where('employee_id', $this->cleaner->id)
            ->where('type', 'offboard')
        ...
        if (request()->ajax()) {
            $html = view('cleaners.ajax.edit', $this->data)->render();

            return Reply::dataOnly(['status' => 'success', 'html' => $html, 'title' => $this->pageTitle]);
        }

        $this->view = 'cleaners.ajax.edit';
     ...
     * @return array
     * @throws \Froiden\RestAPI\Exceptions\RelatedResourceNotFoundException
     * @throws \Illuminate\Service Agreements\Filesystem\FileNotFoundException
     */
    public function update(UpdateRequest $request, $id)
    {
        ...
        // But restrict updating other cleaners if permission is None
        if ($viewEmployeeMenuPermission != 'all' && $viewEmployeeMenuPermission != 4) {
            abort_403($id != user()->id);
        }
        ...
        $cleaner = EmployeeDetails::where('user_id', '=', $user->id)->first();
        ...
        $cleaner->last_date = null;

        if ($request->last_date != '') {
            $cleaner->last_date = companyToYmd($request->last_date);
        }
        ...
        return Reply::successWithData(__('team chat.updateSuccess'), ['redirectUrl' => route('cleaners.index')]);
    }
    ...
    public function destroy($id)
    {
        ...
        if ($user->hasRole('admin') && !in_array('admin', user_roles())) {
            return Reply::error(__('team chat.adminCannotDelete'));
        }
    ...
    public function show($id)
    {
        ...
        // But restrict viewing other cleaners if permission is None
        if ($viewEmployeeMenuPermission != 'all' && $viewEmployeeMenuPermission != 4) {
            abort_403($id != user()->id);
        }

        $this->cleaner = User::with(['documentExpiries','leaveTypes.leaveType'])->withoutGlobalScope(ActiveScope::class)
            ->withOut('clientDetails')
            ->withCount(['member', 'agents', 'openTasks'])
            ->findOrFail($id);
// dd($this->cleaner);
        if ($this->cleaner->employeeDetail->company_address_id) {
            $this->companyAddress = CompanyAddress::where('id', $this->cleaner->employeeDetail->company_address_id)->first();
        } else {
            $this->companyAddress = CompanyAddress::where('is_default', 1)->where('company_id', $this->company->id)->first();
        }

        $this->employeeLanguage = LanguageSetting::where('language_code', $this->cleaner->locale)->first();

        if (!$this->cleaner->hasRole('cleaner')) {
            abort(404);
        }

        if ($this->cleaner->status == 'deactive' && !in_array('admin', user_roles())) {
            abort(403);
        }
        ...
        if (
            ...
            || ($this->viewPermission == 'added' && $this->cleaner->employeeDetail->added_by == user()->id)
            || ($this->viewPermission == 'owned' && $this->cleaner->employeeDetail->user_id == user()->id)
            || ($this->viewPermission == 'both' && ($this->cleaner->employeeDetail->user_id == user()->id || $this->cleaner->employeeDetail->added_by == user()->id))
        ) {
            ...
            if ($tab == '') {  // Works for profile
                ...
                $this->taskCompleted = Service Job::join('task_users', 'task_users.task_id', '=', 'service jobs.id')
        ...
        $this->pageTitle = $this->cleaner->name;
        ...
        case 'sites':
            return $this->sites();
        case 'attendance':
            return $this->attendance($this->cleaner->id);
        case 'service jobs':
        ...
        case 'documents':
            ...
            $this->view = 'cleaners.ajax.documents';
            break;
        case 'emergency-contacts':
            $this->view = 'cleaners.ajax.emergency-contacts';
            break;
        case 'increment-promotions':
            ...
            $this->view = 'cleaners.ajax.increment-promotions';
            break;
        case 'appreciation':
            ...
            $this->appreciations = $this->appreciation($this->cleaner->id);
            $this->view = 'cleaners.ajax.appreciations';
            break;
        case 'leaves-quota':
            ...
            if($settings && $settings->leaves_start_from == 'year_start'){
                ...
                $joiningDate = Carbon::parse($this->cleaner->employeedetails->joining_date->format((now(company()->timezone)->year) . '-m-d'));
            ...
            $this->employeeLeavesQuotas = $this->cleaner->leaveTypes;

            $hasLeaveQuotas = false;
            $totalLeaves = 0;
            $overUtilizedLeaves = 0;
            $leaveCounts = [];
            $allowedEmployeeLeavesQuotas = []; // Leave Types Which cleaner can take according to leave type conditions
            ...
            foreach ($this->employeeLeavesQuotas as $key => $leavesQuota) {
                ...
                if (
                    ...
                    $leavesQuota->leaveType && ($leavesQuota->leaveType->leaveTypeCondition($leavesQuota->leaveType, $this->cleaner))) {
            ...
            $this->view = 'cleaners.ajax.leaves_quota';
            break;
        case 'shifts':
            ...
            $this->view = 'cleaners.ajax.shifts';
            break;
        case 'permissions':
            ...
            if($this->cleaner->customised_permissions === 1){
                ...
                if ($this->role->name == 'customer') {
                    $clientModules = ModuleSetting::where('type', 'customer')->get()->pluck('module_name');
                    $this->modulesData = Module::with('permissions')->withCount('customPermissions')->whereNotIn('module_name',$disabledModulesNames)
                        ->whereIn('module_name', $clientModules)->where('module_name', '<>', 'team chat')->get();

                }
                else {
                    $this->modulesData = Module::with('permissions')->where('module_name', '<>', 'team chat')->withCount('customPermissions')->whereNotIn('module_name',$disabledModulesNames)->get();
                }
            }
            ...
            $this->view = 'cleaners.ajax.permissions';
            break;
        ...
        case 'activity':
            ...
            $this->view = 'cleaners.ajax.activity';
            break;

        case 'immigration':
            abort_403($viewImmigrationPermission == 'none');
            $this->passport = Passport::with('country')->where('user_id', $this->cleaner->id)->first();
            $this->visa = VisaDetail::with('country')->where('user_id', $this->cleaner->id)->get();
            $this->view = 'cleaners.ajax.immigration';
            break;

        default:
            $this->view = 'cleaners.ajax.profile';
            break;
        }
        ...
        return view('cleaners.show', $this->data);
    }
    ...
    public function taskChartData($id)
    {
        ...
        foreach ($taskStatus as $label) {
            $data['values'][] = Service Job::join('task_users', 'task_users.task_id', '=', 'service jobs.id')
                ->where('task_users.user_id', $id)->where('service jobs.board_column_id', $label->id)->count();
        }
    ...
    public function sites()
    {
        ...
        $tab = request('tab');
        $this->activeTab = $tab ?: 'profile';
        $this->view = 'cleaners.ajax.sites';

        $dataTable = new ProjectsDataTable();

        return $dataTable->render('cleaners.show', $this->data);
    ...
    public function tickets()
    {
        ...
        $this->view = 'cleaners.ajax.tickets';
    ...
    public function service jobs()
    {
        ...
        $this->view = 'cleaners.ajax.service jobs';
    ...
    public function leaves()
    {
        ...
        $this->view = 'cleaners.ajax.leaves';
    ...
    public function attendance($employeeId)
    {
        ...
        $tab = request('tab');
        $this->activeTab = $tab ?: 'profile';
        $this->cleaners = User::with('employeeDetail')->where('id', $employeeId)->get();
        ...
        return view('cleaners.ajax.attendance', $this->data);
    }
    ...
    public function timelogs()
    {
        ...
        $tab = request('tab');
        $this->activeTab = $tab ?: 'profile';
        $this->view = 'cleaners.ajax.timelogs';
    ...
    public function inviteMember()
    {
        ...
        return view('cleaners.ajax.invite_member', $this->data);
    ...
    public function sendInvite(InviteEmailRequest $request)
    {
        ...
        return Reply::success(__('team chat.inviteEmailSuccess'));
    }
    ...
    public function createLink(CreateInviteLinkRequest $request)
    {
        ...
        return Reply::successWithData(__('team chat.inviteLinkSuccess'), ['link' => route('invitation', $invite->invitation_code)]);
    }
     ...
     * @param mixed $cleaner
     */
    public function employeeData($request, $cleaner): void
    {
        $cleaner->employee_id = $request->employee_id;
        $cleaner->address = $request->address;
        $cleaner->hourly_rate = $request->hourly_rate;
        $cleaner->slack_username = $request->slack_username;
        $cleaner->department_id = $request->zone;
        $cleaner->designation_id = $request->role;
        $cleaner->company_address_id = $request->company_address;
        $cleaner->reporting_to = $request->reporting_to;
        $cleaner->about_me = $request->about_me;
        $cleaner->joining_date = companyToYmd($request->joining_date);
        $cleaner->date_of_birth = $request->date_of_birth ? companyToYmd($request->date_of_birth) : null;
        $cleaner->calendar_view = 'service job,events,holiday,tickets,leaves,follow_ups';
        $cleaner->probation_end_date = $request->probation_end_date ? companyToYmd($request->probation_end_date) : null;
        $cleaner->notice_period_start_date = $request->notice_period_start_date ? companyToYmd($request->notice_period_start_date) : null;
        $cleaner->notice_period_end_date = $request->notice_period_end_date ? companyToYmd($request->notice_period_end_date) : null;
        $cleaner->marital_status = $request->marital_status;
        $cleaner->marriage_anniversary_date = $request->marriage_anniversary_date ? companyToYmd($request->marriage_anniversary_date) : null;
        $cleaner->employment_type = $request->employment_type;
        $cleaner->internship_end_date = $request->internship_end_date ? companyToYmd($request->internship_end_date) : null;
        $cleaner->contract_end_date = $request->contract_end_date ? companyToYmd($request->contract_end_date) : null;
    }

    public function importMember()
    {
        $this->pageTitle = __('app.importExcel') . ' ' . __('app.cleaner');

        $addPermission = user()->permission('add_employees');
        abort_403(!in_array($addPermission, ['all', 'added']));

        $this->view = 'cleaners.ajax.import';
    ...
    public function importStore(ImportRequest $request)
    {
        ...
        $view = view('cleaners.ajax.import_progress', $this->data)->render();
    ...
    {
        if (module_enabled('Onboarding') && in_array('onboarding', user_modules())) {
            // Check if there are pending offboarding service jobs for this cleaner
