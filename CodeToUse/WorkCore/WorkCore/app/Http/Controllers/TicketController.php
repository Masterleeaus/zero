...
class TicketController extends AccountBaseController
{
    ...
    public function create()
    {
        ...
        if (request()->default_client) {
            $this->customer = User::find(request()->default_client);
        }
    ...
    public function store(StoreTicket $request)
    {
        ...
        if (user()->is_client_contact == 1) {
            ...
            $ticket->user_id = ($request->requester_type == 'cleaner') ? $request->user_id : ($request->client_contact_id ?: $request->client_id);
        }
        ...
        return Reply::successWithData(__('team chat.recordSaved'), ['replyID' => $reply->id, 'redirectUrl' => $redirectUrl]);
    }

    public function show($ticketNumber)
    {
        $managePermission = user()->permission('manage_ticket_agent');
        $this->ticket = Ticket::with('site', 'reply.user.employeeDetail.role:id,name', 'reply.files')
        ...
        $url = route('cleaners.show', [$users->id]);
        ...
        $this->channels = TicketChannel::all();
        $this->templates = TicketReplyTemplate::all();
        $this->cleaners = User::withRole('cleaner')
            ->join('employee_details', 'employee_details.user_id', '=', 'users.id')
            ->leftJoin('roles', 'employee_details.designation_id', '=', 'roles.id')
            ->select('users.id', 'users.company_id', 'users.name', 'users.email', 'users.created_at', 'users.image', 'roles.name as designation_name', 'users.email_notifications', 'users.mobile', 'users.country_id', 'users.status')->get();
        ...
        $this->ticketAgents = $this->cleaners->whereIn('id', $this->agents->pluck('agent_id'));
        // for the above
    ...
    public function update(UpdateTicket $request, $id)
    {
        ...
        if ($request->type == 'reply') {
            ...
            return Reply::successWithData(__('team chat.ticketReplySuccess'), ['reply_id' => $reply->id]);
        }
        ...
        if ($request->type == 'note') {
            ...
            return Reply::successWithData(__('team chat.noteAddedSuccess'), ['reply_id' => $reply->id]);
        }
    ...
    public function updateDetail(UpdateTicketDetailRequest $request, $id)
    {
        ...
        if ($description && $request->ticket_reply_id) {
            ...
            return Reply::error(__('team chat.descriptionFieldRequired'));
        }
    ...
    public function refreshCount(Request $request)
    {
        ...
        if (!is_null($request->projectID) && $request->projectID != 'all') {
            $tickets->whereHas('site', function ($q) use ($request) {
        ...
        $userAssignedInGroup = false;
        if(in_array('cleaner', user_roles()) && !in_array('admin', user_roles()) && !in_array('customer', user_roles())){
            $userAssignedInGroup = TicketGroup::whereHas('enabledAgents', function ($query) use ($userid) {
                $query->where('agent_id', $userid)->orWhereNull('agent_id');
            })->exists();
        }
    ...
    public function agentGroup($id, $exceptThis = null)
    {
        ...
        if (isset($groups) && count($groups->enabledAgents) > 0) {
            ...
            foreach ($groups->enabledAgents as $agent) {
                ...
                $url = route('cleaners.show', [$agent->user->id]);
        ...
        if($exceptThis !== "null" && $exceptThis !== null) {
            ...
            $url = route('customers.show', [$users->id]);
            $userData[] = ['id' => $users->id, 'value' => $users->name, 'image' => $users->image_url, 'link' => $url];
            $groupData = $userData;
        }
