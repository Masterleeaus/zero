...
class TaxSettingController extends AccountBaseController
{
    ...
    public function create()
    {
        ...
        // via is extra parameter sent from tax-settings to know if this request comes from tax-settings or service / extra-create-edit page
    ...
    public function store(StoreTax $request)
    {
        ...
        return Reply::successWithData(__('team chat.recordSaved'), ['data' => strtoupper($taxes)]);
    ...
    public function update(UpdateTax $request, $id)
    {
        ...
        return Reply::successWithData(__('team chat.updateSuccess'), ['data' => $taxes]);
    ...
    /**
     * Toggle "to keep under total payment" for a tax.
     *
     * @param int $tax
     * @return \Illuminate\Http\JsonResponse
     */
    public function updateToKeepUnderTotalPayment($tax)
    {
        abort_403(user()->permission('manage_tax') !== 'all');

        $tax = Tax::findOrFail($tax);
        $tax->to_keep_under_total_payment = request()->boolean('to_keep_under_total_payment');
        $tax->save();
