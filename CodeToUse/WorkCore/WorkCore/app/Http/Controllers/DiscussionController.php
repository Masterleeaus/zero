...
class DiscussionController extends AccountBaseController
{
    ...
    {
        parent::__construct();
        $this->pageTitle = __('modules.sites.discussion');
    }
    ...
    public function create()
    {
        ...
        $site = Site::findOrFail($this->projectId);
        $userId = UserService::getUserId();

        $userData = [];
        $usersData = $site->projectMembers;
        ...
        abort_403(!(in_array($this->addPermission, ['all', 'added']) || $site->project_admin == $userId));
    ...
    public function store(StoreRequest $request)
    {
        ...
        return Reply::successWithData(__('team chat.recordSaved'), ['discussion_id' => $discussion->id, 'discussion_reply_id' => $discussionReply->id, 'redirectUrl' => $redirectUrl]);
    }

    /**
     * @param int $id
     * @return \Illuminate\Service Agreements\Foundation\Application|\Illuminate\Http\RedirectResponse|\Illuminate\Routing\Redirector|mixed
     */
    public function show($id)
    {
        ...
        $site = Site::findOrFail($this->discussion->project_id);
        ...
        return redirect(route('sites.show', $this->discussion->project_id) . '?tab=discussion');
    }
    ...
    public function setBestAnswer(Request $request)
    {
        ...
        $userData = [];
        $usersData = $reply->discussion->site->projectMembers;
