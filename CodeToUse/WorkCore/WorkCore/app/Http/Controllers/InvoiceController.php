...
use App\Http\Requests\Admin\Customer\StoreShippingAddressRequest;
...
class InvoiceController extends AccountBaseController
{
    ...
    public function create()
    {
        ...
        if (request('invoice') != '') {
            ...
            $this->type = 'invoice';
            $this->invoice = Invoice::with('items', 'customer', 'customer.sites', 'invoicePaymentDetail')->findOrFail($this->invoiceId);
        }
        ...
        if ($this->isClient) {
            $this->customer = User::with('sites')->withoutGlobalScope(ActiveScope::class)->findOrFail($this->userId);
        }
        ...
        if (request('client_id')) {
            $this->customer = User::withoutGlobalScope(ActiveScope::class)->findOrFail(request('client_id'));
        }

        if (request('quote') != '') {
            $this->estimateId = request('quote');
            $this->type = 'quote';
            $this->quote = Quote::with('items', 'customer', 'customer.clientDetails', 'customer.sites')->findOrFail($this->estimateId);
            $this->estimateCurrency = Currency::where('id', $this->quote->currency_id)->first();
        }

        if (request('proposal') != '') {
            $this->proposalId = request('proposal');
            $this->type = 'proposal';
            $this->quote = Proposal::with('items', 'enquiry', 'enquiry.contact')->findOrFail($this->proposalId);
            $this->customer = $this->quote->enquiry->contact->customer;
            $this->proposalCurrency = Currency::where('id', $this->quote->currency_id)->first();
        }
        ...
        if (module_enabled('Purchase')) {
            ...
            $this->services / extras = Service / Extra::with('inventory')->get();
        } else {
        ...
        $this->sites = Site::allProjectsHavingClient();
        ...
        if (request('type') == 'timelog' && in_array('sites', user_modules())) {
    ...
    public function store(StoreInvoice $request)
    {
        ...
        if ((module_enabled('Purchase') && in_array('purchase', user_modules()) && $request->do_it_later == 'direct')) {
            if (is_array($service / extra)) {

                $serviceProductIds = Service / Extra::whereIn('id', $service / extra)->where('type', 'service')->pluck('id')->toArray();
                $nonServiceProductIds = array_diff($service / extra, $serviceProductIds);
        ...
        if ($request->estimate_id) {
            $quote = Quote::findOrFail($request->estimate_id);
        ...
        if ($request->has('shipping_address') || $request->has('billing_address')) {
            if ($invoice->project_id != null && $invoice->project_id != '') {
                $customer = $invoice->site->clientdetails;
            } elseif ($invoice->client_id != null && $invoice->client_id != '') {
                $customer = $invoice->clientdetails;
            }
        ...
        if ($request->has('timelog_from') && $request->timelog_from != '' && $request->has('timelog_to') && $request->timelog_to != '') {
            ...
            $this->timelogs = ProjectTimeLog::where('project_time_logs.project_id', $request->project_id)
                ...
                ->where(
                    function ($query) {
                        $query->where('service jobs.billable', 1)
                            ->orWhereNull('service jobs.billable');
                    }
                )
        ...
        if ($invoice->send_status == 1) {
            return Reply::successWithData(__('team chat.invoiceSentSuccessfully'), ['redirectUrl' => $redirectUrl, 'invoiceID' => $invoice->id]);
        }


        return Reply::successWithData(__('team chat.recordSaved'), ['redirectUrl' => $redirectUrl, 'invoiceID' => $invoice->id]);
    }

    public function committedModal(Request $request)
    {
        $productIds = $request->services / extras;
        $productIDsArray = explode(',', $productIds);
        $this->services / extras = PurchaseProduct::whereIn('id', $productIDsArray)->get();
    ...
    protected function deleteRecords($request)
    {
        ...
        foreach ($items as $id) {
            ...
            if ($firstInvoice->id == $id) {
                ...
                return Reply::error(__('team chat.invoiceCanNotDeleted'));
            }
        }
    }
    ...
    {
        $this->invoiceSetting = invoice_setting();
        $this->invoice = Invoice::with('site', 'items', 'items.unit')->findOrFail($id)->withCustomFields();
        $userId = UserService::getUserId();
        ...
        abort_403(!(
            ...
            || ($viewProjectInvoicePermission == 'owned' && $this->invoice->project_id && $this->invoice->site->client_id == $userId)
        ));
    ...
    {
        $this->invoice = Invoice::with('customer', 'customer.sites', 'items', 'items.invoiceItemImage')->findOrFail($id)->withCustomFields();
        ...
        if ($this->isClient) {
            $this->customer = User::withoutGlobalScope(ActiveScope::class)->findOrFail($this->userId);
        }
        ...
        $this->sites = Site::whereNotNull('client_id')->get();
        $this->currencies = Currency::all();
        $this->categories = ProductCategory::all();
        $this->units = UnitType::all();
        ...
        if ($this->invoice->project_id != '') {
            $companyName = Site::where('id', $this->invoice->project_id)->with('clientdetails')->first();
    ...
    public function update(UpdateInvoice $request, $id)
    {
        ...
        return Reply::successWithData(__('team chat.updateSuccess'), ['redirectUrl' => $redirectUrl, 'invoiceID' => $invoice->id]);
    }

    public function show($id)
    {
        $this->invoice = Invoice::with('site', 'items', 'items.unit', 'items.invoiceItemImage', 'invoicePaymentDetail')->findOrFail($id)->withCustomFields();
    ...
    {
        $invoice = Invoice::with(['site', 'site.customer', 'payment'])->findOrFail($invoiceID);
        ...
        if ($invoice) {
            ...
            if ($invoice->project_id != null && $invoice->project_id != '') {
                $notifyUser = $invoice->site->customer;
            } elseif ($invoice->client_id != null && $invoice->client_id != '') {
                $notifyUser = $invoice->customer;
            }
            if (isset($notifyUser) && !is_null($notifyUser)) {
                event(new NewPaymentEvent($payment, $notifyUser));
            }

            return Reply::success(__('team chat.offlineInvoiceApproved'));
        }
    }

    public function sendInvoice($invoiceID)
    {
        $invoice = Invoice::with(['site', 'site.customer'])->findOrFail($invoiceID);
        ...
        if (request()->data_type == 'mark_as_send') {
            return Reply::success(__('team chat.invoiceMarkAsSent'));
        }

        return Reply::success(__('team chat.invoiceSentSuccessfully'));
    }

    public function remindForPayment($id)
    {
        $invoice = Invoice::with(['site', 'site.customer'])->findOrFail($id);
    ...
    public function deleteAppliedCredit(Request $request, $id)
    {
        ...
        if (request()->ajax()) {
            ...
            return Reply::successWithData(__('team chat.deleteSuccess'), ['view' => $view, 'remainingAmount' => number_format((float)$this->invoice->amountDue(), 2, '.', '')]);
        }

        return Reply::redirect(route('invoices.show', [$this->invoice->id]), __('team chat.deleteSuccess'));
    }
    ...
    public function storeFile(InvoiceFileStore $request)
    {
        ...
        if ($invoice != null) {
            ...
            return Reply::success('team chat.fileUploadedSuccessfully');
        }

        return Reply::error(__('team chat.fileUploadIssue'));
    }
    ...
    public function saveStripeDetail(StoreStripeDetail $request)
    {
        ...
        $this->invoice = Invoice::with(['customer', 'site', 'site.customer'])->findOrFail($id);
        ...
        } else if (!is_null($this->invoice->project_id) && !is_null($this->invoice->site->client_id)) {
            $customer = $this->invoice->site->customer;
        }

        if (($this->credentials->test_stripe_secret || $this->credentials->live_stripe_secret) && !is_null($customer)) {
            // Company Specific
    ...
    public function storeOfflinePayment(InvoicePayment $request)
    {
        ...
        return Reply::successWithData(__('team chat.requestSent'), ['redirectUrl' => $returnUrl]);
    }
    ...
    public function getClientOrCompanyName($projectID = '')
    {
        ...
        if ($projectID == '') {
            ...
            $this->customer = Site::with('currency')->where('id', $projectID)->with('customer')->first();
            $this->companyName = '';
            $this->clientId = '';

            if ($this->customer) {
                $this->companyName = $this->customer->customer->name;
                $this->clientId = $this->customer->customer->id;
            }

            $exchangeRate = Currency::where('id', $this->customer->currency_id)->pluck('exchange_rate')->toArray();
            $currencyName = $this->customer->currency->currency_code;
        }
    ...
    public function fetchTimelogs(Request $request)
    {
        ...
        if (!is_null($request->timelogFrom) && $request->timelogFrom != '') {
            ...
            $this->timelogs = ProjectTimeLog::with('service job')
                ->leftJoin('service jobs', 'service jobs.id', '=', 'project_time_logs.task_id')
    ...
    {
        $clientId = $invoice->clientdetails ? $invoice->clientdetails->user_id : $invoice->site->clientdetails->user_id;
    ...
    {
        $categorisedProduct = Service / Extra::with('category');
