...
use App\Http\Requests\Service Jobs\StoreTaskNote;
...
class TaskNoteController extends AccountBaseController
{
    ...
    public function store(StoreTaskNote $request)
    {
        ...
        abort_403(!(
            ...
            || ($this->addPermission == 'added' && ($service job->added_by == user()->id || $service job->added_by == $this->userId || in_array($service job->added_by, $this->clientIds)))
            || ($this->addPermission == 'owned' && in_array(user()->id, $taskUsers))
            || ($this->addPermission == 'added' && (in_array(user()->id, $taskUsers) || $service job->added_by == user()->id || $service job->added_by == $this->userId || in_array($service job->added_by, $this->clientIds)))
        ));
        ...
        $view = view('service jobs.notes.show', $this->data)->render();
    ...
    public function destroy($id)
    {
        ...
        $note->delete();
        $this->notes = TaskNote::with('service job')->where('task_id', $note_task_id)->orderByDesc('id')->get();
        $view = view('service jobs.notes.show', $this->data)->render();
    ...
    {
        $this->note = TaskNote::with('user', 'service job')->findOrFail($id);
        ...
        $taskuserData = [];
        $usersData = $this->note->service job->users;
        ...
        return view('service jobs.notes.edit', $this->data);
    ...
    public function update(StoreTaskNote $request, $id)
    {
        ...
        $this->notes = TaskNote::with('service job')->where('task_id', $note->task_id)->orderByDesc('id')->get();
        $view = view('service jobs.notes.show', $this->data)->render();
