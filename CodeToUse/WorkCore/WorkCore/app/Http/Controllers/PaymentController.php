...
class PaymentController extends AccountBaseController
{
    ...
    public function create()
    {
        ...
        if (request()->has('default_client') && request('default_client') != '') {
            ...
            $this->sites = Site::with('currency')->where('client_id', request('default_client'))->get();
        }
        else {
            $this->sites = Site::with('currency')->whereNotNull('client_id')->get();
        }
        ...
        if (request()->has('site')) {
            $this->projectId = request()->site;
        }

        $this->site = request()->has('site') ? Site::findOrFail(request()->site) : null;

        if ($this->site) {
            $this->currencyCode = $this->site->currency->currency_code;
            $this->exchangeRate = $this->site->currency->exchange_rate;
        }
        ...
        if (request()->get('invoice_id')) {
            ...
            if ($this->invoice->project_id) {
                $this->site = Site::findOrFail($this->invoice->project_id);
            }
    ...
    public function store(StorePayment $request)
    {
        ...
        if ($request->project_id != '') {
            ...
            $payment->currency_id = $site->currency_id;
        }
        ...
        if ($request->invoice_id != '') {
            ...
            if ($request->amount > $invoice->amountDue() + $tolerance) {
                return Reply::error(__('team chat.invoicePaymentExceedError'));
            }
        }
    ...
    public function update(UpdatePayments $request, $id)
    {
        ...
        return Reply::successWithData(__('team chat.recordSaved'), ['redirectUrl' => route('payments.index')]);
    }

    public function show($id)
    {
        $this->payment = Payment::with(['invoice', 'site', 'currency', 'offlineMethods', 'transactions' => function ($q) {
        ...
        abort_403(!($this->viewPermission == 'all'
            ...
            || ($this->viewPermission == 'owned' && !is_null($this->payment->project_id) && $this->payment->site->client_id == user()->id)
            ...
            || ($this->viewPermission == 'both' && ($this->payment->added_by == user()->id || (!is_null($this->payment->project_id) && $this->payment->site->client_id == user()->id) || (!is_null($this->payment->invoice_id) && $this->payment->invoice->client_id == user()->id) || (!is_null($this->payment->order_id) && $this->payment->order->client_id == user()->id)))
        ));
    ...
    public function download($id)
    {
        ...
        $this->payment = Payment::with('invoice', 'site', 'currency')->findOrFail($id);
        $this->viewPermission = user()->permission('view_payments');
    ...
    public function addBulkPayments()
    {
        ...
        $clientId = in_array('customer', user_roles()) ? $this->user->id : request()->client_id;
    ...
    public function saveBulkPayments(StoreBulkPayments $request)
    {
        ...
        if ($totalRecord === $insertRecord) {
            return Reply::error(__('team chat.pleaseEnterSomeData'));
        }
