...
class RazorPayController extends Controller
{
    ...
    public function payWithRazorPay($companyHash)
    {
        ...
        if ($payment->status == 'authorized' && isset($payment->amount) && (isset($invoice) || isset($order))) {
            ...
            Session::put('success', __('team chat.paymentSuccessful'));
            ...
            if (!auth()->check() && isset($invoice)) {
                ...
                return Reply::redirect($redirectRoute, __('team chat.paymentSuccessful'));
            }

            /** @phpstan-ignore-next-line */
            if (isset(request()->type) && request()->type == 'order' && isset($order)) {
                return Reply::redirect(route('orders.show', $order->id), __('team chat.paymentSuccessful'));
            }

            if (isset($invoice)) {
                return Reply::redirect(route('invoices.show', $invoice->id), __('team chat.paymentSuccessful'));
            }
        }
        elseif ($payment->status == 'captured') {
            ...
            if (!auth()->check() && isset($invoice)) {
                return Reply::redirect(url()->temporarySignedRoute('front.invoice', now()->addDays(GlobalSetting::SIGNED_ROUTE_EXPIRY), $invoice->hash), __('team chat.paymentSuccessful'));
            }
