...
class ProposalTemplateController extends AccountBaseController
{
    ...
    public function __construct()
    {
        ...
        $this->middleware(function ($request, $next) {
            abort_403(in_array('service agreement', $this->user->modules));
            return $next($request);
        });
    }
    ...
    public function show($id)
    {
        ...
        $this->invoice = ProposalTemplate::with('items', 'enquiry', 'items.proposalTemplateItemImage', 'units')->findOrFail($id);

        $this->pageTitle = __('modules.enquiry.proposalTemplate') . '#' . $this->invoice->id;
    ...
    public function edit($id)
    {
        ...
        $this->taxes = Tax::all();
        $this->currencies = Currency::all();
        $this->proposal = ProposalTemplate::with('items', 'enquiry')->findOrFail($id);
        $this->services / extras = Service / Extra::all();
        $this->categories = ProductCategory::all();
        $this->units = UnitType::all();
        $this->invoiceSetting = invoice_setting();
    ...
    public function update(StoreRequest $request, $id)
    {
        ...
        return Reply::redirect(route('proposal-template.show', $proposalTemplate->id), __('team chat.updateSuccess'));
    }
    ...
    {
        $this->invoiceSetting = invoice_setting();
        $this->proposalTemplate = ProposalTemplate::with('items', 'enquiry', 'currency')->findOrFail($id);
        ...
        $filename = __('modules.enquiry.proposal') . '-' . $this->proposalTemplate->id;
