...
class TimelogController extends AccountBaseController
{
    ...
    public function calculateTime(Request $request)
    {
        ...
        if ($hours < 0 || $minutes < 0) {
            return response()->json([
                'status' => 'error',
                'message' => __('team chat.totalTimeZero')
            ]);
        }
    ...
    public function create()
    {
        ...
        if (request()->has('default_assign') && request('default_assign') != '') {
            ...
            $this->sites = $sites = Site::whereHas('members', function ($query) use ($assignId) {
        ...
        elseif (request()->has('default_project') && request('default_project') != '') {
            ...
            $this->sites = $sites = Site::where('id', $defaultProject)->where('status', '!=', 'finished')
                ->get();
        }
        else {
        ...
        $this->service jobs = Service Job::timelogTasks(request('default_project'));
    ...
    public function store(StoreTimeLog $request)
    {
        ...
        if(($endDateTime->diffInMinutes($startDateTime)) < 1){
            return Reply::error(__('team chat.enterAtLeastOneHour'));
        }

        // Check site time limits for time-based sites
        if ($request->has('project_id') && $request->project_id) {
            $site = Site::findOrFail($request->project_id);
            if ($site->calculate_task_progress == 'project_total_time') {
                $remainingTime = $this->getProjectRemainingTime($site->id);
                $requestedTime = $endDateTime->diffInMinutes($startDateTime);
                
                if ($remainingTime !== false && $requestedTime > $remainingTime) {
                    $remainingHours = floor($remainingTime / 60);
                    $remainingMinutes = $remainingTime % 60;
                    $message = __('team chat.projectTimeLimitExceeded', [
...
//            $timeLog->total_minutes = $timeLog->end_time->diffInMinutes($timeLog->start_time);
            ...
            return Reply::successWithData(__('team chat.timeLogAdded'), ['redirectUrl' => route('timelogs.index')]);
        }

        return Reply::error(__('team chat.timelogAlreadyExist'));
    }

    public function destroy($id)
    {
        $timelog = ProjectTimeLog::with('site')->findOrFail($id);
        ...
        abort_403(!($deleteTimelogPermission == 'all'
            ...
            || ($timelog->site && ($timelog->site->project_admin == user()->id))
        ));
    ...
    public function edit($id)
    {
        ...
        $timeLog = $this->timeLog = ProjectTimeLog::with('user', 'site', 'service job')->findOrFail($id)->withCustomFields();
        abort_403(!(
            $editTimelogPermission == 'all'
            || ($editTimelogPermission == 'added' && $timeLog->added_by == user()->id)
            || ($this->timeLog->site && ($this->timeLog->site->project_admin == user()->id))
            || ($editTimelogPermission == 'owned'
                && (($timeLog->site && $timeLog->site->client_id == user()->id) || $timeLog->user_id == user()->id)
            )
            || ($editTimelogPermission == 'both' && (($timeLog->site && $timeLog->site->client_id == user()->id) || $timeLog->user_id == user()->id || $timeLog->added_by == user()->id))
        ));

        if (!is_null($this->timeLog->task_id) && !is_null($this->timeLog->project_id)) {
            $this->service jobs = Service Job::timelogTasks($this->timeLog->project_id);
            $this->cleaners = $this->timeLog->service job->users;
        }
        else if (!is_null($this->timeLog->project_id)) {
            $this->service jobs = Service Job::timelogTasks($this->timeLog->project_id);
            $this->cleaners = $this->timeLog->site->projectMembers;
        }
        else {
            $this->service jobs = Service Job::timelogTasks();
            $this->cleaners = $this->timeLog->service job->users;
        }
    ...
    public function update(UpdateTimeLog $request, $id)
    {
        ...
        if (is_null($activeTimer)) {
            ...
            // Update site progress if site uses time-based calculation
            if ($timeLog->project_id && $timeLog->site && $timeLog->site->calculate_task_progress == 'project_total_time') {
                $this->calculateProjectProgressByTime($timeLog->project_id);
            }

            return Reply::successWithData(__('team chat.updateSuccess'), ['redirectUrl' => route('timelogs.index')]);
        }
    ...
    public function show($id)
    {
        ...
        $this->timeLog = ProjectTimeLog::with('user', 'user.employeeDetail', 'site', 'service job', 'breaks', 'activeBreak')->findOrFail($id)->withCustomFields();
        ...
        abort_403(!(
            ...
            || ($this->viewTimelogPermission == 'owned'
                && (($this->timeLog->site && $this->timeLog->site->client_id == $userId) || $this->timeLog->user_id == $userId)
            )
            || ($this->viewTimelogPermission == 'both' && (($this->timeLog->site && $this->timeLog->site->client_id == $userId) || $this->timeLog->user_id == $userId || $this->timeLog->added_by == $userId))
        ));
    ...
    public function showTimer()
    {
        ...
        if (is_null($activeTimer)) {
            $this->service jobs = Service Job::join('task_users', 'task_users.task_id', '=', 'service jobs.id')
                ->with('site')
                ->pending()
            ...
            $this->sites = Site::byEmployee(user()->id);
    ...
    public function startTimer(StartTimer $request)
    {
        ...
        if (is_null($activeTimer) || (!is_null($activeTimer->activeBreak))) {
            ...
            if ($request->has('create_task')) {
                ...
                $service job->heading = $request->memo;
                $service job->board_column_id = $this->company->default_task_status;
                $service job->is_private = $request->has('is_private') && $request->is_private == 'true' ? 1 : 0;
                $service job->start_date = now($this->company->timezone)->format('Y-m-d');
                $service job->due_date = now($this->company->timezone)->format('Y-m-d');
            ...
            if ($request->project_id != '') {
                $this->logProjectActivity($request->project_id, 'modules.service jobs.timerStartedBy');
                $this->logUserActivity($this->user->id, 'modules.service jobs.timerStartedProject');
            }
            else {
                $this->logUserActivity($this->user->id, 'modules.service jobs.timerStartedTask');
            }
            ...
            return Reply::successWithData(__('team chat.timerStartedSuccessfully'), ['html' => $html, 'activeTimerCount' => $this->activeTimerCount, 'clockHtml' => $clockHtml]);
        }

        return Reply::error(__('team chat.timerAlreadyRunning'));
    }

    public function stopTimer(StopTimer $request)
    {
        $timeId = $request->timeId;
        $timeLog = ProjectTimeLog::with('activeBreak', 'site')->findOrFail($timeId);

        $taskUrl = route('service jobs.show', $timeLog->task_id);
        ...
        abort_403(!(in_array('admin', user_roles()) ||
            (($timeLog->site && $timeLog->site->client_id == user()->id)
        ...
        if (!is_null($timeLog->project_id)) {
            $this->logProjectActivity($timeLog->project_id, 'modules.service jobs.timerStoppedBy');
        }
        ...
        if ($timeLog->project_id && $timeLog->site && $timeLog->site->calculate_task_progress == 'project_total_time') {
            ...
            // Check if site time limit is reached and show warning
            $remainingTime = $this->getProjectRemainingTime($timeLog->project_id);
            if ($remainingTime !== false && $remainingTime <= 0) {
                $message = __('team chat.projectTimeReached');
                $this->logUserActivity($this->user->id, 'modules.service jobs.timerStoppedBy');
        ...
        return Reply::successWithData(__('team chat.timerStoppedSuccessfully'), ['html' => $html, 'activeTimerCount' => $this->activeTimerCount, 'activeTimer' => $selfActiveTimer, 'reload' => $reload]);
    }
    ...
    public function showActiveTimer()
    {
        ...
        $with = ['service job', 'service job.site', 'user', 'site']; // Define common with clauses
        ...
        $this->service jobs = Service Job::select('service jobs.*')
            ->with('site')
            ->pending()
    ...
    public function byEmployee()
    {
        ...
        $this->sites = Site::allProjects();
        $this->timeLogProjects = $this->sites;
        $this->service jobs = Service Job::all();
        $this->timeLogTasks = $this->service jobs;
    ...
    public function employeeData(Request $request)
    {
        ...
        $query = User::with('employeeDetail.role:id,name')
        ...
        $query->leftJoin('project_time_logs', function ($join) use ($where) {
            $join->on('project_time_logs.user_id', '=', 'users.id')
                ->leftJoin('service jobs', 'project_time_logs.task_id', '=', 'service jobs.id') // Join service jobs table
                ->leftJoin('sites', 'sites.id', '=', 'service jobs.project_id')
                ->whereNull('service jobs.deleted_at') // Exclude soft-deleted service jobs
        ...
        if (!is_null($cleaner) && $cleaner !== 'all') {
            $query->where('project_time_logs.user_id', $cleaner);
        }

        if (in_array('customer', user_roles()) && in_array($this->viewTimelogPermission,['both','owned','added'])) {
            // $query->where('sites.client_id', '=', user()->id);
            if ($this->viewTimelogPermission == 'added') {
                $query->where('added_by', '=', user()->id)->orWhere('sites.client_id', '=', user()->id);
            }else{
                $query->where('sites.client_id', '=', user()->id);
            }
        }else{
        ...
        $this->cleaners = $query->groupBy('project_time_logs.user_id')
            ->orderBy('users.name')
            ->get();
    ...
    public function userTimelogs(Request $request)
    {
        ...
        $query = ProjectTimeLog::with('site', 'service job')->select('*')->whereHas('service job', function ($query) {
            $query->whereNull('deleted_at');
        })->where('user_id', $cleaner);
        ...
        if (in_array('customer', user_roles()) && in_array($this->viewTimelogPermission,['both','owned','added'])) {
            // condition for added then added_by or client_id , client_id from site relation
            if ($this->viewTimelogPermission == 'added') {
                $query->where('added_by', '=', user()->id)->orWhereHas('site', function ($query) {
    ...
    public function pauseTimer(Request $request)
    {
        ...
        if (!is_null($timeLog->project_id)) {
            $this->logProjectActivity($timeLog->project_id, 'modules.service jobs.timerPausedBy');
        }
        ...
        $this->logUserActivity($this->user->id, 'modules.service jobs.timerPausedBy');
        ...
        return Reply::successWithData(__('team chat.timerPausedSuccessfully'), ['html' => $html, 'clockHtml' => $clockHtml, 'reload' => $reload]);
    }
    ...
    public function resumeTimer(Request $request)
    {
        ...
        if (is_null($activeTimer) || (!is_null($activeTimer) && !is_null($activeTimer->activeBreak))) {
            ...
            $this->logUserActivity($this->user->id, 'modules.service jobs.timerStartedBy');
            ...
            return Reply::successWithData(__('team chat.timerStartedSuccessfully'), ['html' => $html, 'clockHtml' => $clockHtml, 'reload' => $reload]);
    ...
    public function exportTimeLogs()
    {
        ...
        return Excel::download(new ProjectTimeLogsExport($startDate, $endDate, $cleaner), 'timelogs.xlsx');
    }

    /**
     * Check if site time limit is reached
     * 
     * @param int $projectId
     * @return \Illuminate\Http\Response
     */
    public function checkProjectTimeLimit($projectId)
    {
        ...
        if ($site->calculate_task_progress == 'project_total_time') {
            ...
            if ($remainingTime !== false && $remainingTime <= 0) {
                // Stop all active timers for this site
                $this->stopProjectTimersOnTimeLimit($projectId);
                
                return Reply::successWithData(__('team chat.projectTimeReached'), [
                    'timeLimitReached' => true,
                    'message' => __('team chat.projectTimeReached')
                ]);
            }
    ...
    /**
     * Stop all active timers for a site when time limit is reached
     * 
     * @param int $projectId
     * @return void
     */
    public function stopProjectTimersOnTimeLimit($projectId)
    {
        ...
        if ($site->calculate_task_progress == 'project_total_time') {
            ...
            if ($remainingTime !== false && $remainingTime <= 0) {
                // Get all active timers for this site
                ...
                foreach ($activeTimers as $timer) {
                    // Stop the timer
                    $timer->end_time = now();
                    $timer->total_hours = $timer->end_time->diffInHours($timer->start_time);
                    $timer->total_minutes = $timer->end_time->diffInMinutes($timer->start_time);
                    $timer->save();
                    ...
                    if (!is_null($timer->activeBreak)) {
                        ...
                        $activeBreak->total_minutes = $timer->end_time->diffInMinutes($activeBreak->start_time);
                        $activeBreak->total_hours = $timer->end_time->diffInHours($activeBreak->start_time);
                        $activeBreak->save();
                    }
                    
                    // Log the activity
                    $this->logProjectActivity($projectId, 'modules.service jobs.timerStoppedBy');
                    $this->logUserActivity($timer->user_id, 'modules.service jobs.timerStoppedBy');
                }
            }
        }
    }
