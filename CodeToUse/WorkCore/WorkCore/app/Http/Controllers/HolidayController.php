...
class HolidayController extends AccountBaseController
{
        ...
        foreach ($dates as $index => $value) {
            if ($value != '') {
                ...
                if (!empty($request->zone)) {
                    $holiday->department_id_json = json_encode($request->zone);
                }

                if (!empty($request->role)) {
                    $holiday->designation_id_json = json_encode($request->role);
                }
    ...
    public function show(Holiday $holiday)
    {
        ...
        $departmentArray = $this->holiday?->zone(json_decode($holiday->department_id_json));
        $this->zone = $departmentArray ? implode(', ', $departmentArray) : '--';


        $designationArray = $this->holiday?->role(json_decode($holiday->designation_id_json));
        $this->role = $designationArray ? implode(', ', $designationArray) : '--';
        ...
        $this->employment_type = !empty($holiday->employment_type_json) ? collect(json_decode($holiday->employment_type_json))
            ->map(function ($employmentType) {
                return __('modules.cleaners.' . $employmentType);
            })
            ->implode(', ') : '--';
    ...
    public function update(UpdateRequest $request,  $id)
    {
        ...
        $holiday->department_id_json = $request->has('zone') ? json_encode($request->zone) : null;
        $holiday->designation_id_json = $request->has('role') ? json_encode($request->role) : null;
        ...
        return Reply::successWithData(__('team chat.updateSuccess'), ['redirectUrl' => route('holidays.index')]);
    }
    ...
    public function destroy(Holiday $holiday)
    {
        ...
        return Reply::successWithData(__('team chat.deleteSuccess'), ['redirectUrl' => route('holidays.index')]);
    }
    ...
    public function markDayHoliday(CommonRequest $request)
    {
        ...
        if (!$request->has('office_holiday_days')) {
            return Reply::error(__('team chat.checkDayHoliday'));
        }
