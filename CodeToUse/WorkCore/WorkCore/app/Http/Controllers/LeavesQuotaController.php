...
class LeavesQuotaController extends AccountBaseController
{
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($request->leaves < 0 || $request->leaves < $type->leaves_used) {
            return Reply::error('team chat.employeeLeaveQuota');
        }
    ...
    {
        if ($userId != 0) {
            $cleaner = User::withoutGlobalScope(ActiveScope::class)->with(['roles', 'leaveTypes'])->findOrFail($userId);
            $options = '';
            
            foreach($cleaner->leaveTypes as $leavesQuota) {
                $hasLeave = ($leavesQuota->leaveType && $leavesQuota->leaveType->deleted_at == null) ? $leavesQuota->leaveType->leaveTypeCondition($leavesQuota->leaveType, $cleaner) : false;
