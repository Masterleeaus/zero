...
class OrderController extends AccountBaseController
{
    ...
    public function create()
    {
        ...
        abort_403(!in_array('customers', user_modules()) || in_array('customer', user_roles()) || !in_array($this->addPermission, ['all', 'added', 'both']));
    ...
    public function store(PlaceOrder $request)
    {
        ...
        return response(Reply::redirect(route('orders.show', $order->id), __('team chat.recordSaved')))->withCookie(Cookie::forget('productDetails'));
    ...
    public function addItem(Request $request)
    {
        ...
        $this->item = Service / Extra::with('tax')->findOrFail($request->id);
    ...
    {
        $this->order = Order::with('customer', 'unit')->findOrFail($id)->withCustomFields();
        ...
        abort_403(in_array('customer', user_roles()) || !($this->editPermission == 'all' || ($this->editPermission == 'both' && ($this->order->added_by == $userid || $this->order->client_id == $userid)) || ($this->editPermission == 'added' && $this->order->added_by == $userid) || ($this->editPermission == 'owned' && $this->order->client_id == $userid)));
    ...
    public function update(UpdateOrder $request, $id)
    {
        ...
        if ($request->total == 0) {
            return Reply::error(__('team chat.amountIsZero'));
        }
        ...
        if ($order->status == 'completed') {
            return Reply::error(__('team chat.invalidRequest'));
        }
        ...
        return Reply::redirect(route('orders.index'), __('team chat.updateSuccess'));
    }
    ...
    public function show($id)
    {
        ...
        if ($this->viewBankAccountPermission != 'none') {
            ...
            if ($this->viewBankAccountPermission == 'added') {
                $bankAccounts = $bankAccounts->where('added_by', $userid);
            }
    ...
    public function destroy($id)
    {
        ...
        abort_403(in_array('customer', user_roles()) || !($this->deletePermission == 'all' || ($this->deletePermission == 'both' && ($order->added_by == $userid || $order->client_id == $userid)) || ($this->deletePermission == 'added' && $order->added_by == $userid) || ($this->deletePermission == 'owned' && $order->client_id == $userid)));
    ...
    public function paymentFailed($orderId)
    {
        ...
        return Reply::error(__('team chat.paymentFailed'));
    }
    ...
    public function changeStatus(Request $request)
    {
        ...
        return Reply::success(__('team chat.orderStatusChanged'));
    }
    ...
    public function createCreditNote($invoice)
    {
        ...
        elseif (!is_null($invoice->site) && $invoice->site->client_id) {
            $clientId = $invoice->site->client_id;
        }
        ...
        return Reply::redirect(route('creditnotes.index'), __('team chat.recordSaved'));
    }
    ...
    public function download($id)
    {
        ...
        $this->order = Order::with('customer', 'unit')->findOrFail($id);
