...
class UserPermissionController extends AccountBaseController
{
    ...
    {
        $this->cleaner = User::with('role')->findOrFail($id);

        $roleId = $this->cleaner->role[0]->role_id;
        ...
        $html = view('cleaners.ajax.custom_permissions', $this->data)->render();
    ...
    public function resetPermissions($userId)
    {
        ...
        if (count($userRoles) > 1) {
            $role = $userRoles->where('name', '!=', 'cleaner')->first();
        }
        else {
        ...
        if (!$role) {
            return Reply::error(__('team chat.roleNotFound', ['user' => $user->name]));
        }
