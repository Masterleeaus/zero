...
class WeeklyTimesheetController extends AccountBaseController
{
    ...
    public function weekSummaryData($request)
    {
        ...
        $service jobs = Service Job::leftJoin('task_users', 'task_users.task_id', '=', 'service jobs.id')
            ->where('task_users.user_id', '=', user()->id)
            ->where(function ($query) {
                return $query->where(function ($q) {
                    // Service Job starts before or on end date AND ends after or on start date
                    $q->whereDate('service jobs.start_date', '<=', $this->weekEndDate)
                      ->where(function ($q) {
                        $q->whereDate('service jobs.due_date', '>=', $this->weekStartDate)
                          ->orWhereNull('service jobs.due_date');
                      });
                });
            })
            ->with('site:id,project_name')
            ->whereHas('site', function ($query) {
                $query->where('status', '!=', 'finished');
            })
            ->select('service jobs.id', 'service jobs.heading', 'service jobs.project_id')
            ->get();

        $this->tasksForWeek = $service jobs;
    ...
    public function store(Request $request)
    {
        ...
        if ($weeklyTimesheet->status == 'pending') {
            ...
            return Reply::redirect(route('weekly-timesheets.index'), __('team chat.recordSaved'));
        }
    ...
    public function edit($id)
    {
        ...
        ->where('task_users.user_id', '=', user()->id)
        ->where(function ($query) {
            return $query->whereBetween(DB::raw('DATE(service jobs.`start_date`)'), [$this->weekStartDate, $this->weekEndDate])
            ->orWhereBetween(DB::raw('DATE(service jobs.`due_date`)'), [$this->weekStartDate, $this->weekEndDate]);
        })
        ->with('site:id,project_name')
        ->select('service jobs.id', 'service jobs.heading', 'service jobs.project_id')->get();
