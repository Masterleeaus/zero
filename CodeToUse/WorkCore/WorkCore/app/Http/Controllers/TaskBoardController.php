...
class TaskBoardController extends AccountBaseController
{
    ...
    {
        parent::__construct();
        $this->pageTitle = 'modules.service jobs.taskBoard';
    ...
    public function index(Request $request)
    {
        ...
        if (request()->ajax()) {
            ...
            $boardColumns = TaskboardColumn::withCount([
                'service jobs as tasks_count' => function ($q) use ($startDate, $endDate, $request, $userId) {
                    $q->leftJoin('sites', 'sites.id', '=', 'service jobs.project_id')
                        ->leftJoin('users as customer', 'customer.id', '=', 'sites.client_id');

                    if (
                        ($this->viewUnassignedTasksPermission == 'all' && !in_array('customer', user_roles())
                            && ($request->assignedTo == 'unassigned' || $request->assignedTo == 'all'))
                        || ($request->has('project_admin') && $request->project_admin == 1)
                    ) {
                        $q->leftJoin('task_users', 'task_users.task_id', '=', 'service jobs.id')
                    ...
                    $q->leftJoin('task_labels', 'task_labels.task_id', '=', 'service jobs.id')
                    ...
                    if ($startDate && $endDate) {
                        $q->where(function ($service job) use ($startDate, $endDate) {
                            $service job->whereBetween(DB::raw('DATE(service jobs.`due_date`)'), [$startDate, $endDate]);

                            $service job->orWhereBetween(DB::raw('DATE(service jobs.`start_date`)'), [$startDate, $endDate]);
                        });
                    }
                    ...
                    if ($request->projectID != 0 && $request->projectID != null && $request->projectID != 'all') {
                        $q->where('service jobs.project_id', '=', $request->projectID);
                    }

                    if ($request->clientID != '' && $request->clientID != null && $request->clientID != 'all') {
                        $q->where('sites.client_id', '=', $request->clientID);
                    }
                    ...
                    if ($request->category_id != '' && $request->category_id != null && $request->category_id != 'all') {
                        $q->where('service jobs.task_category_id', '=', $request->category_id);
                    }
                    ...
                    if ($request->billable != '' && $request->billable != null && $request->billable != 'all') {
                        $q->where('service jobs.billable', '=', $request->billable);
                    }

                    if ($request->priority != '' && $request->priority != null && $request->priority != 'all') {
                        $q->where('service jobs.priority', '=', $request->priority);
                    }

                    if ($request->milestone_id != '' && $request->milestone_id != null && $request->milestone_id != 'all') {
                        $q->where('service jobs.milestone_id', $request->milestone_id);
                    }
                    ...
                    if (($request->has('project_admin') && $request->project_admin != 1) || !$request->has('project_admin')) {
                        if ($this->viewTaskPermission == 'owned') {
                            $q->where(function ($q1) use ($request, $userId) {
                                ...
                                if (in_array('customer', user_roles())) {
                                    $q1->orWhere('sites.client_id', '=', $userId);
                                }
                        ...
                        if ($this->viewTaskPermission == 'both') {
                            $q->where(function ($q1) use ($request, $userId) {
                                ...
                                $q1->orWhere('service jobs.added_by', '=', $userId);
                    ...
                    $q->select(DB::raw('count(distinct service jobs.id)'));
                }
            ])
                ->with(['service jobs' => function ($q) use ($startDate, $endDate, $request, $userId) {
                    $q->withCount(['checklists', 'completedSubtasks', 'comments'])
                        ->leftJoin('sites', 'sites.id', '=', 'service jobs.project_id')
                        ->leftJoin('users as customer', 'customer.id', '=', 'sites.client_id');

                    if (
                        ($this->viewUnassignedTasksPermission == 'all' && !in_array('customer', user_roles()) && ($request->assignedTo == 'unassigned' || $request->assignedTo == 'all'))
                        || ($request->has('project_admin') && $request->project_admin == 1)
                    ) {
                        $q->leftJoin('task_users', 'task_users.task_id', '=', 'service jobs.id')
                            ->leftJoin('users', 'task_users.user_id', '=', 'users.id');
            ...
            $boardColumns = $boardColumns->get()->map(function ($query) {
                $query->setRelation('service jobs', $query->service jobs->take($this->taskBoardColumnLength));
            ...
            foreach ($boardColumns as $key => $boardColumn) {
                ...
                $result['boardColumns'][$key]['service jobs'] = $boardColumn->service jobs;
            }

            if (request()->projectID != 'all') {
                $this->site = Site::findOrFail($request->projectID);
            }
    ...
    public function loadMore(Request $request)
    {
        ...
        $service jobs = Service Job::with('users', 'site', 'labels')
            ->withCount(['checklists', 'completedSubtasks', 'comments'])
            ->leftJoin('sites', 'sites.id', '=', 'service jobs.project_id')
            ->leftJoin('users as customer', 'customer.id', '=', 'sites.client_id');
        ...
        if (
            ...
            $service jobs->leftJoin('task_users', 'task_users.task_id', '=', 'service jobs.id')
                ->leftJoin('users', 'task_users.user_id', '=', 'users.id');
        } else {
            $service jobs->leftJoin('task_users', 'task_users.task_id', '=', 'service jobs.id')
                ->leftJoin('users', 'task_users.user_id', '=', 'users.id');
        ...
        $service jobs->leftJoin('task_labels', 'task_labels.task_id', '=', 'service jobs.id')
            ->leftJoin('users as creator_user', 'creator_user.id', '=', 'service jobs.created_by')
            ->select('service jobs.*')
            ->where('service jobs.board_column_id', $request->columnId)
        ...
        if ($startDate && $endDate) {
            $service jobs->where(function ($service job) use ($startDate, $endDate) {
                $service job->whereBetween(DB::raw('DATE(service jobs.`due_date`)'), [$startDate, $endDate]);
        ...
        $service jobs->whereNull('sites.deleted_at');

        if ($request->projectID != 0 && $request->projectID != null && $request->projectID != 'all') {
            $service jobs->where('service jobs.project_id', '=', $request->projectID);
        }

        if ($request->clientID != '' && $request->clientID != null && $request->clientID != 'all') {
            $service jobs->where('sites.client_id', '=', $request->clientID);
        }

        if ($request->assignedTo != '' && $request->assignedTo != null && $request->assignedTo != 'all') {
            $service jobs->where('task_users.user_id', '=', $request->assignedTo);
        }

        if ($request->assignedBY != '' && $request->assignedBY != null && $request->assignedBY != 'all') {
            $service jobs->where('creator_user.id', '=', $request->assignedBY);
        }

        if ($request->category_id != '' && $request->category_id != null && $request->category_id != 'all') {
            $service jobs->where('service jobs.task_category_id', '=', $request->category_id);
        }

        if ($request->label_id != '' && $request->label_id != null && $request->label_id != 'all') {
            $service jobs->where('task_labels.label_id', '=', $request->label_id);
        }

        if (($request->has('project_admin') && $request->project_admin != 1) || !$request->has('project_admin')) {
            if ($this->viewTaskPermission == 'owned') {
                $service jobs->where(function ($q1) use ($request, $userId) {
                    $q1->where('task_users.user_id', '=', $userId);
            ...
            if ($this->viewTaskPermission == 'added') {
                $service jobs->where('service jobs.added_by', '=', $userId);
            }
        ...
        if ($request->searchText != '') {
            $service jobs->where(function ($query) {
                $safeTerm = Common::safeString(request('searchText'));
                $query->where('service jobs.heading', 'like', '%' . $safeTerm . '%')
                    ->orWhere('users.name', 'like', '%' . $safeTerm . '%')
                    ->orWhere('sites.project_name', 'like', '%' . $safeTerm . '%');
        ...
        $service jobs->skip($skip)->take($this->taskBoardColumnLength);
        $service jobs = $service jobs->get();
    ...
    public function updateIndex(Request $request)
    {
        ...
        if (isset($taskIds) && count($taskIds) > 0) {
            ...
            foreach ($taskIds as $key => $taskId) {
                if (!is_null($taskId)) {
                    $service job = Service Job::with('labels')->findOrFail($taskId);
            ...
            $this->triggerPusher('service job-updated-channel', 'service job-updated', ['user_id' => $userId, 'task_id' => $request->draggingTaskId]);
        }
    ...
    {
        Service Job::where('board_column_id', $id)->update(['board_column_id' => company()->default_task_status]);
