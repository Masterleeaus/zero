...
class EstimateRequestController extends AccountBaseController
{
    ...
    public function index(EstimateRequestDataTable $dataTable)
    {
        ...
        return $dataTable->render('quote-requests.index', $this->data);
    }
    ...
    public function create()
    {
        ...
        $this->sites = Site::where('client_id', $this->userId)->get();
        $this->customers = User::allClients();
        $this->currencies = Currency::all();

        $this->view = 'quote-requests.ajax.create';

        if (request()->ajax()) {
            return $this->returnAjax($this->view);
        }

        return view('quote-requests.create', $this->data);
    }
    ...
    public function store(StoreEstimateRequest $request)
    {
        ...
        if ($redirectUrl == '') {
            $redirectUrl = route('quote-request.index');
        }
    ...
    public function show(string $id)
    {
        ...
        $this->estimateLink = $this->estimateRequest->quote ? '<p class="mb-0 text-dark-grey f-14 w-70 text-wrap m-0 mt-1"><a href="' . route('quotes.show', [$this->estimateRequest->quote->id]) . '" class="text-dark-grey f-14 w-70 text-wrap">' . $this->estimateRequest->quote->estimate_number . '</a></p>' : '--';
        ...
        $this->view = 'quote-requests.ajax.show';
    ...
    public function edit($id)
    {
        ...
        $this->sites = Site::where('client_id', $this->estimateRequest->client_id)->get();
        $this->customers = User::allClients();
        $this->currencies = Currency::all();

        $this->view = 'quote-requests.ajax.edit';
    ...
    public function rejectConfirmation($id)
    {
        ...
        return view('quote-requests.ajax.confirm-reject', $this->data);
    }

    public function sendEstimateRequest()
    {
        $this->pageTitle = __('modules.estimateRequest.sendEstimateRequest');
        $this->customers = User::allClients()->whereNotNull('email');

        return view('quote-requests.ajax.send-request', $this->data);
    }

    public function sendEstimateMail(Request $request)
    {
        if ($request->client_id == '') {
            return Reply::error(__('validation.required', ['attribute' => 'customer']));
        }

        $customer = User::findOrFail($request->client_id);

        if (isset($customer->email)){
            Notification::send($customer, new EstimateRequestInvite($customer));
    ...
    /* public function createEstiamte($id)
    {
        ...
        $quote = new Quote();
        $quote->client_id = $estimateRequest->client_id;
        $quote->company_id = $estimateRequest->company_id;
        $quote->description = trim_editor($estimateRequest->description);
        $quote->total = $estimateRequest->estimated_budget;
        $quote->valid_till = now()->addDays(30)->format('Y-m-d');
        $quote->status = 'waiting';
        $quote->estimate_number = $this->lastEstimate;
        $quote->currency_id = $estimateRequest->currency_id;
        $quote->save();
