...
class SubTaskFileController extends AccountBaseController
{
    ...
    public function store(Request $request)
    {
        ...
        $this->checklist = SubTask::with(['files'])->findOrFail($request->sub_task_id);
        ...
        if ($request->hasFile('file')) {
            ...
            foreach ($request->file as $fileData) {
                ...
                $this->logTaskActivity($this->checklist->task_id, $this->user->id, 'fileActivity', null, $this->checklist->id);
            }
        }
        ...
        $this->service job = Service Job::with(['checklists', 'checklists.files'])->findOrFail($this->checklist->task_id);
    ...
    public function destroy($id)
    {
        ...
        $view = view('service jobs.sub_tasks.files.show', $this->data)->render();
