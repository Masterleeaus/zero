...
class GdprController extends AccountBaseController
{
    ...
    public function updateConsentBlock(Request $request)
    {
        ...
        $removalRequest->save();
        return Reply::success(__('team chat.gdprRequestUpdated'));
    }

    public function downloadJson(Request $request)
    {
        $table = User::with('clientDetails', 'attendance', 'cleaner', 'employeeDetail', 'sites', 'member', 'group')->findOrFail(user()->id);
