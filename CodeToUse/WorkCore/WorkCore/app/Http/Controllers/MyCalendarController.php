...
class MyCalendarController extends AccountBaseController
{
    ...
    {
        $this->types = ['service job' => __('app.menu.service jobs'),
        ...
        if ($type == null || $type == 'null' || $type == 'all') {
            $calendar_type_array = ['service job', 'events', 'tickets', 'leaves', 'follow_ups'];
        }
        else {
        ...
        if (request('start') && request('end'))
        {
            ...
            if (!is_null($viewTaskPerm) && $viewTaskPerm != 'none') {
                ...
                if (in_array('service job', $calendar_type_array)) {
                    // service jobs
                    $completedTaskColumn = TaskboardColumn::completeColumn();

                    $service jobs = Service Job::with(['boardColumn', 'site.customer'])
                        ->where('board_column_id', '<>', $completedTaskColumn->id)
                        ->where(function ($query) {
                            $query->whereHas('users', function ($q) {
                                $q->where('user_id', user()->id);
                            });
                            $query->orWhereHas('site.customer', function ($q) {
                                $q->where('id', user()->id);
                            });
                        })
                        ->where(function ($q) use ($startDate, $endDate) {
                            $q->whereBetween(DB::raw('DATE(service jobs.`due_date`)'), [$startDate->toDateString(), $endDate->toDateString()]);

                            $q->orWhereBetween(DB::raw('DATE(service jobs.`start_date`)'), [$startDate->toDateString(), $endDate->toDateString()]);
                        })->get();

                    foreach ($service jobs as $service job) {
                        $eventData[] = [
                            'id' => $service job->id,
                            'title' => $service job->heading,
                            'start' => $service job->start_date,
                            'end' => $service job->due_date ?: $service job->start_date,
                            'event_type' => 'service job',
                            'extendedProps' => ['bg_color' => $service job->boardColumn->label_color, 'color' => '#fff', 'icon' => 'fa-list']
                        ];
                    }
                }
            }
            ...
            if (!is_null($viewleavePerm) && $viewleavePerm != 'none') {
                ...
                if (in_array('leaves', $calendar_type_array)) {
                    // approved leaves of all emoloyees with cleaner name
            ...
            if (!is_null($viewDealPerm) && $viewDealPerm != 'none') {
                ...
                if (in_array('follow_ups', $calendar_type_array)) {
                    // follow ups
                    $followUps = DealFollowUp::with('enquiry')->whereHas('enquiry.leadAgent', function ($query) {
                            $query->where('user_id', user()->id);
                    })
