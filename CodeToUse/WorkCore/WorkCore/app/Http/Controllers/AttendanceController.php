...
class AttendanceController extends AccountBaseController
{
    ...
    public function index(Request $request)
    {
        ...
        if ($this->viewAttendancePermission == 'owned') {
            $this->cleaners = User::where('id', user()->id)->get();
        } else {
            $this->cleaners = User::allEmployees(null, false, ($this->viewAttendancePermission == 'all' ? 'all' : null));
        }
    ...
    public function summaryData($request)
    {
        ...
        if ($request->zone != 'all') {
            $cleaners = $cleaners->where('employee_details.department_id', $request->zone);
        }

        if ($request->role != 'all') {
            $cleaners = $cleaners->where('employee_details.designation_id', $request->role);
        }

        if ($request->userId != 'all') {
            $cleaners = $cleaners->where('users.id', $request->userId);
        }

        if ($viewEmployeePermission == 'owned') {
            $cleaners = $cleaners->where('users.id', user()->id);
        } elseif ($viewEmployeePermission == 'both') {
            $cleaners = $cleaners->where('employee_details.user_id', user()->id)
                ->orWhere('employee_details.added_by', user()->id);
        } elseif ($viewEmployeePermission == 'added') {
            $cleaners = $cleaners->where('employee_details.added_by', user()->id);
        }
        ...
        foreach ($cleaners as $cleaner) {
            ...
            if (!$requestedDate->isPast()) {
                $final[$cleaner->id . '#' . $cleaner->name] = array_replace($dataTillToday, $dataFromTomorrow);
            } else {
                $final[$cleaner->id . '#' . $cleaner->name] = array_replace($dataTillRequestedDate, $dataFromTomorrow);
            }

            $shiftScheduleCollection = $cleaner->shifts->keyBy('date');


            foreach ($cleaner->shifts as $shifts) {
                if ($shifts->shift->shift_name == 'Day Off') {
                    $final[$cleaner->id . '#' . $cleaner->name][$shifts->date->day] = 'Day Off';
                }
            }

            $firstAttendanceProcessed = [];

            foreach ($cleaner->attendance as $attendance) {
                ...
                if (!isset($isHalfDay[$cleaner->id][$startOfDayKey]) && !isset($isLate[$cleaner->id][$startOfDayKey])) {
                    $isHalfDay[$cleaner->id][$startOfDayKey] = $isLate[$cleaner->id][$startOfDayKey] = false;
                }

                // Check if this is the first attendance of the day for this cleaner
                if (!isset($firstAttendanceProcessed[$cleaner->id][$startOfDayKey])) {
                    $firstAttendanceProcessed[$cleaner->id][$startOfDayKey] = true; // Mark as processed

                    // Apply "half day" or "late" logic only if it's the first attendance
                    $isHalfDay[$cleaner->id][$startOfDayKey] = $attendance->half_day == 'yes';
                    $isLate[$cleaner->id][$startOfDayKey] = $attendance->late == 'yes';
                }

                $iconClassKey = $isHalfDay[$cleaner->id][$startOfDayKey] ? 'star-half-alt text-red' : ($isLate[$cleaner->id][$startOfDayKey] ? 'exclamation-circle text-warning' : 'check text-success');
                ...
                if ($isWithinShift || $isAssignedShift || $isPreviousShift) {
                    ...
                    $final[$cleaner->id . '#' . $cleaner->name][$dayToAssign] = $attendanceHtml;
                } else {
                    $final[$cleaner->id . '#' . $cleaner->name][$clockInTime->day] = $attendanceHtml;
                }
            }

            $emplolyeeName = view('components.cleaner', [
                'user' => $cleaner
            ]);

            $final[$cleaner->id . '#' . $cleaner->name][] = $emplolyeeName;

            if ($cleaner->employeeDetail->joining_date->greaterThan(Carbon::parse(Carbon::parse('01-' . $request->month . '-' . $request->year)))) {
                if ($request->month == $cleaner->employeeDetail->joining_date->format('m') && $request->year == $cleaner->employeeDetail->joining_date->format('Y')) {
                    if ($cleaner->employeeDetail->joining_date->format('d') == '01') {
                        $dataBeforeJoin = array_fill(1, $cleaner->employeeDetail->joining_date->format('d'), '-');
                    } else {
                        $dataBeforeJoin = array_fill(1, $cleaner->employeeDetail->joining_date->subDay()->format('d'), '-');
                    }
                }

                if (($request->month < $cleaner->employeeDetail->joining_date->format('m') && $request->year == $cleaner->employeeDetail->joining_date->format('Y')) || $request->year < $cleaner->employeeDetail->joining_date->format('Y')) {
            ...
            if (!is_null($dataBeforeJoin)) {
                $final[$cleaner->id . '#' . $cleaner->name] = array_replace($final[$cleaner->id . '#' . $cleaner->name], $dataBeforeJoin);
            }

            foreach ($cleaner->leaves as $leave) {
                if ($leave->duration == 'half day') {
                    if ($final[$cleaner->id . '#' . $cleaner->name][$leave->leave_date->day] == '-' || $final[$cleaner->id . '#' . $cleaner->name][$leave->leave_date->day] == 'Absent') {
                        $final[$cleaner->id . '#' . $cleaner->name][$leave->leave_date->day] = 'Half Day';
                    }
                } else {
                    $final[$cleaner->id . '#' . $cleaner->name][$leave->leave_date->day] = 'Leave';
                    $leaveReasons[$cleaner->id][$leave->leave_date->day] = $leave->type->type_name . ': ' . $leave->reason;
                }
            }

            foreach ($this->holidays as $holiday) {
                $departmentId = $cleaner->employeeDetail->department_id;
                $designationId = $cleaner->employeeDetail->designation_id;
                $employmentType = $cleaner->employeeDetail->employment_type;
                ...
                if (((in_array($departmentId, $holidayDepartment) || $holiday->department_id_json == null) &&
                    ...
                    if ($final[$cleaner->id . '#' . $cleaner->name][$holiday->date->day] == 'Absent' || $final[$cleaner->id . '#' . $cleaner->name][$holiday->date->day] == '-') {
                        $final[$cleaner->id . '#' . $cleaner->name][$holiday->date->day] = 'Holiday';
    ...
    public function update(ClockInRequest $request, $id)
    {
        ...
        if ($conflictingAttendance) {
            return Reply::error(__('team chat.attendanceMarked'));
        }
        ...
        return Reply::success(__('team chat.attendanceSaveSuccess'));
    }
    ...
    public function store(StoreAttendance $request)
    {
        ...
        if (!is_null($attendance) && !$request->user_id) {
            ...
            if ($clockInCount < $this->attendanceSettings->clockin_in_day || $request->user_id) {
                ...
                return Reply::error(__('team chat.maxClockin'));
            }
        }
    ...
    public function create()
    {
        ...
        if ($addPermission == 'added') {
            ...
            $this->defaultAssign = User::with('roles', 'employeeDetail', 'employeeDetail.zone')->findOrFail(request()->default_assign);
        }
    ...
    {
        $cleaners = $request->user_id;
        $employeeData = User::withoutGlobalScope(ActiveScope::class)
            ->with('employeeDetail')->whereIn('id', $cleaners)->get();
        ...
        foreach ($cleaners as $key => $userId) {
            ...
            // Retrieve holidays based on cleaner details
        ...
        if ($holidayError) {
            return Reply::error(__('team chat.holidayError')); // Display error message
        }
        ...
        return Reply::redirect($redirectUrl, __('team chat.attendanceSaveSuccess'));
    }
    ...
    public function exportAllAttendance($year, $month, $id, $zone, $role)
    {
        abort_403(!canDataTableExport());
        ...
        return Excel::download(new AttendanceExport($year, $month, $id, $zone, $role, $startDate, $endDate), 'Attendance_From_' . $startDate->format('d-m-Y') . '_To_' . $date->format('d-m-Y') . '.xlsx');
    }
    ...
    public function byHour(Request $request)
    {
        ...
        if ($this->viewAttendancePermission == 'owned') {
            $this->cleaners = $users->where(function ($query) {
    ...
    public function hourSummaryData($request)
    {
        ...
        foreach ($cleaners as $count => $cleaner) {
            ...
            foreach ($cleaner->attendance as $index => $attendance) {
                ...
                // totalTime() function is used to calculate the total time of an cleaner on particular date
                $diffInMins = ($to && $from) ? $attendance->totalTime($from, $to, $cleaner->id, 'm') : 0;

                // previous date is used to store the previous date of an attendance, so that we can calculate the total time of an cleaner on particular date
                if ($index == 0) {
                ...
                $final[$cleaner->id . '#' . $cleaner->name][Carbon::parse($attendance->clock_in_time)->timezone($this->company->timezone)->day] = '<a href="javascript:;" class="view-attendance" data-attendance-id="' . $attendance->id . '">' . intdiv($mins, 60) . ':' . ($mins % 60) . '</a>';
            }
    ...
    public function byMapLocation(Request $request)
    {
        ...
        $this->cleaners = User::allEmployees(null, true, ($this->viewAttendancePermission == 'all' ? 'all' : null));
        $this->zones = Team::all();
    ...
    protected function byMapLocationData($request)
    {
        ...
        if ($request->zone != 'all') {
            $this->attendances = $this->attendances->where('employee_details.department_id', $request->zone);
        }
    ...
    public function qrCodeStatus(Request $request)
    {
        ...
        if (request()->qr_status == 1 && $attendanceSetting->auto_clock_in == 'yes') {
            return Reply::error(__('team chat.qrCodeAndFirstSignError'));
        }
    ...
    public function qrClockInOut(Request $request, $hash)
    {
        ...
        if ($leaveApplied != null  && ($leaveApplied->duration === 'single' || $leaveApplied->duration === 'multiple') && $currentTime->between($startTime, $endTime)) {
            ...
            $message = __('team chat.leaveApplied');
            $clockInTime = null;
            $clockOutTime = null;
            ...
            $message = __('team chat.leaveForFirstHalf');
            $clockInTime = null;
            $clockOutTime = null;
            $totalWorkingTime = null;
            $leave = true;
            ...
            $message = __('team chat.leaveForSecondHalf');
            $clockInTime = null;
            $clockOutTime = null;
            $totalWorkingTime = null;
            $leave = true;
            ...
            $message = __('team chat.todayHoliday');
            $clockInTime = null;
            $clockOutTime = null;
            $totalWorkingTime = null;
            $holiday = true;
            ...
            $message = __('team chat.notInOfficeHours');
            $clockInTime = null;
            $clockOutTime = null;
            $totalWorkingTime = null;
        ...
        if ($todayAttendance && !session('qr_clock_in')) {
            ...
            $message = __('team chat.attendanceClockOutSuccess');
            ...
            if ($attendanceSetting->qr_enable == 1) {
                ...
                if (isset($res['status']) && $res['status'] == 'success') {
                    ...
                    $message = __('team chat.attendanceClockInSuccess');
                    ...
                    // Clock in successful, set success message
                    $message = $res['message'] ?? __('team chat.attendanceClockInFailed');
                    $clockInTime = null;
                    $clockOutTime = null;
                    $totalWorkingTime = null;
                    $notAuthorize = true;
    ...
    private function clockInUser($user, $request)
    {
        ...
        if (attendance_setting()->ip_check == 'yes') {
            ...
            if (!in_array($request->ip(), $ips)) {
                return ['type' => 'error', 'message' => __('team chat.notAnAuthorisedDevice'), 'notAuthorize' => $notAuthorize, 'time' => null];
            } else {
                $notAuthorize = true;
            }
        }
        ...
        if (attendance_setting()->radius_check == 'yes') {
            ...
            if (!$checkRadius) {
                return ['type' => 'error', 'message' => __('team chat.notAnValidLocation'), 'qrClockIn' => $qrClockIn];
            } else {
                $qrClockIn = true;
        ...
        if ($clockInCount >= $this->attendanceSettings->clockin_in_day) {
            return ['type' => 'error', 'message' => __('team chat.maxClockin')];
        ...
        return Reply::successWithData(__('team chat.attendanceClockInSuccess'), ['time' => $now->format('h:i A'), 'ip' => $attendance->clock_in_ip, 'working_from' => $attendance->working_from, 'notAuthorize' => $notAuthorize, 'qrClockIn' => $qrClockIn]);


        // return ['type' => 'success', 'message' => __('team chat.attendanceSaveSuccess')];

        // return Reply::successWithData(__('team chat.attendanceSaveSuccess'), [
