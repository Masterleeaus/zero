...
class ModuleSettingController extends AccountBaseController
{
    ...
    public function index()
    {
        ...
        $this->modulesData = match ($tab) {
            'cleaner' => ModuleSetting::where('module_name', '<>', 'settings')->where('is_allowed', 1)->where('type', 'cleaner')->get(),
            'customer' => ModuleSetting::where('module_name', '<>', 'settings')->where('is_allowed', 1)->where('type', 'customer')->get(),
