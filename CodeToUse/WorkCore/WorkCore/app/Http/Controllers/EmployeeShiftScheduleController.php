...
class EmployeeShiftScheduleController extends AccountBaseController
{
    ...
    public function index(Request $request)
    {
        ...
        if ($this->viewShiftPermission == 'owned') {
            ...
            $this->cleaners = User::allEmployees(null, false, ($this->viewShiftPermission == 'all' ? 'all' : null));
        }
    ...
    public function summaryData($request)
    {
        ...
        foreach ($cleaners as $cleaner) {
            ...
            if($cleaner->status == 'deactive'){
                $isActive = 'no';
                $deactiveEmp[] = $cleaner->id;
            }
            ...
            foreach ($cleaner->shifts as $shift) {
                if ($shift->shift->shift_name == 'Day Off') {
                    $final[$cleaner->id . '#' . $cleaner->name][Carbon::parse($shift->date)->day] = '<button type="button" class="change-shift'.$isActive.' badge badge-light border f-10 p-1" data-user-id="' . $shift->user_id . '" data-attendance-date="' . $shift->date->day . '"  data-toggle="tooltip" data-original-title="' . __('modules.attendance.dayOff') . '" style="background-color: #E8EEF3">' . $shift->shift->shift_short_code . '</button>';
                    $shiftColorCode[Carbon::parse($shift->date)->day] = $shift->color;

                }
                else {
                    $final[$cleaner->id . '#' . $cleaner->name][Carbon::parse($shift->date)->day] = '<button type="button" class="change-shift'.$isActive.' badge badge-info f-10 p-1" style="background-color: ' . $shift->shift->color . '" data-user-id="' . $shift->user_id . '" data-attendance-date="' . $shift->date->day . '"  data-toggle="tooltip" data-original-title="' . $shift->shift->shift_name . '">' . $shift->shift->shift_short_code . '</button>';
                    $shiftColorCode[Carbon::parse($shift->date)->day] = $shift->color;
                }

            }

            $employeeName = view('components.cleaner', [
                'user' => $cleaner
            ]);

            $final[$cleaner->id . '#' . $cleaner->name][] = $employeeName;

            if ($cleaner->employeeDetail->joining_date->greaterThan(Carbon::parse(Carbon::parse('01-' . $this->month . '-' . $this->year)))) {
                if ($request->month == $cleaner->employeeDetail->joining_date->format('m') && $this->year == $cleaner->employeeDetail->joining_date->format('Y')) {
                    if ($cleaner->employeeDetail->joining_date->format('d') == '01') {
                        ...
                        $shiftColorCode = array_fill(1, $cleaner->employeeDetail->joining_date->format('d'), '');
                    }
                    else {
                ...
                if (($request->month < $cleaner->employeeDetail->joining_date->format('m') && $this->year == $cleaner->employeeDetail->joining_date->format('Y')) || $this->year < $cleaner->employeeDetail->joining_date->format('Y')) {
            ...
            foreach ($this->holidays as $holiday) {
                ...
                if (((in_array($departmentId, $holidayDepartment) || $holiday->department_id_json == null) &&
                    ...
                    if ($final[$cleaner->id . '#' . $cleaner->name][$holiday->date->day] == 'Absent' || $final[$cleaner->id . '#' . $cleaner->name][$holiday->date->day] == 'EMPTY') {
    ...
    public function weekSummaryData($request)
    {
        ...
        foreach ($cleaners as $cleaner) {
            ...
            foreach ($this->weekPeriod->toArray() as $date) {
                $final[$cleaner->id . '#' . $cleaner->name][$date->toDateString()] = 'EMPTY';
            }
            ...
            foreach ($cleaner->shifts as $shift) {
                ...
                if ($shift->shift->shift_name == 'Day Off') {
                    $final[$cleaner->id . '#' . $cleaner->name][$dateString] = '<button type="button" class="change-shift-week'.$isActive.' badge as badge-light f-10 p-1 border f-12 py-4 w-100" data-user-id="' . $shift->user_id . '" data-attendance-date="' . $dateString . '" style="background-color: #E8EEF3"><div>' . __('modules.attendance.dayOff') . '<div></button>';
                    $shiftColorCode[$dateString] = $shift->color;

                } else {
                    $final[$cleaner->id . '#' . $cleaner->name][$dateString] = '<button type="button" class="change-shift-week'.$isActive.' badge ass badge-info text-left f-12 py-3 px-2 w-100" style="background-color: ' . $shift->shift->color . '" data-user-id="' . $shift->user_id . '" data-attendance-date="' . $dateString . '"><div>' . $shift->shift->shift_name . '<div>';

                    if ($shift->shift->shift_type == 'strict')
                    {
                        $final[$cleaner->id . '#' . $cleaner->name][$dateString] .= '<div class="mt-2 f-10">' . Carbon::parse($shift->shift->office_start_time)->format('H:i') . ' - ' . Carbon::parse($shift->shift->office_end_time)->format('H:i') . '</div>';

                    } else {
                        $final[$cleaner->id . '#' . $cleaner->name][$dateString] .= '<div class="mt-2 f-10">' . $shift->shift->flexible_total_hours.' '.__('app.hrs') . '</div>';
                    }

                    $final[$cleaner->id . '#' . $cleaner->name][$dateString] .= '</button>';
                    $shiftColorCode[$dateString] = $shift->color;
                }
            }
            ...
            $joiningDate = Carbon::createFromFormat('Y-m-d', $cleaner->employeeDetail->joining_date->toDateString(), company()->timezone)->startOfDay();

            if ($joiningDate->greaterThan($this->weekEndDate)) {
                foreach ($this->weekPeriod->toArray() as $date) {
                    $final[$cleaner->id . '#' . $cleaner->name][$date->toDateString()] = '-';
                }
            }
            ...
            foreach ($cleaner->leaves as $leave) {
                if ($leave->duration != 'half day') {
                    $final[$cleaner->id . '#' . $cleaner->name][$leave->leave_date->toDateString()] = 'Leave';
                    $shiftColorCode[$leave->leave_date->day] = '';
                    $leaveType[$cleaner->id][$leave->leave_date->toDateString()] = $leave->type->type_name;
                }
            }
            ...
            foreach ($this->holidays as $holiday) {
                ...
                if (((in_array($departmentId, $holidayDepartment) || $holiday->department_id_json == null) &&
                    ...
                    if ($final[$cleaner->id . '#' . $cleaner->name][$holidayDateString] == 'Absent' || $final[$cleaner->id . '#' . $cleaner->name][$holidayDateString] == 'EMPTY') {
                        $final[$cleaner->id . '#' . $cleaner->name][$holidayDateString] = 'Holiday';
    ...
    public function mark(Request $request, $userid, $day, $month, $year)
    {
        ...
        $this->cleaner = User::findOrFail($userid);
    ...
    public function store(Request $request)
    {
        ...
        return Reply::success(__('team chat.employeeShiftAdded'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        if (!$request->hasFile('file')) {
            Files::deleteFile($shift->file, 'cleaner-shift-file/' . $id);
            Files::deleteDirectory('cleaner-shift-file/' . $id);
            $shift->file = null;
        }

        if ($request->hasFile('file')) {
            Files::deleteFile($request->file, 'cleaner-shift-file/' . $id);
            $shift->file = Files::uploadLocalOrS3($request->file, 'cleaner-shift-file/' . $id);
        }
    ...
    public function exportAllShift($year, $month, $id, $zone, $startDate, $viewType)
    {
        ...
        return Excel::download(new ShiftScheduleExport($year, $month, $id, $zone, $startDate, $endDate, $viewType), 'Attendance_From_' . $startDate->format('d-m-Y') . '_To_' . $date->format('d-m-Y') . '.xlsx');
    }
    ...
    public function bulkShift(StoreBulkShift $request)
    {
        ...
        $employeeData = User::with('employeeDetail')->whereIn('id', $cleaners)->get();
        ...
        $previousSchedules = EmployeeShiftSchedule::whereIn('user_id', $cleaners)
        ...
        foreach ($previousSchedules as $previousSchedule) {
            if (!is_null($previousSchedule->file) || $previousSchedule->file != '') {
                Files::deleteFile($previousSchedule->file, 'cleaner-shift-file/' . $previousSchedule->id);
            }
        ...
        if ($invalidShiftDays && $request->assign_shift_by == 'date') {
            $errorMessage = __('team chat.invalidShiftDayError') . ' ' . $dayOfName;
        ...
        return Reply::redirect($redirectUrl, __('team chat.employeeShiftAdded'));
    }
    ...
    public function bulkData($request, $date, $userData, $userId, &$insertData, $officeOpenDays, $overrideShiftId = null)
    {
        ...
        if ($date->greaterThanOrEqualTo($userData->employeeDetail->joining_date) && (is_null($officeOpenDays) || (is_array($officeOpenDays)))) {
            ...
            if ($request->hasFile('file')) {
                $fileName = Files::uploadLocalOrS3($request->file, 'cleaner-shift-file/' . $shift->id);
