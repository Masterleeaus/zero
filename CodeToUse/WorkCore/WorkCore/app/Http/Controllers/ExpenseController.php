...
class ExpenseController extends AccountBaseController
{
    ...
    {
        $this->expense = Expense::with(['user', 'site', 'category', 'transactions' => function($q){
    ...
    public function create()
    {
        ...
        // Get only current login cleaner sites
        if ($this->addPermission == 'added') {
            $this->sites = Site::where('added_by', user()->id)->orWhereHas('projectMembers', function ($query) {
        ...
        if (!is_null($this->projectId)) {
            $this->site = Site::with('projectMembers')->where('id', $this->projectId)->first();
            $this->projectName = $this->site->project_name;
    ...
    public function edit($id)
    {
        ...
        if (!is_null($userId)) {
            $this->sites = Site::with('members')->whereHas('members', function ($q) use ($userId) {
    ...
    public function update(StoreExpense $request, $id)
    {
        ...
        return Reply::successWithData(__('team chat.updateSuccess'), ['redirectUrl' => route('expenses.index')]);
    ...
    protected function getEmployeeProjects(Request $request)
    {
        ...
        // Get cleaner site
        if (!is_null($request->userId)) {
            $sites = Site::with('members')->whereHas('members', function ($q) use ($request) {
        ...
        if ($sites) {
            foreach ($sites as $site) {
                $data .= '<option data-currency-id="'. $site->currency_id .'" value="' . $site->id . '">' . $site->project_name . '</option>';
            }
        }
    ...
    protected function getCategoryEmployee(Request $request)
    {
        ...
        if (count($roleId ) == 1 && $roleId != null) {
            ...
            foreach ($users as $user) {
                ($user->hasRole('Manager')) ? array_push($managers, $user) : array_push($cleaners, $user);
            }
        }
        else {
            $cleaners = User::allEmployees(null, false);
        }
        ...
        if ($cleaners) {
            foreach ($cleaners as $cleaner) {
                if($cleaner->status == 'active' || $cleaner->id == $request->userId){
                    $data .= '<option ';

                    $content = ($cleaner->status == 'deactive') ? "<span class='badge badge-pill badge-danger border align-center ml-2 px-2'>Inactive</span>" : '';
                    $selected = $cleaner->id == $request->userId ? 'selected' : '';
                    $itsYou = $cleaner->id == user()->id ? "<span class='ml-2 badge badge-secondary pr-1'>". __('app.itsYou') .'</span>' : '';

                    $data .= 'data-content="<div class=\'d-inline-block mr-1\'><img class=\'taskEmployeeImg rounded-circle\' src=\'' . $cleaner->image_url . '\' ></div> '.$cleaner->name.$itsYou.$content.'"
                    value="' . $cleaner->id . '"'.$selected.'>'.$cleaner->name.'</option>';
                }
            }
        }
        else {
        ...
        return Reply::dataOnly(['status' => 'success', 'cleaners' => $data]);
    }
