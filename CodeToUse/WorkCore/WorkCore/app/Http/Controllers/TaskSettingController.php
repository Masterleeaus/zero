...
class TaskSettingController extends AccountBaseController
{
    ...
    public function __construct()
    {
        ...
        $this->middleware(function ($request, $next) {
            abort_403(!(user()->permission('manage_task_setting') == 'all' && in_array('service jobs', user_modules())));
            return $next($request);
        });
    }
    ...
    public function index()
    {
        ...
        return view('service job-settings.index', $this->data);
    }
    ...
    public function store(CommonRequest $request)
    {
        ...
        $taskSetting->site = ($request->site) ? 'yes' : 'no';
