...
use App\Http\Requests\Admin\Customer\StoreClientNote;
...
class ClientNoteController extends AccountBaseController
{
    ...
    public function index(ClientNotesDataTable $dataTable)
    {
        ...
        return $dataTable->render('customers.notes.index', $this->data);
    }
    ...
    public function create()
    {
        ...
        if (in_array('customer', user_roles())) {
            ...
            $clientProject = Site::where('client_id', user()->id)->pluck('id')->toArray();
        ...
        $this->view = 'customers.notes.create';
    ...
    public function show($id)
    {
        ...
        if ($this->note->ask_password == 1 && $callingFunction != 'showVerified') { // Password protected note should be called from showVerified function
            abort(403, __('team chat.permissionDenied'));
        }
        ...
        abort_403(!($viewClientNotePermission == 'all'
            ...
            || ($viewClientNotePermission == 'owned' && in_array(user()->id, $this->noteMembers) && in_array('cleaner', user_roles()))

            || ($viewClientNotePermission == 'both' && ($this->note->added_by == user()->id || in_array(user()->id, $this->noteMembers))) /* @phpstan-ignore-line */

            || (in_array('customer', user_roles()) && $this->note->is_client_show == 1)
            || ($this->note->type == 0 && $viewClientNotePermission != 'none')
        )
        );

        $this->pageTitle = __('app.customer') . ' ' . __('app.note');

        $this->view = 'customers.notes.show';
    ...
    public function store(StoreClientNote $request)
    {
        ...
        return Reply::successWithData(__('team chat.recordSaved'), ['redirectUrl' => in_array('customer', user_roles()) ? route('customer-notes.index') : route('customers.show', $note->client_id) . '?tab=notes']);
    }
    ...
    public function edit($id)
    {
        ...
        $this->view = 'customers.notes.edit';
    ...
    public function update(StoreClientNote $request, $id)
    {
        ...
        return Reply::successWithData(__('team chat.updateSuccess'), ['redirectUrl' => in_array('customer', user_roles()) ? route('customer-notes.index') : route('customers.show', $note->client_id) . '?tab=notes']);
    ...
    public function askForPassword($id)
    {
        ...
        return view('customers.notes.verify-password', $this->data);
    }
    ...
    public function checkPassword(Request $request)
    {
        ...
        if (Hash::check($request->password, $this->customer->password)) {
            return Reply::success(__('team chat.passwordMatched'));
        }

        return Reply::error(__('team chat.incorrectPassword'));
    }
