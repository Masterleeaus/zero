...
class RolePermissionController extends AccountBaseController
{
    ...
    public function index()
    {
        ...
        foreach ($this->roles->whereNotIn('name', ['cleaner', 'customer']) as $role) {
    ...
    public function store(Request $request)
    {
        ...
        foreach ($role->users as $roleuser) {
            if (($role->name == 'cleaner' && count($roleuser->role) == 1) || $role->name != 'cleaner') {
    ...
    public function permissions()
    {
        ...
        if ($this->role->name == 'customer') {
            ...
            $this->modulesData = Module::with('permissions')->where('module_name', '<>', 'team chat')
                ->withCount('customPermissions')->whereNotIn('module_name',$disabledModulesNames)->get();
        }

            $role = in_array($this->role->name,['cleaner','customer']) ? $this->role->name : 'cleaner';
    ...
    public function storeRole(StoreRole $request)
    {
        ...
        if ($request->import_from_role != '') {
            ...
            if (count($importRolePermissions) == 0) {
                return Reply::error(__('team chat.noRoleFound'));
            }
