...
class QuickbookController extends AccountBaseController
{
    ...
    public function index()
    {
        if ($this->quickBooksClientId == '' || $this->quickBooksClientSecret == '') {
            return redirect()->back()->withErrors(['credential_error' => __('team chat.quickBooksCredentialsIncorrect')]);
        }
    ...
    public function callback()
    {
        ...
        return redirect(route('invoice-settings.index').'?tab=quickbooks')->with('connect_success', __('team chat.quickBooksConnectSuccess'));
    ...
    public function getCustomerObj($dataService, $invoice)
    {
        ...
        $customerArray = $dataService->Query("select * from Customer where PrimaryEmailAddr='" . $invoice->customer->email . "'");
        $error = $dataService->getLastError();
        ...
        $customerDetails = [
            'DisplayName' => $invoice->customer->clientDetails->company_name ?? $invoice->customer->name,
            'GivenName' => $invoice->customer->name,
            'CurrencyRef' => [
                'value' => $invoice->currency->currency_code,
        ...
        if (!is_null($invoice->customer->email)) {
            $customerDetails['PrimaryEmailAddr'] = ['Address' => $invoice->customer->email];
    ...
    public function createPayment(ModelsPayment $payment)
    {
        ...
        $paymentObj = Payment::create([
            'CustomerRef' => [
                'value' => $payment->invoice->customer->clientDetails->quickbooks_client_id
            ],
