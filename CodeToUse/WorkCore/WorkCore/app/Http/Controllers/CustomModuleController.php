...
class CustomModuleController extends AccountBaseController
{
    ...
    public function store(Request $request)
    {
        ...
        // Filename Like codecanyon-0gOuGKoY-zoom-meeting-module-for-workcore.zip
        ...
        if ($validateModule['status'] == true) {
            // Move files to Modules if modules belongs to this service / extra
    ...
    public function validateModule($moduleName)
    {
        ...
        if (!isset($config['parent_envato_id']) || $config['parent_envato_id'] !== config('froiden_envato.envato_item_id')) {
            return [
                'status' => false,
                'message' => 'You are installing the wrong module for this service / extra'
            ];
        }
        ...
        if ($config['parent_envato_id'] !== config('froiden_envato.envato_item_id')) {
            return [
                'status' => false,
                'message' => 'You are installing wrong module for this service / extra'
            ];
        }
        ...
        // Check if parent_product_name is defined and matches the application's service / extra name
