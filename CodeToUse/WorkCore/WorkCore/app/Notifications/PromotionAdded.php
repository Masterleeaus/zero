...
class PromotionAdded extends BaseNotification
{
    ...
    public function toMail(object $notifiable): MailMessage
    {
        ...
        if($this->promotion->promotion == 1){
            ...
            $zone = $this->promotion->current_department_id == $this->promotion->previous_department_id ? __('email.incrementPromotion.same') : ' <b>'.$this->promotion->currentDepartment->team_name.'</b>';
            $content = __('email.incrementPromotion.text') . ' <b>' . $this->promotion->currentDesignation->name . '</b> ' . __('email.incrementPromotion.in') . ' ' . $zone . ' ' . __('app.menu.teams') . '! 🎊 <br><br>' .
            ...
            $zone = $this->promotion->current_department_id == $this->promotion->previous_department_id ? __('email.decrementPromotion.same') : ' <b>'.$this->promotion->currentDepartment->team_name.'</b>';
            $content = __('email.decrementPromotion.updateText') . ' ' . __('app.from') . ' <b> ' . $this->promotion->previousDesignation->name . ' </b>' . __('app.to') . ' <b>' . $this->promotion->currentDesignation->name . '</b> ' . __('email.decrementPromotion.in') . ' ' . $zone . ' ' . __('app.menu.teams') . '! <br><br>' .
    ...
    public function toSlack($notifiable)
    {
        try {
            ...
            if($this->promotion->promotion == 1){
                $subject = '🚀 ' . __('email.incrementPromotion.subject') . ' ' . $notifiable->name;
                $zone = $this->promotion->current_department_id == $this->promotion->previous_department_id ? __('email.incrementPromotion.same') : ' *' . optional($this->promotion->currentDepartment)->team_name . '*';
                $content = __('email.incrementPromotion.text') . ' *' . optional($this->promotion->currentDesignation)->name . '* ' .
                __('email.incrementPromotion.in') . ' ' . $zone . ' ' . __('app.menu.teams') . '! 🎊' . "\n\n" .
                __('email.incrementPromotion.text1') . "\n\n" . __('email.incrementPromotion.text2');

            }else{
                $subject = '🚀 ' . __('email.decrementPromotion.subject') . ' ' . $notifiable->name;
                $zone = $this->promotion->current_department_id == $this->promotion->previous_department_id ? __('email.decrementPromotion.same') : ' *' . optional($this->promotion->currentDepartment)->team_name . '*';
                $content = __('email.decrementPromotion.updateText') . ' ' . __('app.from') . ' <b> ' . $this->promotion->previousDesignation->name . ' </b>' . __('app.to') . ' <b>' . $this->promotion->currentDesignation->name . '</b> ' . __('email.decrementPromotion.in') . ' ' . $zone . ' ' . __('app.menu.teams') . '! <br><br>' .
                __('email.decrementPromotion.text2') . '<br><br>';
