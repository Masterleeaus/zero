...
class SubTaskAssigneeAdded extends BaseNotification
{
    ...
    public function __construct(SubTask $subTask)
    {
        ...
        $this->company = $this->subTask->service job->company;
    ...
    public function toMail($notifiable): MailMessage
    {
        ...
        $url = route('service jobs.show', [$this->subTask->service job->id, 'view' => 'sub_task']);
        $url = getDomainSpecificUrl($url, $this->company);

        $content = $this->subTask->title . ' ' . __('email.subTaskAssigneeAdded.subject') . '.' . '<br>' . ((!is_null($this->subTask->service job->site)) ? __('app.site') . ' - ' . $this->subTask->service job->site->project_name : '') . '<br>';
    ...
    {
        return [
            'id' => $this->subTask->service job->id,
