...
class EmployeeDocumentExpiryAlert extends Notification implements ShouldQueue
{
    ...
    public function toMail(object $notifiable): MailMessage
    {
        ...
        if ($this->alertType === 'expiring_soon') {
            $message->subject('Document Expiry Alert - ' . $this->document->document_name)
                ...
                // ->action('View Document', route('cleaner-document-expiry.download', md5($this->document->id)))
                // ->line('Thank you for using our application!');
        } else {
            $message->subject('Alert - Cleaner Document Expiring Soon')
                ->greeting('Hello ' . $notifiable->name . '!')
                ->line('An cleaner document is expiring soon and requires attention.')
                ->line('**Document Details:**')
                ->line('- Cleaner: ' . $this->document->user->name)
                ->line('- Document Name: ' . $this->document->document_name)
                ->line('- Document Number: ' . ($this->document->document_number ?: 'N/A'))
                ->line('- Expiry Date: ' . $this->document->expiry_date->format('d M Y'))
                ->line('- Days until expiry: ' . $this->document->days_until_expiry)
                ->line('Please follow up with the cleaner to ensure document renewal.');
                // ->action('View Document', route('cleaner-document-expiry.download', md5($this->document->id)))
                // ->line('Thank you for using our application!');
        }
    ...
    public function toArray(object $notifiable): array
    {
        if ($this->alertType === 'expiring_soon') {
            ...
            return [
                'title' => 'HR Alert - Cleaner Document Expiring',
                'message' => 'Cleaner ' . $this->document->user->name . '\'s document "' . $this->document->document_name . '" is expiring in ' . $this->document->days_until_expiry . ' days.',
                'document_id' => $this->document->id,
