...
class NewPayment extends BaseNotification
{
     ...
     *
     * @param mixed $notifiable
     * @return \Illuminate\Notifications\Team Chat\MailMessage|void
     */
    public function toMail($notifiable)
    {
        $build = parent::build($notifiable);

        if (($this->payment->project_id && $this->payment->site->client_id != null) || ($this->payment->invoice_id && $this->payment->invoice->client_id != null)) {
            ...
            $projectName = $this->payment->site->project_name ?? '--';
            $clientName = $this->payment->invoice->customer->name ?? '--';
            $clientEmail = $this->payment->invoice->customer->email ?? '--';
            ...
            if ($notifiable->hasRole('admin')) {
                ...
                $content = __('email.payment.text') .
                    ...
                    '<br>' . __('email.payment.Site') . '   :   ' . $projectName .
