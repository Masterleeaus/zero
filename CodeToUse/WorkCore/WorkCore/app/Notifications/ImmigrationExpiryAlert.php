...
class ImmigrationExpiryAlert extends Notification implements ShouldQueue
{
    ...
    public function toMail(object $notifiable): MailMessage
    {
        ...
        if ($this->alertType === 'expiring_soon') {
            ...
            $message->subject('Alert - Cleaner ' . $documentName . ' Expiring Soon')
                ->greeting('Hello ' . $notifiable->name . '!')
                ->line('An cleaner ' . strtolower($documentName) . ' is expiring soon and requires attention.')
                ->line('**' . $documentName . ' Details:**')
                ->line('- Cleaner: ' . $this->immigrationDocument->user->name)
                ->line('- ' . $documentName . ' Number: ' . $documentNumber)
                ->line('- Issue Date: ' . $this->immigrationDocument->issue_date->format('d M Y'))
                ->line('- Expiry Date: ' . $this->immigrationDocument->expiry_date->format('d M Y'))
                ->line('- Days until expiry: ' . $daysUntilExpiry)
                ->line('Please follow up with the cleaner to ensure ' . strtolower($documentName) . ' renewal.');
        }
    ...
    public function toArray(object $notifiable): array
    {
        ...
        if ($this->alertType === 'expiring_soon') {
            ...
            return [
                'title' => 'HR Alert - Cleaner ' . $documentName . ' Expiring',
                'message' => 'Cleaner ' . $this->immigrationDocument->user->name . '\'s ' . strtolower($documentName) . ' "' . $documentNumber . '" is expiring in ' . $daysUntilExpiry . ' days.',
                'document_id' => $this->immigrationDocument->id,
                'document_type' => $this->documentType,
