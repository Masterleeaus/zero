...
class DailyScheduleNotification extends BaseNotification
{
    ...
    public function via($notifiable)
    {
        ...
        $modulesToCheck = ['service jobs', 'events', 'holidays', 'leaves', 'recruit'];
        ...
        if ($this->emailSetting->send_push == 'yes' && push_setting()->beams_push_status == 'active') {
            ...
            $pushNotification->sendPushNotifications($pushUsersIds, __('email.dailyScheduleReminder.subject', ['date' => now()->format($this->company->date_format)]), $this->userData['service jobs'] . ' ' . $this->userData['events'] . ' ' . $this->userData['holidays'] . ' ' . $this->userData['leaves'] . ' ' . $this->userData['interview']);
        }
    ...
    public function toMail($notifiable)
    {
        ...
        if (in_array('service jobs', $this->userModules)) {
            $content .= '<br>' . __('email.dailyScheduleReminder.taskText') . ': <a class="text-dark-grey text-decoration-none" href=' . $url . '> ' . $this->userData['service jobs'] . '</a>';
        }
    ...
    public function userModules($userId)
    {
        ...
        elseif (in_array('cleaner', $userRoles)) {
            $module = $module->where('type', 'cleaner');
        }
    ...
    public function toSlack($notifiable)
    {
        ...
        if (in_array('service jobs', $this->userModules)) {
            $content .= __('email.dailyScheduleReminder.taskText') . ': ' . '<' . $url . '|' . $this->userData['service jobs'] . '>' . "\n";
        }
