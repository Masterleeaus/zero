    ...
    public function domPdfObjectForDownload()
    {
        ...
        foreach ($cleaners as $cleaner) {
            ...
            foreach ($cleaner->attendance as $attendance) {
                ...
                if (isset($shiftScheduleCollection[$clockInTime->copy()->startOfDay()->toDateTimeString()])) {
                    ...
                    if ($clockInTime->between($shiftStartTime, $shiftEndTime)) {
                        $final[$cleaner->id . '#' . $cleaner->name][$clockInTime->day] = '&check;';
                    ...
                    elseif ($clockInTime->betweenIncluded($shiftStartTime->copy()->subDay(), $shiftEndTime->copy()->subDay())) {
                        $final[$cleaner->id . '#' . $cleaner->name][$clockInTime->copy()->subDay()->day] = '&check;';
            ...
            if ($cleaner->employeeDetail->joining_date->greaterThan(Carbon::parse(Carbon::parse('01-' . $this->month . '-' . $this->year)))) {
                if ($this->month == $cleaner->employeeDetail->joining_date->format('m') && $this->year == $cleaner->employeeDetail->joining_date->format('Y')) {
                ...
                if (($this->month < $cleaner->employeeDetail->joining_date->format('m') && $this->year == $cleaner->employeeDetail->joining_date->format('Y')) || $this->year < $cleaner->employeeDetail->joining_date->format('Y')) {
