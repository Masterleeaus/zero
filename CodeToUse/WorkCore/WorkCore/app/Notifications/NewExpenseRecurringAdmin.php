...
class NewExpenseRecurringAdmin extends BaseNotification
{
    ...
    public function via($notifiable)
    {
        ...
        if ($this->emailSetting->send_push == 'yes' && push_setting()->beams_push_status == 'active') {
            ...
            $pushNotification->sendPushNotifications($pushUsersIds, __('email.newExpense.subject'), $this->expense->item_name);
        }
    ...
    public function toMail($notifiable)
    {
        ...
        $content = __('email.newExpense.subject') . '.' . '<br>' . __('app.cleaner') . ': ' . $this->expense->user->name . '<br>' . __('modules.expenses.itemName') . ': ' . $this->expense->item_name . '<br>' . __('app.price') . ': ' . $this->expense->currency->currency_symbol . $this->expense->price;
