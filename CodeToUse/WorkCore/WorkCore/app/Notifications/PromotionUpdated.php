...
class PromotionUpdated extends BaseNotification
{
    ...
    public function __construct(Promotion $promotion, $previousDesignationId, $previousDepartmentId)
    {
        ...
        $this->previousDesignation = Role::find($this->previousDesignationId);
    ...
    public function toMail($notifiable)
    {
        ...
        if ($currentDesignation == $previousDesignation) {
            ...
            $role = __('app.from'). ' <b>' .$previousDesignation . ' </b>' . __('app.to') . ' <b>' . $currentDesignation. ' </b>';
        }

        if ($currentDepartment == $previousDepartment) {
            if($this->promotion->promotion == 1){
                $zone = __('email.incrementPromotion.in') . ' ' . __('email.incrementPromotion.same');
            }else{
                $zone = __('email.decrementPromotion.in') . ' ' . __('email.decrementPromotion.same');
            }
        }
        else {
            $zone = __('app.from') . ' <b>' .$previousDepartment . ' </b>' . __('app.to') . ' <b>' . $currentDepartment. ' </b>';
        }

        if($this->promotion->promotion == 1){
            $content = __('email.incrementPromotion.updateText') . ' ' . $role . ' ' .
                $zone . ' ' . __('app.menu.teams') . '! 🎊 <br><br>' .
                __('email.incrementPromotion.text2') . '<br><br>';
        }else{
            $content = __('email.decrementPromotion.updateText') . ' ' . $role . ' ' .
                $zone . ' ' . __('app.menu.teams') . '! <br><br>' .
    ...
    public function toArray(object $notifiable): array
    {
        ...
        if($this->promotion->promotion == 1){
            ...
            $content = __('email.incrementPromotion.updateText') . ' ' . $role . ' ' . $zone . ' ' . __('app.menu.teams') . '! 🎊';
        }else{
            $subject = __('email.decrementPromotion.subject');
            $content = __('email.decrementPromotion.updateText') . ' ' . $role . ' ' . $zone . ' ' . __('app.menu.teams') . '!';
        }
    ...
    public function toSlack($notifiable)
    {
        try {
            ...
            if ($currentDesignation == $previousDesignation) {
                ...
                $role = __('app.from'). ' *' .$previousDesignation . '* ' . __('app.to') . ' *' . $currentDesignation. '* ';
            }
            ...
            if ($currentDepartment == $previousDepartment) {
                ...
                $zone = __('app.from') . ' *' .$previousDepartment . '* ' . __('app.to') . ' *' . $currentDepartment. '* ';
            }
