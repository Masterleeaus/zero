...
class DailyTimeLogReport extends BaseNotification
{
    ...
    public function domPdfObjectForDownload()
    {
        ...
        $cleaners = User::select('users.id', 'users.name')
        ...
        foreach ($cleaners as $cleaner) {
            $employeeData[$cleaner->name] = [];
            $employeeData[$cleaner->name]['timelog'] = 0;
            $employeeData[$cleaner->name]['timelogBreaks'] = 0;

            if (count($cleaner->timeLogs) > 0) {

                foreach ($cleaner->timeLogs as $timeLog) {
                    $employeeData[$cleaner->name]['timelog'] += $timeLog->total_minutes;

                    if (count($timeLog->breaks) > 0) {
                        foreach ($timeLog->breaks as $timeLogBreak) {
                            $employeeData[$cleaner->name]['timelogBreaks'] += $timeLogBreak->total_minutes;
                        }
                    }
                }
            }
        ...
        $pdf->loadView('timelog-report', ['cleaners' => $employeeData, 'date' => $now, 'company' => $company]);
