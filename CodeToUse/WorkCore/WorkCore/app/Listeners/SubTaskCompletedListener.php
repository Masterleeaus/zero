...
class SubTaskCompletedListener
{
    ...
    public function handle(SubTaskCompletedEvent $event)
    {
        if ($event->status == 'completed') {
            Notification::send($event->subTask->service job->users, new SubTaskCompleted($event->subTask));
        }

        elseif ($event->status == 'created') {
            Notification::send($event->subTask->service job->users, new SubTaskCreated($event->subTask));
        }
