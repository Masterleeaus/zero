    ...
    */
    public function collection()
    {
        ...
        foreach ($cleaners as $cleaner) {
            $leaveData = [];
            $leaveQuotaHistory = $this->getAllowedLeavesQuota($cleaner);

            foreach ($months as $month) {
                $leavesTakenInMonth = Leave::where('user_id', $cleaner->id)
            ...
            $futureLeaves = Leave::where('user_id', $cleaner->id)
    ...
    protected function getAllowedLeavesQuota($cleaner)
    {
        if (!$this->thisMonthStartDate->eq($this->forMontDate)) {
            return $cleaner->leaveQuotaHistory;
        }

        $leaveQuotas = $cleaner->leaveTypes;
        $allowedLeavesQuota = collect([]);

        foreach ($leaveQuotas as $leaveQuota) {
            if ($leaveQuota->leaveType && ($leaveQuota->leaveType->leaveTypeCondition($leaveQuota->leaveType, $cleaner))) {
