...
class ShiftScheduleExport implements FromCollection, WithHeadings, WithMapping, WithEvents
{
    ...
    public function __construct($year, $month, $id, $zone, $startdate, $enddate, $viewType)
    {
    ...
    public function collection()
    {
        ...
        if ($this->zone != 'all') {
            $cleaners = $cleaners->where('employee_details.department_id', $this->zone);
        }

        if ($this->userId != 'all') {
            $cleaners = $cleaners->where('users.id', $this->userId);
        }
        ...
        foreach ($cleaners as $cleaner) {
            $employeedata[$employee_index]['employee_name'] = $cleaner->name;
