...
class ProjectwiseTimeLogExport implements FromCollection, WithMapping, WithHeadings, WithStyles, WithCustomStartCell
{
    ...
    {
        $query = ProjectTimeLog::with(['user', 'site', 'service job', 'breaks', 'activeBreak'])
        ...
        if ($this->projectId && $this->projectId !== 'all') {
            $query->where('sites.id', $this->projectId);
        }

        return $query->whereNull('service jobs.deleted_at')
        ...
        foreach ($timelogs as $index => $timelog) {
            ...
            if (!isset($projectLogs[$timelog->project_id])) {
                $projectLogs[$timelog->project_id] = [
                    'project_name' => $timelog->site?->project_name,
                    'total_minutes' => 0,
                    'break_minutes' => 0,
    ...
    public function styles(Worksheet $sheet)
    {
        ...
        // Applying merge cells for cleaner names and center alignment
