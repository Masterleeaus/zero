...
class AttendanceExport implements FromCollection, WithHeadings, WithMapping, WithEvents
{
    ...
    public function __construct($year, $month, $id, $zone, $role, $startdate, $enddate)
    {
    ...
    public static function afterSheet(AfterSheet $event)
    {
        ...
        for ($index = 0; $index < $total; $index++) {
            ...
            for ($i = 1; $i <= $total_day; $i++) {
                if (
                    $emp_status[$index]['dates'][$i]['total_hours'] > 0 ||
                    $emp_status[$index]['dates'][$i]['comments']['clock_in_time'] == 1) 
                    {
    ...
    public function collection()
    {
        ...
        $cleaners = EmployeeDetails::join('users', 'employee_details.user_id', '=', 'users.id');

        if ($id != 'all') {
            if ($this->viewAttendancePermission == 'owned') {
                $cleaners->where('users.id', user()->id);
            }
            else {
                $cleaners->where('users.id', $id);
            }
        }
        ...
        if ($this->zone != 'all') {
            $cleaners->where('employee_details.department_id', $this->zone);
        }

        if ($this->role != 'all') {
            $cleaners->where('employee_details.designation_id', $this->role);
        }

        $cleaners->where(function ($query) use ($startDate, $endDate) {
        ...
        $cleaners = $cleaners->select('users.name', 'users.id')->get();
        $employeedata = array();
        $emp_attendance = 1;
        $employee_index = 0;
        ...
        foreach ($cleaners as $cleaner) {
            ...
            foreach ($period->toArray() as $date) {
                ...
                if ($date->lessThan(now()) && !$attendances->whereBetween('clock_in_time', [$date->copy()->startOfDay(), $date->copy()->endOfDay()])->count()) {
                    ...
                    foreach ($leavesDates as $leave) { // check cleaner leaves
                ...
                if ($employee_temp && $employee_temp[1] == $date) {
                    ...
                    $employeedata[$employee_index]['dates'][$emp_attendance] = [
                        ...
                        'comments' => [
                            ...
                            'clock_in_time' => !empty($clock_in) && $clock_in !== null ? 1 : 0,
                        ],
                    ];
                    $emp_attendance++;
                }
