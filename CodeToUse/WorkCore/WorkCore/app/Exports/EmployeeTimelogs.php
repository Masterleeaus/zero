...
class EmployeeTimelogs extends AccountBaseController implements FromView, ShouldAutoSize, WithStyles, WithEvents
{
    ...
    public function view(): View
    {
        ...
        if (!is_null($cleaner) && $cleaner !== 'all') {
            $this->cleaners = $this->cleaners->where('project_time_logs.user_id', $cleaner);
        }

        if (!is_null($projectId) && $projectId !== 'all') {
            $this->cleaners = $this->cleaners->where('project_time_logs.project_id', '=', $projectId);
        }

        if ($this->viewTimelogPermission == 'owned') {
            $this->cleaners = $this->cleaners->where('project_time_logs.user_id', user()->id);
        }

        if ($this->viewTimelogPermission == 'added') {
            $this->cleaners = $this->cleaners->where('project_time_logs.added_by', user()->id);
        }

        if ($this->viewTimelogPermission == 'both') {
            $this->cleaners = $this->cleaners->where(function ($q) {
                $q->where('project_time_logs.added_by', user()->id)
                    ->orWhere('project_time_logs.user_id', '=', user()->id);
            });
        }

        $this->cleaners = $this->cleaners->with('employeeDetails')->groupBy('project_time_logs.user_id')
            ->orderBy('users.name')
            ->get();

        foreach ($this->cleaners as $cleaner) {
            $cleaner->total_days = $this->countTotalDays($startDate, $endDate);
            $cleaner->holidays = $this->countHolidays($cleaner, $startDate, $endDate);
            $cleaner->total_weekends = $this->countWeekends($startDate, $endDate);
            $cleaner->total_working_days = $cleaner->total_days - $cleaner->total_weekends - $cleaner->holidays->count();
            $cleaner->total_hours = $this->calculateTotalHours($cleaner, $startDate, $endDate);
        }
