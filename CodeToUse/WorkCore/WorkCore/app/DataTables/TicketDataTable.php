...
class   TicketDataTable extends BaseDataTable
{
    ...
    public function dataTable($query)
    {
        ...
        $datatables->addColumn('others', function ($row) {
            ...
            $others .= '<div>' . __('modules.service jobs.priority') . ': ' . __('app.' . $row->priority) . '</div> ';
        ...
        $datatables->editColumn('user_id', function ($row) {
            ...
            $viewComponent = $row->requester->hasRole('cleaner') ? 'components.cleaner' : 'components.customer';
    ...
    public function query(Ticket $model)
    {
        ...
        $model = $model->with('requester', 'agent', 'latestReply.user:id,name,image', 'group.enabledAgents')
            ->select('tickets.*')
            ->leftJoin('sites', 'sites.id', 'tickets.project_id')
            ->join('users', 'users.id', '=', 'tickets.user_id');

        // filter where site is soft deleted
        ...
        if (!is_null($request->projectID) && $request->projectID != 'all') {
            $model->whereHas('site', function ($q) use ($request) {
    ...
    protected function getColumns()
    {
        ...
        $data = [
            ...
            __('modules.tickets.requesterName') => ['data' => 'user_id', 'name' => 'user_id', 'visible' => !in_array('customer', user_roles()), 'exportable' => false, 'title' => __('modules.tickets.requesterName'), 'width' => '20%'],
