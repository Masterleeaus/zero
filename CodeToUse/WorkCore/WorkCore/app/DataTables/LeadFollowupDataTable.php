...
class LeadFollowupDataTable extends BaseDataTable
{
        ...
        return datatables()
            ...
            ->addColumn('action', function ($row) {
                ...
                if ($this->editFollowupPermission == 'all' || ($this->editFollowupPermission == 'added' && user()->id == $row->added_by)) {
                    $action .= '<a class="dropdown-item edit-table-row-enquiry" href="javascript:;" data-followup-id="' . $row->id . '">
                ...
                if ($this->deleteFollowupermission == 'all' || ($this->deleteFollowupermission == 'added' && user()->id == $row->added_by)) {
                    $action .= '<a class="dropdown-item delete-table-row-enquiry" href="javascript:;" data-followup-id="' . $row->id . '">
    ...
    {
        $enquiry = $model->newQuery();

        if (request()->has('leadId') && request()->leadId != '') {
            $enquiry = $enquiry->where('deal_id', request()->leadId);
        }

        if ($this->viewFollowupPermission == 'added') {
            $enquiry->where('added_by', user()->id);
        }
    ...
    protected function getColumns()
    {
        return [
            ...
            __('modules.enquiry.nextFollowUp') => ['data' => 'next_follow_up', 'name' => 'next_follow_up', 'title' => __('modules.enquiry.nextFollowUp')],
