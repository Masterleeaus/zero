...
class TimeLogsDataTable extends BaseDataTable
{
    ...
    public function dataTable($query)
    {
            ...
            if (!is_null($row->end_time)) {
                ...
                if (
                    ...
                    || ($this->editTimelogPermission == 'owned'
                        && (($row->site && $row->site->client_id == $userId) || $row->user_id == $userId)
                    )
                    || ($this->editTimelogPermission == 'both' && (($row->site && $row->site->client_id == $userId) || $row->user_id == $userId || $row->added_by == $userId))
                ) {
                    if (is_null($row->project_id) || ($row->site && is_null($row->site->deleted_at))) {
        ...
        $datatables->editColumn('name', function($row) {
            ...
            return view('components.cleaner', ['user' => $row->user, 'disabledLink' => $isClient]);
        });
        ...
        $datatables->editColumn('project_name', function ($row) {
            ...
            if (!is_null($row->project_id) && !is_null($row->task_id)) {
                $name .= '<h5 class="f-13 text-darkest-grey"><a href="' . route('service jobs.show', [$row->task_id]) . '" class="openRightModal">' . $row->service job->heading . '</a></h5><div class="text-muted">' . $row->service job->site->project_name . '</div>';
            } else if (!is_null($row->project_id)) {
                $name .= '<a href="' . route('sites.show', [$row->project_id]) . '" class="text-darkest-grey ">' . $row->site->project_name . '</a>';
            } else if (!is_null($row->task_id)) {
                $name .= '<a href="' . route('service jobs.show', [$row->task_id]) . '" class="text-darkest-grey openRightModal">' . $row->service job->heading . '</a>';
            }

            return $name;
        });
        $datatables->addColumn('task_name', fn($row) => $row->service job?->heading ?? '--');
        $datatables->addColumn('task_project_name', fn($row) => $row->site?->project_name ?? '--');
        $datatables->addColumn('short_code', fn($row) => $row->site?->project_short_code ?? '--');
        $datatables->addIndexColumn();
        $datatables->setRowId(fn($row) => 'row-' . $row->id);
        $datatables->orderColumn('project_name', 'service jobs.heading $1');
    ...
    public function query(ProjectTimeLog $model)
    {
        ...
        $model = $model->with('user', 'user.employeeDetail', 'user.employeeDetail.role', 'user.session', 'service job', 'service job.site', 'breaks', 'activeBreak', 'site');

        if (!in_array('customer', user_roles()) && $request->has('project_admin') && $request->project_admin == 1) {
        ...
        $model = $model->leftJoin('roles', 'employee_details.designation_id', '=', 'roles.id')
            ->leftJoin('service jobs', 'service jobs.id', '=', 'project_time_logs.task_id')
            ->leftJoin('sites', 'sites.id', '=', 'service jobs.project_id');

        $model = $model->select('project_time_logs.id', 'project_time_logs.start_time', 'project_time_logs.end_time', 'project_time_logs.total_hours', 'project_time_logs.total_minutes', 'project_time_logs.memo', 'project_time_logs.user_id', 'service jobs.project_id', 'project_time_logs.task_id', 'users.name', 'users.image', 'project_time_logs.hourly_rate', 'project_time_logs.earnings', 'project_time_logs.approved', 'service jobs.heading', 'sites.project_name', 'roles.name as designation_name', 'project_time_logs.added_by', 'sites.project_admin', 'employee_details.hourly_rate as user_hour_rate', 'employee_details.reporting_to as reporting_manager', 'project_time_logs.rejected', 'employee_details.department_id');
        ...
        if (!is_null($cleaner) && $cleaner !== 'all') {
            $model->where(function ($q) use ($cleaner, $logTimeFor, $userId) {
                $q->where('project_time_logs.user_id', $cleaner);

                ($this->approveTimelogPermission == 'all' && $logTimeFor->approval_required == 1) ? $q->orWhere('employee_details.reporting_to', $cleaner) : '';
            });
        }

        if (!is_null($zone) && $zone !== 'all') {
            $model->where('employee_details.department_id', $zone);
        }
        ...
        if (($request->has('project_admin') && $request->project_admin != 1) || !$request->has('project_admin')) {
            ...
            if ($this->viewTimelogPermission == 'owned') {
                ...
                if ($projectId != 0 && $projectId != null && $projectId != 'all' && !in_array('customer', user_roles())) {
                    $model->where('sites.project_admin', '<>', $userId);
                }
            }
        ...
        if (!$this->ignoreDeletedAtCondition) {
            $model->whereNull('service jobs.deleted_at');
        }
    ...
    protected function getColumns()
    {
        $data = [
            ...
            __('app.service job') => ['data' => 'project_name', 'name' => 'service jobs.heading', 'exportable' => false, 'width' => '200', 'title' => __('app.service job')],
            __('app.service jobs') => ['data' => 'task_name', 'visible' => false, 'name' => 'task_name', 'title' => __('app.service jobs')],
            __('app.site') => ['data' => 'task_project_name', 'visible' => false, 'name' => 'task_project_name', 'title' => __('app.site')],
            __('app.cleaner') => ['data' => 'name', 'name' => 'users.name', 'exportable' => false, 'title' => __('app.cleaner')],
