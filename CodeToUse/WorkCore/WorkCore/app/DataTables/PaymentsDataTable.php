...
class PaymentsDataTable extends BaseDataTable
{
    ...
    public function dataTable($query)
    {
        ...
        return $datatables
            ...
            ->editColumn('project_id', function ($row) {
                ...
                return '<a class="text-darkest-grey" href="' . route('sites.show', $row->project_id) . '">' . $row->site->project_name . '</a>';
            })
            ...
            ->editColumn('client_id', function ($row) {
                ...
                if (!is_null($row->invoice_id) && isset($row->invoice->customer)) {
                    $user = $row->invoice->customer;
                } elseif (!is_null($row->project_id) && isset($row->site->customer)) {
                    $user = $row->site->customer;
                } elseif (!is_null($row->order_id) && isset($row->order->customer)) {
                    $user = $row->order->customer;
                }

                return $user ? view('components.customer', ['user' => $user]) : '--';
            })
            ->addColumn('client_name', function ($row) {
                if (!is_null($row->invoice_id) && isset($row->invoice->customer)) {
                    return $row->invoice->customer->name;
                }

                if (!is_null($row->project_id) && isset($row->site->customer)) {
                    return $row->site->customer->name;
                }

                if (!is_null($row->order_id) && isset($row->order->customer)) {
            ...
            ->editColumn('client_email', function ($row) {
                if (!is_null($row->invoice_id) && isset($row->invoice->customer->email)) {
                    return '<a class="text-darkest-grey" href="' . route('customers.show', $row->invoice->customer->id) . '">' . ucfirst($row->invoice->customer->email) . '</a>';
                    /** @phpstan-ignore-line */
                }

                if (!is_null($row->project_id) && isset($row->site->customer->email)) {
                    return '<a class="text-darkest-grey" href="' . route('customers.show', $row->site->customer->id) . '">' . ucfirst($row->site->customer->email) . '</a>';
                    /** @phpstan-ignore-line */
                }

                if (!is_null($row->order_id) && isset($row->order->customer->email)) {
                    return '<a class="text-darkest-grey" href="' . route('customers.show', $row->order->customer->id) . '">' . ucfirst($row->order->customer->email) . '</a>';
                    /** @phpstan-ignore-line */
                }
    ...
    public function query()
    {
        ...
        $model = Payment::with([
            'invoice.customer',
            'invoice.customer.clientDetails.company:id,logo,company_name',
            'order.customer',
            'order.customer.clientDetails.company:id,logo,company_name',
            'currency:id,currency_symbol,currency_code',
            'site:id,project_short_code,project_name'
        ])
        ...
        // if (!$this->trashedData) {
        //     $model = $model->whereNull('sites.deleted_at');
        // }
        ...
        if ($request->clientID != 'all' && !is_null($request->clientID)) {
            ...
            $model = $model->where(function ($query) use ($clientId) {
                $query->where('sites.client_id', $clientId)
        ...
        // For cleaners: restrict by view_payments permission (customers are already filtered above)
        if ($this->viewPaymentPermission !== 'all') {
            if ($this->viewPaymentPermission === 'added') {
                $model = $model->where('payments.added_by', user()->id);
            }
            if ($this->viewPaymentPermission === 'owned') {
                $model = $model->where(function ($query) {
            ...
            if ($this->viewPaymentPermission === 'both') {
                $model = $model->where(function ($query) {
                    $query->where('payments.added_by', user()->id)
                        ->orWhere('sites.client_id', user()->id)
                        ->orWhere('invoices.client_id', user()->id)
                        ->orWhere('orders.client_id', user()->id);
                });
            }
    ...
    protected function getColumns()
    {
        return [
            ...
            __('app.site') => ['data' => 'project_id', 'name' => 'project_id', 'title' => __('app.site'), 'width' => '10%'],
            __('app.invoice') . '#' => ['data' => 'invoice_number', 'name' => 'invoices.invoice_number', 'title' => __('app.invoice') . '#'],
            __('app.customer') => ['data' => 'client_id', 'name' => 'client_id.name', 'orderable' => false, 'title' => __('app.customer'), 'exportable' => false, 'visible' => (!in_array('customer', user_roles()) && in_array('customers', user_modules()))],
            __('app.customers') => ['data' => 'client_name', 'name' => 'client_name', 'visible' => false, 'title' => __('app.customer')],
