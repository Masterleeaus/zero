...
class ClientNotesDataTable extends BaseDataTable
{
        ...
        return datatables()
            ...
            ->addColumn('action', function ($row) {
                ...
                if ($row->ask_password == 1) {
                    $action .= '<a href="javascript:;" class="dropdown-item ask-for-password" data-customer-note-id="' . $row->id . '"><i class="fa fa-eye mr-2"></i>' . __('app.view') . '</a>';
                } else {
                    $action .= '<a href="' . route('customer-notes.show', $row->id) . '" class="openRightModal dropdown-item"><i class="fa fa-eye mr-2"></i>' . __('app.view') . '</a>';
                }
                ...
                if ($this->editClientNotePermission == 'all' || ($this->editClientNotePermission == 'added' && user()->id == $row->added_by) || ($this->editClientNotePermission == 'both' && user()->id == $row->added_by)) {
                    $action .= '<a class="dropdown-item openRightModal" href="' . route('customer-notes.edit', [$row->id]) . '">
            ...
            ->editColumn('title', function ($row) {
                if ($row->ask_password == 1) {
                    return '<a href="javascript:;" class="ask-for-password" data-customer-note-id="' . $row->id . '" style="color:black;">' . $row->title . '</a>';
                }

                return '<a href="' . route('customer-notes.show', $row->id) . '" class="openRightModal " style="color:black;">' . $row->title . '</a>';
            })
    ...
    {
        $dataTable = $this->setBuilder('customer-notes-table', 2)
            ->parameters([
                'initComplete' => 'function () {
                   window.LaravelDataTables["customer-notes-table"].buttons().container()
