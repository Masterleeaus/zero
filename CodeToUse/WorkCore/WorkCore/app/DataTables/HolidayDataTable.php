...
class HolidayDataTable extends BaseDataTable
{
    ...
    public function dataTable($query)
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('zone', function ($row) {
                $value = (!empty($row->department_id_json) && $row->department_id_json != 'null') ? collect (Holiday::zone(json_decode($row->department_id_json)))
            ...
            ->addColumn('role', function ($row) {
                $value = (!empty($row->designation_id_json) && $row->designation_id_json != 'null') ? collect( Holiday::role(json_decode($row->designation_id_json)))
                  ->map(function($val){
                    return '<ul>' . $val  . '</ul>';
            ...
            ->addColumn('employment_type', function ($row) {
                $value = !empty($row->employment_type_json) ? collect(json_decode($row->employment_type_json))
                        ->map(function ($employmentType) {
                            return '<ul>' . __('modules.cleaners.' . $employmentType) . '</ul>';
                        })
                        ->implode('') : '--';
                return $value !== '' ? $value : '--';
            })
            ...
            ->rawColumns(['check', 'action', 'employment_type','zone','role']);
    }
    ...
    protected function getColumns()
    {
        return [
            ...
            __('app.zone') => ['data' => 'zone', 'name' => 'zone', 'title' => __('app.zone')],
             __('app.role') => ['data' => 'role', 'name' => 'role', 'title' => __('app.role')],
             __('modules.cleaners.employmentType') => ['data' => 'employment_type', 'name' => 'employment_type', 'title' => __('modules.cleaners.employmentType')],
