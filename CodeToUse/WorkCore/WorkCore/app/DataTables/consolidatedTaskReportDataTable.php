...
class consolidatedTaskReportDataTable extends BaseDataTable
{
    ...
    public function query()
    {
        ...
        $timeLogQuery = DB::table('project_time_logs')
            ...
            ->select(
                ...
                DB::raw('IFNULL(SUM(CAST(project_time_logs.total_minutes AS UNSIGNED)), 0) - IFNULL(SUM(CAST(project_time_log_breaks.total_minutes AS UNSIGNED)), 0) AS spent_minutes')
            )
        ...
        $model = Service Job::leftJoinSub($timeLogQuery, 'time_logs', function ($join) {
                $join->on('service jobs.id', '=', 'time_logs.task_id');
            })
            ->select(
                'service jobs.id',
        ...
        if ($request->project_id !== null && $request->project_id != 'null' && $request->project_id != 'all' && $request->project_id != '') {
            $model = $model->where('service jobs.project_id', $request->project_id);
        }
        ...
        if ($request->assignedTo !== null && $request->assignedTo != 'null' && $request->assignedTo != 'all' && $request->assignedTo != '') {
            $model = $model->whereExists(function ($query) use ($request) {
                $query->select(DB::raw(1))
                    ->from('task_users')
                    ->whereColumn('task_users.task_id', 'service jobs.id')
                    ->where('task_users.user_id', $request->assignedTo);
            });
        }
        $model = $model->groupBy('service jobs.id', 'service jobs.heading', 'service jobs.estimate_hours');
    ...
    {
        $dataTable = $this->setBuilder('consolidated-service job-table')
            ->parameters([
                'initComplete' => 'function () {
                   window.LaravelDataTables["consolidated-service job-table"].buttons().container()
                ...
                'fnDrawCallback' => 'function( oSettings ) {
                    $("#consolidated-service job-table .select-picker").selectpicker();
                }',
                'columnDefs' => [
                    [
                        'targets' => 1,
    ...
    public function getColumns(): array
    {
        return [
            ...
            __('app.service job') => ['data' => 'heading', 'name' => 'heading', 'exportable' => true, 'title' => __('app.service job')],
            __('modules.service jobs.estimateHours') => ['data' => 'estimate_hours', 'name' => 'estimate_hours', 'title' => __('modules.service jobs.estimateHours')],
            __('modules.service jobs.totalHoursSpent') => ['data' => 'spent_minutes', 'name' => 'spent_minutes', 'title' => __('modules.service jobs.totalHoursSpent')],
        ];
