...
class ProductsDataTable extends BaseDataTable
{
    ...
    public function dataTable($query)
    {
        ...
        $datatables->addColumn('action', function ($row) {
            ...
            if (in_array('customer', user_roles())) {
                ...
                return '<button type="button" class="btn-secondary rounded f-14 add-service / extra" data-service / extra-id="' . $row->id . '" id="add-to-cart-btn-' . $row->id . '">
                        ' . $addToCart . '
                    </button>';
            }

            $action = '<div class="task_view">
            <a href="' . route('services / extras.show', [$row->id]) . '"
                class="taskView openRightModal text-darkest-grey f-w-500" data-service / extra-id="' . $row->id . '">' . __('app.view') . '</a>
            ...
            if ($this->editProductPermission == 'all' || ($this->editProductPermission == 'added' && user()->id == $row->added_by)) {
                $action .= '<a class="dropdown-item openRightModal" href="' . route('services / extras.edit', [$row->id]) . '">
            ...
            if ($this->deleteProductPermission == 'all' || ($this->deleteProductPermission == 'added' && user()->id == $row->added_by)) {
                $action .= '<a class="dropdown-item delete-table-row" href="javascript:;" data-service / extra-id="' . $row->id . '">
        ...
        $datatables->editColumn('name', function ($row) {
            ...
            return '<a href="' . route('services / extras.show', [$row->id]) . '" class="openRightModal text-darkest-grey" >' . $row->name . '</a>';
        });
        ...
        $datatables->editColumn('price', function ($row) {
            if (!is_null($row->taxes)) {
                ...
                foreach (json_decode($row->taxes) as $tax) {
                    $prodTax = Service / Extra::taxbyid($tax)->first();
        ...
        $customFieldColumns = CustomField::customFieldData($datatables, Service / Extra::CUSTOM_FIELD_MODEL);
    ...
    /**
     * @param Service / Extra $model
     * @return \Illuminate\Database\Eloquent\Builder
     */
    public function query(Service / Extra $model)
    {
        ...
        if ($request->searchText != '') {
            ...
            $model->where(function ($query) use ($safeTerm) {
                $query->where('services / extras.name', 'like', '%' . $safeTerm . '%')
                    ->orWhere('services / extras.price', 'like', '%' . $safeTerm . '%');
            });
        }

        if (user()->permission('view_product') == 'added') {
            $model->where('services / extras.added_by', user()->id);
        }

        if (in_array('customer', user_roles())) {
            $model->where('services / extras.allow_purchase', 1);
        }
    ...
    {
        $dataTable = $this->setBuilder('services / extras-table', 2)
            ->parameters([
                'initComplete' => 'function () {
                   window.LaravelDataTables["services / extras-table"].buttons().container()
    ...
    protected function getColumns()
    {
        ...
        $data = [
            ...
            __('app.menu.services / extras') => ['data' => 'name', 'name' => 'name', 'title' => __('app.menu.services / extras')],
            ...
            __('app.purchaseAllow') => ['data' => 'allow_purchase', 'name' => 'allow_purchase', 'visible' => !in_array('customer', user_roles()), 'title' => __('app.purchaseAllow')]
        ];
        ...
        return array_merge($data, CustomFieldGroup::customFieldsDataMerge(new Service / Extra()), $action);
    }
