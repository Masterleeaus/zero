...
class ProjectNotesDataTable extends BaseDataTable
{
        ...
        return datatables()
            ...
            ->addColumn('action', function ($row) {
                ...
                if ($row->ask_password == 1 && $row->type == '1') {
                    $action .= '<a href="javascript:;" class="dropdown-item ask-for-password" data-form-type="view" data-site-note-id="' . $row->id . '"><i class="fa fa-eye mr-2"></i>' . __('app.view') . '</a>';
                }
                else {
                    $action .= '<a href="' . route('site-notes.show', $row->id) . '" class="openRightModal dropdown-item"><i class="fa fa-eye mr-2"></i>' . __('app.view') . '</a>';
                }

                if ($this->editProjectNotePermission == 'all' || ($this->editProjectNotePermission == 'added' && user()->id == $row->added_by)) {
                    if ($row->ask_password == 1 && $row->type == '1') {
                        $action .= '<a href="javascript:;" class="dropdown-item ask-for-password" data-form-type="edit" data-site-note-id="' . $row->id . '"><i class="fa fa-edit mr-2"></i>' . __('app.edit') . '</a>';
                    }
                    else {
                        $action .= '<a href="' . route('site-notes.edit', $row->id) . '" class="openRightModal dropdown-item"><i class="fa fa-edit mr-2"></i>' . __('app.edit') . '</a>';
                    }
                }
            ...
            ->editColumn('note_title', function ($row) {
                if ($row->ask_password == 1) {
                    return '<a href="javascript:;" class="ask-for-password" style="color:black;" data-site-note-id="' . $row->id . '">' . $row->title . '</a>';
                }

                return '<a href="' . route('site-notes.show', $row->id) . '" class="openRightModal" style="color:black;">' . $row->title . '</a>';
    ...
    public function query(ProjectNote $model)
    {
        ...
        $sites = $model->where('project_notes.project_id', $request->projectID);
        ...
        if (in_array('customer', user_roles())) {
            ...
            $sites = $sites->where('project_notes.client_id', $userId);

            $sites->where(function ($query) {
                return $query->where('project_notes.is_client_show', 1)
                    ->where('project_notes.type', 1);
            });
            $sites->OrWhere(function ($query) use ($userId) {
                return $query->where('project_notes.type', 0)
                    ->where('project_notes.client_id', $userId);
            });
        }
        ...
        if (!in_array('admin', user_roles())) {
            ...
            if ($this->viewProjectNotePermission == 'added') {
                $sites->where('project_notes.added_by', user()->id);
        ...
        $sites->select('project_notes.*')->groupBy('id');
    ...
    {
        $dataTable = $this->setBuilder('site-notes-table', 2)
            ->parameters([
                'initComplete' => 'function () {
                   window.LaravelDataTables["site-notes-table"].buttons().container()
    ...
    protected function getColumns()
    {
        return [
            ...
            __('modules.customer.noteTitle') => ['data' => 'note_title', 'name' => 'title', 'title' => __('modules.customer.noteTitle')],
            __('modules.customer.noteType') => ['data' => 'note_type', 'name' => 'type', 'title' => __('modules.customer.noteType')],
