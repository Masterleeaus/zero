...
class ArchiveTasksDataTable extends BaseDataTable
{
    ...
    public function dataTable($query)
    {
        ...
        return datatables()
            ...
            ->addColumn('action', function ($row) {
                ...
                $action .= '<a href="' . route('service jobs.show', [$row->id]) . '" class="dropdown-item openRightModal"><i class="fa fa-eye mr-2"></i>' . __('app.view') . '</a>';
            ...
            ->addColumn('service job', fn($row) => $row->heading)
            ...
            ->editColumn('board_column', function ($row) use ($taskBoardColumns) {
                ...
                if (
                    ...
                    $status = '<select class="form-control select-picker change-status" data-service job-id="' . $row->id . '">';
    ...
    public function query(Service Job $model)
    {
        ...
        $model->leftJoin('users as creator_user', 'creator_user.id', '=', 'service jobs.created_by')
            ...
            ->selectRaw('service jobs.id, service jobs.added_by, sites.project_name, sites.project_admin, service jobs.heading, customer.name as clientName, creator_user.name as created_by, creator_user.image as created_image, service jobs.board_column_id,
             service jobs.due_date, taskboard_columns.column_name as board_column, taskboard_columns.label_color,
        ...
        if (($request->has('project_admin') && $request->project_admin != 1) || !$request->has('project_admin')) {
            if ($this->viewTaskPermission == 'owned') {
                ...
                if ($projectId != 0 && $projectId != null && $projectId != 'all') {
                    $model->where('sites.project_admin', '<>', user()->id);
                }
            }
        ...
        if ($request->searchText != '') {
            $model->where(function ($query) {
                ...
                $query->where('service jobs.heading', 'like', '%' . $safeTerm . '%')
                    ...
                    ->orWhere('service jobs.id', 'like', '%' . $safeTerm . '%')
    ...
    protected function getColumns()
    {
        return [
            ...
            __('modules.cleaners.hoursLogged') => ['data' => 'timeLogged', 'name' => 'timeLogged', 'title' => __('modules.cleaners.hoursLogged')],
            ...
            __('app.service job') . ' ' . __('app.status') => ['data' => 'status', 'name' => 'board_column_id', 'visible' => false, 'title' => __('app.service job')],
