...
class EstimateTemplateDataTable extends BaseDataTable
{
    ...
    public function dataTable($query)
    {
        ...
        return datatables()
            ...
            ->addColumn('action', function ($row) use ($userId) {
                ...
                $action .= ' <a href="' . route('quote-template.show', [$row->id]) . '" class="dropdown-item"><i class="fa fa-eye mr-2"></i>' . __('app.view') . '</a>';


                $action .= '<a class="dropdown-item" href="' . route('quotes.create') . '?template=' . $row->id . '">
                ...
                if (
                    ...
                    $action .= '<a class="dropdown-item" href="' . route('quote-template.edit', [$row->id]) . '">
    ...
    {
        $dataTable = $this->setBuilder('quote-template-table')
            ->parameters([
                'initComplete' => 'function () {
                    window.LaravelDataTables["quote-template-table"].buttons().container()
