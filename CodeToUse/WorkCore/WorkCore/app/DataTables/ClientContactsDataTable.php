...
class ClientContactsDataTable extends BaseDataTable
{
    ...
    public function dataTable($query)
    {
        ...
        return datatables()
            ...
            ->addColumn('action', function ($row) {
                ...
                $action .= '<a href="' . route('customer-contacts.show', [$row->id]) . '" class="dropdown-item"><i class="fa fa-eye mr-2"></i>' . __('app.view') . '</a>';
                if ($this->editClientPermission == 'all' || ($this->editClientPermission == 'added' && user()->id == $row->added_by) || ($this->editClientPermission == 'both' && user()->id == $row->added_by)) {
                    $action .= '<a class="dropdown-item openRightModal" href="' . route('customer-contacts.edit', [$row->id]) . '">
            ...
            ->editColumn('contact_name', function ($row) {
                return '<div class="media align-items-center">
                    ...
                    <a href="' . route('customer-contacts.show', [$row->id]) . '">' . $row->contact_name . '</a></h5>
                    </div>
                  </div>';
            })
    ...
    {
        $dataTable = $this->setBuilder('customers-table')
            ->parameters([
