...
class LeadNotesDataTable extends BaseDataTable
{
        ...
        return datatables()
            ...
            ->addColumn('action', function ($row) {
                ...
                if (!in_array('admin', user_roles()) && $row->ask_password == 1) {
                    $action .= '<a href="javascript:;" class="dropdown-item ask-for-password" data-enquiry-note-id="' . $row->id . '"><i class="fa fa-eye mr-2"></i>' . __('app.view') . '</a>';
                }
                else {
                    $action .= '<a href="' . route('enquiry-notes.show', $row->id) . '" class="openRightModal dropdown-item"><i class="fa fa-eye mr-2"></i>' . __('app.view') . '</a>';
                }
                ...
                if ($this->editLeadNotePermission == 'all' || ($this->editLeadNotePermission == 'added' && user()->id == $row->added_by) || ($this->editLeadNotePermission == 'both' && user()->id == $row->added_by)) {
                    $action .= '<a class="dropdown-item openRightModal" href="' . route('enquiry-notes.edit', [$row->id]) . '">
                ...
                if ($this->deleteLeadNotePermission == 'all' || ($this->deleteLeadNotePermission == 'added' && user()->id == $row->added_by) || ($this->deleteLeadNotePermission == 'both' && user()->id == $row->added_by)) {
                    $action .= '<a class="dropdown-item delete-table-row-enquiry" href="javascript:;" data-id="' . $row->id . '">
            ...
            ->editColumn('title', function ($row) {
                if (!in_array('admin', user_roles()) && $row->ask_password == 1) {
                    return '<a href="javascript:;" class="ask-for-password" style="color:black;" data-enquiry-note-id="' . $row->id . '">' . $row->title . '</a>';
                }
                else {
                    return '<a href="' . route('enquiry-notes.show', $row->id) . '" class="openRightModal" style="color:black;">' . $row->title . '</a>';
                }
            })
    ...
    public function query(LeadNote $model)
    {
        ...
        if (in_array('enquiry', user_roles())) {
            $notes->where('type', 0);
    ...
    {
        $dataTable = $this->setBuilder('enquiry-notes-table', 2)
            ->parameters([
                'initComplete' => 'function () {
                   window.LaravelDataTables["enquiry-notes-table"].buttons().container()
    ...
    protected function getColumns()
    {
        return [
            ...
            __('modules.customer.noteTitle') => ['data' => 'title', 'name' => 'title', 'title' => __('modules.customer.noteTitle')],
            __('modules.customer.noteType') => ['data' => 'type', 'name' => 'type', 'title' => __('modules.customer.noteType')],
