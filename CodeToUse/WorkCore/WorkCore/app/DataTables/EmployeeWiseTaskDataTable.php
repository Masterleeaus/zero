...
class EmployeeWiseTaskDataTable extends BaseDataTable
{
    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->editColumn('name', function ($row) {
                return view('components.cleaner', ['user' => $row]);
            })
                ...
                $join->on('users.id', '=', 'task_users.user_id')
                    ->when($startDate !== null && $endDate !== null, function($query) use ($startDate, $endDate) {
                        $query->whereExists(function($subquery) use ($startDate, $endDate) {
                            $subquery->from('service jobs')
                                ->whereRaw('service jobs.id = task_users.task_id')
                                ->where(function($q) use ($startDate, $endDate) {
                                    $q->whereBetween(DB::raw('DATE(service jobs.due_date)'), [$startDate, $endDate])
                                    ->orWhereBetween(DB::raw('DATE(service jobs.start_date)'), [$startDate, $endDate]);
                                });
                        });
                    });
            })
            ->leftJoin('service jobs', 'task_users.task_id', '=', 'service jobs.id')
            ->leftJoin('taskboard_columns', 'service jobs.board_column_id', '=', 'taskboard_columns.id')
            ->select('users.*',
                ...
                DB::raw('SUM(CASE WHEN taskboard_columns.slug = "completed" AND service jobs.completed_on > service jobs.due_date THEN 1 ELSE 0 END) as task_missed_deadline')
            )
            ->groupBy('users.id');

        if ($request->assignedTo == 'unassigned') {
            $model->whereDoesntHave('service jobs');
    ...
    public function getColumns(): array
    {
        return [
            ...
            __('modules.cleaners.employeeName') => ['data' => 'name', 'name' => 'name', 'visible' => true, 'exportable' => false, 'title' => __('modules.cleaners.employeeName')],
            __('app.name') => ['data' => 'employee_name', 'name' => 'employee_name', 'visible' => false, 'title' => __('app.name')],
            __('modules.service jobs.totalTaskAssigned') => ['data' => 'total_task_assigned', 'name' => 'total_task_assigned', 'title' => __('modules.service jobs.totalTaskAssigned')],
            __('modules.service jobs.totalTaskCompleted') => ['data' => 'total_tasks_completed', 'name' => 'total_tasks_completed', 'title' => __('modules.service jobs.totalTaskCompleted')],
            __('modules.service jobs.totalTaskPending') => ['data' => 'total_task_pending', 'name' => 'total_task_pending', 'title' => __('modules.service jobs.totalTaskPending')],
            __('modules.service jobs.missedDeadline') => ['data' => 'task_missed_deadline', 'name' => 'task_missed_deadline', 'title' => __('modules.service jobs.missedDeadline')],
        ];
