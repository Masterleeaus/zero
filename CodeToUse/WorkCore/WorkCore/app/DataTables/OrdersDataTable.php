...
<a href="' . route('orders.show', [$row->id]) . '"
            ...
            if (!in_array('customer', user_roles()) && in_array($row->status, ['pending', 'on-hold', 'failed', 'processing']) && ($this->editOrderPermission == 'all' || (in_array($this->editOrderPermission, ['added', 'both']) && $row->added_by == $id))) {
            ...
            $showEditbtn = (!is_null($row->site) && is_null($row->site->deleted_at)) ? true : (is_null($row->site) ? true : false);

            if ($showEditbtn && in_array('customers', user_modules()) && !in_array('customer', user_roles()) && !in_array($row->status, ['completed', 'canceled', 'refunded']) && ($this->editOrderPermission == 'all' || ($this->editOrderPermission == 'both' && ($row->added_by == $id || $row->client_id == $id)) || ($this->editOrderPermission == 'added' && $row->added_by == $id) || ($this->editOrderPermission == 'owned' && $row->client_id == $id))) {
            ...
            if (
                !in_array('customer', user_roles()) && !in_array($row->status, ['completed', 'refunded']) &&
        ...
        $datatables->addColumn('client_name', fn($row) => $row->customer->name);
        ...
        $datatables->editColumn('status', function ($row) use ($id) {
            ...
            if ((in_array('admin', user_roles()) || in_array('cleaner', user_roles())) && ($this->editOrderPermission == 'all' || ($this->editOrderPermission == 'both' && ($row->added_by == $id || $row->client_id == $id)) || ($this->editOrderPermission == 'added' && $row->added_by == $id) || ($this->editOrderPermission == 'owned' && $row->client_id == $id))) {
    ...
    protected function getColumns()
    {
        $data = [
            ...
            __('app.client_name') => ['data' => 'client_name', 'name' => 'site.customer.name', 'visible' => false, 'title' => __('app.client_name')],
            __('app.customer') => ['data' => 'name', 'name' => 'name', 'visible' => !in_array('customer', user_roles()), 'exportable' => false, 'title' => __('app.customer')],
