...
class ContractTemplatesDataTable extends BaseDataTable
{
    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->addColumn('action', function ($row) {
                ...
                $action .= ' <a href="' . route('service agreement-template.show', [$row->id]) . '" class="dropdown-item openRightModal"><i class="fa fa-eye mr-2"></i>' . __('app.view') . '</a>';

                if ($this->addContractPermission == 'all' || $this->addContractPermission == 'added') {
                    $action .= '<a class="dropdown-item openRightModal" href="' . route('service agreements.create') . '?template=' . $row->id . '">
                        <i class="fa fa-plus mr-2"></i>
                        ' . trans('app.create') . ' ' . trans('app.menu.service agreement') . '
                    </a>';
                }

                if ($this->manageContractTemplate == 'all' || $this->manageContractTemplate == 'added') {
                    $action .= '<a class="dropdown-item openRightModal" href="' . route('service agreement-template.edit', [$row->id]) . '">
            ...
            ->editColumn('subject', function ($row) {
                ...
                return '<div class="media align-items-center">
                    ...
                    <h5 class="mb-0 f-13 text-darkest-grey"><a href="' . route('service agreement-template.show', [$row->id]) . '">' . $row->subject . '</a></h5>
    ...
    {
        $dataTable = $this->setBuilder('service agreement-template-table', 2)
            ->parameters([
                'initComplete' => 'function () {
                    window.LaravelDataTables["service agreement-template-table"].buttons().container()
