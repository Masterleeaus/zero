...
<a href="' . route('zones.show', [$row->id]) . '" class="taskView text-darkest-grey f-w-500 openRightModal">' . __('app.view') . '</a>
                    <div class="dropdown">
                ...
                if ($this->editDepartmentPermission == 'all') {
                    $action .= '<a class="dropdown-item openRightModal" href="' . route('zones.edit', [$row->id]) . '">
                ...
                if ($this->deleteDepartmentPermission == 'all') {
                    $action .= '<a class="dropdown-item delete-table-row" href="javascript:;" data-zone-id="' . $row->id . '">
            ...
            ->editColumn(
                'name',
                function ($row) {
                    return '<h5 class="mb-0 f-13 text-darkest-grey"><a href="' . route('zones.show', [$row->id]) . '" class="openRightModal">' . $row->team_name . '</a></h5>';
                }
            )
            ->editColumn('parent_id', function ($row) {
                // get name of parent zone
    ...
    public function query(Team $model)
    {
        ...
        if ($request->parentId != 'all' && $request->parentId != null) {
            $zones = Team::with('childs')->where('id', $request->parentId)->get();
    ...
    {
        $dataTable = $this->setBuilder('zones-table', 2)
            ->parameters([
                'initComplete' => 'function () {
                   window.LaravelDataTables["zones-table"].buttons().container()
    ...
    protected function getColumns()
    {
        return [
            ...
            __('modules.zone.parentDepartment') => ['data' => 'parent_id', 'name' => 'parent_id', 'exportable' => true, 'title' => __('modules.zone.parentDepartment')],
