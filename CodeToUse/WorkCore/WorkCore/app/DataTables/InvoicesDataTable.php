...
class InvoicesDataTable extends BaseDataTable
{
    ...
    public function dataTable($query)
    {
        ...
        $datatables->addColumn('action', function ($row) use ($firstInvoice, $userId) {
            ...
            if (
                ...
                || $this->viewProjectInvoicePermission == 'owned' && !is_null($row->project_id) && $userId == $row->site->client_id
            ) {
            ...
            if ($row->status != 'canceled' && !in_array('customer', user_roles()) && $row->credit_note == 0) {
            ...
            if ($row->status == 'pending-confirmation' && !in_array('customer', user_roles()) && !empty($row->payment)) {
            ...
            if ($row->status != 'canceled' && !in_array('customer', user_roles()) && $row->credit_note == 0 && $row->send_status == 0) {
                $action .= '<a class="dropdown-item sendButton d-flex justify-content-between align-items-center" data-type="mark_as_send" href="javascript:;"  data-invoice-id="' . $row->id . '" data-amt="' . (($row->total == 0 && $row->status != 'paid') ? 0 : 1) . '" >
                                ...
                                <i class="fa fa-question-circle" data-toggle="tooltip" data-original-title="' . __('team chat.markSentInfo') . '"></i>
                            </a>';
            }

            $edit = (!is_null($row->site) && is_null($row->site->deleted_at)) ? '<a class="dropdown-item" href="' . route('invoices.edit', $row->id) . '" >
            ...
            if ($row->status == 'paid' && !in_array('customer', user_roles()) && $row->credit_note == 0) {
                $action .= '<a class="dropdown-item invoice-upload" href="javascript:;" data-toggle="tooltip" data-original-title="' . __('team chat.uploadOtherInvoice') . '" data-invoice-id="' . $row->id . '">
            ...
            if ($row->status != 'paid' && $row->status != 'canceled') {
                ...
                if (!in_array('customer', user_roles()) && in_array('payments', $this->user->modules) && $row->credit_note == 0 && $row->status != 'draft' && $row->send_status && $row->status !== 'pending-confirmation') {
                    if (
            ...
            if ($row->status != 'canceled' && $row->credit_note == 0) {
                if ($row->clientdetails) {
                    ...
                    if ($row->site && $row->site->clientdetails) {
                        if (!is_null($row->site->clientdetails->shipping_address)) {
                            $action .= ($row->show_shipping_address == 'yes') ? '<a class="dropdown-item toggle-shipping-address" href="javascript:;" data-toggle="tooltip"  data-invoice-id="' . $row->id . '">
                                    <i class="fa fa-eye-slash mr-2"></i>
                                    ' . __('app.hideShippingAddress') . '
                                </a>' : '<a class="dropdown-item toggle-shipping-address" href="javascript:;" data-toggle="tooltip" data-invoice-id="' . $row->id . '">
            ...
            if (($row->status == 'unpaid' || $row->status == 'draft' || $row->status == 'pending-confirmation') && !in_array('customer', user_roles())) {
            ...
            if ($row->credit_note == 0 && $row->status != 'draft' && $row->status != 'canceled' && $row->status != 'unpaid' && !in_array('customer', user_roles())) {
                if ($row->amountPaid() > 0) {
                    if ($row->status == 'paid') {
            ...
            if ($row->status != 'paid' && $row->status != 'draft' && $row->status != 'canceled' && $row->credit_note == 0 && !in_array('customer', user_roles()) && $row->send_status) {
        ...
        $datatables->addColumn('short_code', function ($row) {
            if (!is_null($row->site)) {
                return $row->site->project_short_code;
            }
        ...
        $datatables->addColumn('client_name', function ($row) {
            ...
            if ($row->site && $row->site->customer) {
                return $row->site->customer->name;
            }

            if ($row->quote && $row->quote->customer) {
                return $row->quote->customer->name;
            }
        ...
        $datatables->addColumn('client_email', function ($row) {
            if ($row->site && $row->site->customer) {
                return $row->site->customer->email;
            }
            ...
            if ($row->quote && $row->quote->customer) {
                return $row->quote->customer->email;
            }
        ...
        $datatables->editColumn('name', function ($row) {
            ...
            } else if ($row->site && $row->site->customer) {
                $customer = $row->site->customer;
            } else if ($row->quote && $row->quote->customer) {
    ...
    protected function getColumns()
    {
        $data = [
            ...
            __('app.customer') => ['data' => 'name', 'name' => 'site.customer.name', 'exportable' => false, 'title' => __('app.customer'), 'visible' => (!in_array('customer', user_roles()) && in_array('customers', user_modules()))],
            __('app.customers') => ['data' => 'client_name', 'name' => 'site.customer.name', 'visible' => false, 'title' => __('app.customers')],
            __('app.email') => ['data' => 'client_email', 'name' => 'site.customer.email', 'visible' => false, 'title' => __('app.email')],
