...
class RecurringTasksDataTable extends BaseDataTable
{
    ...
    public function dataTable($query)
    {
        ...
        $datatables->addColumn(
            ...
            function ($row) {
                ...
                $action .= '<a href="' . route('recurring-service job.show', [$row->id]) . $this->tabUrl . '" class="dropdown-item"><i class="fa fa-eye mr-2"></i>' . __('app.view') . '</a>';
                ...
                if ($row->canEditTicket()) {
                    ...
                    $action .= '<a class="dropdown-item openRightModal" href="' . route('recurring-service job.create') . '?duplicate_task=' . $row->id . '">
        ...
        $datatables->editColumn('users', function ($row) {
            ...
            if (count($row->users) > 4 && $key) {
                $members .= '<div class="taskEmployeeImg more-user-count text-center rounded-circle bg-amt-grey position-absolute" style="left:  52px"><a href="' . route('recurring-service job.show', [$row->id]) . '" class="text-dark f-10">+' . (count($row->users) - 4) . '</a></div> ';
            }
        ...
        $datatables->editColumn('short_code', function ($row) {
            ...
            return '<a href="' . route('recurring-service job.show', [$row->id]) . $this->tabUrl . '" class="text-darkest-grey">' . $row->task_short_code . '</a>' . $this->timer($row);
        });
        ...
        $datatables->editColumn('heading', function ($row) {
            ...
            if ($row->subtasks_count > 0) {
                $subTask .= '<a href="' . route('recurring-service job.show', [$row->id]) . '?view=sub_task"><span class="border rounded p-1 f-11 mr-1 text-dark-grey" data-toggle="tooltip" data-original-title="' . __('modules.service jobs.subTask') . '"><i class="bi bi-diagram-2"></i> ' . $row->completed_subtasks_count . '/' . $row->subtasks_count . '</span></a>';
            }
            ...
            return '<div class="media align-items-center"><div class="media-body"><h5 class="mb-0 f-13 text-darkest-grey"><a href="' . route('recurring-service job.show', [$row->id]) . $this->tabUrl . '"
    ...
    public function query(Service Job $model)
    {
        ...
        $model = $model->leftJoin('sites', 'sites.id', '=', 'service jobs.project_id')
            ...
            ->where('service jobs.repeat', 1)
            ->whereNull('service jobs.recurring_task_id');
