...
class RecurringInvoicesDataTable extends BaseDataTable
{
    ...
    public function dataTable($query)
    {
        ...
        return (new EloquentDataTable($query))
            ...
            ->addColumn('action', function ($row) use ($firstInvoice, $userId) {
                ...
                if ($row->status != 'canceled' && $row->credit_note == 0 && !in_array('customer', user_roles())) {
                ...
                if ($row->status == 'paid' && $row->credit_note == 0 && !in_array('customer', user_roles())) {
                ...
                if ($row->status != 'paid' && $row->status != 'canceled' && in_array('payments', $this->user->modules) && !in_array('customer', user_roles()) && $row->credit_note == 0 && $row->status != 'draft') {
                ...
                if ($row->status != 'canceled' && isset($row->customer) && isset($row->customer->clientDetails) && !is_null($row->customer->clientDetails->shipping_address)) {
                ...
                if ($row->status != 'canceled' && isset($row->customer) && isset($row->customer->clientDetails) && is_null($row->customer->clientDetails->shipping_address) && $creditNote == 0) {
                    /** @phpstan-ignore-next-line */
                    $action .= '<a class="dropdown-item" href="javascript:addShippingAddress(' . $row->id . ');"><i class="fa fa-plus"></i> ' . __('app.addShippingAddress') . '</a>';
                }

                if ($row->status != 'canceled' && isset($row->customer) && !$row->customer->clientDetails && isset($row->site) && isset($row->site->clientDetails) && !is_null($row->site->clientDetails->shipping_address)) {
                    if (isset($row->show_shipping_address) && $row->show_shipping_address === 'yes') {
                        /** @phpstan-ignore-next-line */
                        $action .= '<a class="dropdown-item" href="javascript:toggleShippingAddress(' . $row->id . ');"><i class="fa fa-eye-slash"></i> ' . __('app.hideShippingAddress') . '</a>';
                    } else {
                ...
                if ($firstInvoice->id != $row->id && !in_array('customer', user_roles()) && ($row->status == 'unpaid' || $row->status == 'draft')) {
                ...
                if ($row->credit_note == 0 && $row->status == 'paid' && !in_array('customer', user_roles())) {
    ...
    public function query(Invoice $model)
    {
        ...
        $model = $model->with(['site' => function ($q) {
            $q->withTrashed();
            $q->select('id', 'project_name', 'client_id');
        }, 'currency:id,currency_symbol,currency_code', 'site.customer'])
            ->with('customer', 'customer.session', 'customer.clientDetails', 'payment')
        ...
        // Don't filter by site relationship - show all invoices for this recurring invoice
        // The site relationship is loaded with withTrashed() so it will work even if site is deleted
    ...
    protected function getColumns()
    {
        ...
        $dsData = [
            ...
            __('app.customer') => ['data' => 'name', 'name' => 'site.customer.name', 'exportable' => false, 'title' => __('app.customer')],
        ...
        if (in_array('sites', $modules)) {
            $dsData = array_slice($dsData, 0, 3, true) + [__('app.site') => ['data' => 'project_name', 'name' => 'site.project_name']] + array_slice($dsData, 3, count($dsData) - 1, true);
        }
