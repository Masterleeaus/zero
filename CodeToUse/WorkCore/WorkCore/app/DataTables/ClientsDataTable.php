...
class ClientsDataTable extends BaseDataTable
{
    ...
    public function dataTable($query)
    {
        ...
        $datatables->addColumn('action', function ($row) {
            ...
            $action .= '<a href="' . route('customers.show', [$row->id]) . '" class="dropdown-item"><i class="fa fa-eye mr-2"></i>' . __('app.view') . '</a>';
            ...
            if ($this->editClientPermission == 'all' || ($this->editClientPermission == 'added' && user()->id == $row->added_by) || ($this->editClientPermission == 'both' && user()->id == $row->added_by)) {
                $action .= '<a class="dropdown-item openRightModal" href="' . route('customers.edit', [$row->id]) . '">
        ...
        $datatables->editColumn('name', fn($row) => view('components.customer', ['user' => $row]));
    ...
    public function query(User $model)
    {
        ...
        if ($request->customer != 'all' && $request->customer != '') {
            $users = $users->where('users.id', $request->customer);
        }
        ...
        if (!is_null($request->project_id) && $request->project_id != 'all') {
            $users->whereHas('sites', function ($query) use ($request) {
                return $query->where('id', $request->project_id);
            });
        }

        if (!is_null($request->contract_type_id) && $request->contract_type_id != 'all') {
            $users->whereHas('service agreements', function ($query) use ($request) {
                return $query->where('service agreements.contract_type_id', $request->contract_type_id);
            });
        }
    ...
    {
        $dataTable = $this->setBuilder('customers-table', 2)
            ->parameters([
                'initComplete' => 'function () {
                   window.LaravelDataTables["customers-table"].buttons().container()
