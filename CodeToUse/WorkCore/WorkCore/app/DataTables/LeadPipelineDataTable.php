...
class LeadPipelineDataTable extends BaseDataTable
{
    ...
    public function query(LeadPipeline $model)
    {
        ...
        if ($this->viewLeadPermission == 'both') {
            $leadContact = $leadContact->where(function ($query) {
                $query->orWhere('enquiries.added_by', user()->id);
            });
        }
    ...
    {
        $dataTable = $this->setBuilder('enquiry-pipeline-table', 2)
            ->parameters([
                'initComplete' => 'function () {
                   window.LaravelDataTables["enquiry-pipeline-table"].buttons().container()
