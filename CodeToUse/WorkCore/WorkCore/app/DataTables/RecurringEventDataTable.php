...
class RecurringEventDataTable extends BaseDataTable
{
            ...
            ->editColumn('attendees', function ($row) {
                ...
                foreach ($attendees as $attendee) {
                    ...
                    if ($user && $count < 4) {
                        ...
                        $members .= '<div class="taskEmployeeImg rounded-circle ' . $position . '" style="left: ' . ($count * 13) . 'px"><a href="' . route('cleaners.show', $user->id) . '">' . $img . '</a></div> ';
                        $count++;
                    }
                }

                if ($attendees->count() > 4) {
                    $members .= '<div class="taskEmployeeImg more-user-count text-center rounded-circle bg-amt-grey position-absolute" style="left: 52px"><a href="' . route('recurring-event.show', [$row->id]) . '" class="text-dark f-10">+' . ($attendees->count() - 4) . '</a></div>';
                }
    ...
    public function query(Event $model)
    {
        ...
        if (request()->cleaner && request()->cleaner != 'all' && request()->cleaner != 'undefined') {
            $employeeId = request()->cleaner;
    ...
    protected function getColumns()
    {
        $data = [
            ...
            __('modules.cleaners.startTime') => ['data' => 'start_time', 'name' => 'start_time', 'title' => __('modules.cleaners.startTime')],
            __('app.endDate') => ['data' => 'end_date', 'name' => 'end_date', 'title' => __('app.endDate')],
            __('modules.cleaners.endTime') => ['data' => 'end_time', 'name' => 'end_time', 'title' => __('modules.cleaners.endTime')],
