...
class EmployeesDataTable extends BaseDataTable
{
    ...
    public function dataTable($query)
    {
        ...
        $datatables->addColumn('role', function ($row) use ($roles) {
            ...
            if (in_array('admin', $userRole) && !in_array('admin', user_roles())) {
                return $uRole . ' <i data-toggle="tooltip" data-original-title="' . __('team chat.roleCannotChange') . '" class="fa fa-info-circle"></i>';
            }
        ...
        $datatables->addColumn('action', function ($row) {
            ...
            $action .= '<a href="' . route('cleaners.show', [$row->id]) . '" class="dropdown-item"><i class="fa fa-eye mr-2"></i>' . __('app.view') . '</a>';
            ...
            if (
                ...
                if (!in_array('admin', $userRole) || (in_array('admin', $userRole) && in_array('admin', user_roles()))) {
                    $action .= '<a class="dropdown-item openRightModal" href="' . route('cleaners.edit', [$row->id]) . '">
        ...
        $datatables->editColumn('name', function ($row) {
            ...
            $view = view('components.cleaner', ['user' => $row])->render();
        ...
        $datatables->editColumn('employee_id', fn($row) => '<a href="' . route('cleaners.show', [$row->id]) . '" class="text-darkest-grey">' . $row->employee_id . '</a>');
        ...
        $datatables->addColumn('designation_name', fn($row) => $row->employeeDetail->role->name ?? '--');
    ...
    public function query(User $model)
    {
        ...
        $users = $model->with([
            ...
            'employeeDetail.zone:id,team_name',
        ])
        ...
        if ($request->cleaner != 'all' && $request->cleaner != '') {
            $users = $users->where('users.id', $request->cleaner);
        }
        ...
        if ($request->role != 'all' && $request->role != '') {
            $users = $users->where('employee_details.designation_id', $request->role);
        }

        if ($request->zone != 'all' && $request->zone != '') {
            $users = $users->where('employee_details.department_id', $request->zone);
        }

        if ($request->role != 'all' && $request->role != '' && $userRoles) {
            if ($userRoles->name == 'admin') {
                $users = $users->where('roles.id', $request->role);
            } elseif ($userRoles->name == 'cleaner') {
    ...
    {
        $dataTable = $this->setBuilder('cleaners-table', 2)
            ->parameters([
                'initComplete' => 'function () {
                    window.LaravelDataTables["cleaners-table"].buttons().container()
    ...
    protected function getColumns()
    {
        ...
        $data = [
            ...
            __('modules.cleaners.employeeId') => ['data' => 'employee_id', 'name' => 'employee_id', 'title' => __('modules.cleaners.employeeId')],
            __('app.name') => ['data' => 'name', 'name' => 'name', 'exportable' => false, 'title' => __('app.name')],
            __('modules.cleaners.employmentType') => ['data' => 'employment_type', 'name' => 'employment_type', 'visible' => false, 'title' => __('modules.cleaners.employmentType')],
            __('app.cleaner') => ['data' => 'employee_name', 'name' => 'name', 'visible' => false, 'title' => __('app.cleaner')],
            ...
            __('modules.cleaners.role') => ['data' => 'current_role_name', 'name' => 'current_role_name', 'visible' => false, 'title' => __('modules.cleaners.role')],
            __('app.mobile') => ['data' => 'mobile', 'name' => 'mobile', 'visible' => false, 'title' => __('app.mobile')],
            __('modules.cleaners.role') => ['data' => 'designation_name', 'name' => 'designation_name', 'visible' => false, 'title' => __('modules.cleaners.role')],
            __('modules.cleaners.zone') => ['data' => 'department_name', 'name' => 'department_name', 'visible' => false, 'title' => __('modules.cleaners.zone')],
            __('modules.cleaners.reportingTo') => ['data' => 'reporting_to', 'name' => 'reporting_to', 'title' => __('modules.cleaners.reportingTo')],
            __('modules.cleaners.joiningDate') => ['data' => 'joining_date', 'name' => 'joining_date', 'visible' => false, 'title' => __('modules.cleaners.joiningDate')],
