...
class ArchiveProjectsDataTable extends BaseDataTable
{
        ...
        return datatables()
            ...
            ->addColumn('action', function ($row) {
                ...
                if (
                    ...
                    || ($this->editProjectPermission == 'owned' && user()->id == $row->client_id && in_array('customer', user_roles()))
                    || ($this->editProjectPermission == 'owned' && in_array(user()->id, $memberIds) && in_array('cleaner', user_roles()))
                    || ($this->editProjectPermission == 'both' && (user()->id == $row->client_id || user()->id == $row->added_by))
                    || ($this->editProjectPermission == 'both' && in_array(user()->id, $memberIds) && in_array('cleaner', user_roles()))
                ) {
                    $action .= '
                    <a href="javascript:;" class="btn btn-sm btn-secondary restore-site mr-2"
                ...
                if (
                    ...
                    || ($this->deleteProjectPermission == 'owned' && user()->id == $row->client_id && in_array('customer', user_roles()))
            ...
            ->addColumn('members', function ($row) {
                ...
                if (count($row->members) > 0) {
                    foreach ($row->members as $member) {
                        ...
                        $members .= '<div class="taskEmployeeImg rounded-circle"><a href="' . route('cleaners.show', $member->user->id) . '">' . $img . '</a></div> ';
                    }
                } else {
                    $members .= __('team chat.noMemberAddedToProject');
                }
            ...
            ->editColumn('client_id', fn($row) => is_null($row->client_id) ? '' : view('components.customer', ['user' => $row->customer]))
    ...
    public function query(Site $model)
    {
        ...
        $model = $model
            ->with('members', 'members.user', 'customer', 'customer.clientDetails', 'currency', 'customer.session')
            ...
            ->selectRaw('sites.id, sites.added_by, sites.project_name, sites.start_date, sites.deadline, sites.client_id,
              sites.completion_percent, sites.project_budget, sites.currency_id,
              sites.status, users.name, customer.name as client_name,
           ( select count("id") from pinned where pinned.project_id = sites.id and pinned.user_id = ' . user()->id . ') as pinned_project');
        ...
        if ($this->viewProjectPermission == 'added') {
            $model->where('sites.added_by', user()->id);
        }
        ...
        $model->onlyTrashed()->groupBy('sites.id');
    ...
    {
        $dataTable = $this->setBuilder('sites-table')
            ->parameters([
    ...
    protected function getColumns()
    {
        return [
            ...
            __('modules.sites.projectName') => ['data' => 'project_name', 'name' => 'project_name', 'title' => __('modules.sites.projectName')],
            __('modules.sites.members') => ['data' => 'members', 'name' => 'members', 'exportable' => false, 'width' => '25%', 'title' => __('modules.sites.members')],
            __('modules.sites.projectMembers') => ['data' => 'name', 'name' => 'name', 'visible' => false, 'title' => __('modules.sites.projectMembers')],
            __('app.deadline') => ['data' => 'deadline', 'name' => 'deadline', 'title' => __('app.deadline')],
            __('app.customer') => ['data' => 'client_id', 'name' => 'client_id', 'title' => __('app.customer')],
