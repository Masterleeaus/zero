...
class EmployeeAppreciationDataTable extends BaseDataTable
{
        ...
        return datatables()
            ...
            ->editColumn('title', function ($row) {
                ...
                if (!is_null($row->last_reply_by_id)) {
                    $lastReply = '<a class="text-darkest-grey" href="' . route('cleaners.show', $row->last_reply_by_id) . '">' . $row->lastReplyBy->name . '</a> ';
                }
    ...
    {
        $dataTable = $this->setBuilder('cleaner-appreciation-table', 0)
            ->parameters([
