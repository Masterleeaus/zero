...
class ProjectTemplateTasksDataTable extends BaseDataTable
{
    ...
    public function dataTable($query)
    {
        return datatables()
            ...
            ->addColumn('action', function ($row) {
                ...
                $action .= ' <a href="' . route('site-template-service job.show', [$row->id]) . '" class="dropdown-item openRightModal"><i class="fa fa-eye mr-2"></i>' . __('app.view') . '</a>';
                ...
                if ($this->editTaskPermission == 'all' || ($this->editTaskPermission == 'added' && user()->id == $row->added_by) || ($this->editTaskPermission == 'both' && user()->id == $row->added_by)) {
                    $action .= '<a class="dropdown-item openRightModal" href="' . route('site-template-service job.edit', [$row->id]) . '?project_id=' . $row->project_template_id . '">
            ...
            ->editColumn('heading', function ($row) {
                ...
                return '<div class="media align-items-center">
                    ...
                    <h5 class="mb-0 f-13 text-darkest-grey"><a href="' . route('site-template-service job.show', [$row->id]) . '" class="openRightModal">' . $row->heading . '<br/>' . '</a></h5>
    ...
    protected function getColumns()
    {
        return [
            ...
            __('app.service job') => ['data' => 'heading', 'name' => 'heading', 'title' => __('app.service job')],
