...
class ProjectsDataTable extends BaseDataTable
{
    ...
    public function dataTable($query)
    {
        ...
        $datatables->addColumn(
            ...
            function ($row) use ($userId) {
                ...
                $action .= '<a href="' . route('sites.show', [$row->id]) . '" class="dropdown-item"><i class="mr-2 fa fa-eye"></i>' . __('app.view') . '</a>';

                if (
                    $this->editProjectsPermission == 'all'
                    || ($this->editProjectsPermission == 'added' && (user()->id == $row->added_by || $userId == $row->added_by))
                    || ($this->editProjectsPermission == 'owned' && $userId == $row->client_id && in_array('customer', user_roles()))
                    || ($this->editProjectsPermission == 'owned' && in_array(user()->id, $memberIds) && in_array('cleaner', user_roles()))
                    || ($this->editProjectsPermission == 'both' && ($userId == $row->client_id || user()->id == $row->added_by || $userId == $row->added_by))
                    || ($this->editProjectsPermission == 'both' && in_array(user()->id, $memberIds) && in_array('cleaner', user_roles()))
                ) {
                    $action .= '<a class="dropdown-item openRightModal" href="' . route('sites.edit', [$row->id]) . '">
                ...
                if ($this->addProjectPermission == 'all' || $this->addProjectPermission == 'added' || $this->addProjectPermission == 'both') {
                    $action .= '<a class="dropdown-item duplicateProject" href="javascript:;" data-site-id="' . $row->id . '">
                ...
                if ($this->viewGanttPermission == 'all' || ($this->viewGanttPermission == 'added' && user()->id == $row->added_by) || ($this->viewGanttPermission == 'owned' && user()->id == $row->client_id)) {
                    $action .= '<a class="dropdown-item" href="' . route('sites.show', $row->id) . '?tab=gantt' . '">
                            <i class="mr-2 fa fa-site-diagram"></i>
                            ' . trans('modules.sites.viewGanttChart') . '
                        </a>';
                }
                ...
                if ($row->public_gantt_chart == 'enable') {
                    $action .= '<a class="dropdown-item" target="_blank" href="' . url()->temporarySignedRoute('front.gantt', now()->addDays(GlobalSetting::SIGNED_ROUTE_EXPIRY), $row->hash) . '">
                        ...
                        ' . trans('modules.sites.viewPublicGanttChart') . '
                    </a>';
                }
                ...
                if ($row->public_taskboard == 'enable') {
                    $action .= '<a class="dropdown-item" target="_blank" href="' . url()->temporarySignedRoute('front.taskboard', now()->addDays(GlobalSetting::SIGNED_ROUTE_EXPIRY), $row->hash) . '">
                        <i class="mr-2 fa fa-share-square"></i>
                        ' . trans('app.public') . ' ' . __('modules.service jobs.taskBoard') . '
                    </a>';
                }

                if ($row->pinned_project == 1) {
                    $action .= '<a class="dropdown-item" href="javascript:;" id="pinnedItem" data-site-id="' . $row->id . '"
                ...
                if (
                    ...
                    || ($this->deleteProjectPermission == 'owned' && $userId == $row->client_id && in_array('customer', user_roles()))
                    || ($this->deleteProjectPermission == 'owned' && in_array(user()->id, $memberIds) && in_array('cleaner', user_roles()))
                    || ($this->deleteProjectPermission == 'both' && ($userId == $row->client_id || user()->id == $row->added_by || $userId == $row->added_by))
                    || ($this->deleteProjectPermission == 'both' && in_array(user()->id, $memberIds) && in_array('cleaner', user_roles()))
                ) {
            ...
            if (count($row->members) > 0) {
                foreach ($row->members as $key => $member) {
                    if ($key < 4) {
                        ...
                        $route = !in_array('customer', user_roles()) ? route('cleaners.show', $member->user->id): 'javascript:;';
                ...
                if (count($row->members) > 0) {
                    ...
                    $members .= '<a href="' . route('sites.show', $row->id) . '?tab=members" class="f-12 text-dark-grey"><i class="fa fa-plus" ></i> ' . __('modules.sites.addMemberTitle') . '</a>';
                } else {
                ...
                if (count($row->members) > 4) {
                    $members .= '<div class="text-center taskEmployeeImg more-user-count rounded-circle bg-amt-grey position-absolute" style="left:  52px"><a href="' . route('sites.show', $row->id) . '?tab=members" class="text-dark f-10">+' . (count($row->members) - 4) . '</a></div> ';
                }
        ...
        $datatables->addColumn('site', function ($row) {
        ...
        $datatables->addColumn('project_dept', function ($row) {
            ...
            $dept = [];
            if (count($row->zones) > 0) {
                foreach ($row->zones as $dep) {
                    $dept[] = $dep->zone->team_name;
                }
                return implode(', ', $dept);
            }
            return '--';
        ...
        $datatables->editColumn('project_name', function ($row) {
            ...
            return '<div class="media align-items-center">
                    ...
                    <h5 class="mb-0 f-13 text-darkest-grey"><a href="' . route('sites.show', [$row->id]) . '">' . $row->project_name . '</a></h5>
        ...
        $datatables->addColumn('client_name', fn($row) => $row->customer?->name_salutation ?? '-');
        $datatables->addColumn('client_email', fn($row) => $row->customer?->email ?? '-');

        $datatables->addColumn('project_status', fn($row) => ucwords($row->status));
        $datatables->editColumn('client_id', fn($row) => $row->client_id ? view('components.customer', ['user' => $row->customer]) : '');
        $datatables->addColumn('status', function ($row) use ($projectStatus, $userId, $clientIds) {
            ...
            if (
                ...
                || ($this->editProjectsPermission == 'owned' && $userId == $row->client_id && in_array('customer', user_roles()))
                || ($this->editProjectsPermission == 'owned' && in_array(user()->id, $projectUsers) && in_array('cleaner', user_roles()))
                ...
                || ($this->editProjectsPermission == 'both' && in_array(user()->id, $projectUsers) && in_array('cleaner', user_roles()))
            ) {
                $status .= '<select class="form-control select-picker change-status" data-size="5" data-site-id="' . $row->id . '">';
        ...
        $datatables->editColumn('project_short_code', function ($row) {
            return '<a href="' . route('sites.show', [$row->id]) . '" class="text-darkest-grey">' . $row->project_short_code . '</a>';
        });
        ...
        $customFieldColumns = CustomField::customFieldData($datatables, Site::CUSTOM_FIELD_MODEL);
    ...
    public function query(Site $model)
    {
        ...
        $model = $model
            ->with('members', 'members.user', 'customer', 'customer.clientDetails', 'currency', 'customer.session', 'mentionUser', 'customer.clientDetails.company:id,company_name', 'zones', 'category', 'zones.zone')
            ->leftJoin('project_members', 'project_members.project_id', 'sites.id')
            ->leftJoin('project_labels', 'project_labels.project_id', '=', 'sites.id')
            ->leftJoin('users', 'project_members.user_id', 'users.id')
            ->leftJoin('users as customer', 'sites.client_id', 'users.id')
            ->leftJoin('mention_users', 'mention_users.project_id', 'sites.id')
            ->leftJoin('project_departments', 'project_departments.project_id', 'sites.id')
            ->selectRaw(
                'sites.id, sites.public_taskboard, sites.public_gantt_chart, sites.project_short_code, sites.hash, sites.added_by, sites.project_name, sites.start_date, sites.deadline, sites.client_id,
              sites.completion_percent, sites.project_budget, sites.currency_id, sites.category_id,
            sites.status, users.salutation, users.name, customer.name as client_name, customer.email as client_email, sites.public, mention_users.user_id as mention_user,
           ( select count("id") from pinned where pinned.project_id = sites.id and pinned.user_id = ' . $userId . ') as pinned_project'
            );
        ...
        if (!is_null($request->status) && $request->status != 'all') {
            if ($request->status == 'overdue') {
                ...
                if ($request->deadLineStartDate == '' && $request->deadLineEndDate == '') {
                    if ($request->startFilterDate == '' && $request->endFilterDate == '') {
                        $model->whereDate('sites.deadline', '<', $todayDate);
                    } else {
                        ...
                        if ($todayDate >= $startDate && $todayDate <= $endDate) {
                            $model->whereRaw('Date(sites.deadline) >= ?', [$startDate])
                                ->whereRaw('Date(sites.deadline) < ?', [$todayDate]);
                        } else {
                            $model->whereRaw('Date(sites.deadline) >= ?', [$startDate])
                                ->whereRaw('Date(sites.deadline) <= ?', [$endDate]);
                        }
                    }
                } else {
                    if ($request->startFilterDate == '' && $request->endFilterDate == '') {
            ...
            if ($request->startFilterDate !== null && $request->startFilterDate != 'null' && $request->startFilterDate != '') {
                ...
                $model->where(DB::raw('DATE(sites.`start_date`)'), '>=', $startFilterDate);
            }
            ...
            if ($request->endFilterDate !== null && $request->endFilterDate != 'null' && $request->endFilterDate != '') {
                ...
                $model->where(DB::raw('DATE(sites.`start_date`)'), '<=', $endFilterDate);
            }
        }
        ...
        if (!is_null($request->client_id) && $request->client_id != 'all') {
            $model->where('sites.client_id', $request->client_id);
        }
        ...
        if ($this->viewProjectPermission == 'added') {
            $model->where(
                function ($query) use ($userId) {
                    ...
                    return $query->where('sites.added_by', $userId)
                        ->orWhere('mention_users.user_id', user()->id)
                        ->orWhere('sites.public', 1);
                }
            );
        ...
        if ($this->viewProjectPermission == 'owned' && in_array('cleaner', user_roles())) {
            $model->where(
                function ($query) {
        ...
        if ($this->viewProjectPermission == 'both' && in_array('cleaner', user_roles())) {
            $model->where(
                function ($query) {
                    return $query->where('sites.added_by', user()->id)
        ...
        if ($request->searchText != '') {
            ...
            $model->where(
                function ($query) use ($safeTerm) {
                    $query->where('sites.project_name', 'like', '%' . $safeTerm . '%')
                        ->orWhere('users.name', 'like', '%' . $safeTerm . '%')
                        ->orWhere('sites.project_short_code', 'like', '%' . $safeTerm . '%'); // site short code
                }
            );
        }
        ...
        if ($order) {
            ...
            $model->orderByRaw('sites.start_date desc');
        }
    ...
    {
        $dataTable = $this->setBuilder('sites-table', 7)
            ->parameters(
                [
                    ...
                    $("#sites-table .select-picker").selectpicker();
    ...
    protected function getColumns()
    {
        $data = [
            ...
            __('modules.sites.projectName') => ['data' => 'project_name', 'name' => 'project_name', 'exportable' => false, 'width' => '20%', 'title' => __('modules.sites.projectName')],
            __('app.site') => ['data' => 'site', 'name' => 'project_name', 'visible' => false, 'title' => __('app.site')],
        ];

        if (in_array('customer', user_roles())) {
            if (user()->permission('view_project_members') == 'all') {
                $data[__('modules.sites.members')] = ['data' => 'members', 'orderable' => false, 'name' => 'members', 'exportable' => false, 'width' => '15%', 'title' => __('modules.sites.members')];
            }
        } else {
            $data[__('modules.sites.members')] = ['data' => 'members', 'orderable' => false, 'name' => 'members', 'exportable' => false, 'width' => '15%', 'title' => __('modules.sites.members')];
        }
        ...
        $data2 = [
            __('modules.sites.projectMembers') => ['data' => 'name', 'orderable' => false, 'name' => 'name', 'visible' => false, 'title' => __('modules.sites.projectMembers')],
            __('modules.sites.startDate') => ['data' => 'start_date', 'name' => 'start_date', 'title' => __('modules.sites.startDate'), 'width' => '12%'],
            __('app.deadline') => ['data' => 'deadline', 'name' => 'deadline', 'title' => __('app.deadline'), 'width' => '12%'],
            __('app.customer') => ['data' => 'client_id', 'name' => 'client_id', 'width' => '15%', 'exportable' => false, 'title' => __('app.customer'), 'visible' => (!in_array('customer', user_roles()) && in_array('customers', user_modules()))]
        ];

        if (in_array('customer', user_roles())) {
            $data2[__('app.customers')] = ['data' => 'client_name', 'name' => 'client_id', 'visible' => false, 'title' => __('app.customers')];
            $data2[__('app.customer') . ' ' . __('app.email')] = ['data' => 'client_email', 'name' => 'client_id', 'visible' => false, 'title' => __('app.customer') . ' ' . __('app.email')];
        } else {
            $data2[__('app.customers')] = ['data' => 'client_name', 'name' => 'client_id', 'exportable' => (in_array('customers', user_modules()) && user()->permission('view_clients') !== 'none'), 'visible' => false, 'title' => __('app.customers')];
            $data2[__('app.customer') . ' ' . __('app.email')] = ['data' => 'client_email', 'name' => 'client_id', 'exportable' => (in_array('customers', user_modules()) && user()->permission('view_clients') !== 'none'), 'visible' => false, 'title' => __('app.customer') . ' ' . __('app.email')];
        }
        ...
        $data2[__('app.site') . ' ' . __('app.status')] = ['data' => 'project_status', 'name' => 'status', 'visible' => false, 'title' => __('app.site') . ' ' . __('app.status')];
        $data2[__('modules.sites.projectCategory')] = ['data' => 'project_cat', 'name' => 'project_cat', 'visible' => false, 'title' => __('modules.sites.projectCategory')];
        $data2[__('app.zone')] = ['data' => 'project_dept', 'name' => 'project_dept', 'visible' => false, 'title' => __('app.zone')];
        ...
        return array_merge($data, CustomFieldGroup::customFieldsDataMerge(new Site()), $action);
    }
}
