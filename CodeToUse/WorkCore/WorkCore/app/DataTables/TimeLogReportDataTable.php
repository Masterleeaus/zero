...
class TimeLogReportDataTable extends BaseDataTable
{
        ...
        return datatables()
            ...
            ->editColumn('site', function ($row) {
                ...
                if (!is_null($row->project_id)) {
                    $site .= '<a href="' . route('sites.show', [$row->project_id]) . '" class="text-darkest-grey ">' . $row->site->project_name . '</a>';
                }
            ...
            ->editColumn('service job', function ($row) {

                $service job = '';

                if (!is_null($row->task_id)) {
                    $service job .= '<a href="' . route('service jobs.show', [$row->task_id]) . '" class="text-darkest-grey openRightModal">' . $row->service job->heading . '</a>';
                }
            ...
            ->editColumn('task_project', function ($row) {
                ...
                if (!is_null($row->project_id) && !is_null($row->task_id)) {
                    $name .= '<h5 class="f-13 text-darkest-grey"><a href="' . route('service jobs.show', [$row->task_id]) . '" class="openRightModal">' . $row->service job->heading . '</a></h5><div class="text-muted">' . $row->site->project_name . '</div>';
            ...
            ->editColumn('short_code', function ($row) {
                ...
                return '<a href="' . route('service jobs.show', [$row->task_id]) . '" class="text-darkest-grey openRightModal">' . $row->task_short_code . '</a>';
            })
            ->rawColumns(['end_time', 'action', 'site', 'service job', 'task_project', 'name', 'total_hours', 'total_minutes', 'check', 'short_code'])
    ...
    public function query(ProjectTimeLog $model)
    {
        ...
        if (!is_null($customer) && $customer !== 'all') {
            $model->where('sites.client_id', $customer);
        }
    ...
    protected function getColumns()
    {
        return [
            ...
            __('app.site') => ['data' => 'site', 'visible' => false, 'title' => __('app.site')],
            __('app.service job') => ['data' => 'service job', 'visible' => false, 'title' => __('app.service job')],
            __('app.service jobs') => ['data' => 'task_project', 'name' => 'heading', 'width' => '200', 'exportable' => false, 'title' => __('app.service jobs')],
            ...
            __('modules.cleaners.memo') => ['data' => 'memo', 'visible' => false, 'title' => __('modules.cleaners.memo')],
            __('app.customer') => ['data' => 'customer', 'visible' => false, 'title' => __('app.customer')],
