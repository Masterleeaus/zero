...
class EstimatesDataTable extends BaseDataTable
{
    ...
    public function dataTable($query)
    {
        ...
        $datatables->addColumn('action', function ($row) use ($firstEstimate, $userId) {
            ...
            $action .= '<a href="' . route('quotes.show', [$row->id]) . '" class="dropdown-item"><i class="fa fa-eye mr-2"></i>' . __('app.view') . '</a>';

            if ($row->send_status) {
                $action .= '<a class="dropdown-item btn-copy" data-clipboard-text="' . url()->temporarySignedRoute('front.quote.show', now()->addDays(GlobalSetting::SIGNED_ROUTE_EXPIRY), $row->hash) . '">
                        <i class="fa fa-copy mr-2"></i> ' . __('modules.quotes.copyLink') . ' </a>';

                $action .= '<a class="dropdown-item" href="' . url()->temporarySignedRoute('front.quote.show', now()->addDays(GlobalSetting::SIGNED_ROUTE_EXPIRY), $row->hash) . '" target="_blank"><i class="fa fa-external-link-alt mr-2"></i>' . trans('modules.quotes.viewLink') . '</a>';
            }

            if ($row->status != 'draft') {
                $action .= '<a class="dropdown-item" href="' . route('quotes.download', [$row->id]) . '"> <i class="fa fa-download mr-2"></i>' . trans('app.download') . '</a>';
            }
            ...
            if ($row->status == 'waiting' || $row->status == 'draft') {
                if (
                    ...
                    $action .= '<a class="dropdown-item" href="' . route('quotes.edit', [$row->id]) . '"><i class="fa fa-edit mr-2"></i>' . trans('app.edit') . '</a>';
                }
            }

            if ($row->status != 'canceled' && $row->status != 'accepted' && !in_array('customer', user_roles())) {
                $action .= '<a href="javascript:;" data-toggle="tooltip"  data-quote-id="' . $row->id . '" class="dropdown-item sendButton"><i class="fa fa-paper-plane mr-2"></i> ' . __('app.send') . '</a>';
            }
            ...
            if ($firstEstimate->id == $row->id) {
                if (
                    ...
                    $action .= '<a class="dropdown-item delete-table-row" href="javascript:;" data-quote-id="' . $row->id . '"><i class="fa fa-trash mr-2"></i>' . trans('app.delete') . '</a>';
                }
            }
            ...
            if ($row->status == 'waiting' || (is_null($row->estimate_id) && $row->status == 'accepted')) {
                if ($this->addInvoicePermission == 'all' || $this->addInvoicePermission == 'added') {
                    $action .= '<a class="dropdown-item" href="' . route('invoices.create') . '?quote=' . $row->id . '" ><i class="fa fa-plus mr-2"></i> ' . __('app.create') . ' ' . __('app.invoice') . '</a>';
                }

                if ($this->editEstimatePermission == 'all' || ($this->editEstimatePermission == 'added' && $row->added_by == $userId)) {
                    $action .= '<a href="javascript:;" class="dropdown-item change-status" data-quote-id="' . $row->id . '" ><i class="fa fa-times mr-2"></i> ' . __('app.cancelEstimate') . '</a>';
                }
            }

            if ($this->addEstimatePermission == 'all' || $this->addEstimatePermission == 'added') {
                $action .= '<a href="' . route('quotes.create') . '?quote=' . $row->id . '" class="dropdown-item"><i class="fa fa-copy mr-2"></i> ' . __('app.create') . ' ' . __('app.duplicate') . '</a>';
            }
        ...
        $datatables->addColumn('estimate_number', function ($row) {
            return '<a href="' . route('quotes.show', $row->id) . '" class="text-darkest-grey">' . $row->estimate_number . '</a>';
        });
        $datatables->editColumn('project_name', function ($row) {
            if ($row->project_id != null) {
                return '<a href="' . route('sites.show', $row->project_id) . '" class="text-darkest-grey">' . $row->site?->project_name . '</a>';
            }

            return '--';
        });
        $datatables->addColumn('client_name', fn($row) => $row->name);
        $datatables->editColumn('name', fn($row) => view('components.customer', ['user' => $row->customer]));
        $datatables->editColumn('status', function ($row) {
            $status = '';

            if ($row->status == 'waiting') {
                $status .= '<i class="fa fa-circle mr-1 text-yellow f-10"></i>' . __('modules.quotes.' . $row->status) . '</label>';
                ...
                $status .= '<i class="fa fa-circle mr-1 text-red f-10"></i>' . __('modules.quotes.' . $row->status) . '</label>';
            } else {
                $status .= '<i class="fa fa-circle mr-1 text-dark-green f-10"></i>' . __('modules.quotes.' . $row->status) . '</label>';
            }
        ...
        $datatables->addColumn('estimate_request_number', function ($row) {
            if ($row->estimate_request_id) {
                return '<a href="' . route('quote-request.show', $row->estimate_request_id) . '" class="text-darkest-grey">' . $row->estimateRequest->estimate_request_number . '</a>';
            } else {
                return '--';
            }
        });
        ...
        $customFieldColumns = CustomField::customFieldData($datatables, Quote::CUSTOM_FIELD_MODEL);
    ...
    public function query()
    {
        ...
        $model = Quote::with([
            ...
            ->join('client_details', 'quotes.client_id', '=', 'client_details.user_id')
            ->join('currencies', 'currencies.id', '=', 'quotes.currency_id')
            ->join('users', 'users.id', '=', 'quotes.client_id')
            ->leftJoin('invoices', 'invoices.estimate_id', '=', 'quotes.id')
            ->leftJoin('estimate_requests', 'estimate_requests.id', '=', 'quotes.estimate_request_id')
            ->select([
                'quotes.id',
                'quotes.company_id',
                'quotes.client_id',
                ...
                'quotes.project_id',
                'quotes.valid_till',
                'quotes.estimate_number',
                'quotes.send_status',
                'quotes.added_by',
                'quotes.hash',
                'invoices.estimate_id',
                'quotes.created_at',
                'quotes.estimate_request_id',
                'estimate_requests.estimate_request_number',
            ]);

        if ($this->projectID != null && $this->clientID != null) {
            $model = $model->where('quotes.project_id', $this->projectID)
                ->where('quotes.client_id', $this->clientID);
        }

        if ($request->startDate !== null && $request->startDate != 'null' && $request->startDate != '') {
            $startDate = companyToDateString($request->startDate);
            $model = $model->where(DB::raw('DATE(quotes.`valid_till`)'), '>=', $startDate);
        }

        if ($request->endDate !== null && $request->endDate != 'null' && $request->endDate != '') {
            $endDate = companyToDateString($request->endDate);
            $model = $model->where(DB::raw('DATE(quotes.`valid_till`)'), '<=', $endDate);
        }

        if ($request->status != 'all' && !is_null($request->status)) {
            $model = $model->where('quotes.status', '=', $request->status);
        }

        if ($request->clientID != 'all' && !is_null($request->clientID)) {
            $model = $model->where('quotes.client_id', '=', $request->clientID);
        }

        if (in_array('customer', user_roles())) {

            $model = $model->where('quotes.client_id', $userId);
            $model = $model->orWhere('quotes.send_status', 1);
        }
        ...
        if ($request->searchText != '') {
            $model->where(function ($query) use ($safeTerm) {
                ...
                $query->where('quotes.estimate_number', 'like', '%' . $safeTerm . '%')
                    ->orWhere('quotes.id', 'like', '%' . $safeTerm . '%')
                    ->orWhere('quotes.total', 'like', '%' . $safeTerm . '%')
                    ->orWhere(function ($query) use ($safeTerm) {
                        $query->whereHas('customer', function ($q) use ($safeTerm) {
                            $q->where('name', 'like', '%' . $safeTerm . '%');
                        });
                    })
                    ->orWhere(function ($query) use ($safeTerm) {
                        $query->whereHas('site', function ($q) use ($safeTerm) {
                            $q->where('project_name', 'like', '%' . $safeTerm . '%')
                                ->orWhere('project_short_code', 'like', '%' . $safeTerm . '%'); // site short code
                        });
                    })
                    ->orWhere(function ($query) use ($safeTerm) {
                        $query->where('quotes.status', 'like', '%' . $safeTerm . '%');
                    });
            });
        }
        ...
        if ($this->viewEstimatePermission == 'added') {
            $model->where('quotes.added_by', $userId);
        }

        if ($this->viewEstimatePermission == 'both') {
            $model->where(function ($query) use ($userId) {
                $query->where('quotes.added_by', $userId)
                    ->orWhere('quotes.client_id', $userId);
            });
        }

        if ($this->viewEstimatePermission == 'owned') {
            $model->where('quotes.client_id', $userId);
        }
    ...
    protected function getColumns()
    {
        ...
        $data = [
            ...
            __('app.quote') . '#' => ['data' => 'estimate_number', 'name' => 'estimate_number', 'title' => __('app.quote')],
            __('app.site') => ['data' => 'project_name', 'name' => 'site.project_name', 'title' => __('app.site'), 'visible' => in_array('sites', user_modules()), 'exportable' => in_array('sites', user_modules())],
            __('app.customer') => ['data' => 'name', 'name' => 'users.name', 'exportable' => false, 'title' => __('app.customer'), 'visible' => (in_array('customers', user_modules()) && !in_array('customer', user_roles()))],
            ...
            __('modules.quotes.validTill') => ['data' => 'valid_till', 'name' => 'valid_till', 'title' => __('modules.quotes.validTill')],
        ...
        return array_merge($data, CustomFieldGroup::customFieldsDataMerge(new Quote()), $action);
    }
}
