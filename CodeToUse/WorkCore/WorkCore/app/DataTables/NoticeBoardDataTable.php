...
class NoticeBoardDataTable extends BaseDataTable
{
    ...
    public function query(Notice $model)
    {
        ...
        if (in_array('cleaner', user_roles()) && !in_array('admin', user_roles())) {
            $model->where(function ($query) {
                if ($this->user && $this->user->employeeDetail && $this->user->employeeDetail->zone) {
                    $departmentId = $this->user->employeeDetail->zone->id;
    ...
    protected function getColumns()
    {
        return [
            ...
            __('app.to') => ['data' => 'to', 'name' => 'to', 'title' => __('app.to'), 'visible' => !in_array('customer', user_roles())],
