        ...
        if ($this->isColumnExists('name')) {
            ...
            if ($user) {
                ...
                try {
                    ...
                    $role = Role::where('name', 'customer')->where('company_id', $this->company?->id)->select('id')->first();
                    $user->attachRole($role->id);

                    $user->assignUserRolePermission($role->id);

                    if (!is_null($user->email)) {
                        $this->logSearchEntry($user->id, $user->email, 'customers.show', 'customer', $user->company_id);
                    }

                    if (!is_null($user->clientDetails->company_name)) {
                        $this->logSearchEntry($user->id, $user->clientDetails->company_name, 'customers.show', 'customer', $user->company_id);
                    }
