...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        $cleaners = User::withoutGlobalScopes()->onlyEmployee()->get();
        ...
        foreach ($cleaners as $cleaner) {
            ...
            foreach ($leaveQuotas as $leaveQuota) {
                $leave = $this->byUser($cleaner, $leaveQuota->leave_type_id);
    ...
    public function byUser(User $user, $leaveTypeId = null, $status = array('approved'), $leaveDate = null)
    {
        ...
        if ($user->cleaner->first()) {
            if ($setting->leaves_start_from == 'joining_date') {
                $currentYearJoiningDate = Carbon::parse($user->cleaner->first()->joining_date->format((now($setting->timezone)->year) . '-m-d'));
