...
return new class extends Migration
{
    ...
    public function up()
    {
        ...
        foreach ($companies as $company) {
            $LeadCustomProductForm = LeadCustomForm::where('company_id', $company->id)->where('field_name', 'service / extra')->first();

            if(is_null($LeadCustomProductForm)){
                LeadCustomForm::create([
                    'field_display_name' => 'Service / Extra',
                    'field_name' => 'service / extra',
                    'field_order' => 8,
                    'field_type' => 'select',
