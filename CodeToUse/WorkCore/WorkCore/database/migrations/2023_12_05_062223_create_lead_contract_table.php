...
return new class extends Migration {
    ...
    private function createTables()
    {
        ...
        if (!Schema::hasTable('deals')) {
            Schema::create('deals', function (Blueprint $table) {
                ...
                $table->foreign('lead_id')->references('id')->on('enquiries')->onDelete('cascade')->onUpdate('cascade');
    ...
    private function companySpecificChanges()
    {
        ...
        foreach ($companies as $company) {
            $dashboardWidget = DashboardWidget::where('dashboard_type', 'admin-customer-dashboard')
            ...
            if (is_null($dashboardWidget)) {
                $widget = ['dashboard_type' => 'admin-customer-dashboard', 'widget_name' => 'total_deals', 'status' => 1, 'company_id' => $company->id];
    ...
    {
        $enquiries = Enquiry::withoutGlobalScopes()->get();

        foreach ($enquiries as $enquiry) {

            if (Deal::where('lead_id', $enquiry->id)->exists()) {
                continue;
            }

            try {

                $pipeline = LeadPipeline::withoutGlobalScopes()->where('company_id', $enquiry->company_id)->first();

                $pipelineStages = PipelineStage::withoutGlobalScopes()->where('company_id', $enquiry->company_id)->get();

                $leadStatus = DB::table('lead_status')->where('id', $enquiry->status_id)->first();
                ...
                $deal->agent_id = $enquiry->agent_id;
                $deal->next_follow_up = $enquiry->next_follow_up;
                $deal->value = $enquiry->value;
                $deal->currency_id = $enquiry->currency_id;
                $deal->added_by = $enquiry->added_by;
                $deal->last_updated_by = $enquiry->last_updated_by;
                $deal->hash = md5(microtime());
                $deal->next_follow_up = 'yes';
                $deal->saveQuietly();

                DealFollowUp::where('lead_id', $enquiry->id)->update(['deal_id' => $deal->id]);
                LeadProduct::where('lead_id', $enquiry->id)->update(['deal_id' => $deal->id]);
                Proposal::where('lead_id', $enquiry->id)->update(['deal_id' => $deal->id]);
                PurposeConsentLead::where('lead_id', $enquiry->id)->update(['deal_id' => $deal->id]);
                DealFile::where('lead_id', $enquiry->id)->update(['deal_id' => $deal->id]);

            } catch (\Exception $e) {
                echo "\nError in enquiry id: " . $enquiry->id . ' Error: ' . $e->getMessage() . "\n";
            }
        }
    ...
    private function addDealModulePermission()
    {
        ...
        if (!$addDealsPermission) {
            $leadsModule = Module::firstOrCreate(['module_name' => 'enquiries']);
