...
@section('content')
      ...
      <div class="col-xl-12">
        <div class="card">
          <div class="card-header d-flex justify-content-between align-items-center">
            <h5 class="card-title mb-0">{{ __('Edit Expense Request') }} #{{ $expense->expense_number }}</h5>
              ...
              <div class="row">
                <div class="col-md-6">
                  ...
                  <div class="mb-4">
                    ...
                    <input type="text" class="form-control flatpickr" id="expense_date" name="expense_date" 
                      value="{{ $expense->expense_date->format('Y-m-d') }}" placeholder="{{ __('Select Date') }}" required>
                  </div>
                  ...
                  <div class="mb-4">
                    ...
                    <input type="text" class="form-control" id="title" name="title" 
                      value="{{ $expense->title }}" placeholder="{{ __('Enter expense title...') }}" required>
                  </div>
                  
                  <div class="mb-4">
                    <label class="form-label" for="description">{{ __('Description') }}</label>
                    <textarea class="form-control" id="description" name="description" rows="3" 
                      placeholder="{{ __('Enter expense description...') }}">{{ $expense->description }}</textarea>
                  </div>
                </div>
                ...
                <div class="col-md-6">
                  ...
                  <div class="mb-4">
                    ...
                    <input type="text" class="form-control" id="project_code" name="project_code" 
                      value="{{ $expense->project_code }}" placeholder="{{ __('Enter project code') }}">
                  </div>
                  ...
                  <div class="mb-4">
                    ...
                    <input type="text" class="form-control" id="cost_center" name="cost_center" 
                      value="{{ $expense->cost_center }}" placeholder="{{ __('Enter cost center') }}">
                  </div>
                  ...
                  <div class="mb-4">
                    ...
                    <div class="form-text">{{ __('Upload new receipts or invoices (PDF, JPG, PNG, DOC - Max 10MB per file)') }}</div>
                    
                    @if($expense->hasAttachments())
                      <div class="mt-2">
                        <small class="text-muted">{{ __('Current attachments') }}: {{ $expense->files()->count() }} {{ __('file(s)') }}</small>
                      </div>
                    @endif
                  </div>
                </div>
              ...
              <div class="row mt-4">
                <div class="col-12">
                  <button type="submit" class="btn btn-primary">
                    <i class="bx bx-save me-1"></i> {{ __('Update Expense Request') }}
                  </button>
  ...
  <script>
    window.pageData = {
      urls: {
        update: @json(route('hrcore.my.expenses.update', ['id' => $expense->id])),
        list: @json(route('hrcore.my.expenses'))
      },
      labels: {
        success: @json(__('Success')),
        error: @json(__('Error')),
        confirmUpdate: @json(__('Are you sure you want to update this expense request?')),
        updateTitle: @json(__('Update Expense Request')),
        updateButton: @json(__('Yes, Update')),
