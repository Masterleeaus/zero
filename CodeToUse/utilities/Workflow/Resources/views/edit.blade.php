...
<div class="content-wrapper">
    <div class="card">
        ...
        <div class="card-body">
            <form id="editWorkflowForm" onsubmit="updateWorkflow(event)">
                @csrf
                @method('PUT') <!-- Use PUT for updating the workflow -->
                <div class="row">
                    <div class="col-sm-6">
                           
                        <x-forms.text :fieldValue="$workflow->name ?? ''" fieldId="name" :fieldLabel="__('Workflow Name')" fieldName="name" fieldRequired="true"
                    ...
                    <div class="col-sm-6">
                        ...
                        <div class="form-group">
                            ...
                            fieldName="project_category_id" fieldRequired="true" class="mt-0">
                            @foreach ($projectCategories as $category)
                            <option value="{{ $category->id }}" {{ $workflow->project_category_id == $category->id ? 'selected' : '' }}>{{ $category->category_name }}</option>
                            @endforeach
                            </x-forms.select>
                ...
                <div class="form-group">
                    ...
                    <textarea name="description" id="description" class="form-control" rows="4" placeholder="Enter workflow description (optional)">{{ $workflow->description }}</textarea>
                </div>

              
                
                <input type="hidden" id="workflow_id" value="{{ $workflow->id }}">
                <input type="hidden" id="company_id" value="{{ $workflow->company_id }}">
                <div class=" mt-4">
                    ...
                    <div class="mb-3">
                        ...
                        {{-- <button class="btn btn-info" onclick="saveWorkflow()">Save Workflow</button> --}}
                    </div>
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            ...
                            <button type="button" class="btn-close" onclick="closeMilestone()" aria-label="Close"></button>
                        </div>
                            ...
                            <form>
                                ...
                                <div class="mb-3">
                                    <x-forms.text 
                                        ...
                                        :fieldLabel="__('Milestone Title')" 
                                        fieldName="milestone_title" 
                                        fieldRequired="true"
                                        fieldPlaceholder="Enter milestone title">
                                    </x-forms.text>
                                </div>
                                
                                <div class="mb-3">
                                    <x-forms.number 
                                        :fieldValue="$milestone->start_day ?? 0" 
                                ...
                                <div class="mb-3">
                                    <x-forms.number 
                                        :fieldValue="$milestone->end_day ?? 0" 
                                ...
                                <div class="mb-3">
                                    <x-forms.textarea 
                                        :fieldValue="$milestone->description ?? ''" 
                                        fieldId="milestone-description" 
                                        :fieldLabel="__('Milestone Description')" 
                                        fieldName="milestone_description" 
                                        fieldPlaceholder="Enter milestone description">
                                    </x-forms.textarea>
                                </div>
                        ...
                        <div class="modal-footer">
                            <button type="button" class="btn btn-secondary" onclick="closeMilestone()">Close</button>
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        <div class="modal-header">
                            ...
                            <button type="button" class="btn-close" onclick="closeTask()"></button>
                        </div>
                        <div class="modal-body">
                            <form>
                                <div class="mb-3">
                        ...
                        <div class="modal-footer">
                            <button type="button" class="btn btn-secondary" onclick="closeTask()">Close</button>
...
var modal = null;
    function openMilestoneModal(milestone) {
        ...
        const start = milestone.dataset.start || 0;
        const end = milestone.dataset.end || 0;
        ...
        document.getElementById('milestone-start').value = end;
    ...
    function saveMilestone() {
        ...
        const start = document.getElementById('milestone-start').value;
        const end = document.getElementById('milestone-end').value;
        
        
        if (currentMilestone) {
            currentMilestone.querySelector('span').innerText = title;
            currentMilestone.dataset.description = description;
            currentMilestone.dataset.start = start;
            currentMilestone.dataset.end = end;
        } 
...
function closeMilestone(){
    ...
    function openTaskModal(task) {
        ...
        const start = task.dataset.start || 0;
        const end = task.dataset.end || 0;
        let templates;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function updateWorkflow(event) {
        ...
        try {
            const response = await axios.put(`/api/v1/workflows/${workflowId}`, formData, {
                headers: {
            ...
            // Reset the form
            document.getElementById('editWorkflowForm').reset();
        } catch (error) {
...
workflow.forEach((milestoneData) => {
    ...
    milestone.dataset.start = milestoneData.start;
    milestone.dataset.end = milestoneData.end;
...
function renderTask(taskData, parentContainer) {
    ...
    // Append the task to the parent container
...
var workflowData = {!! json_encode($workflow->workflow_data) !!};
